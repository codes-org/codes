...
int cfgp_lex_error(ParserParams* p, int lineno, int colno, const char* msg) {
    char buf[512];
    snprintf(buf, sizeof(buf), "lexer error (line %i, column %i): %s", lineno, colno, msg);
    p->lexer_error_string = strdup(buf);
    p->lexer_error_code = 2;
    return -1;
}

int cfgp_parser_error(ParserParams* p, const char* err, unsigned int first_line,
                      unsigned int first_column, unsigned int last_line, unsigned int last_column) {
    char location[128];
    char buf[512];

    if (first_line) {
        snprintf(location, sizeof(location), "%i:%i until %i:%i", first_line, first_column,
                 last_line, last_column);
    } else if (last_line) {
        snprintf(location, sizeof(location), "line %i, column %i", last_line, last_column);
    } else {
