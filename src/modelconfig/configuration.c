...
int configuration_get_value(ConfigHandle* handle, const char* section_name, const char* key_name,
                            const char* annotation, char* value, size_t len) {
    ...
    if (annotation == NULL) {
        ...
        if (snprintf(key_name_tmp, CONFIGURATION_MAX_NAME, "%s@%s", key_name, annotation) >=
            CONFIGURATION_MAX_NAME) {
            fprintf(stderr, "config error: name@annotation pair too long: %s@%s\n", key_name,
                    annotation);
            return 1;
        } else
...
int configuration_get_value_relpath(ConfigHandle* handle, const char* section_name,
    ...
    int w = configuration_get_value(handle, section_name, key_name, annotation, tmp, length);
    if (w <= 0)
        return w;
...
int configuration_get_value_int(ConfigHandle* handle, const char* section_name,
    ...
    r = configuration_get_value(handle, section_name, key_name, annotation, valuestr,
                                sizeof(valuestr));
    if (r > 0) {
        *value = atoi(valuestr);
        rc = 0;
...
int configuration_get_value_uint(ConfigHandle* handle, const char* section_name,
                                 const char* key_name, const char* annotation,
                                 unsigned int* value) {
    char valuestr[256];
    int rc = 1;
    int r;
...
static void check_add_anno(int anno_offset, config_anno_map_t* map) {
    if (anno_offset == -1) {
        map->has_unanno_lp = 1;
        if (!map->num_annos)
...
static void check_add_lp_type_anno(int lp_name_offset, int anno_offset,
    ...
    if (lpt_anno == lpgroups->lpannos_count) {
        ...
        memset(map->num_anno_lps, 0, CONFIGURATION_MAX_ANNOS * sizeof(*map->num_anno_lps));
        check_add_anno(anno_offset, map);
...
static int check_add_uniq_str(int** offset_array, char** str_buf, int* num_strs,
...
int configuration_get_lpgroups(ConfigHandle* handle, const char* section_name,
    ...
    int* group_names_offsets = malloc(sizeof(*group_names_offsets) * group_names_cap);
    int* lp_names_offsets = malloc(sizeof(*lp_names_offsets) * lp_names_cap);
    int* anno_names_offsets = malloc(sizeof(*anno_names_offsets) * anno_names_cap);
    lpgroups->group_names_buf = malloc(sizeof(*lpgroups->group_names_buf) * group_names_buf_cap);
    lpgroups->lp_names_buf = malloc(sizeof(*lpgroups->lp_names_buf) * lp_names_buf_cap);
    lpgroups->anno_names_buf = malloc(sizeof(*lpgroups->anno_names_buf) * anno_names_buf_cap);
...
#define CHECKED_STRTOL(_val, _field, _data)                                                        \
    do {                                                                                           \
        ...
        if (_rd <= 0 || errno)                                                                     \
            tw_error(TW_LOC,                                                                       \
                     "bad value (expected positive integer) for "                                  \
                     "\"%s\": %s\n",                                                               \
                     _field, _data);                                                               \
        else                                                                                       \
            _val = _rd;                                                                            \
    ...
    for (size_t i = 0; i < se_count; i++) {
        ...
        if (se[i].type == SE_SECTION) {
            ...
            name_pos = check_add_uniq_str(&group_names_offsets, &lpgroups->group_names_buf,
                                          &num_uniq_group_names, &group_names_cap,
                                          &group_names_buf_len, &group_names_buf_cap, se[i].name);
            ...
            if (num_uniq_group_names != lpgroups->lpgroups_count)
                tw_error(TW_LOC, "config error: non-unique group names detected\n");
            ...
            for (size_t j = 0, lpt = 0; j < subse_count; j++) {
                if (subse[j].type == SE_KEY) {
                    ...
                    if (strcmp("repetitions", subse[j].name) == 0) {
                        CHECKED_STRTOL(lpgroups->lpgroups[i].repetitions, "repetitions", data);
                        ...
                        if (c != NULL) {
                            *c = '\0';
                            name_pos =
                                check_add_uniq_str(&anno_names_offsets, &lpgroups->anno_names_buf,
                                                   &lpgroups->num_uniq_annos, &anno_names_cap,
                                                   &anno_names_buf_len, &anno_names_buf_cap, c + 1);
                        ...
                        name_pos =
                            check_add_uniq_str(&lp_names_offsets, &lpgroups->lp_names_buf,
                                               &lpgroups->num_uniq_lptypes, &lp_names_cap,
                                               &lp_names_buf_len, &lp_names_buf_cap, subse[j].name);
                        lpgroups->lpgroups[i].lptypes[lpt].name.offset = lp_names_offsets[name_pos];

                        if (c != NULL)
                            *c = '@';

                        // add to anno map
                        check_add_lp_type_anno(lpgroups->lpgroups[i].lptypes[lpt].name.offset,
                                               lpgroups->lpgroups[i].lptypes[lpt].anno.offset,
                                               lpgroups);
    ...
    lpgroups->group_names = malloc(lpgroups->lpgroups_count * sizeof(*lpgroups->group_names));
    lpgroups->lp_names = malloc(lpgroups->num_uniq_lptypes * sizeof(*lpgroups->lp_names));
    assert(lpgroups->group_names);
    assert(lpgroups->lp_names);

    for (int i = 0; i < lpgroups->lpgroups_count; i++) {
        lpgroups->group_names[i] = lpgroups->group_names_buf + group_names_offsets[i];
    }
    for (int i = 0; i < lpgroups->num_uniq_lptypes; i++) {
        lpgroups->lp_names[i] = lpgroups->lp_names_buf + lp_names_offsets[i];
    }
    if (lpgroups->num_uniq_annos == 0) {
        ...
        lpgroups->anno_names = malloc(lpgroups->num_uniq_annos * sizeof(*lpgroups->anno_names));
        assert(lpgroups->anno_names);
        for (int i = 0; i < lpgroups->num_uniq_annos; i++) {
            lpgroups->anno_names[i] = lpgroups->anno_names_buf + anno_names_offsets[i];
        }
    }
    ...
    for (int g = 0; g < lpgroups->lpgroups_count; g++) {
        ...
        for (int t = 0; t < lpg->lptypes_count; t++) {
            ...
            lpt->anno.ptr =
                (lpt->anno.offset == -1) ? NULL : lpgroups->anno_names_buf + lpt->anno.offset;
        }
    }
    for (int m = 0; m < lpgroups->lpannos_count; m++) {
        ...
        for (int a = 0; a < map->num_annos; a++) {
            map->annotations[a].ptr = (map->annotations[a].offset == -1)
                                          ? NULL
