...
static int cfsa_getKey(void* handle, SectionHandle section, const char* name, char* buf,
                       size_t bufsize) {
...
static int cfsa_getMultiKey(void* handle, SectionHandle section, const char* name, char*** buf,
                            size_t* e) {
    mcs_entry* key = mcs_findkey((mcs_entry*)(section ? section : handle), name);
    int count;
    unsigned int dcount;
...
static int cfsa_listSection(void* handle, SectionHandle section, SectionEntry* entries,
                            size_t* maxentries) {
...
static int cfsa_openSection(void* handle, SectionHandle section, const char* sectionname,
                            SectionHandle* newsection) {
    mcs_entry* e = mcs_findsubsection((mcs_entry*)(section ? section : handle), sectionname);
    *newsection = e;
    return (e != 0 ? 1 : -1);
}
...
static int cfsa_createSection(void* handle, SectionHandle section, const char* name,
                              SectionHandle* newsection) {
    mcs_entry* news = mcs_addsection((mcs_entry*)(section ? section : handle), name);
    *newsection = news;
    return (news != 0);
}

static int cfsa_createKey(void* handle, SectionHandle section, const char* key, const char** data,
                          unsigned int count) {
...
static int cfsa_getSectionSize(void* handle, SectionHandle section, unsigned int* count) {
...
static struct ConfigVTable cfsa_template = {.getKey = cfsa_getKey,
