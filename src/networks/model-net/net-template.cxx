...
static void terminal_final(terminal_state* s, tw_lp* lp) {
    ...
    written += sprintf(s->output_buf + written, "\n %llu %u %llu %lf %ld %lf %lf", LLU(lp->gid),
                       s->terminal_id, s->total_msg_size, s->total_time, s->finished_packets,
...
static void router_packet_receive(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    ...
    if (s->vc_occupancy[next_port][next_vc] + s->params->chunk_size <= max_vc_size) {
        bf->c2 = 1;
...
static void router_packet_send_rc(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    ...
    cur_entry->index = use_vc;
    prepend_to_message_list(s->pending_msgs[output_port], s->pending_msgs_tail[output_port], use_vc,
                            cur_entry);
...
static void router_buf_update(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    ...
    tw_stime ts = codes_local_latency(lp);
    tw_event* e =
        model_net_method_event_new(lp->gid, ts, lp, LOCAL_NETWORK_ROUTER_NAME, (void**)&m, NULL);
    m->type = R_SEND;
    m->vc_index = indx;
    m->magic = router_magic_num;
