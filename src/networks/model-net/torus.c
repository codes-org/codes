...
static void append_to_node_message_list(nodes_message_list** thisq, nodes_message_list** thistail,
                                        int index, nodes_message_list* msg) {
    if (thisq[index] == NULL) {
        thisq[index] = msg;
    } else {
...
static void prepend_to_node_message_list(nodes_message_list** thisq, nodes_message_list** thistail,
                                         int index, nodes_message_list* msg) {
    if (thisq[index] == NULL) {
        thistail[index] = msg;
    } else {
...
static nodes_message_list* return_head(nodes_message_list** thisq, nodes_message_list** thistail,
                                       int index) {
...
static nodes_message_list* return_tail(nodes_message_list** thisq, nodes_message_list** thistail,
                                       int index) {
...
static void torus_read_config(const char* anno, torus_param* params) {
    ...
    if (rc) {
        ...
        fprintf(stderr, "Warning: Number of dimensions not specified, setting to %d\n", p->n_dims);
    }
    ...
    if (rc) {
        p->buffer_size = 2048;
        fprintf(stderr, "Buffer size not specified, setting to %d", p->buffer_size);
    }
...
void torus_collective_init(nodes_state* s, tw_lp* lp) {
    // TODO: be annotation-aware somehow
    codes_mapping_get_lp_info(lp->gid, grp_name, &mapping_grp_id, NULL, &mapping_type_id, NULL,
...
static tw_stime torus_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    xfer_to_nic_time = codes_local_latency(
        sender); /* Throws an error of found last KP time > current event time otherwise */
    //e_new = tw_event_new(local_nic_id, xfer_to_nic_time+offset, sender);
    //msg = tw_event_data(e_new);
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender, TORUS,
...
static void credit_send(nodes_state* s, tw_lp* lp, nodes_message* msg, int sq) {
    tw_event* e;
    nodes_message* m;
    tw_stime ts;

    ts = (1.1 * g_tw_lookahead) + s->params->credit_delay + tw_rand_unif(lp->rng);
    e = model_net_method_event_new(msg->sender_node, ts, lp, TORUS, (void**)&m, NULL);
    if (sq == -1) {
...
static void torus_init(nodes_state* s, tw_lp* lp) {
    int i, j;
    char anno[MAX_NAME_LENGTH];

    rc_stack_create(&s->st);

    codes_mapping_get_lp_info(lp->gid, grp_name, &mapping_grp_id, NULL, &mapping_type_id, anno,
                              &mapping_rep_id, &mapping_offset);
    ...
    s->next_link_available_time = (tw_stime**)malloc(2 * p->n_dims * sizeof(tw_stime*));
    s->next_credit_available_time = (tw_stime**)malloc(2 * p->n_dims * sizeof(tw_stime*));
    s->next_flit_generate_time = (tw_stime**)malloc(2 * p->n_dims * sizeof(tw_stime*));

    for (i = 0; i < 2 * p->n_dims; i++) {
        s->buffer[i] = (int*)malloc(p->num_vc * sizeof(int));
        s->next_link_available_time[i] = (tw_stime*)malloc(p->num_vc * sizeof(tw_stime));
        s->next_credit_available_time[i] = (tw_stime*)malloc(p->num_vc * sizeof(tw_stime));
        s->next_flit_generate_time[i] = (tw_stime*)malloc(p->num_vc * sizeof(tw_stime));
    }
    s->terminal_length = (int*)malloc(2 * p->n_dims * sizeof(int));
    s->queued_length = (int*)malloc(2 * p->n_dims * sizeof(int));
    s->terminal_msgs = (nodes_message_list**)malloc(2 * p->n_dims * sizeof(nodes_message_list*));
    s->terminal_msgs_tail =
        (nodes_message_list**)malloc(2 * p->n_dims * sizeof(nodes_message_list*));
    s->pending_msgs = (nodes_message_list***)malloc(2 * p->n_dims * sizeof(nodes_message_list**));
    s->pending_msgs_tail =
        (nodes_message_list***)malloc(2 * p->n_dims * sizeof(nodes_message_list**));
    s->queued_msgs = (nodes_message_list***)malloc(2 * p->n_dims * sizeof(nodes_message_list**));
    s->queued_msgs_tail =
        (nodes_message_list***)malloc(2 * p->n_dims * sizeof(nodes_message_list**));

    s->busy_time = (tw_stime*)malloc(2 * p->n_dims * sizeof(tw_stime));
    s->last_buf_full = (tw_stime*)malloc(2 * p->n_dims * sizeof(tw_stime));

    for (i = 0; i < 2 * p->n_dims; i++) {
        s->pending_msgs[i] = (nodes_message_list**)malloc(p->num_vc * sizeof(nodes_message_list*));
        s->pending_msgs_tail[i] =
            (nodes_message_list**)malloc(p->num_vc * sizeof(nodes_message_list*));
        s->queued_msgs[i] = (nodes_message_list**)malloc(p->num_vc * sizeof(nodes_message_list*));
        s->queued_msgs_tail[i] =
            (nodes_message_list**)malloc(p->num_vc * sizeof(nodes_message_list*));
    }
    s->other_msgs = (nodes_message_list**)malloc(2 * p->n_dims * sizeof(nodes_message_list*));
    s->other_msgs_tail = (nodes_message_list**)malloc(2 * p->n_dims * sizeof(nodes_message_list*));
    s->in_send_loop = (int*)malloc(2 * p->n_dims * sizeof(int));
    ...
    to_dim_id(codes_mapping_get_lp_relative_id(lp->gid, 0, 1), s->params->n_dims,
              s->params->dim_length, s->dim_position);
    /* DEBUG */
    ...
    */
    ...
    for (j = 0; j < p->n_dims; j++) {
        temp_dim_pos[j] = (s->dim_position[j] - 1 + p->dim_length[j]) % p->dim_length[j];

        s->neighbour_minus_lpID[j] = to_flat_id(p->n_dims, p->dim_length, temp_dim_pos);
    ...
    for (j = 0; j < p->n_dims; j++) {
        ...
        s->neighbour_plus_lpID[j] = to_flat_id(p->n_dims, p->dim_length, temp_dim_pos);
...
void torus_collective(char const* category, int message_size, int remote_event_size,
    ...
    codes_mapping_get_lp_info(sender->gid, grp_name, &mapping_grp_id, NULL, &mapping_type_id, NULL,
                              &mapping_rep_id, &mapping_offset);
    codes_mapping_get_lp_id(grp_name, LP_CONFIG_NM, NULL, 1, mapping_rep_id, mapping_offset,
                            &local_nic_id);

    xfer_to_nic_time = g_tw_lookahead + codes_local_latency(sender);
    e_new = model_net_method_event_new(local_nic_id, xfer_to_nic_time, sender, TORUS, (void**)&msg,
                                       (void**)&tmp_ptr);
...
static void send_remote_event(nodes_state* s, nodes_message* msg, tw_lp* lp) {
...
static void node_collective_init(nodes_state* s, nodes_message* msg, tw_lp* lp) {
    tw_event* e_new;
    ...
    if (s->is_leaf) {
        ...
        codes_mapping_get_lp_info(lp->gid, grp_name, &mapping_grp_id, NULL, &mapping_type_id, NULL,
                                  &mapping_rep_id, &mapping_offset);
        num_lps = codes_mapping_get_lp_count(grp_name, 1, LP_CONFIG_NM, NULL, 1);
        codes_mapping_get_lp_id(grp_name, LP_CONFIG_NM, NULL, 1, s->parent_node_id / num_lps,
                                (s->parent_node_id % num_lps), &parent_nic_id);
        ...
        void* m_data;
        e_new = model_net_method_event_new(parent_nic_id, xfer_to_nic_time, lp, TORUS,
                                           (void**)&msg_new, (void**)&m_data);

        memcpy(msg_new, msg, sizeof(nodes_message));
        if (msg->remote_event_size_bytes) {
            memcpy(m_data, model_net_method_get_edata(TORUS, msg), msg->remote_event_size_bytes);
        }
...
static void node_collective_fan_in(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    int i;
    s->num_fan_nodes++;
    ...
    if (s->is_root && (s->num_fan_nodes == s->num_children)) {
        ...
        for (i = 0; i < s->num_children; i++) {
            ...
            codes_mapping_get_lp_id(grp_name, LP_CONFIG_NM, NULL, 1, s->children[i] / num_lps,
                                    (s->children[i] % num_lps), &child_nic_id);
            //e_new = codes_event_new(child_nic_id, xfer_to_nic_time, lp);

            //msg_new = tw_event_data(e_new);
            void* m_data;
            e_new = model_net_method_event_new(child_nic_id, xfer_to_nic_time, lp, TORUS,
                                               (void**)&msg_new, &m_data);
...
static void node_collective_fan_out(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    int i;
    //TODO: be annotation-aware
    int num_lps = codes_mapping_get_lp_count(grp_name, 1, LP_CONFIG_NM, NULL, 1);
    bf->c1 = 0;
    ...
    if (!s->is_leaf) {
        ...
        for (i = 0; i < s->num_children; i++) {
            xfer_to_nic_time = g_tw_lookahead + TORUS_FAN_OUT_DELAY +
                               tw_rand_exponential(lp->rng, (double)TORUS_FAN_OUT_DELAY / 10);
...
static void dimension_order_routing(nodes_state* s, tw_lpid* dst_lp, int* dim, int* dir) {
    ...
    *dir = -1;
    ...
    to_dim_id(codes_mapping_get_lp_relative_id(*dst_lp, 0, 1), s->params->n_dims,
    ...
    assert(*dim != -1 && *dir != -1);
    *dst_lp = codes_mapping_get_lpid_from_relative(dest_id, NULL, LP_CONFIG_NM, s->anno, 1);
}
static void packet_generate(nodes_state* ns, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    ...
    if (lp->gid == TRACK && msg->packet_ID == TRACE)
        tw_output(lp, "\n packet generated %lld at lp %d dest %d final dest %d", msg->packet_ID,
                  (int)lp->gid, (int)intm_dst, (int)msg->dest_lp);
    ...
    for (uint64_t j = 0; j < num_chunks; j++) {
        nodes_message_list* cur_chunk = (nodes_message_list*)malloc(sizeof(nodes_message_list));
        ...
        append_to_node_message_list(ns->terminal_msgs, ns->terminal_msgs_tail, queue, cur_chunk);
    ...
    total_event_size =
        model_net_get_msg_sz(TORUS) + msg->remote_event_size_bytes + msg->local_event_size_bytes;
    mn_stats* stat;
...
static void packet_generate_rc(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    s->packet_counter--;
    ...
    for (uint64_t j = 0; j < num_chunks; j++) {
        nodes_message_list* cur_entry = return_tail(s->terminal_msgs, s->terminal_msgs_tail, queue);
...
static void packet_send_rc(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    int queue = msg->source_direction + (msg->source_dim * 2);
    ...
    if (bf->c31) {
        prepend_to_node_message_list(s->terminal_msgs, s->terminal_msgs_tail, queue, cur_entry);
        s->terminal_length[queue] += s->params->chunk_size;
        s->all_term_length += s->params->chunk_size;
    }

    if (bf->c8) {
        prepend_to_node_message_list(s->pending_msgs[queue], s->pending_msgs_tail[queue], STATICQ,
                                     cur_entry);
    }
...
static void packet_send(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    tw_stime ts;
    tw_event* e;
    nodes_message* m;
    int isT = 0;
    ...
    if (s->pending_msgs[queue][STATICQ] == NULL && s->terminal_msgs[queue] == NULL) {
        bf->c1 = 1;
        s->in_send_loop[queue] = 0;
        return;
    }
...
*/
    ...
    e = model_net_method_event_new(cur_entry->msg.next_stop, ts, lp, TORUS, (void**)&m, &m_data);
    ...
    if (cur_entry->msg.chunk_id == num_chunks - 1) {
        ...
        if (cur_entry->msg.local_event_size_bytes > 0) {
            ...
            local_event = (char*)cur_entry->event_data + cur_entry->msg.remote_event_size_bytes;
    ...
    if (isT) {
        bf->c31 = 1;
        cur_entry = return_head(s->terminal_msgs, s->terminal_msgs_tail, queue);
        s->terminal_length[queue] -= s->params->chunk_size;
        s->all_term_length -= s->params->chunk_size;
    } else {
        bf->c8 = 1;
        cur_entry = return_head(s->pending_msgs[queue], s->pending_msgs_tail[queue], STATICQ);
    }
...
static void packet_arrive_rc(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    codes_local_latency_reverse(lp);
    s->finished_chunks--;
    ...
    if (bf->c6) {
        ...
        if (bf->c30) {
            cur_entry = return_tail(s->queued_msgs[queue], s->queued_msgs_tail[queue], STATICQ);
        ...
        if (bf->c9 || bf->c11) {
            cur_entry = return_tail(s->pending_msgs[queue], s->pending_msgs_tail[queue], STATICQ);
            s->buffer[queue][STATICQ] -= s->params->chunk_size;
            tw_rand_reverse_unif(lp->rng);
        }

        if (bf->c8) {
            cur_entry = return_tail(s->other_msgs, s->other_msgs_tail, queue);
            if (bf->c24)
                s->last_buf_full[queue] = msg->saved_busy_time;
        }
        assert(cur_entry);
...
static void packet_arrive(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    tw_event* e;
    tw_stime ts;
    nodes_message* m;
    mn_stats* stat;
    ...
    if (lp->gid == msg->dest_lp) {
        if (lp->gid == TRACK && msg->packet_ID == TRACE) {
            tw_output(lp, "\n packet %ld arrived at lp %d from %ld final dest %d hops %d",
                      msg->packet_ID, (int)lp->gid, msg->sender_node, (int)msg->dest_lp,
                      msg->my_N_hop);
        }
        bf->c1 = 1;
        s->total_data_sz += s->params->chunk_size;
        ...
        if (multfactor == 1) {
            if (s->buffer[queue][STATICQ] + s->params->chunk_size > s->params->buffer_size) {
                /* No buffer space available, add it in the queued messages for
                 * now */
                bf->c30 = 1;
                cur_chunk->msg.saved_queue = msg->source_direction + (msg->source_dim * 2);
                append_to_node_message_list(s->queued_msgs[queue], s->queued_msgs_tail[queue],
                                            STATICQ, cur_chunk);
                s->queued_length[queue] += s->params->chunk_size;
                ...
                s->buffer[queue][STATICQ] += s->params->chunk_size;
                credit_send(s, lp, msg, -1);
                append_to_node_message_list(s->pending_msgs[queue], s->pending_msgs_tail[queue],
                                            STATICQ, cur_chunk);
            }
        } else {
            ...
            if (s->buffer[queue][STATICQ] + 2 * s->params->chunk_size <= s->params->buffer_size) {
                ...
                bf->c8 = 1;
                cur_chunk->msg.saved_queue = msg->source_direction + (msg->source_dim * 2);
                append_to_node_message_list(s->other_msgs, s->other_msgs_tail, queue, cur_chunk);
                if (!s->last_buf_full[queue]) {
                    bf->c24 = 1;
                    msg->saved_busy_time = s->last_buf_full[queue];
                    s->last_buf_full[queue] = tw_now(lp);
...
static void torus_report_stats() {
    ...
    if (!g_tw_mynode) {
        printf(" Average number of hops traversed %f average packet latency %lf us maximum packet "
               "latency %lf us finished packets %lld finished hops %lld \n",
               (float)avg_hops / total_finished_packets, avg_time / (total_finished_packets * 1000),
               max_time / 1000, total_finished_packets, avg_hops);
    }
}
/* finalize the torus node and free all event buffers available */
void final(nodes_state* s, tw_lp* lp) {
    int j;
    const torus_param* p = s->params;
    ...
    int written = 0;
    if (!s->node_id) {
        written = sprintf(s->output_buf, "# Format <LP id> <Node ID> <Total Data Size> <Total Time "
                                         "Spent> <# Packets finished> <Avg hops> \n");
    }
    written += sprintf(s->output_buf + written, "%llu %llu %ld %lf %ld %lf\n", LLU(lp->gid),
                       LLU(s->node_id), s->total_data_sz, s->total_time, s->finished_packets,
                       (double)s->total_hops / s->finished_packets);
...
static void packet_buffer_process_rc(nodes_state* s, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    int queue = msg->source_direction + (msg->source_dim * 2);
    s->buffer[queue][STATICQ] += s->params->chunk_size;
    ...
    if (bf->c2) {
        nodes_message_list* tail =
            return_tail(s->pending_msgs[queue], s->pending_msgs_tail[queue], STATICQ);
        prepend_to_node_message_list(s->queued_msgs[queue], s->queued_msgs_tail[queue], STATICQ,
                                     tail);
        s->queued_length[queue] += s->params->chunk_size;
        tw_rand_reverse_unif(lp->rng);
        s->buffer[queue][STATICQ] -= s->params->chunk_size;
    ...
    if (bf->c3) {
        nodes_message_list* tail =
            return_tail(s->pending_msgs[queue], s->pending_msgs_tail[queue], STATICQ);
        prepend_to_node_message_list(s->other_msgs, s->other_msgs_tail, queue, tail);
        tw_rand_reverse_unif(lp->rng);
        s->buffer[queue][STATICQ] -= s->params->chunk_size;
    }
...
static void packet_buffer_process(nodes_state* ns, tw_bf* bf, nodes_message* msg, tw_lp* lp) {
    ...
    if (ns->queued_msgs[queue][STATICQ] != NULL) {
        bf->c2 = 1;
        nodes_message_list* head =
            return_head(ns->queued_msgs[queue], ns->queued_msgs_tail[queue], STATICQ);
        ns->queued_length[queue] -= ns->params->chunk_size;
        credit_send(ns, lp, &head->msg, 1);
        append_to_node_message_list(ns->pending_msgs[queue], ns->pending_msgs_tail[queue], STATICQ,
                                    head);
        ns->buffer[queue][STATICQ] += ns->params->chunk_size;
    } else if (ns->buffer[queue][STATICQ] + 2 * ns->params->chunk_size <= ns->params->buffer_size) {
        if (ns->other_msgs[queue] != NULL) {
            bf->c3 = 1;
            nodes_message_list* head = return_head(ns->other_msgs, ns->other_msgs_tail, queue);
            credit_send(ns, lp, &head->msg, 1);
            append_to_node_message_list(ns->pending_msgs[queue], ns->pending_msgs_tail[queue],
                                        STATICQ, head);
            ns->buffer[queue][STATICQ] += ns->params->chunk_size;
        }
    }
    if (ns->in_send_loop[queue] == 0) {
        ...
        nodes_message* m;
        tw_event* e = model_net_method_event_new(lp->gid, ts, lp, TORUS, (void**)&m, NULL);
        m->type = SEND;
        m->source_direction = msg->source_direction;
        m->source_dim = msg->source_dim;
        ns->in_send_loop[queue] = 1;
...
/* event types */
tw_lptype torus_lp = {
    (init_f)torus_init, (pre_run_f)NULL, (event_f)event_handler, (revent_f)node_rc_handler,
    (commit_f)NULL,     (final_f) final, (map_f)codes_mapping,   sizeof(nodes_state),
};
...
void model_net_torus_get_dims(char const* anno, int ignore_annotations, int* n, int const** dims) {
...
void model_net_torus_get_dim_id(int flat_id, int ndims, const int* dim_lens, int* out_dim_ids) {
    to_dim_id(flat_id, ndims, dim_lens, out_dim_ids);
}

int model_net_torus_get_flat_id(int ndims, const int* dim_lens, const int* dim_ids) {
