...
static tw_stime loggp_packet_event(model_net_request const* req, uint64_t message_offset,
...
tw_stime loggp_recv_msg_event(const char* category, tw_lpid final_dest_lp, tw_lpid src_mn_lp,
                              uint64_t msg_size, int is_pull, uint64_t pull_size, tw_stime offset,
                              int remote_event_size, const void* remote_event, tw_lpid src_lp,
                              tw_lp* sender);
...
static const struct param_table_entry* find_params(uint64_t msg_size, const loggp_param* params);
...
static void loggp_init(loggp_state* ns, tw_lp* lp);
static void loggp_event(loggp_state* ns, tw_bf* b, loggp_message* m, tw_lp* lp);
static void loggp_rev_event(loggp_state* ns, tw_bf* b, loggp_message* m, tw_lp* lp);
static void loggp_finalize(loggp_state* ns, tw_lp* lp);

tw_lptype loggp_lp = {
    (init_f)loggp_init, (pre_run_f)NULL,         (event_f)loggp_event, (revent_f)loggp_rev_event,
    (commit_f)NULL,     (final_f)loggp_finalize, (map_f)codes_mapping, sizeof(loggp_state),
};

static void handle_msg_ready_rev_event(loggp_state* ns, loggp_message* m, tw_lp* lp);
static void handle_msg_ready_event(loggp_state* ns, loggp_message* m, tw_lp* lp);
static void handle_msg_start_rev_event(loggp_state* ns, loggp_message* m, tw_lp* lp);
static void handle_msg_start_event(loggp_state* ns, loggp_message* m, tw_lp* lp);
...
static void handle_msg_ready_rev_event(loggp_state* ns, loggp_message* m, tw_lp* lp) {
    dprintf("%lu (mn): ready msg rc %lu->%lu, size %lu (%3s last)\n"
            ...
            m->event_size_bytes + m->local_event_size_bytes > 0 ? "is" : "not", tw_now(lp),
...
static void handle_msg_ready_event(loggp_state* ns, loggp_message* m, tw_lp* lp) {
    ...
    dprintf("%lu (mn): ready msg    %lu->%lu, size %lu (%3s last)\n"
            ...
            lp->gid, m->src_gid, m->final_dest_gid, m->net_msg_size_bytes,
            m->event_size_bytes + m->local_event_size_bytes > 0 ? "is" : "not", tw_now(lp),
            m->net_recv_next_idle_saved, ns->net_recv_next_idle, recv_queue_time);

    // if we're using a recv-side queue, then we need to tell the scheduler we
    // are idle
#if USE_RECV_QUEUE
    model_net_method_idle_event(codes_local_latency(lp) + ns->net_recv_next_idle - tw_now(lp), 1,
                                lp);
#endif
    ...
    if (m->event_size_bytes) {
        ...
        if (m->is_pull) {
            ...
            m->event_rc = model_net_event_mctx(net_id, &mc_src, &mc_dst, m->category, m->src_gid,
                                               m->pull_size, recv_queue_time, m->event_size_bytes,
                                               tmp_ptr, 0, NULL, lp);
        } else {
...
static void handle_msg_start_event(loggp_state* ns, loggp_message* m, tw_lp* lp) {
    ...
    total_event_size =
        model_net_get_msg_sz(LOGGP) + m->event_size_bytes + m->local_event_size_bytes;
    ...
    dprintf("%lu (mn): start msg    %lu->%lu, size %lu (%3s last)\n"
            ...
            lp->gid, m->src_gid, m->final_dest_gid, m->net_msg_size_bytes,
            m->event_size_bytes + m->local_event_size_bytes > 0 ? "is" : "not", tw_now(lp),
            m->net_send_next_idle_saved, ns->net_send_next_idle, send_queue_time);

#if USE_RECV_QUEUE
    model_net_method_send_msg_recv_event(m->final_dest_gid, m->dest_mn_lp, m->src_gid,
                                         m->net_msg_size_bytes, m->is_pull, m->pull_size,
                                         m->event_size_bytes, &m->sched_params, m->category, LOGGP,
                                         m, send_queue_time, lp);
#else
    void* m_data;
    e_new = model_net_method_event_new(m->dest_mn_lp, send_queue_time, lp, LOGGP, (void**)&m_new,
                                       &m_data);
    ...
    if (m->event_size_bytes) {
        memcpy(m_data, model_net_method_get_edata(LOGGP, m), m->event_size_bytes);
    }
    ...
    model_net_method_idle_event(codes_local_latency(lp) + ns->net_send_next_idle - tw_now(lp), 0,
                                lp);
    ...
    if (m->local_event_size_bytes > 0) {
        ...
        void* m_loc = (char*)model_net_method_get_edata(LOGGP, m) + m->event_size_bytes;
        //local_event = (char*)m;
...
static tw_stime loggp_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender, LOGGP,
...
tw_stime loggp_recv_msg_event(const char* category, tw_lpid final_dest_lp, tw_lpid src_mn_lp,
    ...
    tw_event* e =
        model_net_method_event_new(sender->gid, moffset, sender, LOGGP, (void**)&m, &m_data);
...
static void loggp_configure() {
    ...
    for (int i = 0; i < anno_map->num_annos; i++) {
        ...
        int rc = configuration_get_value_relpath(&config, "PARAMS", "net_config_file", anno,
                                                 config_file, MAX_NAME_LENGTH);
        if (rc <= 0) {
            tw_error(TW_LOC, "unable to read PARAMS:net_config_file@%s", anno);
        }
        loggp_set_params(config_file, &all_params[i]);
    }
    if (anno_map->has_unanno_lp > 0) {
        int rc = configuration_get_value_relpath(&config, "PARAMS", "net_config_file", NULL,
                                                 config_file, MAX_NAME_LENGTH);
        if (rc <= 0) {
...
void loggp_set_params(const char* config_file, loggp_param* params) {
    ...
    while (fgets(buffer, 512, conf)) {
        ...
        ret = sscanf(buffer, "%" PRIu64 " %d %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                     &params->table[params->table_size].size, &params->table[params->table_size].n,
                     ...
                     &params->table[params->table_size].L, &params->table[params->table_size].o_s,
                     &params->table[params->table_size].o_r, &params->table[params->table_size].g,
