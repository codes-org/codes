...
static void fcfs_add(const model_net_request* req, const mn_sched_params* sched_params,
                     int remote_event_size, void* remote_event, int local_event_size,
                     void* local_event, void* sched, model_net_sched_rc* rc, tw_lp* lp);
static void fcfs_add_rc(void* sched, const model_net_sched_rc* rc, tw_lp* lp);
static int fcfs_next(tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc,
                     tw_lp* lp);
static void fcfs_next_rc(void* sched, const void* rc_event_save, const model_net_sched_rc* rc,
                         tw_lp* lp);
...
static int rr_next(tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc,
                   tw_lp* lp);
static void rr_next_rc(void* sched, const void* rc_event_save, const model_net_sched_rc* rc,
                       tw_lp* lp);
...
static int prio_next(tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc,
                     tw_lp* lp);
static void prio_next_rc(void* sched, const void* rc_event_save, const model_net_sched_rc* rc,
                         tw_lp* lp);
...
static const model_net_sched_interface rr_tab = {&rr_init,   &rr_destroy, &rr_add,
                                                 &rr_add_rc, &rr_next,    &rr_next_rc};
...
#define X(a, b, c, d) d,
const crv_checkpointer* sched_checkpointers[] = {SCHEDULER_TYPES};
#undef X
...
void fcfs_add(const model_net_request* req, const mn_sched_params* sched_params,
    ...
    dprintf("%llu (mn):    adding %srequest from %llu to %llu, size %llu, at %lf\n", LLU(lp->gid),
            req->is_pull ? "pull " : "", LLU(req->src_lp), LLU(req->final_dest_lp),
            LLU(req->msg_size), tw_now(lp));
}
...
int fcfs_next(tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc,
              tw_lp* lp) {
    mn_sched_queue* s = sched;
    ...
    if (s->is_recv_queue) {
        ...
        *poffset = s->method->model_net_method_recv_msg_event(q->req.category, q->req.final_dest_lp,
                                                              q->req.dest_mn_lp, psize,
                                                              q->req.is_pull, q->req.pull_size, 0.0,
                                                              q->req.remote_event_size,
        ...
        dprintf("%llu (mn):    issuing packet of size %llu (of %llu) "
                ...
                LLU(q->req.final_dest_lp), tw_now(lp), is_last_packet);
        *poffset =
            s->method->model_net_method_packet_event(&q->req, q->req.msg_size - q->rem, psize, 0.0,
                                                     &q->sched_params, q->remote_event,
                                                     q->local_event, lp, is_last_packet,
...
static bool check_fcfs_state(mn_sched_queue* before, mn_sched_queue* after) {
    ...
    is_same &= are_qlist_equal(&before->reqs, &after->reqs, QLIST_OFFSET(mn_sched_qitem, ql),
                               (bool (*)(void*, void*))check_mn_sched_qitem);
...
void rr_add(const model_net_request* req, const mn_sched_params* sched_params,
            int remote_event_size, void* remote_event, int local_event_size, void* local_event,
            void* sched, model_net_sched_rc* rc, tw_lp* lp) {
    fcfs_add(req, sched_params, remote_event_size, remote_event, local_event_size, local_event,
             sched, rc, lp);
}
...
int rr_next(tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc,
            tw_lp* lp) {
...
int prio_next(tw_stime* poffset, void* sched, void* rc_event_save, model_net_sched_rc* rc,
              tw_lp* lp) {
