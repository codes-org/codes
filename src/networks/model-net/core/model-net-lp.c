...
static void model_net_base_lp_init(model_net_base_state* ns, tw_lp* lp);
static void model_net_base_event(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m,
                                 tw_lp* lp);
static void model_net_base_event_rc(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m,
                                    tw_lp* lp);
static void model_net_base_finalize(model_net_base_state* ns, tw_lp* lp);

/* event type handlers */
static void handle_new_msg(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m, tw_lp* lp);
static void handle_sched_next(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m, tw_lp* lp);
static void handle_new_msg_rc(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m, tw_lp* lp);
static void handle_sched_next_rc(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m,
                                 tw_lp* lp);
static void model_net_commit_event(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m,
                                   tw_lp* lp);
...
static void print_event_state(FILE* out, char const* prefix, model_net_base_state* s,
...
void mn_event_collect(model_net_wrap_msg* m, tw_lp* lp, char* buffer, int* collect_flag) {
    ...
    case MN_BASE_PASS:
        ...
        if (((model_net_base_state*)lp->cur_state)->sub_model_type) {
            if (g_st_ev_trace)
                (((model_net_base_state*)lp->cur_state)->sub_model_type->ev_trace)(sub_msg, lp,
                                                                                   buffer,
                                                                                   collect_flag);
        }
        break;
...
st_model_types mn_model_base_type = {(ev_trace_f)mn_event_collect,
                                     sizeof(int),
    ...
    for (int i = 0; i < MAX_NETS; i++) {
        if (do_config_nets[i]) {
            ...
            if (g_st_ev_trace || g_st_model_stats ||
                g_st_use_analysis_lps) // for ROSS event tracing
            {
...
static void base_read_config(const char* anno, model_net_base_params* p) {
    ...
    ret = configuration_get_value(&config, "PARAMS", "modelnet_scheduler", anno, sched,
                                  MAX_NAME_LENGTH);
    ...
    p->nic_seq_delay = 10;
    ret =
        configuration_get_value_double(&config, "PARAMS", "nic_seq_delay", anno, &p->nic_seq_delay);
    if (ret && !g_tw_mynode) {
        fprintf(stdout,
    ...
    if (p->sched_params.type == MN_SCHED_PRIO) {
        ...
        ret =
            configuration_get_value_int(&config, "PARAMS", "prio-sched-num-prios", anno, num_prios);
        if (ret != 0)
            *num_prios = 10;

        ret = configuration_get_value(&config, "PARAMS", "prio-sched-sub-sched", anno, sched,
                                      MAX_NAME_LENGTH);
        if (ret <= 0)
...
void model_net_base_configure() {
    ...
    msg_offsets[DRAGONFLY_ROUTER] = offsetof(model_net_wrap_msg, msg.m_dfly);
    msg_offsets[DRAGONFLY_CUSTOM] = offsetof(model_net_wrap_msg, msg.m_custom_dfly);
    msg_offsets[DRAGONFLY_CUSTOM_ROUTER] = offsetof(model_net_wrap_msg, msg.m_custom_dfly);
    msg_offsets[DRAGONFLY_PLUS] = offsetof(model_net_wrap_msg, msg.m_dfly_plus);
    msg_offsets[DRAGONFLY_PLUS_ROUTER] = offsetof(model_net_wrap_msg, msg.m_dfly_plus);
    msg_offsets[DRAGONFLY_DALLY] = offsetof(model_net_wrap_msg, msg.m_dally_dfly);
    msg_offsets[DRAGONFLY_DALLY_ROUTER] = offsetof(model_net_wrap_msg, msg.m_dally_dfly);
    msg_offsets[SLIMFLY] = offsetof(model_net_wrap_msg, msg.m_slim);
    msg_offsets[SLIMFLY_ROUTER] = offsetof(model_net_wrap_msg, msg.m_slim);
    msg_offsets[FATTREE] = offsetof(model_net_wrap_msg, msg.m_fat);
    msg_offsets[LOGGP] = offsetof(model_net_wrap_msg, msg.m_loggp);
    msg_offsets[EXPRESS_MESH] = offsetof(model_net_wrap_msg, msg.m_em);
    msg_offsets[EXPRESS_MESH_ROUTER] = offsetof(model_net_wrap_msg, msg.m_em);
...
void model_net_base_event(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m, tw_lp* lp) {
    ...
    if (!is_freezing_on && m->h.event_type == MN_BASE_SCHED_NEXT &&
        m->msg.m_base.created_in_surrogate) {
        return;
    }
...
void handle_new_msg(model_net_base_state* ns, tw_bf* b, model_net_wrap_msg* m, tw_lp* lp) {
    ...
    if (num_servers == -1) {
        ...
        codes_mapping_get_lp_info2(r->src_lp, &sender_group, &sender_lpname, NULL, &rep_id,
                                   &offset);
...
void model_net_method_send_msg_recv_event(
    ...
    const mn_sched_params* sched_params, const char* category, int net_id, void* msg,
    tw_stime offset, tw_lp* sender) {
    ...
    for (int cid = 0; cid < lpconf.num_uniq_lptypes; cid++) {
        ...
        for (int n = 0; n < MAX_NETS; n++) {
            if (strcmp(lp_name, model_net_lp_config_names[n]) == 0) {
                ...
                for (int lid = 0;
                     lid < codes_mapping_get_lp_count("MODELNET_GRP", 0, lp_name, NULL, 1); lid++) {
                    tw_lpid lpgid =
                        codes_mapping_get_lpid_from_relative(lid, NULL, lp_name, NULL, 0);
...
static void save_state_net_state(model_net_base_state* into, model_net_base_state const* from) {
    ...
    if (from->params->num_queues > 0) {
        model_net_sched* sched_send_array =
            malloc(from->params->num_queues * sizeof(model_net_sched));
        for (int i = 0; i < from->params->num_queues; i++) {
...
static bool check_model_net_state(model_net_base_state* before, model_net_base_state* after) {
    ...
    if (chptr && before->sub_state != NULL && chptr->check_lps) {
        ...
        fprintf(stderr,
                "Warning: Network of type \"%s\" has not been fully configured to be checkpointed "
                "(Running this model under SEQUENTIAL_ROLLBACK_CHECK won't capture any issues that "
                "arise from the reverse event handlers).\n",
    ...
    for (int i = 0; i < before->params->node_copy_queues; i++) {
        is_same &=
            before->node_copy_next_available_time[i] == after->node_copy_next_available_time[i];
    }

    return is_same;
}

static void __print_model_net(FILE* out, char const* prefix, model_net_base_state* state,
                              bool is_lp_state) {
    ...
    void (*print_modelnet)(FILE*, char const*, model_net_sched*) =
        is_lp_state ? print_model_net_sched : print_model_net_sched_checkpoint;

    fprintf(out, "%s  |              net_id = %d\n", prefix, state->net_id);
    fprintf(out, "%s  |     nics_per_router = %d\n", prefix, state->nics_per_router);
    fprintf(out, "%s  | *in_sched_send_loop[%d] = [", prefix,
            state->params->num_queues); // deep-all
    for (int i = 0; i < state->params->num_queues; i++) {
        fprintf(out, "%d%s", state->in_sched_send_loop[i],
                i == state->params->num_queues - 1 ? "" : ", ");
    }
    fprintf(out, "]\n");
    ...
    fprintf(out, "%s  | *node_copy_next_available_time[%d] = [", prefix,
            state->params->num_queues); // (done) deep-all
    for (int i = 0; i < state->params->node_copy_queues; i++) {
        fprintf(out, "%g%s", state->node_copy_next_available_time[i],
                i == state->params->node_copy_queues - 1 ? "" : ", ");
    }
    fprintf(out, "]\n");
...
static char const* const event_type_string(enum model_net_base_event_type type) {
    switch (type) {
    case MN_BASE_NEW_MSG:
        return "MN_BASE_NEW_MSG";
    case MN_BASE_SCHED_NEXT:
        return "MN_BASE_SCHED_NEXT";
    case MN_BASE_SAMPLE:
        return "MN_BASE_SAMPLE";
    case MN_BASE_PASS:
        return "MN_BASE_PASS";
    case MN_BASE_END_NOTIF:
        return "MN_BASE_END_NOTIF";
    case MN_CONGESTION_EVENT:
        return "MN_CONGESTION_EVENT";
    }
    return "UNKNOWN TYPE!!";
}
...
static void print_event_state(FILE* out, char const* prefix, model_net_base_state* state,
                              model_net_wrap_msg* msg) {
    fprintf(out, "%sh\n", prefix);
    fprintf(out, "%s| src = %lu\n", prefix, msg->h.src);
    fprintf(out, "%s| event_type = %d (%s)\n", prefix, msg->h.event_type,
            event_type_string(msg->h.event_type));
    ...
    case MN_BASE_END_NOTIF:
        ...
        if (chptr && chptr->print_event) {
            ...
            fprintf(out,
                    "%s    | == cannot print the submessage (event print function not yet defined "
                    "for network of type %s) ==\n",
...
void model_net_method_call_inner(tw_lp* lp, void (*fun)(void* inner, tw_lp* lp, void* data),
                                 void* data) {
    model_net_base_state* const ns = (model_net_base_state*)lp->cur_state;
