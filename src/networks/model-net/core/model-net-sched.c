...
#define X(a, b, c, d) b,
char* sched_names[] = {SCHEDULER_TYPES};
#undef X
...
void model_net_sched_add(const model_net_request* req, const mn_sched_params* sched_params,
                         ...
                         void* local_event, model_net_sched* sched, model_net_sched_rc* sched_rc,
                         tw_lp* lp) {
    sched->impl->add(req, sched_params, remote_event_size, remote_event, local_event_size,
                     local_event, sched->dat, sched_rc, lp);
}

void model_net_sched_add_rc(model_net_sched* sched, const model_net_sched_rc* sched_rc, tw_lp* lp) {
...
bool check_model_net_sched(model_net_sched* before, model_net_sched* after) {
    ...
    tw_error(TW_LOC, "Scheduler of type \"%s\" has not been configured to be checkpointed",
             sched_names[before->type]);
    return false;
}

static void __print_model_net_sched(FILE* out, char const* prefix, model_net_sched* sched,
                                    bool is_lp_state) {
