...
enum ROUTING_ALGO { MINIMAL = 1, NON_MINIMAL, ADAPTIVE, PROG_ADAPTIVE };
...
void custom_dragonfly_event_collect(terminal_custom_message* m, tw_lp* lp, char* buffer,
                                    int* collect_flag);
...
static void ross_custom_dragonfly_rsample_fn(router_state* s, tw_bf* bf, tw_lp* lp,
                                             struct dfly_router_sample* sample);
static void ross_custom_dragonfly_rsample_rc_fn(router_state* s, tw_bf* bf, tw_lp* lp,
                                                struct dfly_router_sample* sample);
static void ross_custom_dragonfly_sample_fn(terminal_state* s, tw_bf* bf, tw_lp* lp,
                                            struct dfly_cn_sample* sample);
static void ross_custom_dragonfly_sample_rc_fn(terminal_state* s, tw_bf* bf, tw_lp* lp,
                                               struct dfly_cn_sample* sample);

st_model_types custom_dragonfly_model_types[] = {
    {(ev_trace_f)custom_dragonfly_event_collect, sizeof(int),
     ...
     (sample_revent_f)ross_custom_dragonfly_sample_rc_fn, sizeof(struct dfly_cn_sample)},
    {(ev_trace_f)custom_dragonfly_event_collect, sizeof(int),
...
static void append_to_terminal_custom_message_list(terminal_custom_message_list** thisq,
                                                   terminal_custom_message_list** thistail,
                                                   int index, terminal_custom_message_list* msg) {
    if (thisq[index] == NULL) {
        thisq[index] = msg;
    } else {
...
static void prepend_to_terminal_custom_message_list(terminal_custom_message_list** thisq,
                                                    terminal_custom_message_list** thistail,
                                                    int index, terminal_custom_message_list* msg) {
    if (thisq[index] == NULL) {
        thistail[index] = msg;
...
static terminal_custom_message_list* return_head(terminal_custom_message_list** thisq,
                                                 terminal_custom_message_list** thistail,
                                                 int index) {
...
static terminal_custom_message_list* return_tail(terminal_custom_message_list** thisq,
                                                 terminal_custom_message_list** thistail,
                                                 int index) {
...
static void dragonfly_read_config(const char* anno, dragonfly_param* params) {
    ...
    rc = configuration_get_value_int(&config, "PARAMS", "num_router_rows", anno,
                                     &p->num_router_rows);
    if (rc) {
        printf("\n Number of router rows not specified, setting to 6 ");
        p->num_router_rows = 6;
    }
    rc = configuration_get_value_int(&config, "PARAMS", "num_router_cols", anno,
                                     &p->num_router_cols);
    if (rc) {
    ...
    if (!myRank) {
        printf("\n Total nodes %d routers %d groups %d routers per group %d radix %d\n",
               p->num_cn * p->total_routers, p->total_routers, p->num_groups, p->num_routers,
               p->radix);
    }
...
void terminal_custom_init(terminal_state* s, tw_lp* lp) {
    s->packet_gen = 0;
    s->packet_fin = 0;
...
void router_custom_setup(router_state* r, tw_lp* lp) {
    ...
    num_routers_per_mgrp =
        codes_mapping_get_lp_count(lp_group_name, 1, "modelnet_dragonfly_custom_router", NULL, 0);
    ...
    for (int i = 0; i < p->radix; i++) {
        ...
        r->pending_msgs[i] = (terminal_custom_message_list**)malloc(
            p->num_vcs * sizeof(terminal_custom_message_list*));
        r->last_buf_full[i] = (tw_stime*)malloc(p->num_vcs * sizeof(tw_stime));
        r->pending_msgs_tail[i] = (terminal_custom_message_list**)malloc(
            p->num_vcs * sizeof(terminal_custom_message_list*));
        r->queued_msgs[i] = (terminal_custom_message_list**)malloc(
            p->num_vcs * sizeof(terminal_custom_message_list*));
        r->queued_msgs_tail[i] = (terminal_custom_message_list**)malloc(
            p->num_vcs * sizeof(terminal_custom_message_list*));
...
static tw_stime dragonfly_custom_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender,
                                       DRAGONFLY_CUSTOM, (void**)&msg, (void**)&tmp_ptr);
...
static void router_credit_send(router_state* s, terminal_custom_message* msg, tw_lp* lp, int sq) {
    tw_event* buf_e;
    tw_stime ts;
    ...
    if (is_terminal) {
        buf_e = model_net_method_event_new(dest, ts, lp, DRAGONFLY_CUSTOM, (void**)&buf_msg, NULL);
        buf_msg->magic = terminal_magic_num;
    } else {
        buf_e = model_net_method_event_new(dest, ts, lp, DRAGONFLY_CUSTOM_ROUTER, (void**)&buf_msg,
                                           NULL);
...
static void packet_generate_rc(terminal_state* s, tw_bf* bf, terminal_custom_message* msg,
                               tw_lp* lp) {
    s->packet_gen--;
    packet_gen--;
    s->packet_counter--;
    ...
    for (i = 0; i < num_chunks; i++) {
        delete_terminal_custom_message_list(
            return_tail(s->terminal_msgs, s->terminal_msgs_tail, 0));
...
static void packet_generate(terminal_state* s, tw_bf* bf, terminal_custom_message* msg, tw_lp* lp) {
    packet_gen++;
    s->packet_gen++;
    ...
    int dest_router_id =
        codes_mapping_get_lp_relative_id(msg->dest_terminal_id, 0, 0) / s->params->num_cn;
    ...
    for (int i = 0; i < num_chunks; i++) {
        terminal_custom_message_list* cur_chunk =
            (terminal_custom_message_list*)malloc(sizeof(terminal_custom_message_list));
        ...
        append_to_terminal_custom_message_list(s->terminal_msgs, s->terminal_msgs_tail, 0,
                                               cur_chunk);
        s->terminal_length += s->params->chunk_size;
    }

    if (s->terminal_length <
        2 * s->params->cn_vc_size) { //TODO This hardcoded 2 * s->params->cn_vc_size seems dubious
    ...
    if (s->in_send_loop == 0) {
        ...
        tw_event* e =
            model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_CUSTOM, (void**)&m, NULL);
        m->type = T_SEND;
    ...
    total_event_size = model_net_get_msg_sz(DRAGONFLY_CUSTOM) + msg->remote_event_size_bytes +
                       msg->local_event_size_bytes;
    mn_stats* stat;
    stat = model_net_find_stats(msg->category, s->dragonfly_stats_array);
    stat->send_count++;
...
static void packet_send_rc(terminal_state* s, tw_bf* bf, terminal_custom_message* msg, tw_lp* lp) {
    if (bf->c10)
        s->last_buf_full[0] = msg->saved_busy_time;
    ...
    prepend_to_terminal_custom_message_list(s->terminal_msgs, s->terminal_msgs_tail, 0, cur_entry);
    if (bf->c3) {
        tw_rand_reverse_unif(lp->rng);
    }
    if (bf->c4) {
...
static void packet_send(terminal_state* s, tw_bf* bf, terminal_custom_message* msg, tw_lp* lp) {
    tw_stime ts;
    tw_event* e;
    terminal_custom_message* m;
    tw_lpid router_id;
    ...
    void* remote_event;
    e = model_net_method_event_new(router_id, ts, lp, DRAGONFLY_CUSTOM_ROUTER, (void**)&m,
                                   &remote_event);
    ...
    if (cur_entry != NULL && s->vc_occupancy[0] + s->params->chunk_size <= s->params->cn_vc_size) {
        ...
        e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_CUSTOM, (void**)&m_new, NULL);
        m_new->type = T_SEND;
...
static void packet_arrive_rc(terminal_state* s, tw_bf* bf, terminal_custom_message* msg,
                             tw_lp* lp) {
    if (bf->c31) {
        s->packet_fin--;
        packet_fin--;
...
static void send_remote_event(terminal_state* s, terminal_custom_message* msg, tw_lp* lp, tw_bf* bf,
...
static void packet_arrive(terminal_state* s, tw_bf* bf, terminal_custom_message* msg, tw_lp* lp) {
    ...
    tw_event* buf_e;
    terminal_custom_message* buf_msg;
    buf_e = model_net_method_event_new(msg->intm_lp_id, ts, lp, DRAGONFLY_CUSTOM_ROUTER,
                                       (void**)&buf_msg, NULL);
    buf_msg->magic = router_magic_num;
    buf_msg->vc_index = msg->vc_index;
    buf_msg->output_chan = msg->output_chan;
    ...
    * them */
    ...
    if (!tmp) {
        ...
        struct dfly_qhash_entry* d_entry =
            (dfly_qhash_entry*)malloc(sizeof(struct dfly_qhash_entry));
...
void dragonfly_custom_rsample_init(router_state* s, tw_lp* lp) {
    (void)lp;
    int i = 0;
    const dragonfly_param* p = s->params;
...
void dragonfly_custom_rsample_rc_fn(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                                    tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
...
void dragonfly_custom_rsample_fn(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                                 tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
    ...
    if (s->op_arr_size >= s->max_arr_size) {
        struct dfly_router_sample* tmp = (dfly_router_sample*)malloc(
            (MAX_STATS + s->max_arr_size) * sizeof(struct dfly_router_sample));
...
void dragonfly_custom_rsample_fin(router_state* s, tw_lp* lp) {
    (void)lp;
    const dragonfly_param* p = s->params;
    ...
    if (s->router_id == 0) {
        ...
        fprintf(fp,
                ...
                "(router-router same column) channels \n %d global (router-router remote group)"
                " channels \n %d terminal channels",
                p->num_router_cols * p->num_row_chans, p->num_router_rows * p->num_col_chans,
                p->num_global_channels, p->num_cn);
        fclose(fp);
    }
...
void dragonfly_custom_sample_init(terminal_state* s, tw_lp* lp) {
    (void)lp;
...
void dragonfly_custom_sample_rc_fn(terminal_state* s, tw_bf* bf, terminal_custom_message* msg,
                                   tw_lp* lp) {
    (void)lp;
    (void)bf;
    (void)msg;
...
void dragonfly_custom_sample_fn(terminal_state* s, tw_bf* bf, terminal_custom_message* msg,
                                tw_lp* lp) {
    (void)lp;
    (void)msg;
    (void)bf;
...
void dragonfly_custom_sample_fin(terminal_state* s, tw_lp* lp) {
    (void)lp;
...
static void terminal_buf_update_rc(terminal_state* s, tw_bf* bf, terminal_custom_message* msg,
                                   tw_lp* lp) {
    s->vc_occupancy[0] += s->params->chunk_size;
    codes_local_latency_reverse(lp);
    if (bf->c1) {
...
static void terminal_buf_update(terminal_state* s, tw_bf* bf, terminal_custom_message* msg,
                                tw_lp* lp) {
    bf->c1 = 0;
    bf->c2 = 0;
    bf->c3 = 0;
...
void terminal_custom_event(terminal_state* s, tw_bf* bf, terminal_custom_message* msg, tw_lp* lp) {
    s->fwd_events++;
...
void dragonfly_custom_terminal_final(terminal_state* s, tw_lp* lp) {
    ...
    if (s->terminal_id == 0) {
        ...
        fprintf(fp, "# Format <LP id> <Terminal ID> <Total Data Size> <Avg packet latency> <# "
                    "Flits/Packets finished> <Avg hops> <Busy Time> <Max packet Latency> <Min "
                    "packet Latency >\n");
    }
    int written = 0;

    written +=
        sprintf(s->output_buf + written, "%llu %u %llu %lf %ld %lf %lf %lf %lf\n", LLU(lp->gid),
...
void dragonfly_custom_router_final(router_state* s, tw_lp* lp) {
...
static tw_lpid get_next_stop(router_state* s, tw_lp* lp, tw_bf* bf, terminal_custom_message* msg,
                             int dest_router_id, int adap_chan, int do_chan_selection) {
    int dest_lp;
    ...
    if (s->group_id == dest_group_id) {
        ...
        vector<int> next_stop =
            get_intra_router(s, local_router_id, dest_router_id, s->params->num_routers);
    ...
    * routing has just decided to take a non-minimal route. */
    if (msg->last_hop == TERMINAL || s->router_id == msg->intm_rtr_id ||
        ...
        if (adap_chan >= 0)
            ...
            bf->c19 = 1;
            select_chan =
                tw_rand_integer(lp->rng, 0, connectionList[my_grp_id][dest_group_id].size() - 1);
        }
   ...
   * groups */
    assert(msg->saved_src_chan >= 0 &&
           msg->saved_src_chan < connectionList[my_grp_id][dest_group_id].size());
    ...
    if (s->router_id == msg->saved_src_dest) {
        ...
        bf->c21 = 1;
        vector<int> dests =
            get_intra_router(s, local_router_id, msg->saved_src_dest, s->params->num_routers);
        assert(!dests.empty());
...
static int get_output_port(router_state* s, terminal_custom_message* msg, tw_lp* lp, tw_bf* bf,
                           int next_stop) {
    int output_port = -1;
    int rand_offset = -1;
    ...
    if ((tw_lpid)next_stop == msg->dest_terminal_id) {
        ...
        if (intm_grp_id != s->group_id) {
            ...
            rand_offset =
                tw_rand_integer(lp->rng, 0, interGroupLinks[src_router][intm_grp_id].size() - 1);
            ...
            if (src_col == dest_col) {
                ...
                output_port =
                    p->num_router_cols * p->num_row_chans + dest_row * p->num_col_chans + offset;
                ...
                tw_error(TW_LOC,
                         "\n Invalid dragonfly connectivity src row %d dest row %d src col %d dest "
                         "col %d src %d dest %d",
...
static void do_local_adaptive_routing(router_state* s, tw_lp* lp, terminal_custom_message* msg,
                                      tw_bf* bf, int dest_router_id, int intm_router_id) {
    ...
    vector<int> next_min_stops =
        get_intra_router(s, s->router_id, dest_router_id, s->params->num_routers);
...
static int do_global_adaptive_routing(router_state* s, tw_lp* lp, terminal_custom_message* msg,
                                      tw_bf* bf, int dest_router_id, int intm_id) {
    int next_chan = -1;
...
static void router_packet_receive_rc(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                                     tw_lp* lp) {
    router_rev_ecount++;
    router_ecount--;
    ...
    if (bf->c2) {
        tw_rand_reverse_unif(lp->rng);
        terminal_custom_message_list* tail =
...
static void router_packet_receive(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                                  tw_lp* lp) {
    router_ecount++;
    ...
    terminal_custom_message_list* cur_chunk =
        (terminal_custom_message_list*)calloc(sizeof(terminal_custom_message_list), 1);
    init_terminal_custom_message_list(cur_chunk, msg);
    ...
    if (src_grp_id != dest_grp_id) {
        ...
        intm_router_id = (src_grp_id * s->params->num_routers) +
                         (((s->router_id % s->params->num_routers) +
                           tw_rand_integer(lp->rng, 1, s->params->num_routers - 1)) %
   ...
   * global channel. It doesn't make sense to use it within a group */
    if (dest_grp_id != src_grp_id &&
        ((cur_chunk->msg.last_hop == TERMINAL && routing == ADAPTIVE) ||
         (cur_chunk->msg.path_type == MINIMAL && routing == PROG_ADAPTIVE &&
          s->group_id == src_grp_id))) {
        bf->c20 = 1;
        adap_chan = do_global_adaptive_routing(s, lp, &(cur_chunk->msg), bf, dest_router_id,
                                               intm_router_id);
    }
   ...
   * triggered */
    ...
    if (dest_grp_id == src_grp_id && dest_router_id != s->router_id &&
        (routing == ADAPTIVE || routing == PROG_ADAPTIVE) && cur_chunk->msg.last_hop == TERMINAL) {
        if (DF_DALLY == 0) {
            bf->c6 = 1;
    ...
    if (cur_chunk->msg.path_type != MINIMAL && cur_chunk->msg.path_type != NON_MINIMAL)
        tw_error(TW_LOC, "\n packet src %d dest %d intm %d src grp %d dest grp %d", s->router_id,
                 dest_router_id, intm_router_id, src_grp_id, dest_grp_id);
    ...
    if ((cur_chunk->msg.last_hop == TERMINAL ||
         (routing == PROG_ADAPTIVE && s->group_id == src_grp_id &&
          prev_path_type != next_path_type)) &&
   ...
   * routing, we have to compute the next stop based on that */
    ...
    next_stop =
        get_next_stop(s, lp, bf, &(cur_chunk->msg), dest_router_id, adap_chan, do_chan_selection);
    ...
    if (output_chan >= s->params->num_vcs || output_chan < 0)
        printf("\n Packet %llu Output chan %d output port %d my rid %d dest rid %d path %d my gid "
               "%d dest gid %d origin terminal id %d ",
               cur_chunk->msg.packet_ID, output_chan, output_port, s->router_id, dest_router_id,
               cur_chunk->msg.path_type, src_grp_id, dest_grp_id, msg->src_terminal_id);
...
static void router_packet_send_rc(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                                  tw_lp* lp) {
    router_ecount--;
    router_rev_ecount++;
...
static void router_packet_send(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                               tw_lp* lp) {
    router_ecount++;
   ...
   * next.*/
    ...
    if (cur_entry != NULL) {
        ...
        e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_CUSTOM_ROUTER, (void**)&m_new,
                                       NULL);
        m_new->type = R_SEND;
...
static void router_buf_update_rc(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                                 tw_lp* lp) {
    ...
    if (bf->c1) {
        terminal_custom_message_list* head =
            return_tail(s->pending_msgs[indx], s->pending_msgs_tail[indx], output_chan);
        tw_rand_reverse_unif(lp->rng);
        prepend_to_terminal_custom_message_list(s->queued_msgs[indx], s->queued_msgs_tail[indx],
                                                output_chan, head);
        s->vc_occupancy[indx][output_chan] -= s->params->chunk_size;
...
static void router_buf_update(router_state* s, tw_bf* bf, terminal_custom_message* msg, tw_lp* lp) {
    ...
    if (s->last_buf_full[indx][output_chan] > 0.0) {
        ...
        if (s->params->counting_bool > 0 && msg->last_bufupdate_time >= s->params->counting_start) {
            ...
            if (current_window < s->params->counting_windows) {
                int full_window =
                    (int)((s->last_buf_full[indx][output_chan] - s->params->counting_start) /
                          s->params->counting_interval);
                if (full_window == current_window) {
                    s->agg_busy_time[current_window][indx] +=
                        (tw_now(lp) - s->last_buf_full[indx][output_chan]);
                } else {
                    s->agg_busy_time[current_window][indx] +=
                    ...
                    s->agg_busy_time[full_window][indx] +=
                        ((s->params->counting_start +
                          current_window * s->params->counting_interval) -
                         s->last_buf_full[indx][output_chan]);
                }
            }
        }
    ...
    if (s->queued_msgs[indx][output_chan] != NULL) {
        ...
        append_to_terminal_custom_message_list(s->pending_msgs[indx], s->pending_msgs_tail[indx],
                                               output_chan, head);
        s->vc_occupancy[indx][output_chan] += s->params->chunk_size;
        s->queued_count[indx] -= s->params->chunk_size;
    }
    if (s->in_send_loop[indx] == 0 && s->pending_msgs[indx][output_chan] != NULL) {
        ...
        tw_stime ts = codes_local_latency(lp);
        tw_event* e =
            model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_CUSTOM_ROUTER, (void**)&m, NULL);
        m->type = R_SEND;
        m->vc_index = indx;
        m->magic = router_magic_num;
...
void router_custom_event(router_state* s, tw_bf* bf, terminal_custom_message* msg, tw_lp* lp) {
    s->fwd_events++;
...
void terminal_custom_rc_event_handler(terminal_state* s, tw_bf* bf, terminal_custom_message* msg,
                                      tw_lp* lp) {
    s->rev_events++;
...
void router_custom_rc_event_handler(router_state* s, tw_bf* bf, terminal_custom_message* msg,
                                    tw_lp* lp) {
    s->rev_events++;
...
tw_lptype dragonfly_custom_lps[] = {
    ...
    {(init_f)terminal_custom_init, (pre_run_f)NULL, (event_f)terminal_custom_event,
     (revent_f)terminal_custom_rc_event_handler, (commit_f)NULL,
     (final_f)dragonfly_custom_terminal_final, (map_f)codes_mapping, sizeof(terminal_state)},
    {
...
static double dragonfly_custom_get_router_link_bandwidth(void* topo, router_id_t r1,
                                                         router_id_t r2) {
...
static void dragonfly_custom_get_router_neighbor_list(void* topo, router_id_t r,
                                                      router_id_t* neighbors) {
...
static int dragonfly_custom_get_router_location(void* topo, router_id_t r, int32_t* location,
                                                int size) {
    // TODO
...
static int dragonfly_custom_get_compute_node_location(void* topo, cn_id_t node, int32_t* location,
                                                      int size) {
    // TODO
...
static void dragonfly_custom_get_router_compute_node_list(void* topo, router_id_t r,
                                                          cn_id_t* nodes) {
