...
void
terminal_plus_init( terminal_state * s,
	       tw_lp * lp )
{
    s->packet_gen = 0;
    s->packet_fin = 0;
...
void router_plus_setup(router_state * r, tw_lp * lp)
{
   ...
   for(int i=0; i < p->radix; i++)
    ...
    r->pending_msgs[i] = (terminal_plus_message_list**)malloc(p->num_vcs *
        sizeof(terminal_plus_message_list*));
    r->last_buf_full[i] = (tw_stime*)malloc(p->num_vcs * sizeof(tw_stime));
    r->pending_msgs_tail[i] = (terminal_plus_message_list**)malloc(p->num_vcs *
        sizeof(terminal_plus_message_list*));
    r->queued_msgs[i] = (terminal_plus_message_list**)malloc(p->num_vcs *
        sizeof(terminal_plus_message_list*));
    r->queued_msgs_tail[i] = (terminal_plus_message_list**)malloc(p->num_vcs *
        sizeof(terminal_plus_message_list*));
        ...
        int is_last_pckt)
{
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time+offset,
            sender, DRAGONFLY_PLUS, (void**)&msg, (void**)&tmp_ptr);
...
static void router_credit_send(router_state * s, terminal_plus_message * msg,
  ...
  if (is_terminal) {
    buf_e = model_net_method_event_new(dest, ts, lp, DRAGONFLY_PLUS,
      (void**)&buf_msg, NULL);
    buf_msg->magic = terminal_magic_num;
  } else {
    buf_e = model_net_method_event_new(dest, ts, lp, DRAGONFLY_PLUS_ROUTER,
            (void**)&buf_msg, NULL);
...
static void packet_generate_rc(terminal_state * s, tw_bf * bf, terminal_plus_message * msg, tw_lp * lp)
{
   ...
   for(i = 0; i < num_chunks; i++) {
        delete_terminal_plus_message_list(return_tail(s->terminal_msgs,
...
static void packet_generate(terminal_state * s, tw_bf * bf, terminal_plus_message * msg,
  tw_lp * lp) {
  packet_gen++;
  s->packet_gen++;
  ...
  {
    terminal_plus_message_list *cur_chunk = (terminal_plus_message_list*)malloc(
      sizeof(terminal_plus_message_list));
    ...
    append_to_terminal_plus_message_list(s->terminal_msgs, s->terminal_msgs_tail,
      0, cur_chunk);
    s->terminal_length += s->params->chunk_size;
  }
  ...
  if(s->in_send_loop == 0) {
    ...
    tw_event* e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_PLUS,
      (void**)&m, NULL);
    m->type = T_SEND;
  ...
  total_event_size = model_net_get_msg_sz(DRAGONFLY_PLUS) +
      msg->remote_event_size_bytes + msg->local_event_size_bytes;
  mn_stats* stat;
  stat = model_net_find_stats(msg->category, s->dragonfly_stats_array);
  stat->send_count++;
...
static void packet_send_rc(terminal_state * s, tw_bf * bf, terminal_plus_message * msg,
        tw_lp * lp)
{
      if(bf->c1) {
        s->in_send_loop = 1;
...
static void packet_send(terminal_state * s, tw_bf * bf, terminal_plus_message * msg,
  tw_lp * lp) {
...
//  printf("\n Local router id %d global router id %d ", s->router_id, router_id);
  ...
  e = model_net_method_event_new(router_id, ts, lp,
          DRAGONFLY_PLUS_ROUTER, (void**)&m, &remote_event);
  ...
  if(cur_entry != NULL &&
    ...
    e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_PLUS,
      (void**)&m_new, NULL);
    m_new->type = T_SEND;
...
static void send_remote_event(terminal_state * s, terminal_plus_message * msg, tw_lp * lp, tw_bf * bf, char * event_data, int remote_event_size)
{
...
static void packet_arrive(terminal_state * s, tw_bf * bf, terminal_plus_message * msg,
  tw_lp * lp) {
  ...
  buf_e = model_net_method_event_new(msg->intm_lp_id, ts, lp,
          DRAGONFLY_PLUS_ROUTER, (void**)&buf_msg, NULL);
  buf_msg->magic = router_magic_num;
  buf_msg->vc_index = msg->vc_index;
  buf_msg->output_chan = msg->output_chan;
...
void dragonfly_plus_rsample_init(router_state * s,
        tw_lp * lp)
{
   (void)lp;
   int i = 0;
...
void dragonfly_plus_rsample_rc_fn(router_state * s,
        tw_bf * bf,
        terminal_plus_message * msg,
        tw_lp * lp)
{
...
void dragonfly_plus_rsample_fn(router_state * s,
        tw_bf * bf,
        terminal_plus_message * msg,
        tw_lp * lp)
{
...
void dragonfly_plus_rsample_fin(router_state * s,
        tw_lp * lp)
{
    (void)lp;
    const dragonfly_plus_param * p = s->params;
...
void dragonfly_plus_sample_init(terminal_state * s,
        tw_lp * lp)
{
    (void)lp;
...
void dragonfly_plus_sample_rc_fn(terminal_state * s,
        tw_bf * bf,
        terminal_plus_message * msg,
        tw_lp * lp)
{
...
void dragonfly_plus_sample_fn(terminal_state * s,
        tw_bf * bf,
        terminal_plus_message * msg,
        tw_lp * lp)
{
...
void dragonfly_plus_sample_fin(terminal_state * s,
        tw_lp * lp)
{
    (void)lp;
...
void
terminal_plus_event( terminal_state * s,
		tw_bf * bf,
		terminal_plus_message * msg,
		tw_lp * lp )
{
...
void
dragonfly_plus_terminal_final( terminal_state * s,
      tw_lp * lp )
{
...
void dragonfly_plus_router_final(router_state * s,
		tw_lp * lp)
{
        ...
        tw_lp * lp)
{
    ...
    if(bf->c2) {
        tw_rand_reverse_unif(lp->rng);
        terminal_plus_message_list * tail = return_tail(s->pending_msgs[output_port], s->pending_msgs_tail[output_port], output_chan);
			...
			tw_lp * lp )
{
  ...
  terminal_plus_message_list * cur_chunk = (terminal_plus_message_list*)malloc(sizeof(terminal_plus_message_list));
  init_terminal_plus_message_list(cur_chunk, msg);
		     ...
		     terminal_plus_message * msg, tw_lp * lp)
{
  ...
  if(cur_entry != NULL) {
    ...
    e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_PLUS_ROUTER,
            (void**)&m_new, NULL);
    m_new->type = R_SEND;
        ...
        tw_lp * lp)
{
      ...
      if(bf->c1) {
        terminal_plus_message_list* head = return_tail(s->pending_msgs[indx],
            s->pending_msgs_tail[indx], output_chan);
        tw_rand_reverse_unif(lp->rng);
        prepend_to_terminal_plus_message_list(s->queued_msgs[indx],
...
static void router_buf_update(router_state * s, tw_bf * bf, terminal_plus_message * msg, tw_lp * lp)
{
  ...
  if(s->queued_msgs[indx][output_chan] != NULL) {
    bf->c1 = 1;
    terminal_plus_message_list *head = return_head(s->queued_msgs[indx],
        s->queued_msgs_tail[indx], output_chan);
    router_credit_send(s, &head->msg, lp, 1);
    append_to_terminal_plus_message_list(s->pending_msgs[indx],
...
void router_plus_event(router_state * s, tw_bf * bf, terminal_plus_message * msg,
    tw_lp * lp) {
  s->fwd_events++;
  rc_stack_gc(lp, s->st);
...
void terminal_plus_rc_event_handler(terminal_state * s, tw_bf * bf,
    terminal_plus_message * msg, tw_lp * lp) {
...
void router_plus_rc_event_handler(router_state * s, tw_bf * bf,
  terminal_plus_message * msg, tw_lp * lp) {
    s->rev_events++;
