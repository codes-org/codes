...
static void init_terminal_message_list(terminal_message_list* this, terminal_message* inmsg) {
    this->msg = *inmsg;
...
enum ROUTING_ALGO { MINIMAL = 0, NON_MINIMAL, ADAPTIVE, PROG_ADAPTIVE };
...
static void ross_dragonfly_rsample_fn(router_state* s, tw_bf* bf, tw_lp* lp,
                                      struct dfly_router_sample* sample);
static void ross_dragonfly_rsample_rc_fn(router_state* s, tw_bf* bf, tw_lp* lp,
                                         struct dfly_router_sample* sample);
static void ross_dragonfly_sample_fn(terminal_state* s, tw_bf* bf, tw_lp* lp,
                                     struct dfly_cn_sample* sample);
static void ross_dragonfly_sample_rc_fn(terminal_state* s, tw_bf* bf, tw_lp* lp,
                                        struct dfly_cn_sample* sample);

st_model_types dragonfly_model_types[] = {
    {(ev_trace_f)dragonfly_event_collect, sizeof(int), (model_stat_f)dragonfly_model_stat_collect,
     sizeof(tw_lpid) + sizeof(long) * 2 + sizeof(double) + sizeof(tw_stime) * 2,
     (sample_event_f)ross_dragonfly_sample_fn, (sample_revent_f)ross_dragonfly_sample_rc_fn,
     sizeof(struct dfly_cn_sample)},
    {(ev_trace_f)dragonfly_event_collect, sizeof(int), (model_stat_f)dfly_router_model_stat_collect,
     0, //updated in router_setup() since it's based on the radix
     (sample_event_f)ross_dragonfly_rsample_fn, (sample_revent_f)ross_dragonfly_rsample_rc_fn,
     0}, //updated in router_setup() since it's based on the radix
...
static void append_to_terminal_message_list(terminal_message_list** thisq,
                                            terminal_message_list** thistail, int index,
...
static void prepend_to_terminal_message_list(terminal_message_list** thisq,
                                             terminal_message_list** thistail, int index,
                                             terminal_message_list* msg) {
    if (thisq[index] == NULL) {
        thistail[index] = msg;
...
static terminal_message_list* return_head(terminal_message_list** thisq,
                                          terminal_message_list** thistail, int index) {
...
static terminal_message_list* return_tail(terminal_message_list** thisq,
                                          terminal_message_list** thistail, int index) {
...
static void dragonfly_read_config(const char* anno, dragonfly_param* params) {
    ...
    int rc = configuration_get_value_int(&config, "PARAMS", "num_routers", anno, &p->num_routers);
    if (rc) {
        p->num_routers = 4;
    ...
    if (!rank) {
        printf("\n Total nodes %d routers %d groups %d radix %d \n", p->num_cn * p->total_routers,
               p->total_routers, p->num_groups, p->radix);
    }
...
static void dragonfly_collective_init(terminal_state* s, tw_lp* lp) {
    // TODO: be annotation-aware
    codes_mapping_get_lp_info(lp->gid, lp_group_name, &mapping_grp_id, NULL, &mapping_type_id, NULL,
                              &mapping_rep_id, &mapping_offset);
    int num_lps = codes_mapping_get_lp_count(lp_group_name, 1, LP_CONFIG_NM_TERM, NULL, 1);
...
static void terminal_init(terminal_state* s, tw_lp* lp) {
    ...
    s->rank_tbl = NULL;
    s->terminal_msgs = (terminal_message_list**)malloc(1 * sizeof(terminal_message_list*));
    s->terminal_msgs_tail = (terminal_message_list**)malloc(1 * sizeof(terminal_message_list*));
...
static void router_setup(router_state* r, tw_lp* lp) {
    ...
    num_routers_per_mgrp = codes_mapping_get_lp_count(lp_group_name, 1, LP_METHOD_NM_ROUT, NULL, 0);
    ...
    */
    ...
    r->pending_msgs = (terminal_message_list***)malloc(p->radix * sizeof(terminal_message_list**));
    r->pending_msgs_tail =
        (terminal_message_list***)malloc(p->radix * sizeof(terminal_message_list**));
    r->queued_msgs = (terminal_message_list***)malloc(p->radix * sizeof(terminal_message_list**));
    r->queued_msgs_tail =
        (terminal_message_list***)malloc(p->radix * sizeof(terminal_message_list**));
    ...
    for (int i = 0; i < p->radix; i++) {
        ...
        r->pending_msgs[i] =
            (terminal_message_list**)malloc(p->num_vcs * sizeof(terminal_message_list*));
...
//   printf("\n LP ID %d VC occupancy radix %d Router %d is connected to ", lp->gid, p->radix, r->router_id);
    ...
    for (int i = 0; i < p->num_global_channels; i++) {
        if (i % 2 != 0) {
            r->global_channel[i] =
                (r->router_id + (router_offset * p->num_routers)) % p->total_routers;
            router_offset++;
        } else {
...
static tw_stime dragonfly_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender, DRAGONFLY,
...
tw_lpid getRouterFromGroupID(int dest_gid, int src_gid, int num_routers, int total_groups) {
#if USE_DIRECT_SCHEME
    int dest = dest_gid;
...
static void router_credit_send(router_state* s, terminal_message* msg, tw_lp* lp, int sq) {
    tw_event* buf_e;
    tw_stime ts;
    ...
    if (is_terminal) {
        buf_e = model_net_method_event_new(dest, ts, lp, DRAGONFLY, (void**)&buf_msg, NULL);
        buf_msg->magic = terminal_magic_num;
    } else {
        buf_e = model_net_method_event_new(dest, ts, lp, DRAGONFLY_ROUTER, (void**)&buf_msg, NULL);
...
static void packet_generate_rc(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    ...
    for (i = 0; i < num_chunks; i++) {
        delete_terminal_message_list(return_tail(s->terminal_msgs, s->terminal_msgs_tail, 0));
...
static void packet_generate(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    packet_gen++;
    s->packet_gen++;
    ...
    for (uint64_t i = 0; i < num_chunks; i++) {
        terminal_message_list* cur_chunk =
            (terminal_message_list*)malloc(sizeof(terminal_message_list));
    ...
    if (s->in_send_loop == 0) {
        ...
        terminal_message* m;
        tw_event* e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY, (void**)&m, NULL);
        m->type = T_SEND;
    ...
    total_event_size = model_net_get_msg_sz(DRAGONFLY) + msg->remote_event_size_bytes +
                       msg->local_event_size_bytes;
    mn_stats* stat;
    stat = model_net_find_stats(msg->category, s->dragonfly_stats_array);
    stat->send_count++;
...
static void packet_send_rc(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    if (bf->c1) {
        s->in_send_loop = 1;
        if (bf->c10)
            s->last_buf_full[0] = msg->saved_busy_time;
    ...
    prepend_to_terminal_message_list(s->terminal_msgs, s->terminal_msgs_tail, 0, cur_entry);
    if (bf->c3) {
        tw_rand_reverse_unif(lp->rng);
    }
    if (bf->c4) {
...
static void packet_send(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    tw_stime ts;
    tw_event* e;
    terminal_message* m;
    tw_lpid router_id;
    ...
    codes_mapping_get_lp_info(lp->gid, lp_group_name, &mapping_grp_id, NULL, &mapping_type_id, NULL,
                              &mapping_rep_id, &mapping_offset);
    codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, NULL, 1, s->router_id, 0, &router_id);
    // we are sending an event to the router, so no method_event here
    void* remote_event;
    e = model_net_method_event_new(router_id, ts, lp, DRAGONFLY_ROUTER, (void**)&m, &remote_event);
    ...
    if (cur_entry != NULL && s->vc_occupancy[0] + s->params->chunk_size <= s->params->cn_vc_size) {
        ...
        e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY, (void**)&m_new, NULL);
        m_new->type = T_SEND;
...
static void send_remote_event(terminal_state* s, terminal_message* msg, tw_lp* lp, tw_bf* bf,
...
static void packet_arrive(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    ...
    tw_event* buf_e;
    terminal_message* buf_msg;
    buf_e = model_net_method_event_new(msg->intm_lp_id, ts, lp, DRAGONFLY_ROUTER, (void**)&buf_msg,
                                       NULL);
    buf_msg->magic = router_magic_num;
    buf_msg->vc_index = msg->vc_index;
    buf_msg->output_chan = msg->output_chan;
...
static void dragonfly_collective(char const* category, int message_size, int remote_event_size,
    ...
    codes_mapping_get_lp_info(sender->gid, lp_group_name, &mapping_grp_id, NULL, &mapping_type_id,
                              NULL, &mapping_rep_id, &mapping_offset);
    codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_TERM, NULL, 1, mapping_rep_id,
                            mapping_offset, &local_nic_id);

    xfer_to_nic_time = codes_local_latency(sender);
    e_new = model_net_method_event_new(local_nic_id, xfer_to_nic_time, sender, DRAGONFLY,
                                       (void**)&msg, (void**)&tmp_ptr);
...
static void send_collective_remote_event(terminal_state* s, terminal_message* msg, tw_lp* lp) {
    // Trigger an event on receiving server
    if (msg->remote_event_size_bytes) {
        tw_event* e;
        tw_stime ts;
...
static void node_collective_init(terminal_state* s, terminal_message* msg, tw_lp* lp) {
    tw_event* e_new;
    ...
    if (s->is_leaf) {
        ...
        codes_mapping_get_lp_info(lp->gid, lp_group_name, &mapping_grp_id, NULL, &mapping_type_id,
                                  NULL, &mapping_rep_id, &mapping_offset);
        num_lps = codes_mapping_get_lp_count(lp_group_name, 1, LP_CONFIG_NM_TERM, s->anno, 0);
        ...
        void* m_data;
        e_new = model_net_method_event_new(parent_nic_id, xfer_to_nic_time, lp, DRAGONFLY,
                                           (void**)&msg_new, (void**)&m_data);
...
static void node_collective_fan_in(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    int i;
    s->num_fan_nodes++;
    ...
    if (s->is_root && (s->num_fan_nodes == s->num_children)) {
        ...
        for (i = 0; i < s->num_children; i++) {
            ...
            xfer_to_nic_time = g_tw_lookahead + COLLECTIVE_COMPUTATION_DELAY + LEVEL_DELAY +
                               tw_rand_exponential(lp->rng, (double)LEVEL_DELAY / 50);
            ...
            //msg_new = tw_event_data(e_new);
            void* m_data;
            e_new = model_net_method_event_new(child_nic_id, xfer_to_nic_time, lp, DRAGONFLY,
                                               (void**)&msg_new, &m_data);
...
static void node_collective_fan_out(terminal_state* s, tw_bf* bf, terminal_message* msg,
                                    tw_lp* lp) {
    int i;
    int num_lps = codes_mapping_get_lp_count(lp_group_name, 1, LP_CONFIG_NM_TERM, NULL, 1);
    bf->c1 = 0;
    ...
    if (!s->is_leaf) {
        ...
        for (i = 0; i < s->num_children; i++) {
            xfer_to_nic_time = g_tw_lookahead + DRAGONFLY_FAN_OUT_DELAY +
                               tw_rand_exponential(lp->rng, (double)DRAGONFLY_FAN_OUT_DELAY / 10);
...
static void dragonfly_rsample_init(router_state* s, tw_lp* lp) {
    (void)lp;
    int i = 0;
    const dragonfly_param* p = s->params;
...
static void dragonfly_rsample_rc_fn(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
...
static void dragonfly_rsample_fn(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
    ...
    if (s->op_arr_size >= s->max_arr_size) {
        struct dfly_router_sample* tmp =
            malloc((MAX_STATS + s->max_arr_size) * sizeof(struct dfly_router_sample));
...
static void dragonfly_rsample_fin(router_state* s, tw_lp* lp) {
    (void)lp;
    const dragonfly_param* p = s->params;
    ...
    if (s->router_id == 0) {
        ...
        fprintf(fp,
                ...
                "(router-router remote group)"
                " channels \n%d terminal channels",
                p->radix / 2, p->radix / 4, p->radix / 4);
        fclose(fp);
    }
...
static void dragonfly_sample_init(terminal_state* s, tw_lp* lp) {
    (void)lp;
...
static void dragonfly_sample_rc_fn(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    (void)lp;
    (void)bf;
    (void)msg;
...
static void dragonfly_sample_fn(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    (void)lp;
    (void)msg;
    (void)bf;
    ...
    if (s->op_arr_size >= s->max_arr_size) {
        ...
        struct dfly_cn_sample* tmp =
            malloc((MAX_STATS + s->max_arr_size) * sizeof(struct dfly_cn_sample));
...
static void dragonfly_sample_fin(terminal_state* s, tw_lp* lp) {
    (void)lp;
...
static void terminal_buf_update_rc(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    (void)msg;
    s->vc_occupancy[0] += s->params->chunk_size;
    codes_local_latency_reverse(lp);
    if (bf->c1) {
...
static void terminal_buf_update(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    (void)msg;
...
static void terminal_event(terminal_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    s->fwd_events++;
...
static void dragonfly_terminal_final(terminal_state* s, tw_lp* lp) {
    ...
    if (s->terminal_id == 0) {
        ...
        FILE* fp = fopen(meta_fname, "w");
        fprintf(fp, "# Format <LP id> <Terminal ID> <Total Data Size> <Aggregate packet latency> "
    ...
    if (!s->terminal_id)
        written = sprintf(s->output_buf,
                          "# Format <LP id> <Terminal ID> <Total Data Size> <Aggregate packet "
                          "latency> <# Flits/Packets finished> <Avg hops> <Busy Time>");

    written +=
        sprintf(s->output_buf + written, "\n %llu %u %" PRId64 " %lf %ld %lf %lf", LLU(lp->gid),
                s->terminal_id, s->total_msg_size, (double)s->total_time / s->finished_packets,
...
static void dragonfly_router_final(router_state* s, tw_lp* lp) {
    //rc_stack_gc(lp, s->st);
...
int get_num_hops(int local_router_id, int dest_router_id, int num_routers, int total_groups) {
    ...
    int src_connecting_router =
        getRouterFromGroupID(dest_group_id, local_grp_id, num_routers, total_groups);

    if (src_connecting_router == local_router_id)
        num_hops--;

    int dest_connecting_router =
        getRouterFromGroupID(local_grp_id, dest_group_id, num_routers, total_groups);
...
tw_lpid get_next_stop(router_state* s, terminal_message* msg, int path, int dest_router_id,
                      int intm_id) {
    int dest_lp;
    ...
    if (s->group_id == dest_group_id) {
        codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, s->anno, 0, dest_router_id, 0,
                                &router_dest_id);
    ...
    if (path == NON_MINIMAL && msg->intm_group_id >= 0 && (s->group_id == origin_grp_id)) {
    ...
    dest_lp = getRouterFromGroupID(dest_group_id, s->group_id, s->params->num_routers,
                                   s->params->num_groups);
    ...
    codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, s->anno, 0, dest_lp, 0,
                            &router_dest_id);

    return router_dest_id;
}
/* gets the output port corresponding to the next stop of the message */
int get_output_port(router_state* s, terminal_message* msg, int next_stop) {
    ...
    if ((tw_lpid)next_stop == msg->dest_terminal_id) {
        ...
        codes_mapping_get_lp_info(next_stop, lp_group_name, &mapping_grp_id, NULL, &mapping_type_id,
                                  NULL, &mapping_rep_id, &mapping_offset);
        ...
        if (intm_grp_id != s->group_id) {
            ...
            output_port = s->params->num_routers + (target_grp) / s->params->num_routers;
#else
            for (int i = 0; i < s->params->num_global_channels; i++) {
...
static int do_adaptive_routing(router_state* s, terminal_message* msg, int dest_router_id,
                               int intm_id) {
    int next_stop;
...
TODO: do we need this code? nomin_vc and min_vc not used anywhere...
  ...
  if(minimal_out_port < s->params->num_routers) {
    ...
    int min_port_count = s->vc_occupancy[minimal_out_port][0] +
                         s->vc_occupancy[minimal_out_port][1] +
                         s->vc_occupancy[minimal_out_port][2] + s->queued_count[minimal_out_port];
    ...
    int intm_router_id =
        getRouterFromGroupID(msg->intm_group_id, s->router_id / s->params->num_routers,
                             s->params->num_routers, s->params->num_groups);

    int num_nonmin_hops = 6;

    if (msg->intm_group_id >= 0) {
        num_nonmin_hops = get_num_hops(s->router_id, intm_router_id, s->params->num_routers,
                                       s->params->num_groups) +
    ...
    for (i = 0; i < s->params->radix; i++) {
        if (i != minimal_out_port)
            q_avg += s->vc_occupancy[i][0] + s->vc_occupancy[i][1] + s->vc_occupancy[i][2];
    }
   ...
   * window number and an average of the previous history window number. */
    ...
    int nonmin_port_count = s->vc_occupancy[nonmin_out_port][0] +
                            s->vc_occupancy[nonmin_out_port][1] +
                            s->vc_occupancy[nonmin_out_port][2] + s->queued_count[nonmin_out_port];
    // VARIATION 1:
...
static void router_packet_receive_rc(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    router_rev_ecount++;
    router_ecount--;
    ...
    if (bf->c2) {
        tw_rand_reverse_unif(lp->rng);
        terminal_message_list* tail = return_tail(s->pending_msgs[output_port],
...
static void router_packet_receive(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    router_ecount++;
    ...
    codes_mapping_get_lp_info(msg->dest_terminal_id, lp_group_name, &mapping_grp_id, NULL,
                              &mapping_type_id, NULL, &mapping_rep_id, &mapping_offset);
    int num_lps = codes_mapping_get_lp_count(lp_group_name, 1, LP_CONFIG_NM_TERM, s->anno, 0);
    int dest_router_id = (mapping_offset + (mapping_rep_id * num_lps)) / s->params->num_cn;
   ...
   * non-minimal path, it does not check any longer. */
    ...
    if (routing == PROG_ADAPTIVE && msg->path_type != NON_MINIMAL &&
        ...
        next_stop =
            get_next_stop(s, &(cur_chunk->msg), cur_chunk->msg.path_type, dest_router_id, intm_id);
    }
    ...
    if (msg->packet_ID == LLU(TRACK_PKT))
        printf("\n Router packet %llu arrived lp id %llu final dest %llu output port %d ",
               msg->packet_ID, LLU(lp->gid), LLU(msg->dest_terminal_id), output_port);
    ...
    } else if (output_port < (s->params->num_routers + s->params->num_global_channels)) {
    ...
    if (s->vc_occupancy[output_port][output_chan] + s->params->chunk_size <= max_vc_size) {
        ...
        if (s->in_send_loop[output_port] == 0) {
            ...
            ts = codes_local_latency(lp);
            tw_event* e =
                model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_ROUTER, (void**)&m, NULL);
            m->type = R_SEND;
            m->magic = router_magic_num;
...
static void router_packet_send_rc(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    router_ecount--;
    router_rev_ecount++;
...
static void router_packet_send(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    router_ecount++;
   ...
   * next.*/
    ...
    if (cur_entry != NULL) {
        ...
        e = model_net_method_event_new(lp->gid, ts, lp, DRAGONFLY_ROUTER, (void**)&m_new, NULL);
        m_new->type = R_SEND;
...
static void router_buf_update_rc(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
...
static void router_event(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    s->fwd_events++;
...
static void terminal_rc_event_handler(terminal_state* s, tw_bf* bf, terminal_message* msg,
                                      tw_lp* lp) {
    s->rev_events++;
...
static void router_rc_event_handler(router_state* s, tw_bf* bf, terminal_message* msg, tw_lp* lp) {
    s->rev_events++;
...
tw_lptype dragonfly_lps[] = {
    ...
    {(init_f)terminal_init, (pre_run_f)NULL, (event_f)terminal_event,
     (revent_f)terminal_rc_event_handler, (commit_f)NULL, (final_f)dragonfly_terminal_final,
...
static int dragonfly_get_compute_node_location(void* topo, cn_id_t node, int32_t* location,
                                               int size) {
    // TODO
    (void)topo;
    (void)node;
