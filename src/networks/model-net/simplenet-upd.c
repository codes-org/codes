...
tw_lptype sn_lp = {
    (init_f)sn_init, (pre_run_f)NULL,      (event_f)sn_event,    (revent_f)sn_rev_event,
    (commit_f)NULL,  (final_f)sn_finalize, (map_f)codes_mapping, sizeof(sn_state),
};
...
static void handle_msg_start_event(sn_state* ns, sn_message* m, tw_lp* lp) {
    ...
    void* m_data;
    e_new = model_net_method_event_new(m->dest_mn_lp, send_queue_time, lp, SIMPLENET,
                                       (void**)&m_new, &m_data);
...
static tw_stime simplenet_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender, SIMPLENET,
...
static void sn_configure() {
    ...
    for (int i = 0; i < anno_map->num_annos; i++) {
        ...
        int rc;
        rc = configuration_get_value_double(&config, "PARAMS", "net_startup_ns", anno,
                                            &all_params[i].net_startup_ns);
        if (rc != 0) {
            tw_error(TW_LOC, "simplenet: unable to read PARAMS:net_startup_ns@%s", anno);
        }
        rc = configuration_get_value_double(&config, "PARAMS", "net_bw_mbps", anno,
                                            &all_params[i].net_bw_mbps);
        if (rc != 0) {
            tw_error(TW_LOC, "simplenet: unable to read PARAMS:net_bw_mbps@%s", anno);
        }
    }
    if (anno_map->has_unanno_lp > 0) {
        int rc;
        rc = configuration_get_value_double(&config, "PARAMS", "net_startup_ns", NULL,
                                            &all_params[num_params - 1].net_startup_ns);
        if (rc != 0) {
            tw_error(TW_LOC, "simplenet: unable to read PARAMS:net_startup_ns");
        }
        rc = configuration_get_value_double(&config, "PARAMS", "net_bw_mbps", NULL,
                                            &all_params[num_params - 1].net_bw_mbps);
        if (rc != 0) {
