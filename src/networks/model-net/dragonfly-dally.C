...
#define indexer2d(_ptr, _x, _y, _maxx, _maxy) ((_ptr) + _y * (_maxx) + _x)
...
static std::vector<std::string> dfdally_event_time_director_request_with_latency(
    const char* operation, const std::vector<std::string>& args, const std::string& bindata) {
    const char* op = operation ? operation : "";
    std::vector<std::string> ret;

    char label[256];
    snprintf(label, sizeof(label), "family=event-time backend=dragonfly-dally operation=%s", op);
...
static void dfdally_event_time_print_global_cache_summary_once(void) {
    ...
    if (mpi_initialized) {
        ...
        MPI_Reduce(local_counts, global_counts, 3, MPI_UNSIGNED_LONG_LONG, MPI_SUM, 0,
                   MPI_COMM_CODES);
    } else {
        for (int i = 0; i < 3; i++) {
...
static void dragonfly_dally_terminal_pre_surrogate_switch_event_queue(terminal_state* s, tw_lp* lp,
                                                                      tw_event* event);
...
typedef enum conn_minimality_t { C_MIN = 1, C_NONMIN } conn_minimality_t;
...
static char* get_routing_alg_chararray(int routing_alg_int) {
    ...
    case SMART_NON_MINIMAL:
        ...
        break;
    default:
        tw_error(TW_LOC, "Routing Algorithm is UNDEFINED - did you call get_routing_alg_string() "
                         "before setting the static global variable: 'routing'?");
        break;
    }
    return rt_alg;
}
...
struct packet_sent {
    ...
    double
        next_packet_delay; // When the packet is initially sent, this value is -1, when the next packet is sent this value is updated to the actual delay to process the next packet
    void*
        message_data; // Yep, we have to save the entire message just because we might need to resend the message when switching to surrogate-mode. It's wasteful but there is no other way
    void*
        remote_event_data; // This and the one above have to be freed. This contains the extra information that the message contains
    void*
        local_data; // This and the one above have to be freed. This contains the extra information that the message contains
};
...
bool operator<(struct packet_id const& lk, struct packet_id const& rk) {
    return lk.packet_ID == rk.packet_ID ? lk.dfdally_src_terminal_id < rk.dfdally_src_terminal_id
                                        : lk.packet_ID < rk.packet_ID;
}
bool operator==(struct packet_id const& lk, struct packet_id const& rk) {
    return lk.packet_ID == rk.packet_ID && lk.dfdally_src_terminal_id == rk.dfdally_src_terminal_id;
}
// Some more function declarations
static void notify_dest_lp_of(terminal_state* s, tw_lp* lp, terminal_dally_message* msg,
...
struct terminal_state {
    ...
    unsigned long*
        stalled_chunks; //Counter for when a packet cannot be immediately routed due to full VC - [rail_id]
...
struct router_state {
    ...
    unsigned long*
        stalled_chunks; //Counter for when a packet is put into queued messages instead of routing due to full VC
...
void custom_dally_dragonfly_event_collect(terminal_dally_message* m, tw_lp* lp, char* buffer,
                                          int* collect_flag);
...
void dragonfly_dally_rsample_init(router_state* s, tw_lp* lp) {
    (void)lp;
    int i = 0;
    const dragonfly_param* p = s->params;
...
void dragonfly_dally_rsample_rc_fn(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                   tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
...
void dragonfly_dally_rsample_fn(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
    ...
    if (s->op_arr_size >= s->max_arr_size) {
        struct dfly_router_sample* tmp =
            (dfly_router_sample*)calloc((MAX_STATS + s->max_arr_size),
...
//TODO redo this
void dragonfly_dally_rsample_fin(router_state* s, tw_lp* lp) {
    (void)lp;
    const dragonfly_param* p = s->params;
...
void dragonfly_dally_sample_init(terminal_state* s, tw_lp* lp) {
    (void)lp;
    ...
    for (int i = 0; i < s->max_arr_size; i++) {
        s->sample_stat[i].busy_time_sample =
            (tw_stime*)calloc(s->params->num_rails, sizeof(tw_stime));
    }
}
void dragonfly_dally_sample_rc_fn(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                  tw_lp* lp) {
    (void)lp;
    (void)bf;
    (void)msg;
...
void dragonfly_dally_sample_fn(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                               tw_lp* lp) {
    (void)lp;
    (void)msg;
    (void)bf;
...
void dragonfly_dally_sample_fin(terminal_state* s, tw_lp* lp) {
    (void)lp;
...
static Connection dfdally_minimal_routing(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                          tw_lp* lp, int fdest_router_id);
static Connection dfdally_nonminimal_routing(router_state* s, tw_bf* bf,
                                             terminal_dally_message* msg, tw_lp* lp,
                                             int fdest_router_id);
static Connection dfdally_prog_adaptive_routing(router_state* s, tw_bf* bf,
                                                terminal_dally_message* msg, tw_lp* lp,
                                                int fdest_router_id);
static Connection dfdally_prog_adaptive_legacy_routing(router_state* s, tw_bf* bf,
                                                       terminal_dally_message* msg, tw_lp* lp,
                                                       int fdest_router_id);
static Connection dfdally_smart_minimal_routing(router_state* s, tw_bf* bf,
                                                terminal_dally_message* msg, tw_lp* lp,
                                                int fdest_router_id);
static Connection dfdally_smart_prog_adaptive_routing(router_state* s, tw_bf* bf,
                                                      terminal_dally_message* msg, tw_lp* lp,
                                                      int fdest_router_id);
static Connection dfdally_smart_minimal_routing(router_state* s, tw_bf* bf,
                                                terminal_dally_message* msg, tw_lp* lp,
                                                int fdest_router_id);
static Connection dfdally_smart_nonminimal_routing(router_state* s, tw_bf* bf,
                                                   terminal_dally_message* msg, tw_lp* lp,
                                                   int fdest_router_id);

/*Routing Helper Declarations*/
static void dfdally_select_intermediate_group(router_state* s, tw_bf* bf,
                                              terminal_dally_message* msg, tw_lp* lp,
                                              int fdest_router_id);
static vector<Connection> get_legal_minimal_stops(router_state* s, tw_bf* bf,
                                                  terminal_dally_message* msg, tw_lp* lp,
                                                  int fdest_router_id);
static set<Connection> get_smart_legal_minimal_stops(router_state* s, tw_bf* bf,
                                                     terminal_dally_message* msg, tw_lp* lp,
                                                     int fdest_router_id,
                                                     int max_global_hops_in_path);
...
static int dfdally_get_assigned_router_id_from_terminal(const dragonfly_param* params, int term_gid,
    ...
    if (num_planes ==
        1) //then all rails go to the same router //TODO: this could change - could be cool!
    {
        if (num_rails == 1) //default behavior
            return term_gid / num_cn_per_router;
        else {
            return term_gid /
                   num_cn_per_router; // all rails go to same router - perhaps change this
        }
    } else {
...
static int dfdally_score_connection(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                    tw_lp* lp, Connection conn, conn_minimality_t c_minimality) {
    int score = 0;
    int port = conn.port;
...
static Connection get_absolute_best_connection_from_conns(router_state* s, tw_bf* bf,
                                                          terminal_dally_message* msg, tw_lp* lp,
                                                          vector<Connection> conns) {
    if (conns.size() ==
        0) { //passed no connections to this but we got to return something - return negative filled conn to force a break if not caught
        Connection bad_conn;
        bad_conn.src_gid = -1;
        bad_conn.port = -1;
        return bad_conn;
...
//Now returns random selection from tied best connections.
static Connection get_absolute_best_connection_from_conn_set(router_state* s, tw_bf* bf,
                                                             terminal_dally_message* msg, tw_lp* lp,
...
static Connection dfdally_get_best_from_k_connection_set(router_state* s, tw_bf* bf,
                                                         terminal_dally_message* msg, tw_lp* lp,
                                                         set<Connection> conns, int k) {
    if (conns.size() == 0) {
        Connection bad_conn;
        bad_conn.src_gid = -1;
        bad_conn.port = -1;
...
static vector<Connection> dfdally_poll_k_connections(router_state* s, tw_bf* bf,
                                                     terminal_dally_message* msg, tw_lp* lp,
                                                     vector<Connection> conns, int k) {
    vector<Connection> k_conns;
    if (conns.size() == 0) {
        return k_conns;
    }
    ...
    for (int i = 0; i < k; i++) {
        ...
        int attempt_offset =
            (last_sel + rand_int) %
            conns
                .size(); //get a hopefully unused index - this method of sampling without replacement results in only about
...
// consider an optimization to implement an efficient shuffle to poll k random sampling instead
// consider an optimization for the default of 2
static Connection dfdally_get_best_from_k_connections(router_state* s, tw_bf* bf,
                                                      terminal_dally_message* msg, tw_lp* lp,
                                                      vector<Connection> conns, int k) {
...
static void copy_terminal_dally_message_qlist(struct qlist_head* into_head,
                                              struct qlist_head* from_head) {
    ...
    qlist_for_each_entry(from_entry, from_head, list) {
        terminal_dally_message_list* copy_entry =
            (terminal_dally_message_list*)malloc(sizeof(terminal_dally_message_list));
        ...
        if (from_entry->event_data != NULL) {
            int event_data_sz =
                from_entry->msg.remote_event_size_bytes + from_entry->msg.local_event_size_bytes;
...
static bool check_terminal_dally_message_qlist(struct qlist_head* before,
                                               struct qlist_head* after) {
    bool is_same = true;
    ...
    while (pos_before != before && pos_after != after) {
        ...
        if (entry_before->event_data != NULL && entry_after->event_data != NULL) {
            int const message_size = entry_before->msg.remote_event_size_bytes +
                                     entry_before->msg.local_event_size_bytes;
            int const message_size_after =
                entry_after->msg.remote_event_size_bytes + entry_after->msg.local_event_size_bytes;
            is_same &= (message_size == message_size_after);
            if (message_size > 0) {
                is_same &=
                    (memcmp(entry_before->event_data, entry_after->event_data, message_size) == 0);
            }
        }
...
static void copy_msgs_qlist(struct qlist_head* into_qlist, struct qlist_head* from_qlist) {
    ...
    qlist_for_each(pos, from_qlist) {
        terminal_dally_message_list* from_entry =
            qlist_entry(pos, terminal_dally_message_list, list);
        terminal_dally_message_list* copy_entry =
            (terminal_dally_message_list*)malloc(sizeof(terminal_dally_message_list));
        ...
        if (from_entry->event_data != NULL) {
            ...
            memcpy(copy_entry->event_data, from_entry->event_data,
                   from_entry->msg.remote_event_size_bytes);
        }
...
static bool check_msgs_qlist(struct qlist_head* before, struct qlist_head* after) {
    ...
    while (pos_before != before && pos_after != after) {
        terminal_dally_message_list* entry_before =
            qlist_entry(pos_before, terminal_dally_message_list, list);
        terminal_dally_message_list* entry_after =
            qlist_entry(pos_after, terminal_dally_message_list, list);

        is_same &= check_terminal_dally_message(&entry_before->msg, &entry_after->msg);
        is_same &= (entry_before->event_data == NULL) == (entry_after->event_data == NULL);

        int const message_size =
            entry_before->msg.remote_event_size_bytes + entry_before->msg.local_event_size_bytes;
        int const message_size_after =
            entry_after->msg.remote_event_size_bytes + entry_after->msg.local_event_size_bytes;
        is_same &= message_size == message_size_after;
...
static void print_terminal_dally_message_qlist(FILE* out, char const* prefix, terminal_state* ns,
...
static void dragonfly_read_config(const char* anno, dragonfly_param* params) {
    ...
    rc = configuration_get_value_double(&config, "PARAMS", "local_bandwidth", anno,
                                        &p->local_bandwidth);
    if (rc) {
    ...
    rc = configuration_get_value_double(&config, "PARAMS", "global_bandwidth", anno,
                                        &p->global_bandwidth);
    if (rc) {
    ...
    rc = configuration_get_value(&config, "PARAMS", "qos_bandwidth", anno, qos_levels_str,
                                 MAX_NAME_LENGTH);
    ...
    rc = configuration_get_value_int(&config, "PARAMS", "adaptive_threshold", anno,
                                     &p->adaptive_threshold);
    if (rc) {
        if (!myRank)
            fprintf(
                stderr,
                "Adaptive Minimal Routing Threshold not specified: setting to default = 0. (Will "
                "consider minimal and nonminimal routes based on scoring metric alone)\n");
    ...
    rc = configuration_get_value_int(&config, "PARAMS", "num_injection_queues", anno,
                                     &p->num_injection_queues);
    if (rc)
    ...
    if (rc) {
        ...
        if (!myRank)
            fprintf(stderr,
                    "global_k_picks for global adaptive routing not specified, setting to %d\n",
                    p->global_k_picks);
    }

    char scoring_str[MAX_NAME_LENGTH] = {'\0'};
    configuration_get_value(&config, "PARAMS", "route_scoring_metric", anno, scoring_str,
                            MAX_NAME_LENGTH);
    if (strcmp(scoring_str, "alpha") == 0) {
        ...
        tw_error(
            TW_LOC,
            "Gamma scoring protocol currently non-functional"); //TODO: Fix gamma scoring protocol
        scoring = GAMMA;
    ...
    rc = configuration_get_value_int(&config, "PARAMS", "notification_on_hops_greater_than", anno,
                                     &p->max_hops_notify);
    if (rc) {
        if (!myRank)
    ...
    if (p->counting_bool) {
        int rc1 = configuration_get_value_double(&config, "PARAMS", "counting_start", anno,
                                                 &p->counting_start);
        int rc2 = configuration_get_value_int(&config, "PARAMS", "counting_windows", anno,
                                              &p->counting_windows);
        int rc3 = configuration_get_value_double(&config, "PARAMS", "counting_interval", anno,
                                                 &p->counting_interval);
        int rc4 = configuration_get_value_int(&config, "PARAMS", "num_apps", anno, &p->num_apps);
        if (rc1 || rc2 || rc3 || rc4)
            tw_error(TW_LOC, "\n Missing couting values, "
                             "(counting_start/windows/interval/num_apps) check for config files\n");
    ...
    rc = configuration_get_value_int(&config, "PARAMS", "num_global_channels", anno,
                                     &p->num_global_channels);
    if (rc) {
        if (!myRank)
    ...
    rc = configuration_get_value_int(&config, "PARAMS", "num_parallel_switch_conns", anno,
                                     &p->num_parallel_switch_conns);
    if (rc) {
    ...
    p->cn_radix =
        (p->num_cn * p->num_rails) /
        p->num_planes; //number of CNs per router times number of rails taht each CN has, divided by how many planes those CNs are shared across
    ...
    netMan = DragonflyNetworkManager(p->total_routers, p->total_terminals, p->num_routers,
                                     p->intra_grp_radix, p->global_radix, p->cn_radix, p->num_rails,
                                     p->num_planes, max_hops_per_group, max_global_hops_nonminimal);
    ...
    configuration_get_value(&config, "PARAMS", "intra-group-connections", anno, intraFile,
                            MAX_NAME_LENGTH);
    ...
    while (fread(&newLink, sizeof(IntraGroupLink), 1, groupFile) != 0) {
        ...
        for (int i = 0; i < p->total_routers;
             i++) //handles all routers in network, including multi planar
        {
    ...
    configuration_get_value(&config, "PARAMS", "inter-group-connections", anno, interFile,
                            MAX_NAME_LENGTH);
    ...
    vector<vector<vector<int>>>
        connectionListEnumerated; //this one will have one final item for EACH link, not just to show existence of some unknown amount of links
    ...
    while (fread(&newInterLink, sizeof(InterGroupLink), 1, systemFile) != 0) {
        for (int i = 0; i < p->num_planes; i++) {
            ...
            if (i == 0) //only need to do this once, not every plane
            {
                ...
                connectionListEnumerated[read_src_group_id][read_dest_group_id].push_back(
                    newInterLink.src);

                int r;
                for (r = 0; r < connectionList[read_src_group_id][read_dest_group_id].size(); r++) {
                    if (connectionList[read_src_group_id][read_dest_group_id][r] ==
                        newInterLink.src)
                        break;
                }
                if (r == connectionList[read_src_group_id][read_dest_group_id].size()) {
                    connectionList[read_src_group_id][read_dest_group_id].push_back(
                        newInterLink.src);
                }
            }
        }
    ...
    if (strlen(g_nm_link_failure_filepath) ==
        0) //was this defined already via a command line argument?
    {
        configuration_get_value(&config, "PARAMS", "link-failure-file", anno, failureFileName,
                                MAX_NAME_LENGTH);
    ...
    netMan
        .solidify_network(); //finalize link failures and burn the network links into the connection managers
    ...
    if (!myRank) {
        fprintf(stderr, "\n Total nodes %d routers %d groups %d routers per group %d radix %d\n\n",
                p->num_cn * p->total_routers, p->total_routers, p->total_groups, p->num_routers,
                p->radix);
    }
    ...
    int credit_delay_unset = configuration_get_value_double(&config, "PARAMS", "credit_delay", anno,
                                                            &general_credit_delay);
    int local_credit_delay_unset =
        configuration_get_value_double(&config, "PARAMS", "local_credit_delay", anno,
                                       &p->local_credit_delay);
    int global_credit_delay_unset =
        configuration_get_value_double(&config, "PARAMS", "global_credit_delay", anno,
                                       &p->global_credit_delay);
    int cn_credit_delay_unset = configuration_get_value_double(&config, "PARAMS", "cn_credit_delay",
                                                               anno, &p->cn_credit_delay);

    int auto_credit_delay_flag;
    rc = configuration_get_value_int(&config, "PARAMS", "auto_credit_delay", anno,
                                     &auto_credit_delay_flag);
    if (rc) {
        auto_credit_delay_flag = 0;
    } else {
        if (!myRank && auto_credit_delay_flag)
            fprintf(stderr, "auto_credit_delay flag enabled. All credit delays will be calculated "
    ...
    if (!credit_delay_unset &&
        !(local_credit_delay_unset || global_credit_delay_unset || cn_credit_delay_unset))
        tw_error(TW_LOC, "\nCannot set both a general credit delay and specific (local/global/cn) "
                         "credit delays. Check configuration file.");
    ...
    if ((!credit_delay_unset || !local_credit_delay_unset || !global_credit_delay_unset ||
         !cn_credit_delay_unset) &&
        auto_credit_delay_flag)
        tw_error(TW_LOC, "\nCannot set both a credit delay (general or specific) and also enable "
                         "auto credit delay calculation. Check Configuration file.");
    ...
    if (credit_delay_unset && local_credit_delay_unset && global_credit_delay_unset &&
        cn_credit_delay_unset && !auto_credit_delay_flag) {
    ...
    //If the user doesn't specify a general credit delay but leaves any of the specific credit delay values unset, then we need to set those (the above conditional handles if none of them had been set)
        ...
        if (local_credit_delay_unset) {
            p->local_credit_delay = bytes_to_ns(p->credit_size, p->local_bandwidth);
            if (!myRank &&
                !auto_credit_delay_flag) //if the auto credit delay flag is true then we've already printed what we're going to do
                fprintf(stderr,
                        "local_credit_delay not specified, using calculation based on local "
                        "bandwidth: %.2f\n",
                        p->local_credit_delay);
        }
        if (global_credit_delay_unset) {
            ...
            if (!myRank && !auto_credit_delay_flag)
                fprintf(stderr,
                        "global_credit_delay not specified, using calculation based on global "
                        "bandwidth: %.2f\n",
                        p->global_credit_delay);
        }
        if (cn_credit_delay_unset) {
            ...
            if (!myRank && !auto_credit_delay_flag)
                fprintf(stderr,
                        "cn_credit_delay not specified, using calculation based on cn bandwidth: "
                        "%.2f\n",
                        p->cn_credit_delay);
        }
    }
    ...
    // CONGESTION CONTROL
    int cc_enabled;
    rc = configuration_get_value_int(&config, "PARAMS", "congestion_control_enabled", anno,
                                     &cc_enabled);
    if (rc) {
        if (!myRank)
    ...
    if (OUTPUT_SNAPSHOT) {
        char** timestamps;
        size_t len;
        configuration_get_multivalue(&config, "PARAMS", "router_buffer_snapshots", anno,
                                     &timestamps, &len);
        if (len) {
            ...
            for (size_t i = 0; i < len; i++) {
                ...
                if (errno == ERANGE || errno == EILSEQ) {
                    tw_error(TW_LOC,
                             "Sequence `%s' could not be succesfully interpreted as a _double_.",
                             timestamps[i]);
                }
            }
    ...
    configuration_get_value(&config, "PARAMS", "save_packet_latency_path", anno,
                            packet_latency_path, MAX_NAME_LENGTH);
    ...
    if (strlen(event_time_trace_stride_str) > 0) {
        ...
        if (parsed_stride > 0) {
            ...
            tw_warning(TW_LOC, "Ignoring invalid save_event_time_trace_stride=%s; using stride=1",
    ...
    event_time_inference_enabled = event_time_surrogate_family_selected &&
    ...
    enable_str[0] = '\0';
    int const rc_enable = configuration_get_value(&config, "NETWORK_SURROGATE", "enable", anno,
                                                  enable_str, MAX_NAME_LENGTH);
    bool enable_network_surrogate = false;
    if (rc_enable > 0) {
        enable_network_surrogate =
            (strcmp(enable_str, "1") == 0 || strcmp(enable_str, "true") == 0);
    }
    ...
    if (event_time_surrogate_family_selected) {
        configuration_get_value(&config, "DIRECTOR", "debug_prints", anno, debug_prints_str,
                                MAX_NAME_LENGTH);
    } else {
            ...
            .lp_types = {
                {
                    ...
                    .check_event_in_queue = (model_check_event_f)
                        dragonfly_dally_terminal_pre_surrogate_switch_event_queue,
...
static void setup_packet_latency_path(char const* const dir_to_save) {
    ...
    if (g_tw_mynode == 0 && stat(dir_to_save, &st) == -1) {
        ...
        if (res != NO_ERROR) {
            tw_error(TW_LOC, "Error (%d) occurred when attempting to mkdir folder `%s`", errno,
                     dir_to_save);
        }
    }
...
static void setup_router_timing_trace_path(char const* const dir_to_save) {
    ...
    fprintf(router_timing_trace_f,
            ...
            "actual_router_delay,is_surrogate_on,is_predicted,now,packet_id,src_terminal,dest_"
            "terminal,next_stop\n");
}

static inline void router_timing_trace_save_to_file(
    router_state const* s, terminal_dally_message const* pkt, int output_port, int output_chan,
    int to_terminal, int is_global, double next_output_available_delta, double nominal_router_delay,
    double actual_router_delay, bool surrogate_on, bool is_predicted, tw_lp* lp) {
    if (!router_timing_trace_f) {
        return;
    }
    ...
    fprintf(router_timing_trace_f,
            ...
            s->group_id, output_port, output_chan, to_terminal, is_global, pkt->packet_size,
            s->params->chunk_size, s->vc_occupancy[output_port][output_chan],
            s->queued_count[output_port], next_output_available_delta, nominal_router_delay,
            router_queueing_delay, actual_router_delay, surrogate_on, is_predicted, tw_now(lp),
            (unsigned long)pkt->packet_ID, pkt->dfdally_src_terminal_id,
            pkt->dfdally_dest_terminal_id, (unsigned long long)pkt->next_stop);
}
...
void dragonfly_dally_report_stats() {
    ...
    MPI_Reduce(&num_local_packets_sr, &total_local_packets_sr, 1, MPI_LONG, MPI_SUM, 0,
               MPI_COMM_CODES);
    MPI_Reduce(&num_local_packets_sg, &total_local_packets_sg, 1, MPI_LONG, MPI_SUM, 0,
               MPI_COMM_CODES);
    MPI_Reduce(&num_remote_packets, &total_remote_packets, 1, MPI_LONG, MPI_SUM, 0, MPI_COMM_CODES);
    if (routing == ADAPTIVE || routing == PROG_ADAPTIVE || routing == PROG_ADAPTIVE_LEGACY ||
        SHOW_ADAP_STATS) {
    ...
    if (!g_tw_mynode) {
        ...
        printf("\nAverage number of hops traversed %f average chunk latency %lf us maximum chunk "
               "latency %lf us avg message size %lf bytes finished messages %lld finished chunks "
               "%lld\n",
               (float)avg_hops / total_finished_chunks,
               (float)avg_time / total_finished_chunks / 1000, max_time / 1000,
               (float)final_msg_sz / total_finished_msgs, total_finished_msgs,
               total_finished_chunks);
        if (routing == ADAPTIVE || routing == PROG_ADAPTIVE || routing == PROG_ADAPTIVE_LEGACY ||
            SHOW_ADAP_STATS)
            printf("\nADAPTIVE ROUTING STATS: %d chunks routed minimally %d chunks routed "
                   "non-minimally completed packets %lld \n",
                   total_minimal_packets, total_nonmin_packets, total_finished_chunks);

        printf("\nTotal packets generated %ld finished %ld Locally routed- same router %ld "
               "different-router %ld Remote (inter-group) %ld \n",
               total_gen, total_fin, total_local_packets_sr, total_local_packets_sg,
               total_remote_packets);
    }
    return;
}

static void dragonfly_dally_terminal_end_sim_notif(terminal_state* s, tw_bf* bf,
                                                   model_net_wrap_msg* msg, tw_lp* lp) {
    s->workloads_finished_flag = 1;
}

static void dragonfly_dally_terminal_end_sim_notif_rc(terminal_state* s, tw_bf* bf,
                                                      model_net_wrap_msg* msg, tw_lp* lp) {
    s->workloads_finished_flag = 0;
}


static void dragonfly_dally_router_end_sim_notif(router_state* s, tw_bf* bf,
                                                 model_net_wrap_msg* msg, tw_lp* lp) {
    s->workloads_finished_flag = 1;
}

static void dragonfly_dally_router_end_sim_notif_rc(router_state* s, tw_bf* bf,
                                                    model_net_wrap_msg* msg, tw_lp* lp) {
    s->workloads_finished_flag = 0;
}

static void dragonfly_dally_terminal_congestion_event(terminal_state* s, tw_bf* bf,
                                                      congestion_control_message* msg, tw_lp* lp) {
    cc_terminal_local_congestion_event(s->local_congestion_controller, bf, msg, lp);
}

static void dragonfly_dally_terminal_congestion_event_rc(terminal_state* s, tw_bf* bf,
                                                         congestion_control_message* msg,
                                                         tw_lp* lp) {
    cc_terminal_local_congestion_event_rc(s->local_congestion_controller, bf, msg, lp);
}

static void dragonfly_dally_terminal_congestion_event_commit(terminal_state* s, tw_bf* bf,
                                                             congestion_control_message* msg,
                                                             tw_lp* lp) {
    cc_terminal_local_congestion_event_commit(s->local_congestion_controller, bf, msg, lp);
}

static void dragonfly_dally_router_congestion_event(router_state* s, tw_bf* bf,
                                                    congestion_control_message* msg, tw_lp* lp) {
    cc_router_local_congestion_event(s->local_congestion_controller, bf, msg, lp);
}

static void dragonfly_dally_router_congestion_event_rc(router_state* s, tw_bf* bf,
                                                       congestion_control_message* msg, tw_lp* lp) {
    cc_router_local_congestion_event_rc(s->local_congestion_controller, bf, msg, lp);
}

static void dragonfly_dally_router_congestion_event_commit(router_state* s, tw_bf* bf,
                                                           congestion_control_message* msg,
                                                           tw_lp* lp) {
...
static int get_rtr_bandwidth_consumption(router_state* s, int qos_lvl, int output_port) {
    assert(qos_lvl >= Q_HIGH && qos_lvl <= Q_LOW);
    assert(output_port <
           s->params->intra_grp_radix + s->params->num_global_channels + s->params->cn_radix);
...
static void issue_bw_monitor_event_rc(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                      tw_lp* lp) {
    ...
    if (msg->rc_is_qos_set == 1) {
        for (int i = 0; i < num_rails; i++) {
            for (int j = 0; j < num_qos_levels; j++) {
                ...
                s->qos_status[i][j] =
                    *(indexer2d(msg->rc_qos_status, i, j, num_rails, num_qos_levels));
            }
        }
...
static void issue_bw_monitor_event(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                   tw_lp* lp) {
    int num_qos_levels = s->params->num_qos_levels;
    int num_rails = s->params->num_rails;
    ...
    msg->rc_qos_data =
        (unsigned long long*)calloc(num_rails * num_qos_levels, sizeof(unsigned long long));
    ...
    if (s->workloads_finished_flag == 0) {
        ...
        tw_event* e =
            model_net_method_event_new(lp->gid, bw_ts, lp, DRAGONFLY_DALLY, (void**)&m, NULL);
        m->type = T_BANDWIDTH;
...
static void issue_rtr_bw_monitor_event_rc(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                          tw_lp* lp) {
...
static void issue_rtr_bw_monitor_event(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                       tw_lp* lp) {
    int radix = s->params->radix;
    int num_qos_levels = s->params->num_qos_levels;
    ...
    //Allocate memory here for these pointers that are stored in the events. FREE THESE IN RC OR IN COMMIT_F
    msg->rc_qos_data =
        (unsigned long long*)calloc(radix * num_qos_levels, sizeof(unsigned long long));
    ...
    for (int i = 0; i < radix; i++) {
        for (int j = 0; j < num_qos_levels; j++) {
            ...
            if (dragonfly_rtr_bw_log != NULL && ROUTER_BW_LOG) {
                if (s->qos_data[i][j] > 0) {
                    fprintf(dragonfly_rtr_bw_log, "\n %d %f %d %d %d %d %d %f", s->router_id,
                            tw_now(lp), i, j, bw_consumed, s->qos_status[i][j], s->qos_data[i][j],
                            s->busy_time_sample[i]);
                }
            }
#endif
        }
    ...
    if (s->workloads_finished_flag == 0) {
        tw_stime bw_ts = bw_reset_window + gen_noise(lp, &msg->num_rngs);
        terminal_dally_message* m;
        tw_event* e = model_net_method_event_new(lp->gid, bw_ts, lp, DRAGONFLY_DALLY_ROUTER,
                                                 (void**)&m, NULL);
        m->type = R_BANDWIDTH;
...
static int get_next_vcg(terminal_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    if (num_qos_levels == 1) {
        if (qlist_empty(&s->terminal_msgs[msg->rail_id][0]) ||
            s->vc_occupancy[msg->rail_id][0] + s->params->chunk_size > s->params->cn_vc_size)
            return -1;
        else
            return 0;
    }
    ...
    * priority traffic first. */
    if (BW_MONITOR == 1) {
        for (int i = 0; i < num_qos_levels; i++) {
            if (s->qos_status[msg->rail_id][i] == Q_ACTIVE) {
                if (!qlist_empty(&s->terminal_msgs[msg->rail_id][i]) &&
                    s->vc_occupancy[msg->rail_id][i] + s->params->chunk_size <=
                        s->params->cn_vc_size)
                    return i;
            }
        }
    ...
    for (int i = 0; i < num_qos_levels; i++) {
        if (!qlist_empty(&s->terminal_msgs[msg->rail_id][i]) &&
            s->vc_occupancy[msg->rail_id][i] + s->params->chunk_size <= s->params->cn_vc_size) {
            bf->c2 = 1;
...
static int get_next_router_vcg(router_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    msg->last_saved_qos =
        s->last_qos_lvl
            [output_port]; // last_qos_lvl stores a vc# not a qos# for routers. Terminals store qos#
...
static unsigned int dfdally_safe_rail_id(terminal_state const* s,
...
static uint32_t dfdally_terminal_src_router_id(terminal_state const* s,
                                               terminal_dally_message const* msg) {
    if (!s || !s->params) {
        return (uint32_t)-1;
    }
    ...
    return dfdally_get_assigned_router_id_from_terminal(s->params, s->terminal_id, rail_id);
}

static inline void packet_latency_save_to_file(unsigned int terminal_id, struct packet_start* start,
                                               struct packet_end* end, bool surrogate_on,
                                               bool is_predicted) {
    if (!packet_latency_f) {
        return;
    } // Don't save if there isn't a file to save to
    if (end->travel_end_time > g_tw_ts_end) {
        return;
    } // This packet could never arrive to its destination!
    double const latency = end->travel_end_time - start->travel_start_time;
    fprintf(packet_latency_f, "%u,%u,%u,%d,%llu,%u,%u,%u,%u,%u,%u,%f,%f,%f,%lu,%d,%d,%f,%f,%f,%f\n",
            terminal_id, start->dfdally_dest_terminal_id, start->packet_size,
            start->is_there_another_pckt_in_queue, (unsigned long long)start->caller_lp_gid,
            start->src_router_id, start->src_group_id, start->dst_router_id, start->dst_group_id,
            start->terminal_queue_length, start->terminal_vc_occupancy,
            start->processing_packet_delay, latency, end->next_packet_delay, start->packet_ID,
            surrogate_on, is_predicted, start->workload_injection_time, start->travel_start_time,
            end->travel_end_time, latency);
}
...
static void setup_event_time_trace_path(char const* const dir_to_save) {
    ...
    snprintf(filename, sizeof(filename), "%s/event-time-node=%ld.csv", dir_to_save,
             (long)g_tw_mynode);
    ...
    fprintf(event_time_trace_f, "schema_version,sample_id,now,current_lp_gid,current_lp_type,"
...
static void dfdally_event_time_zmq_flush(void) {
    ...
    try {
        std::vector<std::string> ret =
            dfdally_event_time_director_request_with_latency("send-records", args,
                                                             event_time_zmq_buffer);
        ...
        if (ret.empty() || ret[0] != "done") {
            ...
            fprintf(stderr, "[event-time records] sent %llu rows to zmqmlserver\n",
                    event_time_zmq_buffered_rows);
            fflush(stderr);
        }
    } catch (...) {
...
static void dfdally_event_time_trace_row(tw_lp* lp, int current_lp_type, int current_event_type,
                                         tw_lpid target_lp_gid, int target_lp_type,
                                         int target_event_type, double nominal_delay,
                                         double target_delay) {
    ...
    int const n =
        snprintf(row, sizeof(row),
                 ...
                 event_time_trace_counter, (double)tw_now(lp), (unsigned long long)lp->gid,
                 current_lp_type, current_event_type, (unsigned long long)target_lp_gid,
                 target_lp_type, target_event_type, nominal_delay, target_delay);
...
static std::string dfdally_event_time_prediction_cache_key(int current_lp_type,
                                                           int current_event_type,
                                                           int target_lp_type,
                                                           int target_event_type,
                                                           double nominal_delay, double now) {
    char key[256];
    ...
    if (time_bucket_width > 0.0) {
        ...
        snprintf(key, sizeof(key), "%d|%d|%d|%d|%.12g|tb=%.0f", current_lp_type, current_event_type,
                 target_lp_type, target_event_type, nominal_delay, time_bucket);
    } else {
        snprintf(key, sizeof(key), "%d|%d|%d|%d|%.12g", current_lp_type, current_event_type,
                 target_lp_type, target_event_type, nominal_delay);
    }

    return std::string(key);
}

static double dfdally_event_time_sanitize_prediction(double predicted, double original_delay) {
    if (!isfinite(predicted) || predicted <= 0.0) {
        return original_delay;
    }

    double const max_reasonable = original_delay > 0.0 ? original_delay * 10.0 : predicted;
...
static double dfdally_event_time_predict_or_original(tw_lp* lp, int current_lp_type,
                                                     int current_event_type, tw_lpid target_lp_gid,
                                                     int target_lp_type, int target_event_type,
                                                     double nominal_delay, double original_delay) {
    if (!event_time_surrogate_family_selected || !event_time_inference_enabled ||
    ...
    std::string const cache_key =
        dfdally_event_time_prediction_cache_key(current_lp_type, current_event_type, target_lp_type,
                                                target_event_type, nominal_delay,
                                                (double)tw_now(lp));
    ...
    if (cached != event_time_prediction_cache.end()) {
        ...
        if (dfdally_surrogate_debug_prints &&
            (event_time_cache_hits <= 20 || (event_time_cache_hits % 65536ULL) == 0)) {
            fprintf(stderr,
                    "[event-time inference cache-hit] hits=%llu misses=%llu "
                    "cache_size=%llu original=%.17g predicted=%.17g\n",
                    event_time_cache_hits, event_time_cache_misses,
                    (unsigned long long)event_time_prediction_cache.size(), original_delay,
                    predicted);
            fflush(stderr);
        }
    ...
    row << "schema_version,sample_id,now,current_lp_gid,current_lp_type,"
        << "current_event_type,target_lp_gid,target_lp_type,target_event_type,"
        << "nominal_delay\n"
        << "1,0," << (double)tw_now(lp) << "," << (unsigned long long)lp->gid << ","
        << current_lp_type << "," << current_event_type << "," << (unsigned long long)target_lp_gid
        << "," << target_lp_type << "," << target_event_type << "," << nominal_delay << "\n";
    ...
    double const predicted = dfdally_event_time_sanitize_prediction(predicted_raw, original_delay);

    if (dfdally_surrogate_debug_prints &&
        (event_time_cache_misses <= 20 || (event_time_cache_misses % 1024ULL) == 0)) {
        fprintf(stderr,
                "[event-time inference cache-miss] hits=%llu misses=%llu "
                "cache_size=%llu original=%.17g predicted=%.17g\n",
                event_time_cache_hits, event_time_cache_misses,
                (unsigned long long)event_time_prediction_cache.size(), original_delay, predicted);
        fflush(stderr);
    }
...
extern "C" int dfdally_event_time_surrogate_set_active(int enabled) {
    ...
    if (dfdally_surrogate_debug_prints) {
        fprintf(stderr, "[event-time surrogate] active=%d active_count=%d inference_enabled=%d\n",
                event_time_inference_active, event_time_inference_active_count,
...
static void feed_packet_to_predictor(terminal_state* s, tw_lp* lp, uint64_t packet_ID,
                                     double end_time) {
...
static void dragonfly_dally_terminal_highdef_to_surrogate(terminal_state* s, tw_lp* lp,
                                                          tw_event** terminal_events) {
    (void)terminal_events;

    if (s->arrival_of_last_packet.packet_ID != -1) {
        assert(s->sent_packets.count(s->arrival_of_last_packet.packet_ID) ==
               1); // packet_ID is in s->sent_packets
        assert(s->sent_packets[s->arrival_of_last_packet.packet_ID].next_packet_delay <
               0); // next_packet_delay is -1
    ...
    for (auto it = s->sent_packets.begin(); it != s->sent_packets.end();
         it = s->sent_packets.erase(it)) {
        ...
        if (freeze_network_on_switch) {
            ...
            packet_latency_save_to_file(s->terminal_id, &sent.start, &predicted_end,
                                        is_dally_surrogate_on, true);
            ...
            void* remote_event;
            tw_event* const e =
                model_net_method_event_new(sent.start.dest_terminal_lpid, latency, lp,
                                           DRAGONFLY_DALLY, (void**)&m, &remote_event);
    ...
    memcpy(&s->arrival_of_last_packet, &frozen_state->arrival_of_last_packet,
           sizeof(s->arrival_of_last_packet));
    ...
    memcpy(&s->is_pending_local_send, &frozen_state->is_pending_local_send,
           sizeof(s->is_pending_local_send));
    memcpy(&s->remaining_sz_packets, &frozen_state->remaining_sz_packets,
           sizeof(s->remaining_sz_packets));
...
static void dragonfly_dally_terminal_surrogate_to_highdef(terminal_state* s, tw_lp* lp,
    ...
    memcpy(&frozen_state->arrival_of_last_packet, &s->arrival_of_last_packet,
           sizeof(s->arrival_of_last_packet));
    ...
    memcpy(&frozen_state->is_pending_local_send, &s->is_pending_local_send,
           sizeof(s->is_pending_local_send));
    memcpy(&frozen_state->remaining_sz_packets, &s->remaining_sz_packets,
           sizeof(s->remaining_sz_packets));
...
static bool dragonfly_dally_terminal_should_event_be_frozen(tw_lp* lp, tw_event* event) {
    ...
    int const event_types_to_freeze = MN_BASE_SCHED_NEXT | MN_BASE_SAMPLE | MN_BASE_PASS |
                                      MN_BASE_END_NOTIF | MN_CONGESTION_EVENT;
    int const event_type = model_net_get_event_type_lp((model_net_wrap_msg*)tw_event_data(event));
    if (event_type &
        event_types_to_freeze) { // Finding out whether current event type is one of freeze types
        return true;
    }
    return false;
}
...
static bool dragonfly_dally_router_should_event_be_frozen(tw_lp* lp, tw_event* event) {
    ...
    int const event_types_to_freeze = MN_BASE_NEW_MSG | MN_BASE_SCHED_NEXT | MN_BASE_SAMPLE |
                                      MN_BASE_END_NOTIF | MN_CONGESTION_EVENT;
    int const event_type = model_net_get_event_type_lp((model_net_wrap_msg*)tw_event_data(event));
    if (event_type &
        event_types_to_freeze) { // Finding out whether current event type is one of freeze types
...
static void router_send_snapshot_events(router_state* s, tw_lp* lp) {
    ...
    if (s->router_id == 0) //add header information
    {
        if (OUTPUT_SNAPSHOT) {
            ...
            for (int i = 0, j = 0; i < s->params->radix;) {
                int sz = snprintf(snapshot_line + offset, sizeof(snapshot_line) - offset,
                                  "Port %d VC %d,", i, j);
                offset += sz;
    ...
    for (int i = 0; i < num_snapshots; i++) {
        terminal_dally_message* m;
        tw_event* e = model_net_method_event_new(lp->gid, snapshot_times[i], lp, DRAGONFLY_DALLY,
                                                 (void**)&m, NULL);
        m->type = R_SNAPSHOT;
        m->magic = router_magic_num;
...
static void router_handle_snapshot_event(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                         tw_lp* lp) {
    if (msg->packet_ID >= num_snapshots) {
        fprintf(stderr,
                "Warning: packet_ID = %llu will not be saved in the snapshot because there are "
                "only %d spaces available.\n",
...
static void terminal_commit_packet_generate(terminal_state* s, tw_bf* bf,
                                            terminal_dally_message* msg, tw_lp* lp) {
    ...
    if (msg->remote_event_size_bytes) {
        ...
        memcpy(remote_data, model_net_method_get_edata(DRAGONFLY_DALLY, msg),
    ...
    if (msg->local_event_size_bytes) {
        ...
        memcpy(local_data,
               (char*)model_net_method_get_edata(DRAGONFLY_DALLY, msg) +
                   msg->remote_event_size_bytes,
    ...
    uint32_t const dst_router_id =
        dfdally_get_assigned_router_id_from_terminal(s->params, msg->dfdally_dest_terminal_id,
                                                     feature_rail_id);
...
static void terminal_dally_commit(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                  tw_lp* lp) {
    switch (msg->type) {
    case T_GENERATE:
        if (bf->c10) { // if the packet was sent as a prediction, store the prediction in memory
            ...
            uint32_t const src_group_id = src_router_id / s->params->num_routers;
            uint32_t const dst_router_id =
                dfdally_get_assigned_router_id_from_terminal(s->params,
            ...
            if (s->arrival_of_last_packet.packet_ID != -1) {
                ...
                packet_latency_save_to_file(s->terminal_id, &sent.start, &end,
                                            is_dally_surrogate_on, false);
    ...
    case T_NOTIFY:
        if (msg->notify_type == NOTIFY_LATENCY) {
            ...
            if (s->sent_packets.count(packet_ID) == 1) { // packet_ID is in s->sent_packets
                ...
                if (packet_ID ==
                    s->last_packet_sent_id) { // packet_ID is last, we cannot compute the next_packet_delay
...
static void router_dally_commit(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                tw_lp* lp) {
    ...
    if (msg->type == R_SNAPSHOT) {
        if (OUTPUT_SNAPSHOT == 1 && msg->packet_ID < num_snapshots) {
            ...
            written =
                sprintf(snapshot_line, "%.4f, %d, ", snapshot_times[msg->packet_ID], s->router_id);
...
static void terminal_dally_init(terminal_state* s, tw_lp* lp) {
    ...
    for (i = 0; i < p->num_rails; i++) {
        s->router_id[i] =
            dfdally_get_assigned_router_id_from_terminal(s->params, s->terminal_id, i);
        num_routers_per_mgrp =
            codes_mapping_get_lp_count(lp_group_name, 1, "modelnet_dragonfly_dally_router", NULL,
                                       0);
        codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, NULL, 1,
                                s->router_id[i] / num_routers_per_mgrp,
                                s->router_id[i] % num_routers_per_mgrp, &s->router_lp[i]);
    }
    ...
    s->vc_occupancy =
        (int**)calloc(p->num_rails, sizeof(int*)); //1 vc times the number of qos levels
    s->last_buf_full = (tw_stime*)calloc(p->num_rails, sizeof(tw_stime));

    s->terminal_length =
        (int**)calloc(p->num_rails, sizeof(int*)); //1 vc times number of qos levels
    ...
    s->rank_tbl =
        qhash_init(dragonfly_rank_hash_compare, dragonfly_hash_func, DFLY_HASH_TABLE_SIZE);
    ...
    for (i = 0; i < p->num_rails; i++) {
        s->in_send_loop[i] = 0;
        s->terminal_msgs[i] =
            (struct qlist_head*)calloc(s->params->num_vcs, sizeof(struct qlist_head));
    ...
    if (g_congestion_control_enabled) {
        ...
        cc_terminal_local_controller_init(s->local_congestion_controller, lp, s->terminal_id,
                                          &s->workloads_finished_flag);
    }
...
static void router_dally_init(router_state* r, tw_lp* lp) {
    ...
    num_routers_per_mgrp =
        codes_mapping_get_lp_count(lp_group_name, 1, "modelnet_dragonfly_dally_router", NULL, 0);
    ...
    r->pending_msgs = (struct qlist_head**)calloc(p->radix, sizeof(struct qlist_head*));
    r->queued_msgs = (struct qlist_head**)calloc(p->radix, sizeof(struct qlist_head*));
    ...
    if (!r->connMan.check_is_solidified())
        tw_error(TW_LOC,
                 "Connection Manager not solidified - should be performed after loading topology");

    if (g_congestion_control_enabled) {
        r->local_congestion_controller = (rlc_state*)calloc(1, sizeof(rlc_state));
        cc_router_local_controller_init(r->local_congestion_controller, lp, p->total_terminals,
                                        r->router_id, p->radix, p->num_vcs, r->vc_max_sizes,
                                        r->port_bandwidths, &r->workloads_finished_flag);
    ...
    if (num_snapshots) {
        ...
        for (int i = 0; i < num_snapshots; i++) {
            r->snapshot_data[i] = (int*)calloc(r->params->num_vcs * r->params->radix,
                                               sizeof(int)); //capturing VC occupancies of each port
        }
...
static tw_stime dragonfly_dally_packet_event(model_net_request const* req, uint64_t message_offset,
                                             uint64_t packet_size, tw_stime offset,
                                             mn_sched_params const* sched_params,
                                             void const* remote_event, void const* self_event,
                                             tw_lp* sender, int is_last_pckt,
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender,
                                       DRAGONFLY_DALLY, (void**)&msg, (void**)&tmp_ptr);
...
static void packet_generate_predicted_rc(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                         tw_lp* lp) {
...
static void packet_generate_predicted(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                      tw_lp* lp) {
    packet_gen++;
    s->packet_gen++;
    s->total_gen_size += msg->packet_size;
    ...
    auto start =
        (struct packet_start){.packet_ID = msg->packet_ID,
                              .dest_terminal_lpid = msg->dest_terminal_lpid,
                              .dfdally_dest_terminal_id = msg->dfdally_dest_terminal_id,
    ...
    double const arrival =
        start.travel_start_time + latency; // this is "equivalent" to end.travel_end_time
    ...
    tw_event* const e = model_net_method_event_new(msg->dest_terminal_lpid, latency, lp,
                                                   DRAGONFLY_DALLY, (void**)&m, &remote_event);
    ...
    if (msg->local_event_size_bytes > 0) {
        ...
        void* m_new = tw_event_data(e_new);
        void* local_event =
            (char*)model_net_method_get_edata(DRAGONFLY_DALLY, msg) + msg->remote_event_size_bytes;
        memcpy(m_new, local_event, msg->local_event_size_bytes);
        tw_event_send(e_new);
    }
}

static void packet_generate_rc(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                               tw_lp* lp) {
    int num_qos_levels = s->params->num_qos_levels;
    if (bf->c1)
        s->is_monitoring_bw = 0;
    ...
    for (i = 0; i < num_chunks; i++) {
        delete_terminal_dally_message_list(
            return_tail_from_qlist(&s->terminal_msgs[msg->rail_id][vcg]));
...
static void packet_generate(terminal_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    if (s->params->num_injection_queues > 1 || netMan.is_link_failures_enabled()) {
        ...
        vector<Connection> injection_connections =
            s->connMan.get_connections_by_type(CONN_INJECTION, false);
        if (injection_connections.size() < 1)
            tw_error(TW_LOC,
                     "Packet Generation Failure: No non-failed injection connections available on "
                     "terminal %d\n",
                     s->terminal_id);
            ...
            for (int i = 0; i < injection_connections.size(); i++) {
                ...
                int dest_router_id =
                    dfdally_get_assigned_router_id_from_terminal(s->params,
                                                                 ...
                                                                 rail_id);
                int src_router_id =
                    dfdally_get_assigned_router_id_from_terminal(s->params, s->terminal_id,
                                                                 rail_id);
                ...
                if (isRoutingMinimal(routing)) {
                    if (src_group_id == dest_group_id) {
                        set<Connection> valid_next_stops = netMan.get_valid_next_hops_conns(
                            src_router_id, dest_router_id, max_hops_per_group,
                            0); //max global hops for local group routing == 0
                        if (valid_next_stops.size() > 0)
                            valid_rails.push_back(injection_connections[i]);
                    } else {
                        set<Connection> valid_next_stops = netMan.get_valid_next_hops_conns(
                            src_router_id, dest_router_id, max_hops_per_group,
                            1); //max global hops for local group routing == 0
                        if (valid_next_stops.size() > 0)
                            valid_rails.push_back(injection_connections[i]);
                    }
                } else {
                    ...
                    set<Connection> valid_next_stops = netMan.get_valid_next_hops_conns(
                        src_router_id, dest_router_id, max_hops_per_group,
                        max_global_hops_nonminimal); //max global hops for local group routing == 0
                    if (valid_next_stops.size() > 0)
                        valid_rails.push_back(injection_connections[i]);
                    // }
                }
        ...
        if (valid_rails.size() > 1) {
            ...
            if (s->params->rail_select ==
                RAIL_DEDICATED) { //then attempt to inject on rail based on the injection queue from the workload
                Connection specific_injection_conn =
                    s->connMan.get_connection_on_port(msg->rail_id, true);
                if (specific_injection_conn.port == -1)
                    tw_error(TW_LOC,
                             "Packet Generation Failure: No connection on specified rail\n");
            ...
            if (s->params->rail_select == RAIL_PATH) {
                ...
                for (; it != valid_rails.end(); it++) {
                    ...
                    path_lens[index] =
                        netMan.get_shortest_dist_between_routers(src_router_id, dest_router_id);
        ...
        dest_router_id =
            dfdally_get_assigned_router_id_from_terminal(p, msg->dfdally_dest_terminal_id,
                                                         msg->rail_id);
    } else {
        dest_router_id =
            dfdally_get_assigned_router_id_from_terminal(p, msg->dfdally_dest_terminal_id, 0);
    }
    ...
    for (int i = 0; i < num_chunks; i++) {
        terminal_dally_message_list* cur_chunk =
            (terminal_dally_message_list*)calloc(1, sizeof(terminal_dally_message_list));
    ...
    if (s->params->num_injection_queues > 1) {
        tw_stime* bts =
            buff_time_storage_create(s); //mallocs space to push onto the rc stack -- free'd in rc
    ...
    total_event_size = model_net_get_msg_sz(DRAGONFLY_DALLY) + msg->remote_event_size_bytes +
                       msg->local_event_size_bytes;
    mn_stats* stat;
    stat = model_net_find_stats(msg->category, s->dragonfly_stats_array);
    stat->send_count++;
...
static void packet_send(terminal_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    * of VCs*/
    ...
    msg->saved_vc = vcg;
    terminal_dally_message_list* cur_entry =
        return_head_from_qlist(&s->terminal_msgs[msg->rail_id][vcg]);
    int data_size = s->params->chunk_size;
    uint64_t const num_chunks = num_chunks_for(cur_entry->msg.packet_size, s->params->chunk_size);

    msg->saved_avg_time =
        cur_entry->msg
            .travel_start_time; // reusing field saved_avg_time. It is only used in another event handler path (arrive). So, no interruptions here
    ...
    s->terminal_available_time[msg->rail_id] =
        maxd(s->terminal_available_time[msg->rail_id], tw_now(lp));
    ...
    dfdally_event_time_trace_row(lp, DRAGONFLY_DALLY, T_SEND, router_id, DRAGONFLY_DALLY_ROUTER,
                                 R_ARRIVE, propagation_ts, event_time_target_delay);
    event_time_target_delay =
        dfdally_event_time_predict_or_original(lp, DRAGONFLY_DALLY, T_SEND, router_id,
                                               DRAGONFLY_DALLY_ROUTER, R_ARRIVE, propagation_ts,
                                               event_time_target_delay);

    void* remote_event;
    e = model_net_method_event_new(router_id, event_time_target_delay, lp, DRAGONFLY_DALLY_ROUTER,
                                   (void**)&m, &remote_event);
    ...
    if (cur_entry->msg.packet_ID == LLU(TRACK_PKT) && lp->gid == T_ID)
        printf("\n Packet %llu generated at terminal %d dest %llu size %llu num chunks %llu "
               ...
               LLU(cur_entry->msg.packet_size), LLU(num_chunks), s->router_id[msg->rail_id],
               LLU(router_id));
#endif
    ...
    if (num_qos_levels >
        1) //I think this one is OK since the default is that terminals have only 1 VC anyway so leaving vcg as
        next_vcg = get_next_vcg(s, bf, msg, lp);

    /* if there is another packet inline then schedule another send event */
    if (next_vcg >= 0 && !qlist_empty(&s->terminal_msgs[msg->rail_id][next_vcg]) &&
        s->vc_occupancy[msg->rail_id][next_vcg] + s->params->chunk_size <= s->params->cn_vc_size) {
        terminal_dally_message* m_new;
        e = model_net_method_event_new(lp->gid, injection_ts + gen_noise(lp, &msg->num_rngs), lp,
                                       DRAGONFLY_DALLY, (void**)&m_new, NULL);
        m_new->type = T_SEND;
    ...
    if (s->issueIdle[msg->rail_id] &&
        s->terminal_length[msg->rail_id][vcg] < s->params->cn_vc_size) {
        bf->c5 = 1;
        s->issueIdle[msg->rail_id] = 0;
        ...
        if (s->last_buf_full[msg->rail_id] > 0.0) {
            ...
            s->ross_sample.busy_time_sample[msg->rail_id] +=
                (tw_now(lp) - s->last_buf_full[msg->rail_id]);
...
static void notify_dest_lp_of(terminal_state* s, tw_lp* lp, terminal_dally_message* msg,
    ...
    case NOTIFY_ZOMBIE:
        offset =
            0.0; // Zero-offset events are ugly, but we want to guarantee correctness, so this prevents funky stuff from happening (like in the case of events arriving and being processed before the notification reaches them)
        break;
    default:
    ...
    tw_event* e = model_net_method_event_new_user_prio(msg->dest_terminal_lpid, offset, lp,
                                                       DRAGONFLY_DALLY, (void**)&new_msg, NULL, 1);

    memcpy(
        new_msg, msg,
        sizeof(
            terminal_dally_message)); // Just making sure that if the simulation breaks because we didn't set some value below, it breaks in a spectacular manner (~0 can be -1)
...
static void notify_src_lp_on_total_latency(tw_lp* lp, terminal_dally_message* msg) {
    terminal_dally_message* new_msg;
    tw_event* e = model_net_method_event_new(msg->src_terminal_id, g_tw_lookahead, lp,
                                             DRAGONFLY_DALLY, (void**)&new_msg, NULL);
...
static void process_terminal_notification_event_rc(terminal_state* s, tw_bf* bf,
                                                   terminal_dally_message* msg, tw_lp* lp) {
    ...
    case NOTIFY_ZOMBIE: {
        struct packet_id const zombie = {.packet_ID = msg->packet_ID,
...
static void process_terminal_notification_event(terminal_state* s, tw_bf* bf,
                                                terminal_dally_message* msg, tw_lp* lp) {
    switch ((enum notify_t)msg->notify_type) {
    case NOTIFY_LATENCY:
        break;
...
static void vacuous_msg_to_itself(terminal_state* s, terminal_dally_message* msg, tw_lp* lp) {
    ...
    (void)msg;
    terminal_dally_message* new_msg;
    tw_event* e = model_net_method_event_new(lp->gid, g_tw_lookahead, lp, DRAGONFLY_DALLY,
                                             (void**)&new_msg, NULL);
...
static void send_remote_event(terminal_state* s, terminal_dally_message* msg, tw_lp* lp, tw_bf* bf,
    ...
    if (msg->is_pull) {
        ...
        msg->event_rc =
            model_net_event_mctx(net_id, &mc_src, &mc_dst, msg->category, msg->sender_lp,
                                 msg->pull_size, ts + gen_noise(lp, &msg->num_rngs),
...
static void packet_arrive_predicted_rc(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                       tw_lp* lp) {
    struct dfly_hash_key key = {
...
static void packet_arrive_predicted(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                    tw_lp* lp) {
    assert(lp->gid == msg->dest_terminal_lpid);
    ...
    if (msg->packet_ID == LLU(TRACK_PKT) && msg->src_terminal_id == T_ID)
        printf("\n Packet %llu arrived at lp %llu hops %d ", LLU(msg->sender_lp), LLU(lp->gid),
               msg->my_N_hop);
#endif
    ...
    if (hash_link) {
        ...
        uint64_t total_packets =
            msg->total_size / packet_size + (msg->total_size % packet_size ? 1 : 0);
        if (total_packets == 0) {
            total_packets = 1;
        }

        struct dfly_qhash_entry* const d_entry =
            (dfly_qhash_entry*)calloc(1, sizeof(struct dfly_qhash_entry));
...
static void packet_arrive_rc(terminal_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    struct packet_id const packet_key = {.packet_ID = msg->packet_ID,
                                         .dfdally_src_terminal_id = msg->dfdally_src_terminal_id};
    ...
    if (bf->c14) {
        s->zombies.emplace(
            (struct packet_id){.packet_ID = msg->packet_ID,
                               .dfdally_src_terminal_id = msg->dfdally_src_terminal_id});
    }
    if (bf->c15) {
        return;
...
static void packet_arrive(terminal_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    if (has_remaining_sz) {
        bf->c28 = 1;
        int const actual_chunk_size =
            std::min(chunk_size, (int)s->remaining_sz_packets[packet_key]);
...
static void terminal_buf_update_rc(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                   tw_lp* lp) {
    int vcg = 0;
    int num_qos_levels = s->params->num_qos_levels;
...
static void terminal_buf_update(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                tw_lp* lp) {
    bf->c1 = 0;
    bf->c2 = 0;
    bf->c3 = 0;
...
static void dragonfly_dally_terminal_final(terminal_state* s, tw_lp* lp) {
    ...
    if (s->terminal_id == 0) {
        written += sprintf(s->output_buf + written,
                           "# Format <source_id> <source_type> <dest_id> < dest_type>  <link_type> "
                           "<link_traffic> <link_saturation> <stalled_chunks>\n");
    ...
    for (int i = 0; i < s->params->num_rails; i++) {
        ...
        written += sprintf(s->output_buf + written, "\n%u %s %u %s %s %llu %lf %lu", s->terminal_id,
                           "T", s->router_id[i], "R", "CN", LLU(s->link_traffic[i]),
                           s->busy_time[i], s->stalled_chunks[i]);
    }
    ...
    if (s->terminal_id == 0) {
        written += sprintf(s->output_buf2 + written,
                           "# Format <LP id> <Terminal ID> <Total Data Sent> <Total Data Received> "
                           "<Avg packet latency> <Max packet Latency> <Min packet Latency> <# "
                           "Packets finished> <Avg Hops> <Avg Busy Time (over rails)>\n");
    }
    ...
    written +=
        sprintf(s->output_buf2 + written, "%llu %u %d %llu %lf %lf %lf %ld %lf %lf\n", LLU(lp->gid),
                s->terminal_id, s->total_gen_size, LLU(s->total_msg_size),
                s->total_time / s->finished_chunks, s->max_latency, s->min_latency,
    ...
    for (auto&& kv : s->sent_packets) {
        if (kv.second.message_data) {
            free(kv.second.message_data);
        }
        if (kv.second.remote_event_data) {
            free(kv.second.remote_event_data);
        }
        if (kv.second.local_data) {
            free(kv.second.local_data);
        }
    }
    s->sent_packets.~map();
...
void dragonfly_dally_router_final(router_state* s, tw_lp* lp) {
    ...
    for (int d = 0; d <= p->intra_grp_radix; d++) {
        if (d != src_rel_id) {
            ...
            written += sprintf(s->output_buf + written, "\n%d %s %d %s %s %llu %lf %lu",
                               s->router_id, "R", dest_ab_id, "R", "L", LLU(s->link_traffic[d]),
                               s->busy_time[d], s->stalled_chunks[d]);
        }
    }
    ...
    for (; it != my_global_links.end(); it++) {
        ...
        written += sprintf(s->output_buf + written, "\n%d %s %d %s %s %llu %lf %lu", s->router_id,
                           "R", dest_rtr_id, "R", "G", LLU(s->link_traffic[port_no]),
                           s->busy_time[port_no], s->stalled_chunks[port_no]);
    }
    ...
    for (; it != my_terminal_links.end(); it++) {
        ...
        int port_no = it->port;
        written += sprintf(s->output_buf + written, "\n%d %s %d %s %s %llu %lf %lu", s->router_id,
                           "R", dest_term_id, "T", "CN", LLU(s->link_traffic[port_no]),
                           s->busy_time[port_no], s->stalled_chunks[port_no]);
    }
    ...
    */
    ...
    if (p->counting_bool) {
        ...
        if (!s->router_id) {
            written =
                sprintf(s->output_buf5,
                        "# Format <LP ID> <Group ID> <Router ID> <Window ID> <Link Traffic>\n");
            lp_io_write(lp->gid, (char*)"dragonfly-router-traffic-sample", written, s->output_buf5);
        }
        for (int i = 0; i < p->counting_windows; i++) {
            written = sprintf(s->output_buf5, "\n %llu %d %d %lf", LLU(lp->gid),
                              s->router_id / p->num_routers, s->router_id,
                              (p->counting_start + (i + 1) * p->counting_interval));
            for (int d = 0; d < p->radix; d++)
        ...
        if (!s->router_id) {
            written =
                sprintf(s->output_buf6,
                        "# Format <LP ID> <Group ID> <Router ID> <Window ID> <Link Busy Time>\n");
            lp_io_write(lp->gid, (char*)"dragonfly-router-busytime-sample", written,
                        s->output_buf6);
        }
        for (int i = 0; i < p->counting_windows; i++) {
...
static Connection do_dfdally_routing(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                     tw_lp* lp, int fdest_router_id) {
    ...
    if (!isRoutingSmart(routing)) {
        ...
        if (routing != PROG_ADAPTIVE_LEGACY) {
            ...
            if (my_router_id ==
                fdest_router_id) { //destination router reached, next dest = final terminal destination
                vector<Connection> poss_next_stops =
                    s->connMan.get_connections_to_gid(msg->dfdally_dest_terminal_id, CONN_TERMINAL);
                if (poss_next_stops.size() < 1)
                    tw_error(
                        TW_LOC, "Destination Router %d: No connection to destination terminal %d\n",
                        s->router_id,
                        msg->dfdally_dest_terminal_id); //shouldn't happen unless math was wrong
                ...
                my_group_id ==
                fdest_group_id) { //Then we're already in the destination group and should just route to the fdest router
                vector<Connection> conns_to_fdest =
                    s->connMan.get_connections_to_gid(fdest_router_id, CONN_LOCAL);
                if (conns_to_fdest.size() < 1) {
                    vector<Connection> poss_next_stops =
                        get_legal_minimal_stops(s, bf, msg, lp, fdest_router_id);
                    if (poss_next_stops.size() < 1)
                        tw_error(
                            TW_LOC,
                            "Destination Group %d: No connection to destination router %d\n",
                            s->router_id,
                            fdest_router_id); //shouldn't happen unless the connections weren't set up / loaded correctly
                ...
                if (isRoutingAdaptive(routing)) { // Pick the best connection
                    Connection best_conn =
                        get_absolute_best_connection_from_conns(s, bf, msg, lp, conns_to_fdest);
                    return best_conn;
            ...
            if (msg->last_hop ==
                TERMINAL) //This is used by dfdally_nonminimal_routing and dfdally_prog_adaptive_routing
            {
    ...
    case ADAPTIVE:
        tw_error(TW_LOC,
                 "Standard Adaptive Routing not implemented - try Progressive Adaptive instead");
        break;
    case PROG_ADAPTIVE:
...
static void router_verify_valid_receipt(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                        tw_lp* lp) {
    if (msg->my_N_hop > s->params->max_hops_notify) {
...
static void router_credit_send(router_state* s, terminal_dally_message* msg, tw_lp* lp, int sq,
                               short* rng_counter) {
    tw_event* buf_e;
    tw_stime ts;
    terminal_dally_message* buf_msg;
    ...
    if (is_terminal) {
        buf_e = model_net_method_event_new(dest, ts + gen_noise(lp, &msg->num_rngs), lp,
                                           DRAGONFLY_DALLY, (void**)&buf_msg, NULL);
...
static void router_packet_receive_rc(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                     tw_lp* lp) {
    ...
    if (bf->c2) {
        if (!qlist_empty(&s->pending_msgs[output_port][output_chan])) {
            ...
            qlist_del(last);
            terminal_dally_message_list* tail =
                qlist_entry(last, terminal_dally_message_list, list);
    ...
    if (g_congestion_control_enabled) {
        ...
        cc_router_received_packet_rc(s->local_congestion_controller, lp, s->params->chunk_size,
                                     output_port, output_chan, src_term_id, app_id, cc_msg_rc);
        cc_msg_rc_storage_delete(cc_msg_rc);
    }
}

/* Packet arrives at the router and a credit is sent back to the sending terminal/router */
static void router_packet_receive(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                  tw_lp* lp) {
    ...
    if (num_qos_levels > 1) {
        if (s->is_monitoring_bw == 0) {
            ...
            terminal_dally_message* m;
            tw_event* e = model_net_method_event_new(lp->gid, bw_ts + gen_noise(lp, &msg->num_rngs),
                                                     lp, DRAGONFLY_DALLY_ROUTER, (void**)&m, NULL);
            m->type = R_BANDWIDTH;
            m->magic = router_magic_num;
            tw_event_send(e);
            s->is_monitoring_bw = 1;
    ...
    msg->num_rngs +=
        (cur_chunk->msg).num_rngs -
        num_rngs_before; //make sure we're counting the rngs called during do_dfdally_routing()
    ...
    if (next_stop_conn.conn_type != CONN_TERMINAL) {
        ...
        codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, s->anno, 0,
                                id / num_routers_per_mgrp, id % num_routers_per_mgrp,
                                &router_dest_id);
    ...
    if (cur_chunk->msg.packet_ID == LLU(TRACK_PKT) && cur_chunk->msg.src_terminal_id == T_ID)
        printf("\n Packet %llu arrived at router %u next stop %d final stop %d local hops %d "
               "global hops %d",
               cur_chunk->msg.packet_ID, s->router_id, next_stop, dest_router_id,
               cur_chunk->msg.my_l_hop, cur_chunk->msg.my_g_hop);
#endif
    ...
    if (s->vc_occupancy[output_port][output_chan] + s->params->chunk_size <= max_vc_size) {
        ...
        if (s->in_send_loop[output_port] == 0) {
            ...
            tw_event* e = model_net_method_event_new(lp->gid, ts + gen_noise(lp, &msg->num_rngs),
                                                     lp, DRAGONFLY_DALLY_ROUTER, (void**)&m, NULL);
            m->rail_id = msg->rail_id;
            m->type = R_SEND;
            m->magic = router_magic_num;
    ...
    if (g_congestion_control_enabled) {
        ...
        cc_router_received_packet(s->local_congestion_controller, lp, s->params->chunk_size,
                                  output_port, output_chan, cur_chunk->msg.dfdally_src_terminal_id,
                                  cur_chunk->msg.app_id, cc_msg_rc);
...
static void router_packet_send_rc(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                  tw_lp* lp) {
    int num_qos_levels = s->params->num_qos_levels;
    ...
    if (s->params->counting_bool > 0 && msg->last_sent_time >= s->params->counting_start) {
        current_window =
            (int)((msg->last_sent_time - s->params->counting_start) / s->params->counting_interval);
    ...
    if (bf->c11) {
        ...
        s->ross_rsample.link_traffic_sample[output_port] -=
            cur_entry->msg.packet_size % s->params->chunk_size;
        s->link_traffic_ross_sample[output_port] -=
            cur_entry->msg.packet_size % s->params->chunk_size;
        msg_size = cur_entry->msg.packet_size % s->params->chunk_size;

        //Xin: reverse link traffic
        if (rolback && current_window >= 0) {
            s->agg_link_traffic[current_window][output_port] -=
                cur_entry->msg.packet_size % s->params->chunk_size;
        }
    }
    if (bf->c12) {
    ...
    if (g_congestion_control_enabled) {
        congestion_control_message* cc_msg_rc = (congestion_control_message*)rc_stack_pop(s->cc_st);
        cc_router_forwarded_packet_rc(s->local_congestion_controller, lp, s->params->chunk_size,
                                      output_port, output_chan, src_term_id, app_id, cc_msg_rc);
        cc_msg_rc_storage_delete(cc_msg_rc);
    }
...
static void router_packet_send(router_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    int num_qos_levels = s->params->num_qos_levels;
    int output_chan = get_next_router_vcg(
        s, bf, msg, lp); //includes default output_chan setting functionality if qos not enabled
    ...
    if (output_chan < 0) {
        bf->c1 = 1;
        s->in_send_loop[output_port] = 0;
        if (s->queued_count[output_port] &&
            !s->last_buf_full
                 [output_port]) //5-21-19, not sure why this was added here with the qos stuff
        {
            bf->c2 = 1;
            msg->saved_busy_time = s->last_buf_full[output_port];
            s->last_buf_full[output_port] = tw_now(lp);
    ...
    if (s->last_buf_full[output_port]) //5-12-19, same here as above comment
    {
        ...
        if (update && current_window >= 0) {
            s->agg_busy_time[current_window][output_port] +=
                (tw_now(lp) - s->last_buf_full[output_port]);
        }
    }
    ...
    if ((cur_entry->msg.packet_size < s->params->chunk_size) &&
        (cur_entry->msg.chunk_id == num_chunks - 1))
        injection_delay =
            bytes_to_ns(cur_entry->msg.packet_size % s->params->chunk_size, bandwidth);
    ...
    router_timing_trace_save_to_file(s, &cur_entry->msg, output_port, output_chan, to_terminal,
                                     global, next_output_available_delta, nominal_router_delay,
                                     highdef_actual_router_delay, is_dally_surrogate_on,
                                     router_timing_prediction_used, lp);

    cur_entry->msg.this_router_ptp_latency =
        s->next_output_available_time[output_port] - cur_entry->msg.this_router_arrival;
    ...
    tw_stime event_time_target_delay = propagation_ts + gen_noise(lp, &msg->num_rngs);
    dfdally_event_time_trace_row(lp, DRAGONFLY_DALLY_ROUTER, R_SEND, cur_entry->msg.next_stop,
                                 event_time_target_lp_type, event_time_target_event_type,
                                 propagation_ts, event_time_target_delay);
    event_time_target_delay =
        dfdally_event_time_predict_or_original(lp, DRAGONFLY_DALLY_ROUTER, R_SEND,
                                               cur_entry->msg.next_stop, event_time_target_lp_type,
                                               event_time_target_event_type, propagation_ts,
                                               event_time_target_delay);
    ...
    if (to_terminal) {
        ...
        if (cur_entry->msg.next_stop != cur_entry->msg.dest_terminal_lpid)
            printf("\n intra-group radix %d output port %d next stop %d",
                   s->params->intra_grp_radix, output_port, cur_entry->msg.next_stop);
        assert(cur_entry->msg.next_stop == cur_entry->msg.dest_terminal_lpid);
        e = model_net_method_event_new(cur_entry->msg.next_stop, event_time_target_delay, lp,
    ...
    int msg_size = s->params->chunk_size;
    if (((cur_entry->msg.packet_size % s->params->chunk_size) || cur_entry->msg.packet_size == 0) &&
        (cur_entry->msg.chunk_id == num_chunks - 1)) {
        bf->c11 = 1;
        s->link_traffic[output_port] += (cur_entry->msg.packet_size % s->params->chunk_size);
        s->link_traffic_sample[output_port] += (cur_entry->msg.packet_size % s->params->chunk_size);
    ...
    * next.*/
    ...
    if (g_congestion_control_enabled) {
        congestion_control_message* cc_msg_rc = cc_msg_rc_storage_create();
        cc_router_forwarded_packet(s->local_congestion_controller, lp, s->params->chunk_size,
                                   output_port, output_chan, cur_entry->msg.dfdally_src_terminal_id,
                                   cur_entry->msg.app_id, cc_msg_rc);
        rc_stack_push(lp, cc_msg_rc, cc_msg_rc_storage_delete, s->cc_st);
    }
    ...
    terminal_dally_message* m_new;
    e = model_net_method_event_new(lp->gid, injection_ts + gen_noise(lp, &msg->num_rngs), lp,
                                   DRAGONFLY_DALLY_ROUTER, (void**)&m_new, NULL);
    m_new->type = R_SEND;
    m_new->rail_id = msg->rail_id;
...
static void router_buf_update_rc(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                 tw_lp* lp) {
    ...
    if (bf->c3) {
        ...
        if (s->params->counting_bool > 0 && msg->last_bufupdate_time >= s->params->counting_start) {
            int current_window = (int)((msg->last_bufupdate_time - s->params->counting_start) /
                                       s->params->counting_interval);
            if (current_window < s->params->counting_windows) {
    ...
    if (bf->c1) {
        terminal_dally_message_list* head =
            return_tail_from_qlist(&s->pending_msgs[indx][output_chan]);
...
static void router_buf_update(router_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    ...
    if (s->last_buf_full[indx] > 0.0) {
        ...
        if (s->params->counting_bool > 0 && msg->last_bufupdate_time >= s->params->counting_start) {
            int current_window = (int)((msg->last_bufupdate_time - s->params->counting_start) /
                                       s->params->counting_interval);
            if (current_window < s->params->counting_windows) {
                int full_window = (int)((s->last_buf_full[indx] - s->params->counting_start) /
                                        s->params->counting_interval);
                if (full_window == current_window) {
                    s->agg_busy_time[current_window][indx] += (tw_now(lp) - s->last_buf_full[indx]);
                } else {
                    s->agg_busy_time[current_window][indx] +=
                        (tw_now(lp) - (s->params->counting_start +
                                       current_window * s->params->counting_interval));
                    s->agg_busy_time[full_window][indx] +=
                        ((s->params->counting_start +
                          current_window * s->params->counting_interval) -
                         s->last_buf_full[indx]);
                }
            }
        }
    ...
    if (!qlist_empty(&s->queued_msgs[indx][output_chan])) {
        ...
        terminal_dally_message_list* head =
            item ? qlist_entry(item, terminal_dally_message_list, list) : NULL;
...
static void terminal_dally_event(terminal_state* s, tw_bf* bf, terminal_dally_message* msg,
                                 tw_lp* lp) {
    if (dfdally_surrogate_debug_prints && dfdally_surrogate_has_switched_once &&
        ...
        if (dfdally_post_switch_terminal_events <= 20 ||
            ...
            fprintf(stderr,
                    ...
                    (unsigned long long)dfdally_post_switch_terminal_events, tw_now(lp),
                    (unsigned long long)lp->gid, msg->type, dfdally_last_post_switch_terminal_now,
    ...
    if (is_dally_surrogate_on && freeze_network_on_switch) {
        ...
        if (!(msg->type == T_GENERATE || msg->type == T_ARRIVE_PREDICTED ||
              msg->type == T_NOTIFY)) {
            bf->c20 = 1;
...
static void router_dally_event(router_state* s, tw_bf* bf, terminal_dally_message* msg, tw_lp* lp) {
    if (dfdally_surrogate_debug_prints && dfdally_surrogate_has_switched_once &&
        !is_dally_surrogate_on) {
        ...
        if (dfdally_post_switch_router_events <= 20 ||
            ...
            fprintf(stderr,
                    ...
                    "last_now=%f delta_now=%f\n",
                    (unsigned long long)dfdally_post_switch_router_events, tw_now(lp),
                    (unsigned long long)lp->gid, msg->type, dfdally_last_post_switch_router_now,
    ...
    default:
        printf("\n (%lf) [Router %d] Router Message type not supported %d dest "
               ...
               tw_now(lp), (int)lp->gid, msg->type, (int)msg->dest_terminal_lpid,
               (int)msg->packet_ID);
        tw_error(TW_LOC, "Msg type not supported");
        break;
    }
...
static void terminal_dally_rc_event_handler(terminal_state* s, tw_bf* bf,
                                            terminal_dally_message* msg, tw_lp* lp) {
    s->rev_events++;
...
static void router_dally_rc_event_handler(router_state* s, tw_bf* bf, terminal_dally_message* msg,
                                          tw_lp* lp) {
...
static void copy_rank_tbl(struct qhash_table* into, struct qhash_table const* from) {
    ...
    for (int i = 0; i < from->table_size; i++) {
        ...
        qlist_for_each_entry(entry, &from->array[i], hash_link) {
            ...
            *new_entry =
                *entry; // There is no need to copy contents of pointer because we don't check it
...
static bool check_dfly_qhash_entry(struct dfly_qhash_entry* before,
                                   struct dfly_qhash_entry* after) {
    ...
    for (int i = 0; i < rank_tbl->table_size; i++) {
        ...
        qlist_for_each_entry(entry, &rank_tbl->array[i], hash_link) {
            ...
            if (entry->remote_event_size) {
                tw_fprint_binary_array(out, subprefix, entry->remote_event_data,
                                       entry->remote_event_size);
            }
            fprintf(out, "%s     | },\n", prefix);
        }
    }
...
static void save_terminal_state(terminal_state* into, terminal_state const* from) {
    ...
    if (!is_dally_surrogate_on) {
        ...
        for (int i = 0; i < num_rails; i++) {
            ...
            into->terminal_msgs[i] =
                (struct qlist_head*)malloc(num_qos_levels * sizeof(struct qlist_head));
            for (int j = 0; j < num_qos_levels; j++) {
                ...
                copy_terminal_dally_message_qlist(&into->terminal_msgs[i][j],
                                                  &from->terminal_msgs[i][j]);
            }
    ...
    map<struct packet_id, uint32_t>* from_remaining_sz_packets =
        (map<struct packet_id, uint32_t>*)&from->remaining_sz_packets;
    set<struct packet_id>* from_zombies = (set<struct packet_id>*)&from->zombies;

    std::map<struct packet_id, uint32_t>::iterator it_map;
    for (it_map = from_remaining_sz_packets->begin(); it_map != from_remaining_sz_packets->end();
         ++it_map) {
    ...
    for (it_set = from_zombies->begin(); it_set != from_zombies->end(); ++it_set) {
        struct packet_id const zombie = {.packet_ID = it_set->packet_ID,
                                         .dfdally_src_terminal_id =
                                             it_set->dfdally_src_terminal_id};
...
static bool check_terminal_state(terminal_state* before, terminal_state* after) {
    ...
    if (!is_dally_surrogate_on) {
        ...
        for (int i = 0; i < num_rails; i++) {
            for (int j = 0; j < num_qos_levels; j++) {
                ...
                is_same &= check_terminal_dally_message_qlist(&before->terminal_msgs[i][j],
                                                              &after->terminal_msgs[i][j]);
            }
    ...
    if (after->local_congestion_controller != NULL) {
        is_same &= check_tlc_state(before->local_congestion_controller,
                                   after->local_congestion_controller);
    }
...
static void print_terminal_state(FILE* out, char const* prefix, terminal_state* state) {
    ...
    if (is_dally_surrogate_on) {
        ...
        fprintf(out, "%s  | **  vc_occupancy[%d][%d] = [\n", prefix, state->params->num_rails,
                state->params->num_qos_levels);
        for (int i = 0; i < state->params->num_rails; i++) {
        ...
        fprintf(out, "%s  | ***        terminal_msgs[%d][%d] = [\n", prefix,
                state->params->num_rails, state->params->num_qos_levels);
        for (int i = 0; i < state->params->num_rails; i++) {
            fprintf(out, "%s  |   rail %d: [\n", prefix, i);
            for (int j = 0; j < state->params->num_qos_levels; j++) {
                fprintf(out, "%s  |    | qos level %d\n", prefix, j);
                print_terminal_dally_message_qlist(out, subprefix, state,
                                                   &state->terminal_msgs[i][j]);
            }
        }
        fprintf(out, "%s  | ]\n", prefix);
        free(subprefix);
    ...
    fprintf(
        out, "%s  | *            sample_stat = %p\n", prefix,
        state
            ->sample_stat); // ingnoring as this part of the code is never used. Originally part of instrumentation
    ...
    std::map<struct packet_id, uint32_t>::iterator it_map;
    for (it_map = state->remaining_sz_packets.begin(); it_map != state->remaining_sz_packets.end();
         ++it_map) {
        fprintf(out, "%s  |         {packet_ID: %lu, dfdally_src_terminal_id: %u} -> %d,\n", prefix,
                it_map->first.packet_ID, it_map->first.dfdally_src_terminal_id, it_map->second);
    }
    fprintf(out, "%s  |     }\n", prefix);
    ...
    for (int i = 0; i < radix; i++) {
        ...
        for (int j = 0; j < p->num_vcs; j++) {
            ...
            while (!qlist_empty(&state->pending_msgs[i][j])) {
                ...
                terminal_dally_message_list* entry =
                    qlist_entry(item, terminal_dally_message_list, list);
                free(entry->event_data);
                free(entry);
            }
...
static bool check_router_state(router_state const* before, router_state const* after) {
    ...
    if (before->router_id != after->router_id || before->group_id != after->group_id ||
        before->plane_id != after->plane_id || before->op_arr_size != after->op_arr_size ||
...
char const* const string_event_t(enum event_t type) {
    ...
    case T_NOTIFY:
        return "T_NOTIFY";
    case T_ARRIVE_PREDICTED:
        return "T_ARRIVE_PREDICTED";
#if ALWAYS_DETERMINISTIC_NETWORK
    case T_VACUOUS_EVENT:
        return "T_VACUOUS_EVENT";
#endif /* ALWAYS_DETERMINISTIC_NETWORK */
    default:
        return "UNKNOWN TYPE!!";
    }
...
bool check_terminal_dally_message(struct terminal_dally_message* before,
                                  struct terminal_dally_message* after) {
    bool is_same = true;
...
// Print fuction originally constructed with help from Claude.ai
void print_terminal_dally_message(FILE* out, char const* prefix, void* s,
                                  struct terminal_dally_message* msg) {
    ...
    fprintf(out, "%s  |                       type = %d (%s)\n", prefix, msg->type,
            string_event_t((enum event_t)msg->type));
...
tw_lptype dragonfly_dally_lps[] = {
    ...
    {(init_f)terminal_dally_init, (pre_run_f)NULL, (event_f)terminal_dally_event,
     (revent_f)terminal_dally_rc_event_handler, (commit_f)terminal_dally_commit,
     (final_f)dragonfly_dally_terminal_final, (map_f)codes_mapping, sizeof(terminal_state)},
    {
...
static void dfdally_select_intermediate_group(router_state* s, tw_bf* bf,
    ...
    if (msg->intm_grp_id ==
        -1) { // Intermediate group hasn't been chosen yet, choose one randomly and route toward it
        assert(s->router_id == msg->origin_router_id);
        msg->num_rngs++;
        int rand_group_id;
        if (NONMIN_INCLUDE_SOURCE_DEST) //then any group is a valid intermediate group
            rand_group_id = tw_rand_integer(lp->rng, s->params->num_groups * s->plane_id,
                                            (s->params->num_groups * (s->plane_id + 1)) - 1);
        else { //then we don't consider source or dest groups as valid intermediate groups
            vector<int> group_list;
            for (int i = s->params->num_groups * s->plane_id;
                 i < s->params->num_groups * (s->plane_id + 1); i++) {
        ...
        for (vector<Connection>::iterator it = global_conns.begin(); it != global_conns.end();
             it++) {
            Connection conn = *it;
            if (NONMIN_INCLUDE_SOURCE_DEST) //then any group I connect to is valid
            {
                ...
                if ((conn.dest_group_id != fdest_group_id) &&
                    (conn.dest_group_id != origin_group_id))
                    valid_intm_groups.insert(conn.dest_group_id);
            }
        }
...
static vector<Connection> get_legal_minimal_stops(router_state* s, tw_bf* bf,
    ...
    if (my_group_id !=
        fdest_group_id) { //we're in origin group or intermediate group - either way we need to route to fdest group minimally
        vector<Connection> conns_to_dest_group =
            s->connMan.get_connections_to_group(fdest_group_id);
        ...
        // }
            ...
            vector<Connection> poss_next_conns_to_group =
                s->connMan.get_routed_connections_to_group(fdest_group_id, true);
        ...
        vector<Connection> conns_to_fdest_router =
            s->connMan.get_connections_to_gid(fdest_router_id, CONN_LOCAL);
...
static vector<Connection> get_legal_nonminimal_stops(router_state* s, tw_bf* bf,
                                                     terminal_dally_message* msg, tw_lp* lp,
                                                     int fdest_router_id) {
    int my_router_id = s->router_id;
    int my_group_id = s->group_id;
    int origin_group_id = msg->origin_router_id / s->params->num_routers;
    int fdest_group_id = fdest_router_id / s->params->num_routers;
    bool in_intermediate_group =
        (my_group_id != origin_group_id) && (my_group_id != fdest_group_id);
    int preset_intm_group_id = msg->intm_grp_id;

    if (my_group_id == origin_group_id) {
        vector<Connection> conns_to_intm_group =
            s->connMan.get_connections_to_group(preset_intm_group_id);
        //are we the originating router
        if (my_router_id ==
            msg->origin_router_id) { //then we are able to route within our own group if necessary
            ...
            if (conns_to_intm_group.size() > 0) { //yes
                ...
                vector<Connection> conns_to_connecting_routers =
                    s->connMan.get_routed_connections_to_group(preset_intm_group_id, true);
            ...
            if (conns_to_intm_group.size() > 0) {
                ...
                dfdally_select_intermediate_group(s, bf, msg, lp, fdest_router_id);
                conns_to_intm_group =
                    s->connMan.get_connections_to_group(msg->intm_grp_id); //new intm group id
                return conns_to_intm_group;
            }
        }
        ...
        tw_error(TW_LOC,
                 "Invalid group somehow: not origin, not intermediate, and not fdest group\n");
        vector<Connection> empty;
        return empty;
    }
}
...
static Connection dfdally_minimal_routing(router_state* s, tw_bf* bf, terminal_dally_message* msg,
    ...
    if (poss_next_stops.size() < 1)
        tw_error(TW_LOC, "%d group %d: MINIMAL DEAD END to %d in group %d - (s%d -> d%d)\n",
                 s->router_id, s->group_id, fdest_router_id,
                 fdest_router_id / s->params->num_routers, msg->origin_router_id, fdest_router_id);
...
static Connection dfdally_nonminimal_routing(router_state* s, tw_bf* bf,
    ...
    vector<Connection> conns_to_next_group =
        s->connMan.get_connections_to_group(next_dest_group_id);
    if (conns_to_next_group.size() > 0) { //Then yes I do
        ...
        vector<Connection> connections_toward_next_group =
            s->connMan.get_routed_connections_to_group(next_dest_group_id, true);
...
static Connection dfdally_prog_adaptive_routing(router_state* s, tw_bf* bf,
    ...
    vector<Connection> poss_min_next_stops =
        get_legal_minimal_stops(s, bf, msg, lp, fdest_router_id);
    vector<Connection> poss_nonmin_next_stops =
        get_legal_nonminimal_stops(s, bf, msg, lp, fdest_router_id);
    ...
    if (poss_min_next_stops.size() > 0) {
        conn_type_of_mins =
            poss_min_next_stops[0].conn_type; // All of these in this vector should be the same...
    }
    ...
    if (conn_type_of_mins == CONN_GLOBAL)
        best_min_conn = dfdally_get_best_from_k_connections(s, bf, msg, lp, poss_min_next_stops,
                                                            s->params->global_k_picks);
    else
        best_min_conn = get_absolute_best_connection_from_conns(
            s, bf, msg, lp,
            poss_min_next_stops); //could use from_k_connections function but that's very expensive when k == size of input connections

    if (conn_type_of_nonmins == CONN_GLOBAL)
        best_nonmin_conn =
            dfdally_get_best_from_k_connections(s, bf, msg, lp, poss_nonmin_next_stops,
                                                s->params->global_k_picks);
    else
        best_nonmin_conn =
            get_absolute_best_connection_from_conns(s, bf, msg, lp, poss_nonmin_next_stops);
    ...
    if ((msg->path_type == NON_MINIMAL) &&
        (msg->is_intm_visited !=
         1)) { //if we're nonminimal and haven't reached the intermediate group yet
...
static set<Connection> get_smart_legal_minimal_stops(router_state* s, tw_bf* bf,
    ...
    if (s->group_id == fdest_router_id / s->params->num_routers) {
        possible_next_dests =
            netMan.get_valid_next_hops_conns(s->router_id, fdest_router_id,
                                             (max_hops_per_group - msg->my_hops_cur_group), 0);
    } else {
        possible_next_dests =
            netMan.get_valid_next_hops_conns(s->router_id, fdest_router_id,
                                             (max_hops_per_group - msg->my_hops_cur_group),
                                             (max_global_hops_in_path - msg->my_g_hop));
    }
...
static set<Connection> get_smart_legal_nonminimal_stops(router_state* s, tw_bf* bf,
                                                        terminal_dally_message* msg, tw_lp* lp,
                                                        int fdest_router_id) {
    set<Connection> possible_next_dests;
    if (s->group_id == fdest_router_id / s->params->num_routers) {
        ...
        if (possible_next_dests.size() == 0)
            possible_next_dests =
                netMan.get_valid_next_hops_conns(s->router_id, fdest_router_id,
                                                 (max_hops_per_group - msg->my_hops_cur_group),
                                                 (max_global_hops_nonminimal - msg->my_g_hop));
    } else {
        possible_next_dests =
            netMan.get_valid_next_hops_conns(s->router_id, fdest_router_id,
                                             (max_hops_per_group - msg->my_hops_cur_group),
...
static Connection dfdally_smart_minimal_routing(router_state* s, tw_bf* bf,
    ...
    if (direct_conns.size() > 0) {
        ...
        set<Connection> possible_conns =
            get_smart_legal_minimal_stops(s, bf, msg, lp, fdest_router_id);
...
static Connection dfdally_smart_nonminimal_routing(router_state* s, tw_bf* bf,
    ...
    set<Connection> possible_conns =
        get_smart_legal_nonminimal_stops(s, bf, msg, lp, fdest_router_id);
...
static Connection dfdally_smart_prog_adaptive_routing(router_state* s, tw_bf* bf,
    ...
    if (my_router_id == fdest_router_id) // we're the destination, send to connected terminal
    {
        if (s->router_id == fdest_router_id) {
            ...
            if (poss_next_stops.size() < 1)
                tw_error(TW_LOC,
                         "Destination Router %d: No connection to destination terminal %d\n",
                         s->router_id,
                         msg->dfdally_dest_terminal_id); //shouldn't happen unless math was wrong
            Connection best_conn =
                get_absolute_best_connection_from_conns(s, bf, msg, lp, poss_next_stops);
    ...
    {
        set<Connection> poss_min_next_stops =
            get_smart_legal_minimal_stops(s, bf, msg, lp, fdest_router_id,
                                          max_global_hops_nonminimal);
        return get_absolute_best_connection_from_conn_set(s, bf, msg, lp, poss_min_next_stops);
    } else {
        if (msg->path_type == NON_MINIMAL) {
            set<Connection> poss_next_stops =
                get_smart_legal_nonminimal_stops(s, bf, msg, lp, fdest_router_id);
            if (poss_next_stops.size() < 1)
                tw_error(TW_LOC, "Smart Prog Adaptive Routing: intm can't reach intm rtr");
            Connection best_conn =
                dfdally_get_best_from_k_connection_set(s, bf, msg, lp, poss_next_stops,
                                                       s->params->global_k_picks);
            return best_conn;
        } else {
            if (my_group_id == origin_group_id) {
                set<Connection> poss_min_next_stops =
                    get_smart_legal_minimal_stops(s, bf, msg, lp, fdest_router_id);
                set<Connection> poss_non_min_next_stops =
                    get_smart_legal_nonminimal_stops(s, bf, msg, lp, fdest_router_id);

                Connection best_min_conn =
                    dfdally_get_best_from_k_connection_set(s, bf, msg, lp, poss_min_next_stops,
                                                           s->params->global_k_picks);
                Connection best_nonmin_conn =
                    dfdally_get_best_from_k_connection_set(s, bf, msg, lp, poss_non_min_next_stops,
                                                           s->params->global_k_picks);

                int min_score = dfdally_score_connection(s, bf, msg, lp, best_min_conn, C_MIN);
                int nonmin_score =
                    dfdally_score_connection(s, bf, msg, lp, best_nonmin_conn, C_NONMIN);
                ...
                set<Connection> poss_min_next_stops =
                    get_smart_legal_minimal_stops(s, bf, msg, lp, fdest_router_id,
                                                  max_global_hops_nonminimal);
                return dfdally_get_best_from_k_connection_set(s, bf, msg, lp, poss_min_next_stops,
                                                              s->params->global_k_picks);
            }
        }
    }
...
static int get_port_score_legacy(router_state* s, int port, int bias) {
    int port_count = 0;
...
static void do_local_adaptive_routing_legacy(router_state* s, tw_lp* lp,
                                             terminal_dally_message* msg, tw_bf* bf,
                                             int dest_router_id, int intm_router_id,
                                             short* rng_counter) {
    //this is an adaptation of the do_local_adaptive_routing() from the original dragonfly-dally model found below
    int next_min_stop =
        dest_router_id; //was a vector of one in original - this is equivalent to what it did
...
static int get_output_port_legacy(router_state* s, terminal_dally_message* msg, tw_lp* lp,
                                  tw_bf* bf, int next_stop, short* rng_counter) {
    int output_port = -1;
    int rand_offset = -1;
    ...
    if ((tw_lpid)next_stop == msg->dest_terminal_lpid) {
        Connection term_conn =
            s->connMan.get_connections_to_gid(msg->dfdally_dest_terminal_id, CONN_TERMINAL)[0];
        ...
        if (intm_grp_id != s->group_id) {
            ...
            vector<Connection> conns_to_local_router =
                s->connMan.get_connections_to_gid(local_router_id, CONN_LOCAL);
...
static tw_lpid get_next_stop_legacy(router_state* s, tw_lp* lp, tw_bf* bf,
                                    terminal_dally_message* msg, int dest_router_id, int adap_chan,
                                    int do_chan_selection, int get_direct_con, short* rng_counter) {
    int dest_lp;
    tw_lpid router_dest_id;
    int dest_group_id;
    ...
    if (s->group_id == dest_group_id) {
        int next_stop =
            dest_router_id; //trimmed down from old as the old had a lot of superflouous code to poll randomly from a vector of one.

        codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, s->anno, 0,
                                next_stop / num_routers_per_mgrp, next_stop % num_routers_per_mgrp,
                                &router_dest_id);
    ...
    if (msg->last_hop == TERMINAL || s->router_id == msg->intm_rtr_id ||
        ...
        if (adap_chan >= 0) //
            ...
            * (global ports) */
            if (get_direct_con) {
                if (s->connMan.get_connections_to_group(dest_group_id).size() >
                    1) //NTODO - this looks like maybe it should be >= 1
                    select_chan =
                        find_chan_legacy(s->router_id, dest_group_id, s->params->num_routers);
                else {
                ...
                (*rng_counter)++;
                select_chan = tw_rand_integer(lp->rng, 0,
                                              connectionList[my_grp_id][dest_group_id].size() - 1);
            }
        }
    ...
    if (s->router_id == msg->saved_src_dest) {
        ...
        int dests =
            msg->saved_src_dest; //trimmed down from old as the old had a lot of superflouous code to poll randomly from a vector of one.
...
#endif
    codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, s->anno, 0,
                            dest_lp / num_routers_per_mgrp, dest_lp % num_routers_per_mgrp,
                            &router_dest_id);
...
static int do_global_adaptive_routing_legacy(router_state* s, tw_lp* lp,
                                             terminal_dally_message* msg, tw_bf* bf,
                                             int dest_router_id, int intm_id_a, int intm_id_b,
                                             short* rng_counter) {
    int next_chan = -1;
    ...
    codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, s->anno, 0,
                            dest_rtr_as[dest_rtr_a_sel] / num_routers_per_mgrp,
    ...
    if (num_min_chans > 1) {
        ...
        codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, s->anno, 0,
                                dest_rtr_bs[dest_rtr_b_sel] / num_routers_per_mgrp,
    ...
    if (nonmin_chan_a != -1) {
        ...
        if (nonmin_rtr_a == s->router_id) {
            noIntraA = true;
            nonmin_rtr_a =
                s->connMan.get_connections_to_group(intm_grp_id_a)[0]
                    .dest_gid; //NOTE: This line did not exist in the original but I believe this was what was supposed to happen and it won't work without it, otherwise nonmin_rtr_* is THIS ROUTER
        }
    }
    ...
    if (num_nonmin_chans_b > 0) {
        ...
        if (nonmin_chan_b != -1) {
            ...
            if (nonmin_rtr_b == s->router_id) {
                noIntraB = true;
                nonmin_rtr_b =
                    s->connMan.get_connections_to_group(intm_grp_id_b)[0]
                        .dest_gid; //NOTE: This line did not exist in the original but I believe this was what was supposed to happen and it won't work without it, otherwise nonmin_rtr_* is THIS ROUTER
            }
        }
...
static Connection dfdally_prog_adaptive_legacy_routing(router_state* s, tw_bf* bf,
    ...
    int dest_router_id =
        codes_mapping_get_lp_relative_id(msg->dest_terminal_lpid, 0, 0) / s->params->num_cn;
    ...
    if (src_grp_id != dest_grp_id) {
        if (msg->my_l_hop == max_lvc_src_g) {
            ...
            for (; it != groups_i_connect_to.end(); it++) {
                ...
                for (
                    int i = 0; i < num_conns_to_group;
                    i++) //old version adds same router the order of the number of connections to said group //NTODO this is probably an error in the original
                {
        ...
        msg->num_rngs++;
        intm_rtr_a =
            (src_grp_id * num_routers) +
            (((s->router_id % num_routers) + tw_rand_integer(lp->rng, 1, num_routers - 1)) %
             num_routers);
    }
    ...
    * global channel. It doesn't make sense to use it within a group */
    if (dest_grp_id != src_grp_id && ((msg->last_hop == TERMINAL && routing == ADAPTIVE) ||
                                      (msg->path_type == MINIMAL &&
                                       routing == PROG_ADAPTIVE_LEGACY
                                       // && s->router_id != dest_router_id)))
                                       && my_group_id == src_grp_id))) {
        adap_chan = do_global_adaptive_routing_legacy(s, lp, msg, bf, dest_router_id, intm_rtr_a,
                                                      intm_rtr_b, &(msg->num_rngs));
    }
    ...
    * triggered */
    ...
    if (dest_grp_id == src_grp_id && dest_router_id != s->router_id &&
        (routing == ADAPTIVE || routing == PROG_ADAPTIVE_LEGACY) && msg->last_hop == TERMINAL) {
        do_local_adaptive_routing_legacy(s, lp, msg, bf, dest_router_id, intm_rtr_a,
                                         &(msg->num_rngs));
    }
    ...
    if (msg->path_type != MINIMAL && msg->path_type != NON_MINIMAL)
        tw_error(TW_LOC, "\n packet src %d dest %d intm %d src grp %d dest grp %d", s->router_id,
                 dest_router_id, intm_rtr_a, src_grp_id, dest_grp_id);

    assert(msg->path_type == MINIMAL || msg->path_type == NON_MINIMAL);

    /* If non-minimal, set the random destination */
    if (msg->last_hop == TERMINAL && msg->path_type == NON_MINIMAL && msg->intm_rtr_id == -1) {
    ...
    * routing, we have to compute the next stop based on that */
    ...
    if (routing == PROG_ADAPTIVE_LEGACY && prev_path_type != next_path_type &&
        s->group_id == src_grp_id)
        do_chan_selection = 1;

    next_stop = get_next_stop_legacy(s, lp, bf, msg, dest_router_id, adap_chan, do_chan_selection,
                                     get_direct_con, &(msg->num_rngs));

#if DEBUG == 1
    if (msg->packet_ID == LLU(TRACK_PKT) && msg->src_terminal_id == T_ID)
        printf("\n Packet %llu arrived at router %u next stop %d final stop %d local hops %d "
               "global hops %d",
               msg->packet_ID, s->router_id, next_stop, dest_router_id, msg->my_l_hop,
               msg->my_g_hop);
#endif
