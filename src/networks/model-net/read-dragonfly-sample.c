...
int main(int argc, char** argv) {
   ...
   if(!RADIX)
   {
    ...
    if (my_rank == 0) {
        ...
        fprintf(fp_meta,
                "Rank_ID num_finished_chunks data_size_finished(bytes) finished_hops "
                "time_spent(ns) busy_time(ns) num_fwd_events num_rev_events sample_end_time(ns)");
        fclose(fp_meta);
    }
    ...
    for (i = 0; i < in_sz / sizeof(struct dfly_samples); i++) {
        ...
        fprintf(writeFile, "%ld %ld %ld %lf %lf %lf %ld %ld %lf \n", event_array[i].terminal_id,
                event_array[i].fin_chunks_sample, event_array[i].data_size_sample,
                event_array[i].fin_hops_sample, event_array[i].fin_chunks_time,
                event_array[i].busy_time_sample, event_array[i].fwd_events,
                event_array[i].rev_events, event_array[i].end_time);
    }
    fclose(pFile);
    fclose(writeFile);
    ...
    if (my_rank == 0) {
        ...
        fprintf(fp_rtr_meta,
                "Format: Router_ID Busy_time_per_channel(ns) Link_traffic_per_channel(ns) "
                "Sample_end_time(ns) fwd_events reverse_events");
        fclose(fp_rtr_meta);
    }
    ...
    if (my_rank == 0) {
        ...
        fprintf(fp_ops_meta,
                "network_node_id app_id num_sends num_bytes_sent num_waits sample_end_time(ns)");
        fclose(fp_ops_meta);
    }
    fread(mpi_event_array, mpi_sample_sz, in_sz_mpi / mpi_sample_sz, pFile);
    for (i = 0; i < in_sz_mpi / mpi_sample_sz; i++) {
        fprintf(writeFile, "\n %d %d %lu %lu %lu %lf ", mpi_event_array[i].nw_id,
                mpi_event_array[i].app_id, mpi_event_array[i].num_sends_sample,
                mpi_event_array[i].num_bytes_sample, mpi_event_array[i].num_waits_sample,
