...
int num_indirect_routes =
    4; //Number of indirect (Valiant) routes to use in Adaptive routing methods
...
struct slimfly_param {
    ...
    double
        local_bandwidth; /*LOCAL_BANDWIDTH bandwidth of the router-router channels within a group */
    double global_bandwidth; /*GLOBAL_BANDWIDTH bandwidth of the inter-group router connections */
    double
        cn_bandwidth; /*NODE_BANDWIDTH bandwidth of the compute node channels connected to routers */
...
enum event_t { T_GENERATE = 1, T_ARRIVE, T_SEND, T_BUFFER, R_SEND, R_ARRIVE, R_BUFFER };
/* status of a virtual channel can be idle, active, allocated or wait for credit */
enum vc_status { VC_IDLE, VC_ACTIVE, VC_ALLOC, VC_CREDIT };

/* whether the last hop of a packet was global, local or a terminal */
enum last_hop { GLOBAL, LOCAL, TERMINAL };
...
enum ROUTING_ALGO { MINIMAL = 0, NON_MINIMAL, ADAPTIVE };
...
static void ross_slimfly_sample_fn(terminal_state* s, tw_bf* bf, tw_lp* lp,
                                   struct slimfly_cn_sample* sample);
static void ross_slimfly_sample_rc_fn(terminal_state* s, tw_bf* bf, tw_lp* lp,
                                      struct slimfly_cn_sample* sample);
static void ross_slimfly_rsample_fn(router_state* s, tw_bf* bf, tw_lp* lp,
                                    struct slimfly_router_sample* sample);
static void ross_slimfly_rsample_rc_fn(router_state* s, tw_bf* bf, tw_lp* lp,
                                       struct slimfly_router_sample* sample);
...
void get_router_connections(int src_router_id, int num_global_channels, int num_local_channels,
                            int total_routers, int* local_channels, int* global_channels,
                            int sf_type, const slimfly_param* p);

st_model_types slimfly_model_types[] = {
    {(ev_trace_f)slimfly_event_collect, sizeof(int), (model_stat_f)slimfly_model_stat_collect,
     0, // update this when changing slimfly_model_stat_collect
     (sample_event_f)ross_slimfly_sample_fn, (sample_revent_f)ross_slimfly_sample_rc_fn,
     sizeof(struct slimfly_cn_sample)},
    {(ev_trace_f)slimfly_event_collect, sizeof(int), (model_stat_f)slimfly_model_stat_collect,
     0, // update this when changing slimfly_model_stat_collect
     (sample_event_f)ross_slimfly_rsample_fn, (sample_revent_f)ross_slimfly_rsample_rc_fn,
...
static int slimfly_rank_hash_compare(void* key, struct qhash_head* link) {
...
static void append_to_terminal_message_list(slim_terminal_message_list** thisq,
                                            slim_terminal_message_list** thistail, int index,
...
static void prepend_to_terminal_message_list(slim_terminal_message_list** thisq,
                                             slim_terminal_message_list** thistail, int index,
                                             slim_terminal_message_list* msg) {
    if (thisq[index] == NULL) {
        thistail[index] = msg;
...
static slim_terminal_message_list* return_head(slim_terminal_message_list** thisq,
                                               slim_terminal_message_list** thistail, int index) {
...
static slim_terminal_message_list* return_tail(slim_terminal_message_list** thisq,
                                               slim_terminal_message_list** thistail, int index) {
...
tw_stime* buff_time_storage_create(terminal_state* s) {
    tw_stime* storage = (tw_stime*)malloc(s->params->ports_per_nic * sizeof(tw_stime));
    return storage;
}
...
static void slimfly_read_config(const char* anno, slimfly_param* params) {
    ...
    if (!g_tw_mynode)
        printf("SF num rails is %d\n", p->num_rails);

    p->num_injection_queues = 1;
    configuration_get_value_int(&config, "PARAMS", "num_injection_queues", anno,
    ...
    configuration_get_value_int(&config, "PARAMS", "num_routers", anno, &p->num_routers);
    if (p->num_routers <= 0) {
        p->num_routers = 4;
    ...
    configuration_get_value_double(&config, "PARAMS", "global_bandwidth", anno,
                                   &p->global_bandwidth);
    ...
    configuration_get_value_int(&config, "PARAMS", "global_channels", anno,
    ...
    size_t length;
    int ret =
        configuration_get_multivalue(&config, "PARAMS", "generator_set_X", anno, &values, &length);
    if (ret != 1)
    ...
    ret = configuration_get_multivalue(&config, "PARAMS", "generator_set_X_prime", anno, &values,
                                       &length);
    if (ret != 1)
    ...
    X_size = length;
    X_prime = (int*)calloc(length, sizeof(int));
    for (size_t i = 0; i < length; i++) {
    ...
    if (!rank) {
        printf("\n Total nodes %d total routers %d total groups %d num_terminals %d num_routers %d "
               "radix %d local_channels %d global_channels %d \n",
               p->num_cn * p->slim_total_routers, p->slim_total_routers, p->num_groups, p->num_cn,
               p->num_routers, p->radix, p->num_local_channels, p->num_global_channels);
    }
            ...
            for (t = 0; t < N_COLLECT_POINTS; t++) {
                ...
                for (j = 0; j < TEMP_RADIX; j++) {
                    for (i = 0; i < TEMP_NUM_VC; i++) {
                        ...
                        fprintf(slimfly_router_occupancy_log, "%d, ",
                                vc_occupancy_storage_router[k][j][i][t]);
                    }
                }
            }
        }
    ...
    if (tw_ismaster()) {
        ...
        for (k = 0; k < slim_total_terminals_noah; k++) {
            ...
            if (slimfly_terminal_occupancy_log == NULL) {
                printf("Failed to open slimfly terminal occupancy log file "
                       "vc-occupancy/terminals/slimfly_terminal_occupancy_log.%d.txt\n",
                       k);
            ...
            for (i = 0; i < N_COLLECT_POINTS; i++) {
                ...
                for (j = 0; j < 1; j++) {
                    fprintf(slimfly_terminal_occupancy_log, "%d, ",
                            vc_occupancy_storage_terminal[k][j][i]);
                }
            }
        }
    }
...
int slim_get_associated_router_id_from_terminal(int term_gid, int rail_id, int num_rails) {
    ...
    int num_term_lps = codes_mapping_get_lp_count(lp_group_name, 1, LP_CONFIG_NM_TERM, NULL,
                                                  0); // number of terminals per rep
    int num_routers = codes_mapping_get_lp_count(lp_group_name, 0, LP_CONFIG_NM_ROUT, NULL, 0);

    int default_router_id =
        term_rel_id /
        num_term_lps; // the router id that is assigned to the given terminal by default - relative.

    int num_routers_per_rail = num_routers / num_rails;

    tw_lpid router_lpgid;
    if (rail_id % 2 == 0) { //even rails have normal mapping
        codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, NULL, 1, default_router_id,
                                rail_id, &router_lpgid);
    } else { //odd rails have reverse mapping
        int rep_id = num_routers_per_rail - default_router_id - 1;
        codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, NULL, 1, rep_id, rail_id,
                                &router_lpgid);
    }

    int router_id =
        codes_mapping_get_lp_relative_id(router_lpgid, 0, 0); //get the router ID from the lpid

    return router_id;
}

/* initialize a slimfly compute node terminal */
void slim_terminal_init(terminal_state* s, tw_lp* lp) {
    s->packet_gen = 0;
    s->packet_fin = 0;
    ...
    int num_term_lps =
        codes_mapping_get_lp_count(lp_group_name, 1, LP_CONFIG_NM_TERM, s->anno,
                                   0); //keep in mind that this is the number of terminals per rep!

    int num_routers =
        codes_mapping_get_lp_count(lp_group_name, 0, "modelnet_slimfly_router", s->anno, 0);
    ...
    if (s->params->sf_type == 1) { //fitfly
        ...
        for (int i = 0; i < s->params->ports_per_nic; i++) {
            if (i % 2 == 0) { //even rails have normal mapping
                codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, NULL, 1, s->router_id, i,
                                        &s->router_lp[i]);
                // printf("  - Rail %d = %d",i, codes_mapping_get_lp_relative_id(s->router_lp[i],0,0));
            } else { //odd rails have reverse mapping
                int rep_id = num_routers_per_rail - s->router_id - 1;
                codes_mapping_get_lp_id(lp_group_name, LP_CONFIG_NM_ROUT, NULL, 1, rep_id, i,
                                        &s->router_lp[i]);
                // printf("  - Rail %d = %d",i, codes_mapping_get_lp_relative_id(s->router_lp[i],0,0));
            }
        }
        ...
        codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", NULL, 1, s->router_id, 0,
                                &s->router_lp[0]);
    }

#if SLIMFLY_CONNECTIONS
    int written = 0;
    written += sprintf(s->output_buf + written, "%d, %d, ", s->terminal_id,
                       s->params->slim_total_terminals + s->router_id);
#endif
    ...
    if (s->terminal_id == 1723)
        printf("terminal_id:%d, router_id:%d, router_lp[0]:%d, router_lp[1]:%d\n", s->terminal_id,
               s->router_id, (int)s->router_lp[0], (int)s->router_lp[1]);

    s->terminal_available_time = (tw_stime*)calloc(s->params->ports_per_nic, sizeof(tw_stime));
    s->busy_time = (tw_stime*)calloc(s->params->ports_per_nic, sizeof(tw_stime));
    s->issueIdle = (int*)calloc(s->params->ports_per_nic, sizeof(int));
    s->in_send_loop = (int*)calloc(s->params->ports_per_nic, sizeof(int));
    s->terminal_length = (int*)calloc(s->params->ports_per_nic, sizeof(int));
    s->vc_occupancy = (int*)calloc(s->params->ports_per_nic, sizeof(int));
    s->last_buf_full = (tw_stime*)calloc(s->params->ports_per_nic, sizeof(tw_stime));
    s->terminal_msgs = (slim_terminal_message_list**)calloc(s->params->ports_per_nic,
                                                            sizeof(slim_terminal_message_list*));
    s->terminal_msgs_tail =
        (slim_terminal_message_list**)calloc(s->params->ports_per_nic,
                                             sizeof(slim_terminal_message_list*));
...
void slim_router_setup(router_state* r, tw_lp* lp) {
    ...
    r->local_channel = (int*)calloc(p->num_local_channels, sizeof(int));
    ...
    r->pending_msgs =
        (slim_terminal_message_list***)calloc(p->radix, sizeof(slim_terminal_message_list**));
    r->pending_msgs_tail =
        (slim_terminal_message_list***)calloc(p->radix, sizeof(slim_terminal_message_list**));
    r->queued_msgs =
        (slim_terminal_message_list***)calloc(p->radix, sizeof(slim_terminal_message_list**));
    ...
    if (g_st_use_analysis_lps && g_st_model_stats)
        lp->model_types->sample_struct_sz =
            sizeof(struct slimfly_router_sample) + sizeof(int) * p->radix;
    ...
    for (int i = 0; i < p->radix; i++) {
        ...
        r->pending_msgs[i] =
            (slim_terminal_message_list**)calloc(p->num_vcs, sizeof(slim_terminal_message_list*));
    ...
    char log[500];
    sprintf(log, "simulation-input-files/MMS.%d/MMS.%d.%d.bsconf",
            p->num_global_channels + p->num_local_channels,
            p->num_global_channels + p->num_local_channels, p->num_cn);
    MMS_input_file = fopen(log, "r");
    if (MMS_input_file == NULL)
        tw_error(
            TW_LOC,
            "Failed to Open Slim_fly input MMS layout file: %s The reason *may* have been %s\n",
            log, strerror(errno));
    ...
    for (i = 0; i < r->params->num_local_channels; i++) {
        written += sprintf(r->output_buf + written, "%d, %d, ",
                           r->params->slim_total_terminals + r->router_id,
                           r->params->slim_total_terminals + r->local_channel[i]);
    }
    for (i = 0; i < r->params->num_global_channels; i++) {
        written += sprintf(r->output_buf + written, "%d, %d, ",
                           r->params->slim_total_terminals + r->router_id,
                           r->params->slim_total_terminals + r->global_channel[i]);
    }
...
static tw_stime slimfly_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender, SLIMFLY,
...
static void slimfly_packet_event_rc(tw_lp* sender) {
    // printf("slimfly packet event rc\n");
...
void slim_router_credit_send(router_state* s, slim_terminal_message* msg, tw_lp* lp, int sq) {
    // printf("slim router credit send\n");
    ...
    if (is_terminal) {
        buf_e = model_net_method_event_new(dest, ts, lp, SLIMFLY, (void**)&buf_msg, NULL);
...
void slim_packet_generate_rc(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slimfly packet gen rc\n");

    s->packet_gen--;
    slimfly_packet_gen--;

    for (int i = 0; i < msg->rng_calls; i++) {
        tw_rand_reverse_unif(lp->rng);
    }

    // tw_rand_reverse_unif(lp->rng); //rng_calls
...
void slim_packet_generate(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    ...
    if (s->params->rail_select == RAIL_PATH) {
        if (s->params->num_rails > 2)
            tw_error(TW_LOC, "This rail path selection method doesn't support greater than 2 "
                             "rails. see notes in slim_packet_generate()\n");
        // Set starting rail to the first rail (rail 0)
        target_queue = 0;
        // Get information on destination terminal from its LP ID
        codes_mapping_get_lp_info(msg->dest_terminal_id, lp_group_name, &mapping_grp_id, NULL,
        ...
        int rail_one_dest_router_rel_id =
            (mapping_offset + (mapping_rep_id * num_lps)) / s->params->num_cn;
        // Get path length from rail 1 source router to rail 1 dest router
        int path_length_rail1 =
            get_path_length_from_terminal(s->router_id, rail_one_dest_router_rel_id, p);
        // Compute relative id of router in the second rail connected to destination compute terminal
        int rail_two_dest_router_rel_id =
            s->params->slim_total_routers - 1 - rail_one_dest_router_rel_id;
        // Get path length from rail 2 src router to rail 2 dest router
        int path_length_rail2 =
            get_path_length_from_terminal(s->params->slim_total_routers - 1 - s->router_id,
                                          rail_two_dest_router_rel_id, p);
    ...
    for (i = 0; i < num_chunks; i++) {
        slim_terminal_message_list* cur_chunk =
            (slim_terminal_message_list*)calloc(1, sizeof(slim_terminal_message_list));
        ...
        append_to_terminal_message_list(s->terminal_msgs, s->terminal_msgs_tail, target_queue,
                                        cur_chunk);
    ...
    total_event_size =
        model_net_get_msg_sz(SLIMFLY) + msg->remote_event_size_bytes + msg->local_event_size_bytes;
    mn_stats* stat;
    stat = model_net_find_stats(msg->category, s->slimfly_stats_array);
    stat->send_count++;
    stat->send_bytes += msg->packet_size;
...
void slim_packet_send_rc(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    ...
    if (bf->c1) {
        s->in_send_loop[msg->vc_index] = msg->saved_send_loop;
        return;
    }
    ...
    if (bf->c5) {
        ...
        if (bf->c6) {
            ...
            s->last_buf_full[msg->vc_index] = bts[msg->vc_index];
            buff_time_storage_delete(bts);
        }
    }
    ...
    prepend_to_terminal_message_list(s->terminal_msgs, s->terminal_msgs_tail, msg->vc_index,
                                     cur_entry);
...
void slim_packet_send(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slim pacekt send\n");
    tw_stime ts;
    tw_event* e;
    slim_terminal_message* m;
    ...
    if (s->vc_occupancy[msg->vc_index] + s->params->chunk_size > s->params->cn_vc_size ||
        cur_entry == NULL) {
        bf->c1 = 1;
        msg->saved_send_loop = s->in_send_loop[msg->vc_index];
    ...
    void* remote_event;
    e = model_net_method_event_new(s->router_lp[msg->vc_index], ts, lp, SLIMFLY_ROUTER, (void**)&m,
                                   &remote_event);
    ...
    //if(msg->packet_ID == TRACK)
#if DEBUG
    printf("%llu \x1B[34m-->Packet sending from terminal %d sending to router LP:%d relID:%d to "
           "arrive at %llu\x1b[0m\n",
           LLU(tw_now(lp)), (int)lp->gid, (int)s->router_lp[msg->vc_index], s->router_id,
           LLU(ts + tw_now(lp)));
#endif
    ...
    if (m->packet_ID == TRACK) {
        printf("(%lf) [Terminal lp:%d, rel:%d] packet_ID %lld message_ID %d is sending ",
               tw_now(lp), (int)lp->gid, s->terminal_id, m->packet_ID, (int)m->message_id);
    ...
    s->vc_occupancy[msg->vc_index] += s->params->chunk_size;
#if TERMINAL_OCCUPANCY_LOG
    vc_occupancy_storage_terminal[s->terminal_id][0][index] =
    ...
    if (cur_entry != NULL &&
        ...
        msg->rng_calls++;
        tw_event* e_new =
            model_net_method_event_new(lp->gid, ts, lp, SLIMFLY, (void**)&m_new, NULL);
        m_new->type = T_SEND;
    ...
    if (s->issueIdle[msg->vc_index]) {
        ...
        if (s->last_buf_full[msg->vc_index] > 0.0) {
            ...
            tw_stime* bts = buff_time_storage_create(s);
            bts[msg->vc_index] = s->last_buf_full[msg->vc_index];
            rc_stack_push(lp, bts, buff_time_storage_delete, s->st);
...
void slim_packet_arrive_rc(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slimfly packet arrive rc\n");
    for (int i = 0; i < msg->rng_calls; i++) {
        tw_rand_reverse_unif(lp->rng);
    }
...
void slim_send_remote_event(terminal_state* s, slim_terminal_message* msg, tw_lp* lp, tw_bf* bf,
                            char* event_data, int remote_event_size) {
    // printf("slim send remote event\n");
...
void slim_packet_arrive(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slim packet arrive\n");
    ...
    tw_event* buf_e;
    slim_terminal_message* buf_msg;
    buf_e = model_net_method_event_new(s->router_lp[msg->rail_id], ts, lp, SLIMFLY_ROUTER,
                                       (void**)&buf_msg, NULL);
    buf_msg->magic = slim_router_magic_num;
    buf_msg->vc_index = msg->vc_index;
    buf_msg->output_chan = msg->output_chan;
    ...
    {
        printf("(%lf) [Terminal %d] packet %lld has arrived. ", tw_now(lp), (int)lp->gid,
               msg->packet_ID);
    ...
    if (!hash_link) {
        bf->c5 = 1;
        struct sfly_qhash_entry* d_entry = calloc(1, sizeof(struct sfly_qhash_entry));
...
void slim_terminal_buf_update_rc(terminal_state* s, tw_bf* bf, slim_terminal_message* msg,
                                 tw_lp* lp) {
    // printf("slimfly term buf update rc\n");
...
void slim_terminal_buf_update(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slim terminal buf update\n");
    (void)msg;
    bf->c1 = 0;
...
void slim_terminal_event(terminal_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slim terminal event\n");
...
void slimfly_terminal_final(terminal_state* s, tw_lp* lp) {
    // printf("slim terminal final\n");
    ...
    if (!s->terminal_id)
        written =
            sprintf(s->output_buf,
                    "# Format <LP id> <Terminal ID> <Total Data Size> <Total Packet Latency> <# "
                    "Flits/Packets finished> <Packets Generated> <Avg hops> <Busy Time>\n");
    ...
    written += sprintf(s->output_buf + written, "%llu %u %llu %lf %ld %d %lf %lf\n", LLU(lp->gid),
                       s->terminal_id, LLU(s->total_msg_size), s->total_time, s->finished_packets,
                       s->packet_gen, (double)s->total_hops / s->finished_chunks,
    ...
    for (int i = 0; i < 200; i++)
        written2 += sprintf(s->output_buf2 + written2, "%u %d %d\n", s->terminal_id,
                            s->msg_send_times[i], s->msg_rail_select[i]);
...
void slimfly_router_final(router_state* s, tw_lp* lp) {
    // printf("slim router final\n");
    ...
    written +=
        sprintf(s->output_buf + written, "\n %llu %d %d", LLU(lp->gid), s->group_id, s->router_id);
 ...
 */
int get_path_length_from_terminal(int src, int dest, const slimfly_param* p) {
    int* local_channel = (int*)calloc(
        p->num_local_channels,
        sizeof(
            int)); //NM TODO: DYNAMIC ALLOCS ARE TIME EXPENSIVE - this function is called every packet generate?!
    ...
    if (match == 0) {
        ...
        for (i = 0; i < r->params->num_global_channels + r->params->num_local_channels; i++) {
            ...
            if (i < r->params->num_global_channels) {
                ...
                get3DCoordinates(r->local_channel[i - r->params->num_global_channels], &s_m, &i_m,
                                 &j_m, r);
            }
#if DEBUG_ROUTING
            if (msg->packet_ID == TRACK && msg->chunk_id == num_chunks - 1) {
                ...
                if (i < r->params->num_global_channels) {
                    ...
                    printf("case 4b,2a result comparing global%d and dest%d: %d=%d\n",
                           r->global_channel[i % GLOBAL_CHANNELS], rid, j_m,
                           (i_d * i_m + j_d) % NUM_ROUTER);
                    printf("case 4a,2c result comparing global%d and dest%d: %d=%d\n",
                           r->global_channel[i % GLOBAL_CHANNELS], rid, j_d,
                           (i_m * i_d + j_m) % NUM_ROUTER);
                } else {
                    ...
                    printf("case 4b,2a result comparing local%d and dest%d: %d=%d\n",
                           r->local_channel[i % GLOBAL_CHANNELS], rid, j_m,
                           (i_d * i_m + j_d) % NUM_ROUTER);
                    printf("case 4a,2c result comparing local%d and dest%d: %d=%d\n",
                           r->local_channel[i % GLOBAL_CHANNELS], rid, j_d,
                           (i_m * i_d + j_m) % NUM_ROUTER);
                }
            }
#endif
                            ...
                            for (j = 0; j < X_size; j++) {
                                if (abs(j_m - j_d) == X[j]) {
                                    if (i < r->params->num_global_channels) {
                                        ...
                                        router_id =
                                            r->local_channel[i - r->params->num_global_channels];
                                    }
                                    match = 1; // Case 3a
#if DEBUG_ROUTING
                                    if (msg->packet_ID == TRACK &&
...
tw_lpid slim_get_next_stop(router_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp,
                           int path, int dest_router_rel_id, int intm_id) {
...
#if TRACK_OUTPUT
        if (msg->packet_ID == TRACK) {
            printf("\x1b[32m-->Finishing Routing at current router:%d (gid:%d) to "
                   "dest_terminal(globalID):%d through randm intm router:%d next stop:%d "
                   "(globalid:%d)\x1b[0m\n",
                   s->router_id, (int)lp->gid, (int)msg->dest_terminal_id, msg->intm_router_id,
                   next_stop_rel_id, (int)next_stop_lp_id);
        }
#endif
    ...
    if (msg->last_hop == TERMINAL && path == NON_MINIMAL) {
        ...
        if (next_stop_rel_id > s->params->slim_total_routers - 1)
            codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", NULL, 1,
                                    next_stop_rel_id - (rpr * rail_id), rail_id, &next_stop_lp_id);
        else
            codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", s->anno, 0,
                                    next_stop_rel_id, 0, &next_stop_lp_id);
#if TRACK_OUTPUT
        //if( msg->packet_ID == TRACK )
        {
            printf(
                "\x1b[32m-->Starting First leg NonMinimal from current router:%d (gid:%d) to "
                "dest_router:%d through randm intm router:%d next stop:%d (globalid:%d)\x1b[0m\n",
                s->router_id, (int)lp->gid, dest_router_rel_id, msg->intm_router_id,
                next_stop_rel_id, (int)next_stop_lp_id);
        }
#endif
        return next_stop_lp_id;
    ...
    if (path == NON_MINIMAL && msg->intm_router_id == s->router_id) {
        ...
        //if( msg->packet_ID == TRACK )
        {
            printf("\x1b[32m-->Arrived at NonMinimal intm_router %d s->router_id:%d "
                   "local_router_rel_id:%d (gid:%d) dest_router:%d next stop:%d "
                   "(globalid:%d)\x1b[0m\n",
                   msg->intm_router_id, s->router_id, local_router_rel_id, (int)lp->gid,
                   dest_router_rel_id, next_stop_rel_id, (int)next_stop_lp_id);
        }
#endif
    ...
    if (path == NON_MINIMAL && msg->intm_router_id >= 0 &&
        (msg->intm_router_id != local_router_rel_id)) {
        next_stop_rel_id = getMinimalRouterFromEquations(msg, msg->intm_router_id, s);
        if (next_stop_rel_id > s->params->slim_total_routers - 1)
            codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", NULL, 1,
                                    next_stop_rel_id - (rpr * rail_id), rail_id, &next_stop_lp_id);
        ...
        //if( msg->packet_ID == TRACK )
        {
            printf("\x1b[32m-->Continuing First leg Non-Minimal Routing from current router:%d "
                   "(gid:%d) to dest_router:%d through randm intm router:%d next stop:%d "
                   "(globalid:%d)\x1b[0m\n",
                   s->router_id, (int)lp->gid, dest_router_rel_id, msg->intm_router_id,
                   next_stop_rel_id, (int)next_stop_lp_id);
        }
    ...
    if (path == NON_MINIMAL && msg->intm_router_id < 0) {
        ...
        {
            printf(
                "\x1b[32m-->Second leg Non-Minimal Routing from current router:%d (gid:%d) to "
                "dest_router:%d through randm intm router:%d next stop:%d (globalid:%d)\x1b[0m\n",
                s->router_id, (int)lp->gid, dest_router_rel_id, msg->intm_router_id,
                next_stop_rel_id, (int)next_stop_lp_id);
        }
    ...
    {
        printf("\x1b[32m-->Starting Minimal Routing from current router:%d (gid:%d) to "
               "dest_router:%d next stop:%d (globalid:%d)\x1b[0m\n",
               s->router_id, (int)lp->gid, dest_router_rel_id, next_stop_rel_id,
               (int)next_stop_lp_id);
    }
    //if( msg->packet_ID == TRACK && msg->last_hop != TERMINAL)
    {
        printf("\x1b[32m-->Continuing Minimal Routing from current router:%d (gid:%d) to "
               "dest_router:%d \x1b[0m\n",
...
int slim_get_output_port(router_state* s, slim_terminal_message* msg, tw_lp* lp,
                         tw_lpid next_stop_lp_id) {
    // printf("slim get output port\n");
    ...
    if (next_stop_lp_id == msg->dest_terminal_id) {
        ...
        codes_mapping_get_lp_info(next_stop_lp_id, lp_group_name, &mapping_grp_id, NULL,
                                  &mapping_type_id, NULL, &mapping_rep_id, &mapping_offset);
        ...
        for (int i = 0; i < s->params->num_global_channels; i++) {
            if (s->global_channel[i] == next_stop_rel_id) {
                ...
                break;
            } else if (i < s->params->num_local_channels &&
                       s->local_channel[i] == next_stop_rel_id) {
                output_port = i;
                break;
            }
        }
        if (output_port == -1) {
            printf("\n output port not found %d. Local LPID:%d local_rel_id:%d next stop lp_id %d "
                   "next_stop rel_id %d \n",
...
static int do_adaptive_routing(router_state* s, slim_terminal_message* msg, tw_lp* lp,
                               int dest_router_rel_id, int* intm_id) {
    int i;
    ...
    if (minimal_next_stop_rel_id > s->params->slim_total_routers - 1)
        codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", NULL, 1,
                                minimal_next_stop_rel_id - (rpr * rail_id), rail_id,
                                &minimal_next_stop_lp_id);
    else
        codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", s->anno, 0,
                                minimal_next_stop_rel_id, 0, &minimal_next_stop_lp_id);
    ...
    for (i = 0; i < num_indirect_routes; i++) {
        ...
        if (nonmin_next_stop_rel_id > s->params->slim_total_routers - 1)
            codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", NULL, 1,
                                    nonmin_next_stop_rel_id - (rpr * rail_id), rail_id,
                                    &nonmin_next_stop_lp_id[i]);
        else
            codes_mapping_get_lp_id(lp_group_name, "modelnet_slimfly_router", s->anno, 0,
                                    nonmin_next_stop_rel_id, 0, &nonmin_next_stop_lp_id[i]);
    ...
    for (i = 0; i < num_indirect_routes; i++) {
        ...
        num_nonmin_hops[i] +=
            get_path_length_from_terminal(intm_id[i], dest_router_rel_id, s->params);
    }
    ...
    for (i = 0; i < num_indirect_routes; i++) {
        cost_nonminimal[i] = ((float)num_nonmin_hops[i] / (float)num_min_hops) *
                             (float)nonmin_port_count[i] * csf_ratio;
#if DEBUG
        //if( msg->packet_ID == TRACK)
        {
            printf("\x1b[31mnonmin_next_stop_lp_id[%d]:%d num_hops_nonmin[%d]:%d, "
                   "output_port_nonmin[%d]:%d port_occupancy_non_min[%d]:%d cost_indirect[%d]:%f\n "
                   "     min_next_stop:%d       num_hops_min:%d,       output_port_min:%d        "
                   "port_occupancy_min:%d     cost_minimal:%f\x1b[0m\n",
                   i, (int)nonmin_next_stop_lp_id[i], i, num_nonmin_hops[i], i, nonmin_out_port[i],
                   i, nonmin_port_count[i], i, cost_nonminimal[i], (int)minimal_next_stop_lp_id,
                   num_min_hops, minimal_out_port, min_port_count, cost_minimal);
        }
#endif
    }
...
void slim_router_packet_receive_rc(router_state* s, tw_bf* bf, slim_terminal_message* msg,
                                   tw_lp* lp) {
    // printf("slimfly router packet receive rc\n");
    ...
    if (bf->c2) {
        // tw_rand_reverse_unif(lp->rng);
        slim_terminal_message_list* tail =
...
void slim_router_packet_receive(router_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slim router packet receive\n");
    ...
    int default_rail_dest_router_rel_id =
        (mapping_offset + (mapping_rep_id * num_lps)) / s->params->num_cn;
    if (s->router_id > s->params->slim_total_routers - 1)
        ...
        if (rail_id % 2 == 0)
            dest_router_rel_id =
                (rail_id)*s->params->slim_total_routers + default_rail_dest_router_rel_id;
        else
            dest_router_rel_id =
                (rail_id + 1) * s->params->slim_total_routers - 1 - default_rail_dest_router_rel_id;
    else
...
#if TRACK_OUTPUT
    if (msg->packet_ID == TRACK) {
        printf("\x1b[32m-->arrived at router relID:%d lpID:%d. Final destination terminal lp_id:%d "
               "router rel_id:%d\x1b[0m\n",
               s->router_id, (int)lp->gid, (int)msg->dest_terminal_id, dest_router_rel_id);
    }
#endif
    ...
    if (msg->last_hop == TERMINAL && routing == ADAPTIVE) {
        if (dest_router_rel_id == s->router_id) {
            cur_chunk->msg.path_type =
                MINIMAL; /*defaults to the routing algorithm if we don't have adaptive routing here*/
            next_stop = slim_get_next_stop(s, bf, &(cur_chunk->msg), lp, cur_chunk->msg.path_type,
                                           dest_router_rel_id, intm_id);
        } else {
            intm_router =
                (int*)malloc(num_indirect_routes * sizeof(int)); //indirect == nonMinimal == valiant
            ...
            next_stop =
                do_adaptive_routing(s, &(cur_chunk->msg), lp, dest_router_rel_id, intm_router);
            free(intm_router);
        }
    } else {
        if (routing == MINIMAL || routing == NON_MINIMAL)
            cur_chunk->msg.path_type =
                routing; /*defaults to the routing algorithm if we don't have adaptive routing here*/
        next_stop = slim_get_next_stop(s, bf, &(cur_chunk->msg), lp, cur_chunk->msg.path_type,
                                       dest_router_rel_id, intm_id);
    }
    ...
    //    if(output_chan > 2)
    {
        codes_mapping_get_lp_info(lp->gid, lp_group_name, &mapping_grp_id, NULL, &mapping_type_id,
                                  NULL, &mapping_rep_id, &mapping_offset);
        int local_router_id = (mapping_offset * s->params->slim_total_routers + mapping_rep_id);
        printf("packet_id:%d, current router:%d dest_router:%d next stop lp_id:%d output port:%d "
               "output chan:%d\n",
               (int)msg->packet_ID, local_router_id, dest_router_rel_id, next_stop, output_port,
               output_chan);
    }
#endif
    ...
    if (s->vc_occupancy[output_port][output_chan] + s->params->chunk_size <= max_vc_size) {
...
#if ROUTER_OCCUPANCY_LOG
        int index = floor(N_COLLECT_POINTS * (tw_now(lp) / g_tw_ts_end));
        vc_occupancy_storage_router[s->router_id][output_port][output_chan][index] =
            s->vc_occupancy[output_port][output_chan] / s->params->chunk_size;
#endif
        ...
        if (s->in_send_loop[output_port] == 0) {
            ...
            ts = codes_local_latency(lp);
            tw_event* e =
                model_net_method_event_new(lp->gid, ts, lp, SLIMFLY_ROUTER, (void**)&m, NULL);
            m->type = R_SEND;
            m->magic = slim_router_magic_num;
...
void slim_router_packet_send_rc(router_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slimfly router packet send rc\n");
    for (int i = 0; i < msg->rng_calls; i++) {
        tw_rand_reverse_unif(lp->rng);
    }
...
void slim_router_packet_send(router_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slim router packet send\n");
    ...
    if (cur_entry != NULL) {
        ...
        msg->rng_calls++;
        tw_event* e_new =
            model_net_method_event_new(lp->gid, ts, lp, SLIMFLY_ROUTER, (void**)&m_new, NULL);
        m_new->type = R_SEND;
...
void slim_router_buf_update_rc(router_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
    // printf("slimfly router buf update rc\n");
    for (int i = 0; i < msg->rng_calls; i++) {
        tw_rand_reverse_unif(lp->rng);
    }
    ...
    if (bf->c1) {
        slim_terminal_message_list* head =
            return_tail(s->pending_msgs[indx], s->pending_msgs_tail[indx], output_chan);
        // tw_rand_reverse_unif(lp->rng);
        prepend_to_terminal_message_list(s->queued_msgs[indx], s->queued_msgs_tail[indx],
                                         output_chan, head);
        s->vc_occupancy[indx][output_chan] -= s->params->chunk_size;
    }
    if (bf->c2) {
...
void slim_router_buf_update(router_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
...
#if ROUTER_OCCUPANCY_LOG
    int index = floor(N_COLLECT_POINTS * (tw_now(lp) / g_tw_ts_end));
    vc_occupancy_storage_router[s->router_id][indx][output_chan][index] =
        s->vc_occupancy[indx][output_chan] / s->params->chunk_size;
#endif
    if (s->queued_msgs[indx][output_chan] != NULL) {
        ...
        append_to_terminal_message_list(s->pending_msgs[indx], s->pending_msgs_tail[indx],
                                        output_chan, head);
        s->vc_occupancy[indx][output_chan] += s->params->chunk_size;
#if ROUTER_OCCUPANCY_LOG
        vc_occupancy_storage_router[s->router_id][indx][output_chan][index] =
...
void slim_router_event(router_state* s, tw_bf* bf, slim_terminal_message* msg, tw_lp* lp) {
...
void slim_terminal_rc_event_handler(terminal_state* s, tw_bf* bf, slim_terminal_message* msg,
                                    tw_lp* lp) {
    switch (msg->type) {
    case T_GENERATE:
...
void slim_router_rc_event_handler(router_state* s, tw_bf* bf, slim_terminal_message* msg,
                                  tw_lp* lp) {
    switch (msg->type) {
    case R_SEND:
...
tw_lptype slimfly_lps[] = {
    ...
    {(init_f)slim_terminal_init, (pre_run_f)NULL, (event_f)slim_terminal_event,
     (revent_f)slim_terminal_rc_event_handler, (commit_f)NULL, (final_f)slimfly_terminal_final,
     (map_f)codes_mapping, sizeof(terminal_state)},
    {
