...
static void init_message_list(message_list* thism, LOCAL_MSG_STRUCT* inmsg) {
...
enum last_hop { ROUTER = 1, TERMINAL };
...
static void to_dim_id(int flat_id, int ndims, const int* dim_lens, int* out_dim_ids) {
...
static int to_flat_id(int ndims, const int* dim_lens, const int* dim_ids) {
...
static void local_read_config(const char* anno, local_param* params) {
    ...
    int rc = configuration_get_value_double(&config, "PARAMS", "link_bandwidth", anno,
                                            &p->link_bandwidth);
    if (rc) {
    ...
    if (rc) {
        p->cn_bandwidth = 5.25;
        fprintf(stderr,
                "Bandwidth of compute node channels not specified, setting "
                "to %lf\n",
                p->cn_bandwidth);
    }

    rc = configuration_get_value_int(&config, "PARAMS", "num_cn", anno, &p->num_cn);
    if (rc) {
        tw_error(TW_LOC, "Nodes per router (num_cn) not specified\n");
    }

    rc = configuration_get_value_int(&config, "PARAMS", "num_vcs", anno, &p->num_vcs);
    if (rc) {
        p->num_vcs = 1;
    }
    ...
    if (rc) {
        p->cn_vc_size = 65536;
        fprintf(stderr,
                "Buffer size of compute node channels not specified, "
                "setting to %d\n",
                p->cn_vc_size);
    }

    p->router_delay = 50;
    configuration_get_value_int(&config, "PARAMS", "router_delay", anno, &p->router_delay);

    configuration_get_value(&config, "PARAMS", "cn_sample_file", anno, local_cn_sample_file,
                            MAX_NAME_LENGTH);
    configuration_get_value(&config, "PARAMS", "rt_sample_file", anno, local_rtr_sample_file,
                            MAX_NAME_LENGTH);

    //CHANGE: add network specific parameters here
    rc = configuration_get_value_int(&config, "PARAMS", "n_dims", anno, &p->n_dims);
    if (rc) {
    ...
    rc = configuration_get_value(&config, "PARAMS", "dim_length", anno, dim_length_str,
                                 MAX_NAME_LENGTH);
    if (rc == 0) {
    ...
    while (token != NULL) {
        ...
        if (p->dim_length[i] <= 0) {
            tw_error(TW_LOC,
                     "Invalid torus dimension specified "
                     "(%d at pos %d), exiting... ",
                     p->dim_length[i], i);
        }
        i++;
    ...
    if (strcmp(routing_str, "static") == 0)
        ...
        p->routing = STATIC;
        fprintf(stderr, "No routing protocol specified, setting to static routing\n");
    }
...
static void local_report_stats() {
    ...
    MPI_Reduce(&local_total_time, &avg_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_CODES);
    MPI_Reduce(&local_max_latency, &max_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_CODES);
...
static void terminal_init(terminal_state* s, tw_lp* lp) {
    ...
    int num_lps = codes_mapping_get_lp_count(lp_group_name, 0, LP_CONFIG_NM_TERM, s->anno, 0);
    ...
    s->router_gid =
        codes_mapping_get_lpid_from_relative(s->router_id, NULL, LP_CONFIG_NM_ROUT, s->anno, 1);
    ...
    s->terminal_msgs[0] = (message_list**)malloc(s->params->num_vcs * sizeof(message_list*));
    s->terminal_msgs_tail[0] = (message_list**)malloc(s->params->num_vcs * sizeof(message_list*));
    for (i = 0; i < s->params->num_vcs; i++) {
...
static void create_router_connections(router_state* r, tw_lp* lp) {
    ...
    to_dim_id(r->router_id, r->params->n_dims, r->params->dim_length, r->dim_position);

    //set up connections
    int curr_con = 0;
    int first = r->router_id * p->num_cn;
    for (; curr_con < p->num_cn; curr_con++) {
        r->link_connections[curr_con] =
            codes_mapping_get_lpid_from_relative(first, NULL, LP_CONFIG_NM_TERM, r->anno, 1);
        first++;
    }
    ...
    for (int curr_dim = 0; curr_dim < p->n_dims; curr_dim++) {
        ...
        for (int loc = 0; loc < p->dim_length[curr_dim]; loc++) {
            if (loc != r->dim_position[curr_dim]) {
                if (loc == r->dim_position[curr_dim] - 1 || loc == r->dim_position[curr_dim] + 1 ||
                    ...
                    r->link_connections[curr_con] =
                        codes_mapping_get_lpid_from_relative(neigh_id, NULL, LP_CONFIG_NM_ROUT,
                                                             r->anno, 1);
                    curr_con++;
                }
            }
...
static void router_setup(router_state* r, tw_lp* lp) {
    ...
    r->pending_msgs = (message_list***)malloc(p->radix * sizeof(message_list**));
    r->pending_msgs_tail = (message_list***)malloc(p->radix * sizeof(message_list**));
    r->queued_msgs = (message_list***)malloc(p->radix * sizeof(message_list**));
    r->queued_msgs_tail = (message_list***)malloc(p->radix * sizeof(message_list**));
    ...
    for (int i = 0; i < p->radix; i++) {
        ...
        r->pending_msgs[i] = (message_list**)malloc(p->num_vcs * sizeof(message_list*));
        r->pending_msgs_tail[i] = (message_list**)malloc(p->num_vcs * sizeof(message_list*));
        r->queued_msgs[i] = (message_list**)malloc(p->num_vcs * sizeof(message_list*));
        r->queued_msgs_tail[i] = (message_list**)malloc(p->num_vcs * sizeof(message_list*));
...
static tw_stime local_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender,
                                       LOCAL_NETWORK_NAME, (void**)&msg, (void**)&tmp_ptr);
...
static void packet_generate(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    packet_gen++;
    s->packet_gen++;
    ...
    for (uint64_t i = 0; i < num_chunks; i++) {
        ...
        cur_chunk->port = 0;
        cur_chunk->index = use_vc;
        append_to_message_list(s->terminal_msgs[0], s->terminal_msgs_tail[0], use_vc, cur_chunk);
    ...
    if (s->in_send_loop == 0) {
        ...
        LOCAL_MSG_STRUCT* m;
        tw_event* e =
            model_net_method_event_new(lp->gid, ts, lp, LOCAL_NETWORK_NAME, (void**)&m, NULL);
        m->type = T_SEND;
    ...
    total_event_size = model_net_get_msg_sz(LOCAL_NETWORK_NAME) + msg->remote_event_size_bytes +
                       msg->local_event_size_bytes;
    mn_stats* stat;
...
static void packet_generate_rc(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    ...
    for (i = 0; i < num_chunks; i++) {
        delete_message_list(
            return_tail(s->terminal_msgs[0], s->terminal_msgs_tail[0], msg->saved_channel));
        s->terminal_length -= s->params->chunk_size;
    }
    if (bf->c11) {
        s->issueIdle = 0;
...
static void packet_send(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    tw_stime ts;
    tw_event* e;
    LOCAL_MSG_STRUCT* m;
    tw_lpid router_id;
    ...
    void* remote_event;
    e = model_net_method_event_new(s->router_gid, ts, lp, LOCAL_NETWORK_ROUTER_NAME, (void**)&m,
                                   &remote_event);
    ...
    if (cur_entry->LOCAL_MSG_NAME_FROM_UNION.chunk_id == num_chunks - 1 &&
        ...
        tw_event* e_new =
            tw_event_new(cur_entry->LOCAL_MSG_NAME_FROM_UNION.sender_lp, local_ts, lp);
    ...
    e = model_net_method_event_new(lp->gid, ts, lp, LOCAL_NETWORK_NAME, (void**)&m_new, NULL);
    m_new->type = T_SEND;
...
static void packet_send_rc(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    if (bf->c1) {
        s->in_send_loop = 1;
        s->last_buf_full = msg->saved_busy_time;
        return;
    ...
    cur_entry->port = 0;
    cur_entry->index = use_vc;
    prepend_to_message_list(s->terminal_msgs[0], s->terminal_msgs_tail[0], use_vc, cur_entry);
    s->terminal_length += s->params->chunk_size;
...
static void send_remote_event(terminal_state* s, LOCAL_MSG_STRUCT* msg, tw_lp* lp, tw_bf* bf,
...
static void packet_arrive(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    assert(lp->gid == msg->dest_terminal_id);
    ...
    buf_e = model_net_method_event_new(msg->intm_lp_id, ts, lp, LOCAL_NETWORK_ROUTER_NAME,
                                       (void**)&buf_msg, NULL);
   ...
   * them */
    ...
    if (!tmp) {
        bf->c5 = 1;
        struct mn_qhash_entry* d_entry =
            (struct mn_qhash_entry*)malloc(sizeof(struct mn_qhash_entry));
...
static void packet_arrive_rc(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    ...
    if (bf->c7) {
        ...
        struct mn_qhash_entry* d_entry_pop = (struct mn_qhash_entry*)rc_stack_pop(s->st);
...
static void terminal_buf_update(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
...
static void terminal_buf_update_rc(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    s->vc_occupancy[0][msg->output_chan] += s->params->chunk_size;
    if (bf->c1) {
        codes_local_latency_reverse(lp);
        s->in_send_loop = 0;
...
static void terminal_event(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    s->fwd_events++;
...
static void terminal_event_rc(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    s->rev_events++;
    switch (msg->type) {
    case T_GENERATE:
...
static void terminal_final(terminal_state* s, tw_lp* lp) {
    model_net_print_stats(lp->gid, s->local_stats_array);

    int written = 0;
    if (!s->terminal_id)
        written =
            sprintf(s->output_buf, "# Format <LP id> <Terminal ID> <Total Data Size> <Avg packet "
                                   "latency> <# Flits/Packets finished> <Avg hops> <Busy Time>");

    written += sprintf(s->output_buf + written, "\n %llu %u %llu %lf %ld %lf %lf", LLU(lp->gid),
                       s->terminal_id, LLU(s->total_msg_size), s->total_time, s->finished_packets,
                       (double)s->total_hops / s->finished_chunks, s->busy_time);
...
static void get_next_stop(router_state* s, LOCAL_MSG_STRUCT* msg, tw_bf* bf, int* port, int* vc,
                          int* src_dim, int* dst_dim, int* static_port) {
    *static_port = -1;
    *src_dim = -2;
    ...
    to_dim_id(msg->dest_terminal / s->params->num_cn, s->params->n_dims, s->params->dim_length,
              dest);
                ...
                *port -= (s->dim_position[i] == 0 ? 1 : 0);
                ...
                if (dest[i] < s->dim_position[i]) {
                    ...
                    if ((dest[i] - s->dim_position[i] - 1) % s->params->gap == 0) {
                        *port += (s->dim_position[i] - 1 - first_dim_con) / s->params->gap + 1 +
                                 (dest[i] - s->dim_position[i] - 1) / s->params->gap;
                        *port -= (s->dim_position[i] == 0 ? 1 : 0);
                        port_options.push_back(NextHop(i, *port));
                    } else {
                        int long_hop = *port +
                                       (s->dim_position[i] - 1 - first_dim_con) / s->params->gap +
        ...
        *dst_dim = port_options[0].dim;
        ...
        if ((s->vc_occupancy[*port][*vc] + s->params->chunk_size > s->params->vc_size)) {
            ...
            *static_port = port_options[0].port;
            ...
            for (int i = 0; (i < port_options.size()) && (port_options[i].dim == first_dim); i++) {
    ...
    printf("[%lld-%d] Me: %d %d %d: Dest %d %d %d Found %d %d\n", msg->packet_ID, s->params->n_dims,
           s->dim_position[0], s->dim_position[1], s->dim_position[2], dest[0], dest[1], dest[2],
           *dst_dim, *port);
#endif
}
...
/* get the next stop for the current packet */
static void get_next_stop_rc(router_state* s, LOCAL_MSG_STRUCT* msg, tw_bf* bf) {
}
...
static void router_credit_send(router_state* s, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    tw_event* buf_e;
    tw_stime ts;
    LOCAL_MSG_STRUCT* buf_msg;
    ...
    if (is_terminal) {
        buf_e =
            model_net_method_event_new(dest, ts, lp, LOCAL_NETWORK_NAME, (void**)&buf_msg, NULL);
...
static void router_packet_receive(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    tw_stime ts;
    ...
    if (s->vc_occupancy[next_port][next_vc] + multfactor * s->params->chunk_size <= max_vc_size) {
        bf->c2 = 1;
        router_credit_send(s, msg, lp);
        cur_chunk->port = next_port;
        cur_chunk->index = next_vc;
        append_to_message_list(s->pending_msgs[next_port], s->pending_msgs_tail[next_port], next_vc,
                               cur_chunk);
        ...
        cur_chunk->port = next_port;
        cur_chunk->index = next_vc;
        append_to_message_list(s->queued_msgs[next_port], s->queued_msgs_tail[next_port], next_vc,
                               cur_chunk);
        ...
        if (s->params->routing != STATIC && next_dim != -1) {
            ...
            bool triggerSend =
                (s->vc_occupancy[static_port][0] + MULT_FACTOR * s->params->chunk_size <=
                 s->params->vc_size);
...
static void router_packet_receive_rc(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    ...
    if (bf->c2) {
        tw_rand_reverse_unif(lp->rng);
        message_list* tail =
            return_tail(s->pending_msgs[next_port], s->pending_msgs_tail[next_port], next_vc);
    ...
    if (bf->c4) {
        message_list* tail =
            return_tail(s->queued_msgs[next_port], s->queued_msgs_tail[next_port], next_vc);
        s->queued_count[next_port] -= s->params->chunk_size;
        ...
        if (bf->c6) {
            ...
            message_list* stail =
                altq_return_tail(s->queued_msgs[static_port], s->queued_msgs_tail[static_port], 0);
            assert(stail == tail);
            if (bf->c5) {
                codes_local_latency_reverse(lp);
...
static void router_packet_send(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    tw_stime ts;
    tw_event* e;
    LOCAL_MSG_STRUCT* m;
    ...
    for (int i = 0; i < s->params->num_vcs; i++) {
        if (s->pending_msgs[output_port][i] != NULL) {
            VC_Entry tmp;
            tmp.vc = i;
            tmp.entry = s->pending_msgs[output_port][i];
            entries.push_back(tmp);
        }
    }
    ...
    if (entries.size() == 0) {
        if ((output_port >= s->params->num_cn) && (s->params->routing != STATIC) &&
            (s->vc_occupancy[output_port][0] + MULT_FACTOR * s->params->chunk_size <=
             s->params->vc_size)) {
    ...
    void* m_data;
    if (to_terminal) {
        assert(s->link_connections[output_port] ==
               cur_entry->LOCAL_MSG_NAME_FROM_UNION.dest_terminal_id);
        e = model_net_method_event_new(s->link_connections[output_port], ts, lp, LOCAL_NETWORK_NAME,
                                       (void**)&m, &m_data);
    } else {
        e = model_net_method_event_new(s->link_connections[output_port], ts, lp,
                                       LOCAL_NETWORK_ROUTER_NAME, (void**)&m, &m_data);
    }
    memcpy(m, &cur_entry->LOCAL_MSG_NAME_FROM_UNION, sizeof(LOCAL_MSG_STRUCT));
    if (m->remote_event_size_bytes) {
    ...
    if ((cur_entry->LOCAL_MSG_NAME_FROM_UNION.packet_size % s->params->chunk_size) &&
        (cur_entry->LOCAL_MSG_NAME_FROM_UNION.chunk_id == num_chunks - 1)) {
        bf->c11 = 1;
        s->link_traffic[output_port] +=
            (cur_entry->LOCAL_MSG_NAME_FROM_UNION.packet_size % s->params->chunk_size);
        s->link_traffic_sample[output_port] +=
            (cur_entry->LOCAL_MSG_NAME_FROM_UNION.packet_size % s->params->chunk_size);
    } else {
        bf->c12 = 1;
   ...
   * next.*/
    ...
    if (!bf->c21) {
        cur_entry =
            return_head(s->pending_msgs[output_port], s->pending_msgs_tail[output_port], use_vc);
    }
    rc_stack_push(lp, cur_entry, delete_message_list, s->st);
    ...
    e = model_net_method_event_new(lp->gid, ts, lp, LOCAL_NETWORK_ROUTER_NAME, (void**)&m_new,
                                   NULL);
    m_new->type = R_SEND;
...
static void router_packet_send_rc(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    int output_port = msg->vc_index;
    ...
    if (bf->c21) {
        tw_rand_reverse_unif(lp->rng);
        cur_entry->port = msg->saved_vc;
        cur_entry->index = msg->saved_channel;
        cur_entry->in_alt_q = 1;
        altq_prepend_to_message_list(s->queued_msgs[output_port], s->queued_msgs_tail[output_port],
                                     0, cur_entry);
        add_to_message_list(s->queued_msgs, s->queued_msgs_tail, cur_entry);
        s->vc_occupancy[output_port][0] -= s->params->chunk_size;
        s->queued_count[msg->saved_vc] += s->params->chunk_size;
    } else {
        cur_entry->port = output_port;
        cur_entry->index = use_vc;
        prepend_to_message_list(s->pending_msgs[output_port], s->pending_msgs_tail[output_port],
                                use_vc, cur_entry);
    }
...
static void router_buf_update(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    ...
    if ((s->params->routing == STATIC || output_chan != 0 || indx < s->params->num_cn) &&
        ...
        if (!s->queued_msgs[indx][output_chan]->LOCAL_MSG_NAME_FROM_UNION.dim_change ||
            ...
            head->port = indx;
            head->index = output_chan;
            append_to_message_list(s->pending_msgs[indx], s->pending_msgs_tail[indx], output_chan,
                                   head);
            s->vc_occupancy[indx][output_chan] += s->params->chunk_size;
    ...
    if (s->in_send_loop[indx] == 0 &&
        ((s->pending_msgs[indx][output_chan] != NULL) ||
...
static void router_buf_update_rc(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    int indx = msg->vc_index;
    int output_chan = msg->output_chan;
    s->vc_occupancy[indx][output_chan] += s->params->chunk_size;
    if (bf->c3) {
    ...
    if (bf->c1) {
        ...
        head->port = indx;
        head->index = output_chan;
        prepend_to_message_list(s->queued_msgs[indx], s->queued_msgs_tail[indx], output_chan, head);
        s->vc_occupancy[indx][output_chan] -= s->params->chunk_size;
        s->queued_count[indx] += s->params->chunk_size;
    }
    if (bf->c2) {
...
static void router_event(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    s->fwd_events++;
    rc_stack_gc(lp, s->st);
...
static void router_rc_event_handler(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    s->rev_events++;
...
static void router_final(router_state* s, tw_lp* lp) {
    int i, j;
    ...
    if (!s->router_id) {
        written =
            sprintf(s->output_buf, "# Format <LP ID> <Router ID> <Busy time per router port(s)>");
    }
    written += sprintf(s->output_buf + written, "\n %llu %d ", LLU(lp->gid), s->router_id);
    ...
    if (!s->router_id) {
        written = sprintf(s->output_buf2,
                          "# Format <LP ID> <Router ID> <Link traffic per router port(s)>");
    }
...
static void local_rsample_init(router_state* s, tw_lp* lp) {
    (void)lp;
    int i = 0;
    const local_param* p = s->params;
    ...
    s->rsamples =
        (struct local_router_sample*)malloc(MAX_STATS * sizeof(struct local_router_sample));
...
void local_rsample_rc_fn(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
...
static void local_rsample_fn(router_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    (void)bf;
    (void)lp;
    (void)msg;
    ...
    if (s->op_arr_size >= s->max_arr_size) {
        struct local_router_sample* tmp = (struct local_router_sample*)malloc(
            (MAX_STATS + s->max_arr_size) * sizeof(struct local_router_sample));
...
static void local_rsample_fin(router_state* s, tw_lp* lp) {
    (void)lp;
    const local_param* p = s->params;
...
static void local_sample_init(terminal_state* s, tw_lp* lp) {
    (void)lp;
...
void local_sample_rc_fn(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    (void)lp;
    (void)bf;
    (void)msg;
...
static void local_sample_fn(terminal_state* s, tw_bf* bf, LOCAL_MSG_STRUCT* msg, tw_lp* lp) {
    (void)lp;
    (void)msg;
    (void)bf;
     ...
     * expensive operation though */
        struct local_cn_sample* tmp = (struct local_cn_sample*)malloc(
            (MAX_STATS + s->max_arr_size) * sizeof(struct local_cn_sample));
...
static void local_sample_fin(terminal_state* s, tw_lp* lp) {
    (void)lp;
...
tw_lptype local_lps[] = {
    ...
    {(init_f)terminal_init, (pre_run_f)NULL, (event_f)terminal_event, (revent_f)terminal_event_rc,
     (commit_f)NULL, (final_f)terminal_final, (map_f)codes_mapping, sizeof(terminal_state)},
    {
        (init_f)router_setup,
        (pre_run_f)NULL,
