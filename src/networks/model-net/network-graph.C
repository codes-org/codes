...
terminal_final( terminal_state * s, tw_lp * lp )
{
  ...
  for(int port = 0; port < s->radix; port++) {
    for(int i = 0; i < s->params->num_vcs; i++) {
      if(s->terminal_msgs[port][i] != NULL)
  ...
  for(int port = 0; port < s->radix; port++) {
    free(s->vc_occupancy[port]);
    free(s->terminal_msgs[port]);
    free(s->terminal_msgs_tail[port]);
    free(s->terminal_length[port]);
  }
  free(s->vc_occupancy);
  free(s->terminal_msgs);
  free(s->terminal_msgs_tail);
  free(s->terminal_length);
  free(s->total_terminal_length);
}
...
{
  tw_stime ts;
  int next_port, next_vc;

  get_next_stop(s, msg, bf, &next_port, &next_vc);
  ...
  msg->saved_port = next_port;
  msg->saved_vc = next_vc;
  return;
}
    ...
    tw_lp * lp)
{
  ...
  int next_port = msg->saved_port;
  int next_vc = msg->saved_vc;
    ...
    LOCAL_MSG_STRUCT * msg, tw_lp * lp)
{
  ...
  void * m_data;
  if (to_terminal) {
    assert(s->conn_gids[output_port] == cur_entry->LOCAL_MSG_NAME_FROM_UNION.dest_terminal_id);
    e = model_net_method_event_new(s->conn_gids[output_port],
