...
tw_lptype sp_lp = {
    (init_f)sp_init, (pre_run_f)NULL,      (event_f)sp_event,    (revent_f)sp_rev_event,
    (commit_f)NULL,  (final_f)sp_finalize, (map_f)codes_mapping, sizeof(sp_state),
};
...
static void sp_set_params(const char* latency_fname, const char* bw_fname,
    ...
    if (is_tri_mat) {
        ...
        params->net_bw_mbps_table = malloc(2 * params->mat_len * params->mat_len * sizeof(double));
...
static void sp_init(sp_state* ns, tw_lp* lp) {
    ...
    ns->send_next_idle = malloc(ns->params->num_lps * sizeof(ns->send_next_idle));
    ns->recv_next_idle = malloc(ns->params->num_lps * sizeof(ns->recv_next_idle));
...
static void handle_msg_ready_event(sp_state* ns, sp_message* m, tw_lp* lp) {
    ...
    double bw = sp_get_table_ent(m->src_mn_rel_id, ns->id, 1, ns->params->num_lps,
                                 ns->params->net_bw_mbps_table);
    double latency = sp_get_table_ent(m->src_mn_rel_id, ns->id, 1, ns->params->num_lps,
                                      ns->params->net_latency_ns_table);
...
static void handle_msg_start_event(sp_state* ns, sp_message* m, tw_lp* lp) {
    ...
    bw = sp_get_table_ent(ns->id, dest_rel_id, 0, ns->params->num_lps,
                          ns->params->net_bw_mbps_table);
    latency = sp_get_table_ent(ns->id, dest_rel_id, 0, ns->params->num_lps,
                               ns->params->net_latency_ns_table);
    ...
    void* m_data;
    e_new = model_net_method_event_new(m->dest_mn_lp, send_queue_time + latency, lp, SIMPLEP2P,
                                       (void**)&m_new, &m_data);
...
static tw_stime simplep2p_packet_event(model_net_request const* req, uint64_t message_offset,
    ...
    printf("%lu: final %lu packet sz %d remote sz %d self sz %d is_last_pckt %d latency %lf\n",
           (src_lp - 1) / 2, req->final_dest_lp, packet_size, req->remote_event_size,
           req->self_event_size, is_last_pckt, xfer_to_nic_time + offset);
#endif

    e_new = model_net_method_event_new(sender->gid, xfer_to_nic_time + offset, sender, SIMPLEP2P,
...
static void sp_read_config(const char* anno, simplep2p_param* p) {
    ...
    int rc;
    rc = configuration_get_value_relpath(&config, "PARAMS", "net_latency_ns_file", anno,
                                         latency_file, MAX_NAME_LENGTH);
    if (rc <= 0) {
        if (anno == NULL)
    ...
    rc = configuration_get_value_relpath(&config, "PARAMS", "net_bw_mbps_file", anno, bw_file,
                                         MAX_NAME_LENGTH);
    if (rc <= 0) {
        if (anno == NULL)
...
static double
sp_get_table_ent(int from_id, int to_id,
