...
int main(int argc, char** argv) {
    ...
    printf("\n Cortex needs to be enabled in order to run the test. See "
   ...
   * topology node and router counts. Right now it is hard coded for Theta. */
    ...
    for (int i = 0; i < num_router_cols - 1; i++) {
        double router_bw =
            dragonfly_custom_cortex_topology.get_router_link_bandwidth(topo_arg, i, i + 1);
    ...
    for (int i = 0; i < num_router_rows - 1; i++) {
        ...
        double router_bw =
            dragonfly_custom_cortex_topology.get_router_link_bandwidth(topo_arg, src, dest);
        assert(router_bw == local_bandwidth);
    }
   ...
   * connection in between them */
    ...
    for (int i = 0; i < 4; i++) {
        ...
        double router_bw =
            dragonfly_custom_cortex_topology.get_router_link_bandwidth(topo_arg, i, dest);
    ...
    for (int i = grp1_offset; i < grp1_offset + num_router_cols; i++) {
        ...
        dragonfly_custom_cortex_topology.get_router_location(topo_arg, i, location,
                                                             sizeof(location));
        ...
        for (int k = 0; k < num_cns; k++) {
            ...
            dragonfly_custom_cortex_topology.get_compute_node_location(topo_arg, cn_id, loc_cn,
                                                                       sizeof(loc_cn));
            ...
            router_id_t rid =
                dragonfly_custom_cortex_topology.get_router_from_compute_node(topo_arg, cn_id);
            assert(rid == i);
        }
