#!/usr/bin/env python3
"""
Generate a WAN-like topology YAML file for model-net-fluid-switch.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a random WAN-like topology YAML for model-net-fluid-switch."
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("doc/example/fluid-switch-topology.generated.yaml"),
        help=(
            "output YAML path, relative to the current working directory unless absolute; "
            "default: doc/example/fluid-switch-topology.generated.yaml"
        ),
    )
    parser.add_argument(
        "--name-prefix",
...
def write_topology(args: argparse.Namespace, links: dict[int, dict[int, float]]) -> None:
    ...
    with args.output.open("w", encoding="utf-8") as f:
        f.write("# Generated by src/network-workloads/generate-fluid-switch-topology.py\n")
