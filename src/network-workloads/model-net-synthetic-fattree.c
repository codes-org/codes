...
st_model_types ft_svr_model_types[] = {{(ev_trace_f)ft_svr_event_collect, sizeof(int),
                                        (model_stat_f)ft_svr_model_stat_collect, 0, NULL, NULL, 0},
...
static void svr_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    default:
        printf("\n LP: %d has received invalid message from src lpID: %d of message type:%d",
               (int)lp->gid, (int)m->src, m->svr_event_type);
        assert(0);
        break;
    }
}
...
int main(int argc, char** argv) {
    ...
    if (!g_tw_mynode) {
        ...
        fprintf(fattree_results_log_header, "<Total Events>, <Rollbacks>, <GVT Computations>, <Net "
                                            "Events>, <Running Time>, <Efficiency>, <Event Rate>");
        fprintf(fattree_results_log, "%14llu, %11llu, %18d, %12llu, %14.4f, %12.2f, %12.2f\n",
                total_events, rollbacks, gvt_computations, net_events, running_time, efficiency,
                event_rate);
        fclose(fattree_results_log);
        fclose(fattree_results_log_header);
