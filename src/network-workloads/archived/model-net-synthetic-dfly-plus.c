...
enum TRAFFIC {
    ...
    STENCIL_3D = 11,
    ND_BROADCAST =
        12, /*Non delayed BC, difference from 7 is that the first broadcasting is immediately issued at time 0*/
...
static void issue_event_non_delay(svr_state* ns, tw_lp* lp) {
    (void)ns;
    tw_event* e;
    svr_msg* m;
...
static void handle_kickoff_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    assert(net_id == DRAGONFLY || net_id == DRAGONFLY_PLUS ||
           net_id == DRAGONFLY_CUSTOM); /* only supported for dragonfly model right now. */
    ...
    //background noise3
        ...
        default:
            tw_error(TW_LOC, "Node %llu in group %d should not generate BACKGROUND_NOISE3",
                     LLU(ns->svr_id), ns->group_id);
        }
    }
    ...
    if (!sender_receiver_from_alloc) {
        ...
        if (multi_syn_job)
            m->event_rc = model_net_event(net_id, "test", global_dest, app_payload_sz[ns->app_id],
        ...
        for (i = 0; i < dest_num; ++i) {
            ...
            global_dest = codes_mapping_get_lpid_from_relative(intm_dest_id, group_name,
                                                               lp_type_name, NULL, 0); //node lpid
...
static void handle_remote_rev_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    if (app_traffic[m->app_id] < BACKGROUND_NOISE3) {
        if ((*ns->comm_time)[m->app_id][index] == (m->arrive_time - m->issued_time)) {
            ...
            tw_error(TW_LOC,
                     "Error:remote reverse calculatioin failed \n svr %llu found travel time %lf "
                     "but record %lf at index %d\n",
                     ns->svr_id, m->arrive_time - m->issued_time,
                     (*ns->comm_time)[m->app_id][index], index);
        }
    }
}
...
static void handle_remote_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    if (m->dest != ns->svr_id)
        tw_error(TW_LOC,
                 "Error: msg issued by %llu at time %lfns should arrive at %llu but reached %llu\n",
                 (unsigned long long)m->src, m->issued_time, (unsigned long long)m->dest,
                 (unsigned long long)ns->svr_id);
...
static void svr_finalize(svr_state* ns, tw_lp* lp) {
        ...
        for (i = 0; i < num_apps; ++i) {
            ...
            if (ns->svr_id == 0) {
                written = sprintf(ns->output_buf0,
                                  "### Application %d, Synthetic Traffic: %d, comm. time (ns)\n "
                                  "<msg comm. time>, <Avg. chunck comm.time>",
                                  i, app_traffic[i]);
                result = lp_io_write(lp->gid, file, written, ns->output_buf0);
                if (result != 0)
                    tw_error(TW_LOC,
                             "\nERROR: lpio result %d, svr %llu, lpgid %llu, writting %s failed, "
                             "written %d, buf %s\n",
                             result, LLU(ns->svr_id), LLU(lp->gid), file, written, ns->output_buf0);
            }

            for (j = 0; j < index; j++) {
                written = sprintf(ns->output_buf0, "\n%lf %lf", (*ns->comm_time)[i][j],
                                  (*ns->comm_time)[i][j] / app_msg_chunk_ratio[i]);
    ...
    written = 0;
    if (ns->svr_id == 0)
        written = sprintf(ns->output_buf1, "#<LP ID> <Terminal ID> <Job ID> <Local Rank> <Total "
                                           "sends> <Total Recvs> <Local count> <lp running time>");

    written += sprintf(ns->output_buf1 + written, "\n %llu %llu %d %d %d %d %d %lf", LLU(lp->gid),
                       LLU(ns->svr_id), ns->app_id, ns->local_rank, ns->msg_sent_count,
                       ns->msg_recvd_count, ns->local_recvd_count, ns->end_ts - ns->start_ts);

    result = lp_io_write(lp->gid, (char*)"mpi-synthetic-stats", written, ns->output_buf1);
    if (result != 0)
        tw_error(TW_LOC,
                 "\nERROR: lpio result %d, svr %llu, lpgid %llu, writting %s failed, written %d, "
                 "buf %s\n",
                 result, LLU(ns->svr_id), LLU(lp->gid), (char*)"mpi-synthetic-stats", written,
                 ns->output_buf1);
...
int main(int argc, char** argv) {
    ...
    if (g_st_ross_rank) { // keep damaris ranks from running code between here up until tw_end()
        ...
        if (!(net_id == DRAGONFLY || net_id == DRAGONFLY_PLUS || net_id == DRAGONFLY_CUSTOM)) {
            printf("\n The test works with dragonfly model configuration only! %d %d ",
                   DRAGONFLY_PLUS, net_id);
            MPI_Finalize();
            return 0;
        }
        ...
        if (strlen(workloads_file) > 0 && strlen(alloc_file)) {
            ...
            while (!feof(name_file)) {
                ...
                ref = fscanf(name_file, "%d %s %d %lf %d %d", &app_job_nodes[i], app_job_name[i],
                             &app_traffic[i], &app_arrival_time[i], &app_payload_sz[i],
                             &app_num_msgs[i]);
                ...
                if (ref != EOF && strncmp(app_job_name[i], "synthetic", 9) == 0) {
                    ...
                    if (rank == 0)
                        printf("\nApp id %d, #nodes %d, traffic#: %d, arrival-time %lf, payload "
                               "size %d, num msgs %d, msg chunk ratio %d\n",
                               i, app_job_nodes[i], app_traffic[i], app_arrival_time[i],
                               app_payload_sz[i], app_num_msgs[i], app_msg_chunk_ratio[i]);
