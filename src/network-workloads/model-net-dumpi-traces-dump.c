...
void nw_test_finalize(nw_state* s, tw_lp* lp) {
    ...
    printf("\n LP %llu total sends %ld receives %ld wait_alls %ld waits %ld ", LLU(lp->gid),
           s->num_sends, s->num_recvs, s->num_waitall, s->num_wait);
...
const tw_optdef app_opt[] = {TWOPT_GROUP("Network workload test"),
                             TWOPT_CHAR("workload_type", workload_type,
                                        "workload type (either \"scalatrace\" or \"dumpi\")"),
...
int main(int argc, char** argv) {
    ...
    if (g_st_ross_rank) { // keep damaris ranks from running code between here up until tw_end()
        ...
        if (strlen(workload_file) == 0) {
            if (tw_ismaster())
                printf("\n Usage: mpirun -np n ./codes-nw-test --sync=1/2/3 --workload_type=type "
                       "--workload_file=workload-file-name");
            tw_end();
            return -1;
        }
        ...
        MPI_Reduce(&avg_compute_time, &total_avg_comp_time, 1, MPI_DOUBLE, MPI_SUM, 0,
                   MPI_COMM_CODES);
        ...
        if (!g_tw_mynode)
            printf(
                "\n Total bytes sent %lld recvd %lld \n avg runtime %lf \n avg comm time %lf avg "
                "compute time %lf \n avg send time %lf \n avg recv time %lf \n avg wait time %lf "
                "\n total sends %ld total recvs %ld total waits %ld total collectives %ld ",
                total_bytes_sent, total_bytes_recvd, avg_run_time / num_net_lps,
                avg_comm_run_time / num_net_lps, total_avg_comp_time / num_net_lps,
                total_avg_send_time / num_net_lps, total_avg_recv_time / num_net_lps,
                total_avg_wait_time / num_net_lps, overall_sends, overall_recvs, overall_waits,
                overall_cols);
#ifdef USE_RDAMARIS
