...
#define NEAR_ZERO                                                                                  \
    .0001 //timestamp for use to be 'close to zero' but still allow progress, zero offset events are hard on the PDES engine
...
enum MPI_NW_EVENTS {
    ...
    CODES_CMD_SWITCH_TO_SURR, // transition simulation from PDES -> Surrogate
    CODES_CMD_SWITCH_TO_PDES, // transition simulation from Surrogate -> PDES
};
...
enum TRAFFIC {
    ...
    NEAREST_NEIGHBOR =
        2,        /* sends message to the next node (potentially connected to the same router) */
...
struct nw_state {
    ...
    /* For hybrid simulations with DIRECTOR and surrogate */
    int director_enabled;
    tw_lpid director_lpid;
    int simulation_mode;
};
...
/*  ==========================================================
    START of model-net replay DIRECTOR interface
    ==========================================================
*/


void codes_register_director_events(nw_state* s, int dir_event_type, int nw_event_type,
                                    size_t event_value, tw_lp* lp) {
    tw_event* e;
    director_message* msg;

    nw_message registered_nw_msg;
    ...
    //printf("==DIR[%d] NW Registering dir_event_type:%d | nw_event_type:%d (time: %lf)\n",
    //        s->nw_id, dir_event_type, nw_event_type, tw_now(lp));

    //printf("==DIR: ts: %lf\n", ts);
    if (sizeof(nw_message) + sizeof(director_message) > g_tw_msg_sz) {
        tw_error(TW_LOC,
                 "Error: NW-director trying to transmit an event of size "
                 "%d but ROSS is configured for events of size %zd\n",
                 sizeof(nw_message) + sizeof(director_message), g_tw_msg_sz);
    }

    e = tw_event_new(s->director_lpid, ts, lp);
    msg = (director_message*)tw_event_data(e);
    msg->msg_type = dir_event_type;
    msg->op_type = nw_event_type;
    msg->value = sizeof(nw_message);

    memcpy(&msg->buffer, &registered_nw_msg, msg->value);

    tw_event_send(e);
}

/* Trigger Director Event from within network model*/
static void codes_issue_director_event(tw_lp* lp, tw_lpid director_lpid, int dir_event_type,
                                       int value) {
    tw_event* e;
...
static void update_completed_queue(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp,
                                   dumpi_req_id req_id);
...
static void update_message_size(struct nw_state* ns, tw_lp* lp, tw_bf* bf, struct nw_message* m,
                                mpi_msgs_queue* qitem, int is_eager, int is_send) {
    (void)bf;
    (void)is_eager;
    ...
    if (ns->msg_sz_table == NULL)
        ns->msg_sz_table =
            qhash_init(msg_size_hash_compare, quickhash_64bit_hash, RANK_HASH_TABLE_SZ);
    ...
    /* update hash table */
    if (!hash_link) {
        struct msg_size_info* msg_info =
            (struct msg_size_info*)malloc(sizeof(struct msg_size_info));
...
static void notify_background_traffic_rc(struct nw_state* ns, tw_lp* lp, tw_bf* bf,
                                         struct nw_message* m) {
    (void)ns;
    (void)bf;
    (void)m;
...
static void notify_background_traffic(struct nw_state* ns, tw_lp* lp, tw_bf* bf,
                                      struct nw_message* m) {
    (void)bf;
    (void)m;
    ...
    for (int other_id = 0; other_id < num_jobs; other_id++) {
        if (is_job_synthetic[other_id] ==
            0) //we only want to notify the synthetic (background) ranks
            continue;
        ...
        for (int k = 0; k < num_other_ranks; k++) {
            ...
            global_dest_id =
                codes_mapping_get_lpid_from_relative(intm_dest_id, NULL, NW_LP_NM, NULL, 0);
...
static void notify_root_workload_rc(struct nw_state* ns, tw_lp* lp, tw_bf* bf,
                                    struct nw_message* m) {
    (void)ns;
    (void)bf;
    (void)m;
...
static void notify_root_workload(struct nw_state* ns, tw_lp* lp, tw_bf* bf, struct nw_message* m) {
    (void)bf;
    (void)m;
    ...
    m_new->fwd.app_id =
        ns->app_id; //just borrowing the fwd struct even though this isn't an injected message
    tw_event_send(e);
}

void handle_other_finish_rc(struct nw_state* ns, tw_lp* lp, tw_bf* bf, struct nw_message* m) {
...
void handle_other_finish(struct nw_state* ns, tw_lp* lp, tw_bf* bf, struct nw_message* m) {
    ...
    if (total_non_syn_completed_jobs ==
        num_nonsyn_jobs) //then all nonsynthetic jobs have completed, the background synthetic workloads must be notified
    {
        ...
        if (total_non_syn_completed_jobs < num_jobs) {
            if (max_gen_data <= 0) {
                ...
                printf("App %d: Not notifying background traffic as max_gen_data > 0. Will let "
                       "synthetic workloads finish",
                       ns->app_id);
            }
        }
...
static void handle_neighbor_finish_rc(struct nw_state* ns, tw_lp* lp, tw_bf* bf,
                                      struct nw_message* m) {
...
static void handle_neighbor_finish(struct nw_state* ns, tw_lp* lp, tw_bf* bf,
                                   struct nw_message* m) {
    ns->num_own_job_ranks_completed++;
...
static void notify_root_rank_rc(struct nw_state* ns, tw_lp* lp, tw_bf* bf, struct nw_message* m) {
}

static void notify_root_rank(struct nw_state* ns, tw_lp* lp, tw_bf* bf, struct nw_message* m) {
...
void finish_bckgnd_traffic_rc(struct nw_state* ns, tw_bf* b, struct nw_message* msg, tw_lp* lp) {
    (void)b;
    (void)msg;
    (void)lp;
...
void finish_bckgnd_traffic(struct nw_state* ns, tw_bf* b, struct nw_message* msg, tw_lp* lp) {
    (void)b;
    (void)msg;
    ns->is_finished = 1;
    ...
    printf("\n LP %llu App %d completed sending data %llu completed at time %lf ", LLU(lp->gid),
           ns->app_id, ns->gen_data, tw_now(lp));
...
static void gen_synthetic_tr(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp) {
    ...
    if (iter_predictor &&
        iter_predictor->model.have_we_hit_switch(
            lp, s->nw_id_in_pe, 0)) { // background synthetic lps have no iterations
        bf->c13 = 1;
        ...
        if (restarting_background_at > tw_now(lp)) {
            long const periods_to_jump =
                ceil((restarting_background_at - tw_now(lp)) / mean_interval_of_job[s->app_id]);
            ts *= periods_to_jump;
    ...
    if (max_gen_data !=
        0) { //max_gen_data is by default 0 (off). If it's on, then we use it to determine when to finish synth ranks
...
void arrive_syn_tr(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp) {
    ...
    if (PRINT_SYNTH_TRAFFIC) {
        if (s->local_rank == 0) {
            printf("\n Data arrived %llu rank %llu total data %llu ", LLU(m->fwd.num_bytes),
                   LLU(s->nw_id), s->syn_data);
    ...
    if (op_type == CODES_WK_WAIT && (wait_elem->req_ids[0] == completed_req)) {
        ...
        for (i = 0; i < wait_elem->count; i++) {
            if (wait_elem->req_ids[i] == completed_req) {
                ...
                m->fwd
                    .wait_completed++; //This is just the individual request handle - not the entire wait.
                if (wait_elem->num_completed > wait_elem->count)
                    printf("\n Num completed %d count %d LP %llu ", wait_elem->num_completed,
...
static void codes_exec_mpi_wait(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp,
    ...
    if (enable_debug) {
        fprintf(workload_log, "\n (%lf) APP ID %d MPI WAIT POSTED SOURCE %d", tw_now(lp), s->app_id,
                s->local_rank);
    }
...
static void codes_exec_mpi_wait_all(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp,
    ...
    if (enable_debug) {
        ...
        fprintf(workload_log, "\n (%lf) APP ID %d MPI WAITALL POSTED SOURCE %d", tw_now(lp),
    ...
    if (enable_sampling) {
        ...
        if (s->sampling_indx >= MAX_STATS) {
            struct mpi_workload_sample* tmp =
                (struct mpi_workload_sample*)calloc((MAX_STATS + s->max_arr_size),
...
static int rm_matching_rcv(nw_state* ns, tw_bf* bf, nw_message* m, tw_lp* lp,
...
static int rm_matching_send(nw_state* ns, tw_bf* bf, nw_message* m, tw_lp* lp,
                            mpi_msgs_queue* qitem) {
    int matched = 0;
    struct qlist_head* ent = NULL;
    mpi_msgs_queue* qi = NULL;
...
static void codes_exec_comp_delay(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp,
    ...
    if (enable_debug) {
        fprintf(workload_log, "\n (%lf) APP %d MPI DELAY SOURCE %d DURATION %lf", tw_now(lp),
                s->app_id, s->local_rank, ts);
    }
...
static void codes_exec_mpi_recv_rc(nw_state* ns, tw_bf* bf, nw_message* m, tw_lp* lp) {
    ...
    if (enable_debug) {
        if (mpi_op->op_type == CODES_WK_RECV) {
            fprintf(workload_log, "\n (%lf) APP %d MPI RECV SOURCE %d DEST %d BYTES %" PRId64,
                    tw_now(lp), s->app_id, recv_op->source_rank, recv_op->dest_rank,
                    recv_op->num_bytes);
        } else {
...
static void codes_exec_mpi_send(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp,
    ...
    if (lp->gid == TRACK_LP)
        printf("\n Sender rank %llu global dest rank %d dest-rank %d bytes %" PRIu64 " Tag %d",
               LLU(s->nw_id), global_dest_rank, mpi_op->u.send.dest_rank, mpi_op->u.send.num_bytes,
               mpi_op->u.send.tag);
    m->rc.mpi_ack.saved_num_bytes = mpi_op->u.send.num_bytes;
    /* model-net event */
    tw_lpid dest_rank =
        codes_mapping_get_lpid_from_relative(global_dest_rank, NULL, "nw-lp", NULL, 0);
    ...
    if (mpi_op->u.send.num_bytes < EAGER_THRESHOLD) {
        ...
        remote_m.fwd.app_id = s->app_id;
        m->event_rc =
            model_net_event_mctx(net_id, &mapping_context, &mapping_context, prio, dest_rank,
                                 mpi_op->u.send.num_bytes,
                                 (self_overhead + copy_overhead + soft_delay_mpi + nic_delay),
                                 sizeof(nw_message), (const void*)&remote_m, sizeof(nw_message),
                                 (const void*)&local_m, lp);
    } else if (is_rend == 0) {
        ...
        m->event_rc =
            model_net_event_mctx(net_id, &mapping_context, &mapping_context, prio, dest_rank,
                                 CONTROL_MSG_SZ, (self_overhead + soft_delay_mpi + nic_delay),
                                 sizeof(nw_message), (const void*)&remote_m, 0, NULL, lp);
    } else if (is_rend == 1) {
        bf->c17 = 1;
        ...
        m->event_rc = model_net_event_mctx(net_id, &mapping_context, &mapping_context, prio,
                                           dest_rank, mpi_op->u.send.num_bytes,
                                           (self_overhead + soft_delay_mpi + nic_delay),
                                           sizeof(nw_message), (const void*)&remote_m,
                                           sizeof(nw_message), (const void*)&local_m, lp);
    }
...
static void send_ack_back(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp, mpi_msgs_queue* mpi_op,
    ...
    m->event_rc = model_net_event_mctx(net_id, &mapping_context, &mapping_context, prio, dest_rank,
                                       CONTROL_MSG_SZ, (self_overhead + soft_delay_mpi + nic_delay),
                                       sizeof(nw_message), (const void*)&remote_m, 0, NULL, lp);
}
...
static void update_arrival_queue(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp) {
    if (s->app_id != m->fwd.app_id)
        printf("\n Received message for app %d my id %d my rank %llu ", m->fwd.app_id, s->app_id,
               LLU(s->nw_id));
...
void nw_test_init(nw_state* s, tw_lp* lp) {
    ...
    s->mpi_wkld_samples =
        (struct mpi_workload_sample*)calloc(MAX_STATS, sizeof(struct mpi_workload_sample));
    s->sampling_indx = 0;
    s->is_finished = 0;
    ...
    s->max_time = 0;
    s->qos_level =
        0; //TODO:  We need a more elegant solution for determining if qos is enabled or not.
           //       This had been -1 but if qos is not configured (single job no workload conf file)
           //       then this will error out
    s->wrkld_id = -1;

    /* START of DIRECTOR setup 
    */
    ...
    if (num_dir_per_mgrp > 0) {
        ...
        codes_mapping_get_lp_id("MODELNET_GRP", "dir-nw-lp", NULL, 1, s->nw_id / num_dir_per_mgrp,
                                s->nw_id % num_dir_per_mgrp, &s->director_lpid);

        // register callbacks with director
        codes_register_director_events(s, DIR_REGISTERED_EVENT__SWITCH_TO_SURR,
                                       CODES_CMD_SWITCH_TO_SURR, sizeof(nw_message), lp);
        codes_register_director_events(s, DIR_REGISTERED_EVENT__SWITCH_TO_PDES,
                                       CODES_CMD_SWITCH_TO_PDES, sizeof(nw_message), lp);
        codes_register_director_events(s, DIR_REGISTERED_EVENT__MOVE_TO_NEXT, MPI_OP_GET_NEXT,
                                       sizeof(nw_message), lp);
...
//               s->app_id, s->local_rank, lp->gid);
    ...
    int rc =
        configuration_get_value_int(&config, "PARAMS", "num_qos_levels", NULL, &num_qos_levels);
    if (rc)
        num_qos_levels = 1;
    ...
    if (strncmp(file_name_of_job[lid.job], "synthetic", 9) == 0) {
        ...
        if (lid.rank == 0) {
            for (int k = 0; k < period_count[lid.job]; k++) {
                ...
                m_new2->fwd.msg_send_time = period_interval
                    [lid.job]
                    [k]; // Warning: this is overwriting a variable meant for message type MPI_SEND_ARRIVED_CB
                tw_event_send(e2);
            }
        }
    ...
    if (iter_predictor) {
        if (am_i_synthetic) {
            ...
            assert(s->wrkld_id != -1);
            int const ending_iter =
                codes_workload_get_final_iteration(s->wrkld_id, s->app_id, s->local_rank);
            if (ending_iter == -1) {
                tw_warning(TW_LOC,
                           "Predictor for non-synthetic job cannot be initialized. app id=%d",
                           s->app_id);
            } else {
                struct app_iter_node_config conf = {
                    .app_id = s->app_id,
...
void nw_test_event_handler(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp) {
    ...
    case CLI_BCKGND_CHANGE:
        m->rc.change.saved_send_time = mean_interval_of_job
            [s->app_id]; // Warning: this is overwriting a variable meant for message type MPI_OP_GET_NEXT (specifically CODES_WK_ALLREDUCE) and CLI_BCKGND_ARRIVE
    ...
    case CODES_CMD_SWITCH_TO_PDES:
        ...
        break;
        /* END of Sim. transition event sent by DIRECTOR */
    }
}
...
static void get_next_mpi_operation(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp) {
    ...
    struct codes_workload_op* mpi_op =
        (struct codes_workload_op*)malloc(sizeof(struct codes_workload_op));
    codes_workload_get_next(s->wrkld_id, s->app_id, s->local_rank, mpi_op);
    m->mpi_op = mpi_op;
    ...
    if (mpi_op->op_type == CODES_WK_MARK) // If annotation type
    { // TODO: extend this section to include checks for all annotations
        m->rc.mpi_next.mark.saved_marker_time = tw_now(lp);
    ...
    case CODES_WK_MARK: {
        ...
        if (iter_predictor &&
            iter_predictor->model.have_we_hit_switch(lp, s->nw_id_in_pe, iteration_i)) {
            bf->c13 = 1;
            struct iteration_pred iter_pred = iter_predictor->model.predict(lp, s->nw_id_in_pe);
...
void nw_test_finalize(nw_state* s, tw_lp* lp) {
    ...
    if (enable_msg_tracking) {
        qlist_for_each(ent, &s->msg_sz_list) {
            ...
            printf("\n Rank %d Msg size %" PRId64 " num_msgs %d agg_latency %f avg_latency %f",
                   s->local_rank, tmp_msg->msg_size, tmp_msg->num_msgs, tmp_msg->agg_latency,
                   tmp_msg->avg_latency);
            ...
            if (s->local_rank == 0) {
                fprintf(msg_size_log, "\n %llu %" PRId64 " %d %f", LLU(s->nw_id), tmp_msg->msg_size,
                        tmp_msg->num_msgs, tmp_msg->avg_latency);
            }
        }
    }
    ...
    if (count_irecv > 0 || count_isend > 0) {
        unmatched = 1;
        printf("\n nw-id %llu unmatched irecvs %d unmatched sends %d Total sends %ld receives %ld "
               "collectives %ld delays %ld wait alls %ld waits %ld send time %lf wait %lf",
               LLU(s->nw_id), count_irecv, count_isend, s->num_sends, s->num_recvs, s->num_cols,
    ...
    if (!s->nw_id)
        written =
            sprintf(s->output_buf, "# Format <LP ID> <Terminal ID> <Job ID> <Local Rank> <Total "
                                   "sends> <Total Recvs> <Bytes sent> <Bytes recvd> <Send time> "
                                   "<Comm. time> <Compute time> <Avg msg time> <Max Msg Time>");

    written +=
        sprintf(s->output_buf + written, "\n %llu %llu %d %d %ld %ld %llu %llu %lf %lf %lf %lf %lf",
                LLU(lp->gid), LLU(s->nw_id), s->app_id, s->local_rank, s->num_sends, s->num_recvs,
                s->num_bytes_sent, s->num_bytes_recvd, s->send_time,
                s->elapsed_time - s->compute_time, s->compute_time, avg_msg_time, s->max_time);
    ...
    if (enable_sampling) {
        ...
        fwrite(s->mpi_wkld_samples, sizeof(struct mpi_workload_sample), s->sampling_indx + 1,
               workload_agg_log);
    }
    ...
    if (debug_cols)
        written += sprintf(s->col_stats + written, "%llu \t %lf \n", LLU(s->nw_id),
                           ns_to_s(s->all_reduce_time / s->num_all_reduce));
...
void nw_test_event_handler_commit(nw_state* s, tw_bf* bf, nw_message* m, tw_lp* lp) {
    ...
    case MPI_OP_GET_NEXT:
        ...
        case CODES_WK_END:
            printf("Network node %d Rank %llu App %d finished at %lf \n", s->local_rank,
                   LLU(s->nw_id), s->app_id, m->rc.mpi_next.mark.saved_marker_time);
            if (iter_predictor) {
                iter_predictor->model.ended(lp, s->nw_id_in_pe,
                                            m->rc.mpi_next.mark.saved_marker_time);
            }
            break;
        ...
        case CODES_WK_MARK:
            if (!m->rc.mpi_next.mark.was_skipped) {
                fprintf(iteration_log, "ITERATION %d node %llu job %d rank %d time %lf\n",
                        ...
                        m->rc.mpi_next.mark.saved_marker_time);
                if (iter_predictor) {
                    iter_predictor->model.feed(lp, s->nw_id_in_pe, m->mpi_op->u.send.tag,
                                               m->rc.mpi_next.mark.saved_marker_time);
                }
            }
    ...
    case SURR_SKIP_ITERATION:
        fprintf(iteration_log, "SKIPPED TO ITERATION %d node %llu job %d rank %d time %lf\n",
                ...
                m->rc.surr_skip.saved_marker_time);
        break;
...
static void make_qlist_cpy(struct qlist_head* into, struct qlist_head const* from,
                           unsigned int sizeof_elem, unsigned int offset_ql) {
    ...
    if (num_elems) {
        ...
        struct qlist_head* last_ql =
            (void*)(pending_recvs + (num_elems - 1) * sizeof_elem + offset_ql);
        into->next = first_ql;
        into->prev = last_ql;
        first_ql->prev = into;
        last_ql->next = into;
...
static bool compare_msg_size_info(struct msg_size_info* left, struct msg_size_info* right) {
    ...
    is_same &=
        left->hash_link.next ==
        right->hash_link
            .next; // This is not correct, we have to do deep copy this and chek that it is the same
...
static void save_nw_lp_state(nw_state* into, nw_state const* from) {
    ...
    make_qlist_cpy(&into->arrival_queue, &from->arrival_queue, sizeof(mpi_msgs_queue),
                   QLIST_OFFSET(mpi_msgs_queue, ql));
    make_qlist_cpy(&into->pending_recvs_queue, &from->pending_recvs_queue, sizeof(mpi_msgs_queue),
                   QLIST_OFFSET(mpi_msgs_queue, ql));
    make_qlist_cpy(&into->completed_reqs, &from->completed_reqs, sizeof(completed_requests),
                   QLIST_OFFSET(completed_requests, ql));
    make_qlist_cpy(&into->msg_sz_list, &from->msg_sz_list, sizeof(struct msg_size_info),
                   QLIST_OFFSET(struct msg_size_info, ql));
...
static void print_mpi_msgs_queue(FILE* out, char const* prefix, struct qlist_head* head) {
    ...
    qlist_for_each_entry(current, head, ql) {
        fprintf(out,
                "%sMsg: OpType: %d Tag %d Source %d Dest %d bytes %" PRId64
                " req_init_time %g req_id %u\n",
                prefix, current->op_type, current->tag, current->source_rank, current->dest_rank,
                current->num_bytes, current->req_init_time, current->req_id);
    }
}
...
static bool check_nw_lp_state(nw_state* before, nw_state const* after) {
    ...
    // Complex elements
    is_same &= are_qlist_equal(&before->arrival_queue, &after->arrival_queue,
                               QLIST_OFFSET(mpi_msgs_queue, ql),
                               (bool (*)(void*, void*))compare_mpi_msg_queues);
    is_same &= are_qlist_equal(&before->pending_recvs_queue, &after->pending_recvs_queue,
                               QLIST_OFFSET(mpi_msgs_queue, ql),
                               (bool (*)(void*, void*))compare_mpi_msg_queues);
    is_same &= are_qlist_equal(&before->completed_reqs, &after->completed_reqs,
                               QLIST_OFFSET(completed_requests, ql),
                               (bool (*)(void*, void*))compare_completed_requests);
    is_same &= are_qlist_equal(&before->msg_sz_list, &after->msg_sz_list,
                               QLIST_OFFSET(struct msg_size_info, ql),
                               (bool (*)(void*, void*))compare_msg_size_info);

    is_same &= !memcmp(&before->ross_sample, &after->ross_sample, sizeof(struct ross_model_sample));

    int num_jobs = codes_jobmap_get_num_jobs(jobmap_ctx);
    is_same &=
        !memcmp(before->known_completed_jobs, after->known_completed_jobs, num_jobs * sizeof(int));
...
static void print_nw_lp_state(FILE* out, char const* prefix, nw_state* state) {
    ...
    for (int i = 0; i < num_jobs; i++) {
        fprintf(out, "%s%d%s", prefix, state->known_completed_jobs[i],
                i + 1 == num_jobs ? "" : ", ");
    }
    fprintf(out, "]\n");
    ...
    if (state->wait_op != NULL) {
        ...
        for (int i = 0; i < state->wait_op->count; i++) {
            fprintf(out, "%d%s", state->wait_op->req_ids[i],
                    i + 1 == state->wait_op->count ? "" : ", ");
        }
        fprintf(out, "]\n");
    ...
    qlist_for_each_entry(ms_info, &state->msg_sz_list, ql) {
        fprintf(out,
                "%s |  | MsSizeInfo: msg_size: %lu num_msgs: %d agg_latency: %g avg_latency: %g "
                "hash_link.next: %p  hash_link.prev: %p\n",
                prefix, ms_info->msg_size, ms_info->num_msgs, ms_info->agg_latency,
                ms_info->avg_latency, ms_info->hash_link.next, ms_info->hash_link.prev);
    }
    fprintf(out, "%s | ]\n", prefix);
...
static char const* const MPI_NW_EVENTS_to_string(enum MPI_NW_EVENTS event_type) {
    ...
    case MPI_REND_ARRIVED:
        return "MPI_REND_ARRIVED";
    case MPI_REND_ACK_ARRIVED:
        return "MPI_REND_ACK_ARRIVED";
    case CLI_BCKGND_FIN:
        return "CLI_BCKGND_FIN";
    case CLI_BCKGND_ARRIVE:
        return "CLI_BCKGND_ARRIVE";
    case CLI_BCKGND_GEN:
        return "CLI_BCKGND_GEN";
    case CLI_BCKGND_CHANGE:
        return "CLI_BCKGND_CHANGE";
    case CLI_NBR_FINISH:
        return "CLI_NBR_FINISH";
    case CLI_OTHER_FINISH:
        return "CLI_OTHER_FINISH";
    case SURR_SKIP_ITERATION:
        return "SURR_SKIP_ITERATION";
    default:
        return "UNKNOWN!!";
    }
}
...
const tw_optdef app_opt[] = {
    ...
    TWOPT_CHAR("workload_name", workload_name,
               "lammps or nekbone (for online workload generation)"),
    ...
    TWOPT_CHAR(
        "link_failure_file", g_nm_link_failure_filepath,
        "filepath for override of link failure file from configuration for supporting models"),
    TWOPT_CHAR("workload_timer_file", workloads_timer_file,
               "workload timer file name (for starting/pausing/stopping synthetic traffic)"),
    TWOPT_CHAR("workload_period_file", workloads_period_file,
               "workload periods file name (for changing the per-job synthetic traffic load at "
               "specified periods/times)"),
    TWOPT_UINT("num_net_traces", num_net_traces, "number of network traces"),
    TWOPT_UINT("priority_type", prioritize_collectives,
               "Priority type (zero): Normal, (one): high priority to collective operations "),
    TWOPT_UINT("payload_sz", payload_sz, "size of payload for synthetic traffic "),
    TWOPT_ULONGLONG("max_gen_data", max_gen_data,
                    "maximum data to be generated for synthetic traffic (Default 0 (OFF))"),
    TWOPT_UINT("eager_threshold", EAGER_THRESHOLD,
    ...
    TWOPT_UINT("preserve_wait_ordering", preserve_wait_ordering,
               "only enable when getting unmatched send/recv errors in optimistic mode (turning on "
               "slows down simulation)"),
    TWOPT_UINT("debug_cols", debug_cols,
               "completion time of collective operations (currently MPI_AllReduce)"),
    TWOPT_UINT("enable_mpi_debug", enable_debug,
    ...
    TWOPT_UINT("enable_sampling", enable_sampling,
...
#ifdef ENABLE_CORTEX_PYTHON
    TWOPT_CHAR("cortex-file", cortex_file,
...
tw_lptype nw_lp = {(init_f)nw_test_init,
                   (pre_run_f)NULL,
...
st_model_types nw_lp_model_types[] = {{(ev_trace_f)nw_lp_event_collect, sizeof(int),
                                       (model_stat_f)nw_lp_model_stat_collect, 0,
...
void modelnet_mpi_replay_read_config() {
    ...
    int rc = configuration_get_value_double(&config, "PARAMS", "compute_time_speedup", NULL,
...
void modelnet_mpi_replay_configure_app_surrogate() {
    ...
    int app_surrogate_len = configuration_get_value(&config, "APPLICATION_SURROGATE", "enable",
                                                    NULL, app_surrogate_test, MAX_NAME_LENGTH);
...
#ifdef USE_RDAMARIS
    if (g_st_ross_rank) { // keep damaris ranks from running code between here up until tw_end()
#endif
        codes_comm_update();
        //Xin: add conceptual online workload
        if (strcmp(workload_type, "dumpi") != 0 && strcmp(workload_type, "swm-online") != 0 &&
            strcmp(workload_type, "conc-online") != 0) {
            if (tw_ismaster())
        ...
        if (strlen(workloads_conf_file) > 0) {
            ...
            while (fgets(line, sizeof(line), name_file)) {
                int const fields =
                    sscanf(line, "%d %s %d %f", &num_traces_of_job[i], file_name_of_job[i],
                           &qos_level_of_job[i], &mean_interval_of_job[i]);
                if (fields != 4) {
                    tw_error(TW_LOC, "Invalid format in %s at line %d: expected 4 fields, got %d",
                             workloads_conf_file, i + 1, fields);
                }
                ...
                if (strncmp(file_name_of_job[i], "synthetic", 9) == 0) {
                    ...
                    if (strcmp(file_name_of_job[i], "synthetic6") == 0 &&
                        num_traces_of_job[i] % 2 != 0)
            ...
            if (strlen(workloads_period_file) > 0) {
                ...
                while (!feof(period_file)) {
                    if (j >= MAX_JOBS && !g_tw_mynode) {
                        tw_error(
                            TW_LOC,
                            "Exceeded number of max workloads in workloads period file. Max: %d",
                            MAX_JOBS);
                    }
                    ...
                    if (ref2 != EOF) {
                        ...
                        for (int k = 0; k < period_count[j]; k++) {
                            fscanf(period_file, "%lf:%f", &period_time[j][k],
                                   &period_interval[j][k]);
                            if (!g_tw_mynode) {
            ...
            if (is_conc_enabled && strlen(workload_json_files) > 0) {
                ...
                while (!feof(json_file) && workload_json_mapping_count < MAX_JOBS) {
                    if (fscanf(json_file, "%s %s",
                               ...
                               workload_json_mappings[workload_json_mapping_count].json_path) ==
                        2) {
            ...
            if (!is_conc_enabled && strlen(workload_json_files) > 0) {
                printf("\n Conceptual online worloads will not run, thus, we won't read any json "
                       "files from --workload_json_files\n");
            }
        } else {
        ...
        if (jobmap_ctx ==
            NULL) //then we need to set up an identity jobmap for the ranks that do exist
        {
        ...
        if (enable_msg_tracking) {
            char log_name[512];
            sprintf(log_name, "%s/mpi-msg-sz-logs-%s-syn-sz-%d-mean-%f-%d", mpi_msg_dir,
                    file_name_of_job[0], payload_sz, mean_interval, rand());
        ...
        codes_mapping_setup();
        congestion_control_set_jobmap(
            jobmap_ctx,
            net_id); //must be placed after codes_mapping_setup - where g_congestion_control_enabled is set

        num_mpi_lps = codes_mapping_get_lp_count("MODELNET_GRP", 0, "nw-lp", NULL, 0);

        num_nw_lps = codes_mapping_get_lp_count("MODELNET_GRP", 1, "nw-lp", NULL, 1);
        ...
        MPI_Reduce(&num_bytes_sent, &total_bytes_sent, 1, MPI_LONG_LONG, MPI_SUM, 0,
                   MPI_COMM_CODES);
        MPI_Reduce(&num_bytes_recvd, &total_bytes_recvd, 1, MPI_LONG_LONG, MPI_SUM, 0,
                   MPI_COMM_CODES);
        ...
        MPI_Reduce(max_elapsed_time_per_job, g_max_elapsed_time_per_job, num_total_jobs, MPI_DOUBLE,
                   MPI_MAX, 0, MPI_COMM_CODES);
        ...
        if (!g_tw_mynode) {
            printf("\n Total bytes sent %lld recvd %lld \n max runtime %lf ns avg runtime %lf \n "
                   ...
                   total_bytes_sent, total_bytes_recvd, max_run_time, avg_run_time / num_net_traces,
                   max_comm_run_time, avg_comm_run_time / num_net_traces, total_max_send_time,
                   total_avg_send_time / num_net_traces, total_max_recv_time,
                   total_avg_recv_time / num_net_traces, total_max_wait_time,
                   total_avg_wait_time / num_net_traces);
