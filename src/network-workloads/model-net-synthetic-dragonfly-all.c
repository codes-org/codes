...
static const struct codes_workload_traffic_name traffic_names[] = {
    ...
    {"random_other_group", RANDOM_OTHER_GROUP},
    {NULL, 0},
};
...
st_model_types dragonfly_svr_model_types[] = {{(ev_trace_f)dragonfly_svr_event_collect, sizeof(int),
                                               (model_stat_f)dragonfly_svr_model_stat_collect, 0,
                                               NULL, NULL, 0},
...
const tw_optdef app_opt[] = {
    ...
    TWOPT_UINT("rperm_threshold", rperm_threshold,
               "size of rperm data sent before selecting new dest "),
    ...
    TWOPT_STIME("warm_up_time", warm_up_time,
                "Time delay before starting stats colleciton. For generating accurate observed "
...
static tw_stime issue_event(svr_state* ns, tw_lp* lp) {
    (void)ns;
    tw_event* e;
    svr_msg* m;
...
static void handle_kickoff_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    m->event_rc =
        model_net_event(net_id, "test", global_dest, PAYLOAD_SZ, 0.0, sizeof(svr_msg),
...
static void svr_finalize(svr_state* ns, tw_lp* lp) {
    ...
    double offered_load =
        ((double)PAYLOAD_SZ * (double)ns->warm_msg_sent_count) / offered_load_time;
    ...
    written += sprintf(output_buf + written, "%llu %d %d %d %d %f %f %f %f\n", LLU(lp->gid),
                       ns->msg_sent_count, ns->msg_recvd_count, PAYLOAD_SZ * ns->msg_sent_count,
                       PAYLOAD_SZ * ns->msg_recvd_count, load * link_bandwidth, observed_load,
                       ns->last_recv_ts, observed_load_time);
...
static void aggregate_svr_stats(int myrank) {
    ...
    MPI_Reduce(&pe_total_offered_load, &agg_offered_load, 1, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_CODES);
    MPI_Reduce(&pe_total_observed_load, &agg_observed_load, 1, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_CODES);
...
int main(int argc, char** argv) {
    ...
    if (g_st_ross_rank) { // keep damaris ranks from running code between here up until tw_end()
        ...
        codes_workload_config_check_unsupported_jobs("model-net-synthetic-dragonfly-all");
         ...
         * effect, which the previous unconditional override silently ignored. */
        if (g_tw_ts_end == 100000.0)
            g_tw_ts_end = s_to_ns(5 * 24 * 60 * 60);
        model_net_enable_sampling(sampling_interval, sampling_end_time);

        if (!(net_id == DRAGONFLY_DALLY || net_id == DRAGONFLY_PLUS || net_id == DRAGONFLY_CUSTOM ||
              net_id == DRAGONFLY)) {
            printf("\n The workload generator is designed to only work with Dragonfly based model "
                   "(Dally, Plus, Custom, Original) configuration only! %d %d ",
                   DRAGONFLY_DALLY, net_id);
            MPI_Finalize();
            return 0;
        }
        ...
        if (net_id == DRAGONFLY_DALLY) {
            ...
            configuration_get_value_int(&config, "PARAMS", "num_cns_per_router", NULL,
                                        &num_nodes_per_router);
            total_terminals =
                codes_mapping_get_lp_count("MODELNET_GRP", 0, "modelnet_dragonfly_dally", NULL, 1);
            ...
            configuration_get_value_int(&config, "PARAMS", "num_router_leaf", NULL,
                                        &num_router_leaf);
            configuration_get_value_int(&config, "PARAMS", "num_router_spine", NULL,
                                        &num_router_spine);
            configuration_get_value_int(&config, "PARAMS", "num_routers", NULL, &num_routers);
            configuration_get_value_int(&config, "PARAMS", "num_groups", NULL, &num_groups);
            configuration_get_value_int(&config, "PARAMS", "num_cns_per_router", NULL,
                                        &num_nodes_per_router);
            total_terminals =
                codes_mapping_get_lp_count("MODELNET_GRP", 0, "modelnet_dragonfly_plus", NULL, 1);
            ...
            configuration_get_value_int(&config, "PARAMS", "num_router_rows", NULL,
                                        &num_router_rows);
            configuration_get_value_int(&config, "PARAMS", "num_router_cols", NULL,
                                        &num_router_cols);
            configuration_get_value_int(&config, "PARAMS", "num_groups", NULL, &num_groups);
            configuration_get_value_int(&config, "PARAMS", "num_cns_per_router", NULL,
                                        &num_nodes_per_router);
            total_terminals =
                codes_mapping_get_lp_count("MODELNET_GRP", 0, "modelnet_dragonfly_custom", NULL, 1);
        ...
        struct codes_jobmap_ctx* jobmap_ctx =
            codes_jobmap_configure(CODES_JOBMAP_IDENTITY, &jobmap_ident_p);
