...
/*
 * The test program generates some synthetic traffic patterns for the slim fly network models.
 * currently it only supports uniform random and worst-case traffic patterns.
 */
...
enum TRAFFIC {
    ...
    NEAREST_NEIGHBOR_2D =
        4, /* sends messages to nearest neighbor nodes following 2D node mapping */
    NEAREST_NEIGHBOR_3D =
        5,        /* sends messages to nearest neighbor nodes following 3D node mapping */
...
//NOTE: load_per_svr is the percent of cn_bandwidth to inject to each terminal per server from (0,X].
// So if you have 2 injection queues (and configured for two servers per terminal) and a load of 1, then each
// terminal will get 200% of its cn_bandwidth supplied to it.
const tw_optdef app_opt[] = {
    TWOPT_GROUP("Model net synthetic traffic "),
    TWOPT_UINT("traffic", traffic,
               "1: Uniform Random, 2: Worst-Case, 3: 1D Nearest Neighbor, 4: 2D Nearest Neighbor, "
               "5: 3D Nearest Neighbor, 6: Gather, 7: Scatter, 8: Bisection"),
    ...
    TWOPT_STIME("warm_up_time", warm_up_time,
                "Time delay before starting stats collection. For generating accurate observed "
                "bandwidth calculations"),
    TWOPT_STIME("load_per_svr", load,
                "percentage of packet inter-arrival rate to simulate per server"),
...
static void svr_init(svr_state* ns, tw_lp* lp) {
    ...
    // Initiailize comm_map 2D array - TODO: NM: this is broken in conservative/optimistic
    // if(!lp->gid){
...
static void handle_kickoff_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    assert(net_id == SLIMFLY); /* only supported for slimfly model right now. */
    ns->start_ts = tw_now(lp);

    codes_mapping_get_lp_info(lp->gid, group_name, &group_index, lp_type_name, &lp_type_index, anno,
                              &rep_id, &offset);

    int num_server_lps = codes_mapping_get_lp_count(group_name, 1, LP_CONFIG_NM, NULL, 0);
    ...
    if (traffic == UNIFORM) {
        ...
        num_transfers = 1;
        local_dest = (int*)malloc(num_transfers * sizeof(int));
        local_dest[0] = (server_id + (int)(num_nodes / 3)) % num_nodes;
    }
    ...
    for (int i = 0; i < num_transfers; i++) {
        ...
        codes_mapping_get_lp_id(group_name, lp_type_name, anno, 1,
                                local_dest[i] / num_servers_per_rep,
                                local_dest[i] % num_servers_per_rep, &global_dest);
        // Increment send count in communication heat map
        // comm_map[server_id][local_dest[i]]++; //TODO: NM: This is broken in cons/opt
        // Increment send count
        ns->msg_sent_count++;
        // Issue event //TODO: NM: This is broken in optimistic when num_transfers > 1
        m->event_rc =
            model_net_event(net_id, "test", global_dest, payload_size, i * 0.2, sizeof(svr_msg),
...
static void svr_finalize(svr_state* ns, tw_lp* lp) {
    ...
    double observed_load =
        ((double)payload_size * (double)ns->msg_recvd_count) / observed_load_time;
    ...
    if (lp->gid == 0) {
        written = sprintf(ns->output_buf,
                          "# Format <LP id> <Msgs Sent> <Msgs Recvd> <Bytes Sent> <Bytes Recvd> "
                          "<Offered Load [GBps]> <Observed Load [GBps]> <End Time [ns]>\n");
        written2 = sprintf(
            ns->output_buf2,
            "# Format <server ID> <sends to svr 0> <sends to svr 1> ... <sends to svr N>\n");
    }

    written += sprintf(ns->output_buf + written, "%llu %d %d %d %d %f %f %f %f\n", LLU(lp->gid),
                       ns->msg_sent_count, ns->msg_recvd_count, payload_size * ns->msg_sent_count,
                       payload_size * ns->msg_recvd_count, load * link_bandwidth, observed_load,
                       ns->end_ts, observed_load_time);

    lp_io_write(lp->gid, "synthetic-stats", written, ns->output_buf);


    //TODO: NM: This is broken for cons/opt
    // Get server mapping info
    // char anno[MAX_NAME_LENGTH];
    // int num_server_lps = codes_mapping_get_lp_count(group_name, 1, model_net_lp_config_names[SLIMFLY], NULL, 0);
...
int main(int argc, char** argv) {
    ...
    if (g_st_ross_rank) { // keep damaris ranks from running code between here up until tw_end()
        ...
        if (traffic == WORST_CASE) {
            configuration_get_value_int(&config, "PARAMS", "local_channels", NULL,
                                        &num_local_channels);
            ...
            configuration_get_value_int(&config, "PARAMS", "global_channels", NULL,
                                        &num_global_channels);
            ...
            char** values;
            size_t length;
            int ret = configuration_get_multivalue(&config, "PARAMS", "generator_set_X", NULL,
                                                   &values, &length);
            if (ret != 1)
            ...
            ret = configuration_get_multivalue(&config, "PARAMS", "generator_set_X_prime", NULL,
                                               &values, &length);
            if (ret != 1)
...
void init_worst_case_mapping() {
    ...
    for (int src_idx = start; src_idx < total_routers + start; src_idx++) {
        ...
        if (worst_dest[src] < 0) {
            ...
            for (int dst_idx = src + total_routers / 2;
                 dst_idx < total_routers + src + total_routers / 2; dst_idx++) {
                    ...
                    if (is_two_hops_away(src, dst, intm, &len_intm)) {
                        for (int intm_indx = 0; intm_indx < len_intm; intm_indx++) {
                            ...
                            if (worst_dest[intm[intm_indx]] < 0) {
                                ...
                                for (int intm_dst_idx = intm[intm_indx] + total_routers / 2;
                                     intm_dst_idx <
                                     total_routers + intm[intm_indx] + total_routers / 2;
                                     intm_dst_idx++) {
                                    ...
                                    if (intm_dst != dst && intm_dst != src) {
                                        ...
                                        if (worst_dest[intm_dst] < 0) {
                                            ...
                                            int len_intm2 = 0;
                                            // Check if the destination is two hops from the source
                                            if (is_two_hops_away(intm[intm_indx], intm_dst, intm2,
                                                                 &len_intm2)) {
                                                for (int intm2_indx = 0; intm2_indx < len_intm2;
                                                     intm2_indx++) {
                                                    ...
                                                    if (middled[intm2[intm2_indx]] < 1) {
                                                        ...
                                                        printf(
                                                            "intm2:%d is already paired with %d\n",
                                                            intm2[intm2_indx],
                                                            worst_dest[intm2[intm2_indx]]);
                                                }
                                                if (worst_dest[intm[intm_indx]] < 0) {
                                                    if (src == srcID)
                                                        printf("No worst-case pairing found for "
                                                               "the intermediate router:%d with "
                                                               "src:%d and dst:%d\n",
                                                ...
                                                printf("intm_dst:%d is not two hops away from "
                                                       "intm:%d\n",
                                ...
                                if (worst_dest[intm[intm_indx]] < 0) {
                                    if (src == srcID) {
                                        printf("No worst-case pairing found for the intermediate "
                                               "router:%d with src:%d and dst:%d\n",
                                               intm[intm_indx], src, dst);
                                    }
                                } else {
                                    break;
    ...
    for (k = 0; k < 2; k++) {
        for (j = 0; j < num_routers_per_grp - 1; j += 2) {
            for (i = 0; i < num_routers_per_grp; i++) {
                ...
                r2 = i + (j + 1) * num_routers_per_grp +
                     k * num_routers_per_grp * num_routers_per_grp;
                worst_dest[r1] = r2;
                worst_dest[r2] = r1;
