...
static void codes_kernel_helper_parse_cf(char* io_kernel_path, char* io_kernel_meta_path,
                                         int task_rank, int max_ranks_default,
    ...
    while (fgets(line, CK_LINE_LIMIT, ikmp) != NULL) {
        ...
        if (gid == CL_DEFAULT_GID) {
            fprintf(stderr, "%s:%i incorrect GID detected in kernel meta\
                           ...
                           CL_DEFAULT_GID(%i)\n",
                    __func__, __LINE__, CL_DEFAULT_GID);
        }
...
int codes_kernel_helper_parse_input(CodesIOKernel_pstate* ps, CodesIOKernelContext* c,
    ...
    do {
        ...
        case DELETE: {
            c->inst_ready = 0;
            status =
                CodesIOKernel_push_parse(ps, yychar, (codesYYType*)c->lval, (YYLTYPE*)c->locval, c);
...
int codes_kernel_helper_bootstrap(char* io_kernel_path, char* io_kernel_meta_path, int rank,
    ...
    codes_kernel_helper_parse_cf(io_kernel_path, io_kernel_meta_path, rank, num_ranks, task_info,
                                 use_relpath);
