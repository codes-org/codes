...
bool network_surrogate_configure(char const* const anno, struct network_surrogate_config* const sc,
    ...
    director_mode[0] = '\0';
    configuration_get_value(&config, "NETWORK_SURROGATE", "director_mode", anno, director_mode,
                            MAX_NAME_LENGTH);
    if (strcmp(director_mode, "at-fixed-virtual-times") == 0) {
        ...
        char** timestamps;
        size_t len;
        configuration_get_multivalue(&config, "NETWORK_SURROGATE", "fixed_switch_timestamps", anno,
                                     &timestamps, &len);
        ...
        master_printf("\nNetwork surrogate enabled but director won't run. Network surrogate will "
                      "be triggered by app director if present\n");
    } else {
    ...
    configuration_get_value(&config, "NETWORK_SURROGATE", "debug_prints", anno, debug_prints_str,
                            MAX_NAME_LENGTH);
    bool debug_prints =
        (strcmp(debug_prints_str, "1") == 0 || strcmp(debug_prints_str, "true") == 0 ||
         strcmp(debug_prints_str, "TRUE") == 0 || strcmp(debug_prints_str, "yes") == 0 ||
         strcmp(debug_prints_str, "YES") == 0);

    char latency_pred_name[MAX_NAME_LENGTH];
    latency_pred_name[0] = '\0';
    configuration_get_value(&config, "NETWORK_SURROGATE", "packet_latency_predictor", anno,
                            latency_pred_name, MAX_NAME_LENGTH);
    if (*latency_pred_name) {
        if (strcmp(latency_pred_name, "average") == 0) {
            ...
            *pl_pred = &current_net_predictor;
            ...
            torch_jit_mode[0] = '\0';
            configuration_get_value(&config, "NETWORK_SURROGATE", "torch_jit_mode", anno,
                                    torch_jit_mode, MAX_NAME_LENGTH);
            ...
            if (strcmp(torch_jit_mode, "single-static-model-for-all-terminals") == 0) {
                ...
                configuration_get_value(&config, "NETWORK_SURROGATE",
                                        "torch_jit_terminal_model_path", anno, terminal_model_path,
                                        MAX_NAME_LENGTH);
                configuration_get_value(&config, "NETWORK_SURROGATE",
                                        "torch_jit_router_timing_model_path", anno,
                                        router_timing_model_path, MAX_NAME_LENGTH);
                configuration_get_value(&config, "NETWORK_SURROGATE",
                                        "torch_jit_default_model_path", anno, default_model_path,
                                        MAX_NAME_LENGTH);

                surrogate_torch_init_lp_type_models(terminal_model_path, router_timing_model_path,
                                                    default_model_path);
            } else {
                tw_error(
                    TW_LOC,
    ...
    int rc = configuration_get_value_double(&config, "NETWORK_SURROGATE", "ignore_until", anno,
                                            &ignore_until);
    if (rc) {
        ignore_until =
            -1; // any negative number disables ignore_until, all packet latencies will be considered
        master_printf("`ignore_until` disabled (all packet latencies will be used in training the "
                      "predictor)\n");
    } else {
        master_printf("ignore_until=%g a packet delievered before this time stamp will not be used "
                      "in training any predictor\n",
                      ignore_until);
    }
    ...
    configuration_get_value(&config, "NETWORK_SURROGATE", "network_treatment_on_switch", anno,
                            network_treatment_name, MAX_NAME_LENGTH);
    if (*network_treatment_name) {
        if (strcmp(network_treatment_name, "freeze") == 0) {
            ...
            master_printf("The network will be left alone on switch to surrogate (it will run on "
                          "the background until it empties by itself)\n");
        } else {
            tw_error(TW_LOC,
                     "Unknown network treatment `%s` (possibilities include: frezee or nothing)",
                     network_treatment_name);
        }
    } else {
        freeze_network_on_switch = true;
    ...
    network_director_configure(sc, network_director_enabled ? &switch_network_at : NULL,
...
static int load_and_validate_int_param(const char* param_name, int default_value) {
    ...
    param_str[0] = '\0';
    int const rc = configuration_get_value(&config, "APPLICATION_SURROGATE", param_name, NULL,
                                           param_str, MAX_NAME_LENGTH);
    int value = (rc > 0) ? atoi(param_str) : default_value;

    if (value <= 0) {
        tw_warning(TW_LOC, "%s must be a positive integer, got %d. Using default value %d.",
...
static struct application_director_config load_director_config(void) {
    ...
    enum { MODE_NOT_SET, MODE_EVERY_N_GVT, MODE_EVERY_N_NANOSECONDS, MODE_UNKNOWN } mode;

    char director_mode[MAX_NAME_LENGTH];
    director_mode[0] = '\0';
    int const rc_mode = configuration_get_value(&config, "APPLICATION_SURROGATE", "director_mode",
                                                NULL, director_mode, MAX_NAME_LENGTH);
    ...
    case MODE_EVERY_N_GVT:
        if (is_sequential) {
            tw_warning(TW_LOC, "Cannot use 'every-n-gvt' mode in sequential simulation. Forcing "
                               "'every-n-nanoseconds' mode.");
    ...
    case MODE_UNKNOWN:
        tw_warning(TW_LOC, "Unknown director_mode '%s'. Using default mode 'every-n-nanoseconds'.",
                   director_mode);
        config.option = APP_DIRECTOR_OPTS_call_every_ns;
        config.call_every_ns = every_n_ns;
        break;
...
void application_surrogate_configure(int num_terminals_in_pe, int num_apps,
                                     struct app_iteration_predictor** iter_pred) {
    char num_iters_str[MAX_NAME_LENGTH];
    num_iters_str[0] = '\0';
    int const rc = configuration_get_value(&config, "APPLICATION_SURROGATE", "num_iters_to_collect",
                                           NULL, num_iters_str, MAX_NAME_LENGTH);
    int const num_of_iters_to_feed =
        (rc > 0) ? atoi(num_iters_str) : 5; // default to 5 if not specified
    ...
    *iter_pred = &current_iter_predictor;
    ...
    if (network_director_enabled) {
        master_printf("  The network director has been replaced by the application director. The "
                      "application director will trigger the network surrogate on and off.\n");
    }
    master_printf("\n");
}
