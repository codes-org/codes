...
static struct application_director_config conf = {.option = APP_DIRECTOR_OPTS_call_every_ns,
                                                  .every_n_gvt = 1000000,
...
static void application_director_pre_switch(tw_pe* pe, bool is_queue_empty) {
    ...
    double const restarting_at =
        jump_to.restarting_at > gvt_for(pe) ? jump_to.restarting_at : gvt_for(pe);
    switch (jump_to.status) {
    case FAST_FORWARD_switching:
        tw_trigger_gvt_hook_at(
            restarting_at +
            1); // + 1 to force director to run right after we have fully fast-forward
        master_printf(
            "Triggering switch to application iteration surrogate mode at GVT %d time %f\n",
...
void application_director_configure(struct application_director_config* conf_,
                                    struct app_iteration_predictor* iter_predictor_) {
    conf = *conf_;
