...
static void feed_pred(struct latency_surrogate* data, tw_lp* lp, unsigned int src_terminal,
...
static struct packet_end predict_latency(struct latency_surrogate* data, tw_lp* lp,
    ...
    if (total_total_datapoints == 0) {
        ...
        tw_error(TW_LOC,
                 "Terminal %u doesn't have any packet delay information available to predict "
                 "future packet latency!\n",
                 src_terminal);
    ...
    unsigned int const total_datapoints_for_term =
        data->aggregated_latency[dest_terminal].total_msgs;
    double latency = -1.0;
    ...
    double const next_packet_delay = data->aggregated_next_packet_delay.total_msgs == 0
                                         ? 10
                                         : data->aggregated_next_packet_delay.sum_latency /
                                               data->aggregated_next_packet_delay.total_msgs;
    ...
    if (average_predictor_debug_prints &&
        ...
        fprintf(stderr,
                ...
                average_predict_calls, latency, next_packet_delay,
                data->aggregated_next_packet_delay.total_msgs,
                data->aggregated_latency_for_all.total_msgs,
                data->aggregated_latency[dest_terminal].total_msgs);
        fflush(stderr);
...
struct packet_latency_predictor average_latency_predictor(int num_terminals_) {
    ...
    return (struct packet_latency_predictor){
        ...
        .predictor_data_sz = sizeof(struct latency_surrogate) +
                             num_terminals * sizeof(struct aggregated_latency_one_terminal)};
}
//
