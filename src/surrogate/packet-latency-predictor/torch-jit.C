...
static void load_module_or_die(torch::jit::Module *module, char const *path, char const *label) {
    ...
    if (module->is_training()) {
        std::cerr << "The Torch-JIT " << label
                  << " model was saved before running .eval(); inference will use training-mode behavior."
                  << std::endl;
    }
}

static void validate_output_dims(at::Tensor const &output, int expected_last_dim, char const *label) {
    int const dims = output.ndimension();
    if (dims < 1) {
        tw_error(TW_LOC, "Torch-JIT %s model returned a scalar; expected [1,%d]", label, expected_last_dim);
    }
    ...
    if (at::size(output, dims - 1) != expected_last_dim) {
        tw_error(TW_LOC, "Torch-JIT %s model returned last dim %lld; expected %d",
                label, (long long)at::size(output, dims - 1), expected_last_dim);
    }
}

static void validate_packet_latency_model(torch::jit::Module *model, int feature_count, at::ScalarType dtype, char const *label) {
    std::vector<torch::jit::IValue> inputs;
    torch::NoGradGuard no_grad;

    if (dtype == at::kFloat) {
        std::vector<float> data_input(feature_count, 0.0f);
        inputs.emplace_back(torch::from_blob(data_input.data(), {1, feature_count}, at::kFloat).clone());
    } else if (dtype == at::kLong) {
        std::vector<long int> data_input(feature_count, 0);
        inputs.emplace_back(torch::from_blob(data_input.data(), {1, feature_count}, at::kLong).clone());
    } else {
...
static void validate_router_timing_model(torch::jit::Module *model, char const *label) {
    ...
    inputs.emplace_back(torch::from_blob(data_input.data(),
                                         {1, TORCH_JIT_ROUTER_TIMING_FEATURE_COUNT},
                                         at::kFloat).clone());
    at::Tensor output = model->forward(inputs).toTensor();
...
void surrogate_torch_init(char const *dir) {
    ...
    if (torch_jit_lp_aware_mode) {
        validate_packet_latency_model(&packet_latency_model,
                                      TORCH_JIT_LP_AWARE_FEATURE_COUNT,
                                      at::kFloat,
                                      "lp-aware packet-latency");
    } else {
        validate_packet_latency_model(&packet_latency_model,
                                      TORCH_JIT_LEGACY_FEATURE_COUNT,
                                      at::kLong,
                                      "legacy packet-latency");
    }
        ...
        char const *default_model_path)
{
    ...
    if (has_path(terminal_model_path)) {
        load_module_or_die(&terminal_packet_latency_model, terminal_model_path, "terminal packet-latency");
    ...
    if (has_path(default_model_path)) {
        load_module_or_die(&default_packet_latency_model, default_model_path, "default packet-latency");
    ...
    if (!terminal_packet_latency_model_loaded && !default_packet_latency_model_loaded) {
        tw_error(TW_LOC, "torch_jit_mode=lp-aware-lp-type-models requires torch_jit_terminal_model_path or torch_jit_default_model_path");
    }
    ...
    if (has_path(router_timing_model_path)) {
        ...
        tw_warning(TW_LOC, "No torch_jit_router_timing_model_path configured; router timing inference disabled.");
    }
...
static struct packet_end surrogate_torch_predict(void *, tw_lp *lp, unsigned int src_terminal, struct packet_start const *packet_dest) {
    ...
    if (torch_jit_lp_type_model_mode) {
        model = terminal_packet_latency_model_loaded ? &terminal_packet_latency_model : &default_packet_latency_model;
    }

    if (torch_jit_lp_aware_mode || torch_jit_lp_type_model_mode) {
        std::vector<float> data_input = build_lp_aware_packet_features(lp, src_terminal, packet_dest);
        ...
        inputs.emplace_back(torch::from_blob(data_input,
    ...
    torch_jit_forward_total_sec += std::chrono::duration<double>(torch_jit_t1 - torch_jit_t0).count();
    ...
    if (raw_travel_delta < torch_jit_min_travel_delta) torch_jit_min_travel_delta = raw_travel_delta;
    if (raw_travel_delta > torch_jit_max_travel_delta) torch_jit_max_travel_delta = raw_travel_delta;
    if (raw_next_delay < torch_jit_min_next_delay) torch_jit_min_next_delay = raw_next_delay;
    if (raw_next_delay > torch_jit_max_next_delay) torch_jit_max_next_delay = raw_next_delay;
    ...
    if (torch_jit_debug_prints &&
        ...
        fprintf(stderr,
            ...
            raw_travel_delta, raw_next_delay,
            predicted_travel_delta, predicted_next_packet_delay,
            torch_jit_min_travel_delta, torch_jit_max_travel_delta,
        ...
        double fallback_queueing_delay)
{
    ...
    torch_jit_router_forward_total_sec += std::chrono::duration<double>(torch_jit_t1 - torch_jit_t0).count();
    ...
    double const predicted_queueing_delay =
            std::isfinite(raw_queueing_delay) && raw_queueing_delay >= 0.0
            ? raw_queueing_delay
            : fallback_queueing_delay;

    if (raw_queueing_delay < torch_jit_min_router_queueing_delay) torch_jit_min_router_queueing_delay = raw_queueing_delay;
    if (raw_queueing_delay > torch_jit_max_router_queueing_delay) torch_jit_max_router_queueing_delay = raw_queueing_delay;
    ...
    if (torch_jit_debug_prints &&
        ...
        fprintf(stderr,
            ...
            raw_queueing_delay,
            predicted_queueing_delay,
            torch_jit_min_router_queueing_delay,
