...
static void* darshan_io_workload_read_config(ConfigHandle* handle, char const* section_name,
...
static void generate_mpiio_file_events(struct darshan_mpiio_file* mfile,
                                       struct rank_io_context* io_context);
static double generate_open_event(darshan_record_id id, enum codes_workload_op_type type,
                                  int create_flag, double meta_op_time, double cur_time,
                                  struct rank_io_context* io_context, int insert_flag);
static double generate_close_event(darshan_record_id id, enum codes_workload_op_type type,
                                   double meta_op_time, double cur_time,
                                   struct rank_io_context* io_context, int insert_flag);
static double generate_psx_io_events(struct darshan_posix_file* file, double inter_io_delay,
                                     double cur_time, struct rank_io_context* io_context);
static double generate_mpiio_io_events(struct darshan_mpiio_file* mfile, double inter_io_delay,
                                       double cur_time, struct rank_io_context* io_context);
static void determine_mpiio_io_params(struct darshan_mpiio_file* mfile, int write_flag,
...
static void mpiio_calc_io_delays(struct darshan_mpiio_file* mfile, int64_t num_io_ops,
                                 double total_delay, double* first_io_delay, double* close_delay,
                                 double* inter_io_delay);
static void file_sanity_check(struct darshan_posix_file* file, struct darshan_mpiio_file* mfile,
                              struct darshan_job* job, darshan_fd fd);

static int darshan_psx_io_workload_get_time(const char* params, int app_id, int rank,
...
static void* darshan_io_workload_read_config(ConfigHandle* handle, char const* section_name,
    ...
    int rc = configuration_get_value_relpath(handle, section_name, "darshan_log_file", annotation,
                                             d->log_file_path, MAX_NAME_LENGTH_WKLD);
    assert(rc > 0);
...
static int darshan_psx_io_workload_load(const void* params, int app_id, int rank) {
    ...
    while ((ret = mpiio_utils->log_get_record(logfile_fd, (void**)&mpiio_file_rec)) > 0) {
        for (dur_cur = dur_head; dur_cur; dur_cur = dur_cur->next) {
            if ((dur_cur->psx_file_rec.base_rec.rank == mpiio_file_rec->base_rec.rank) &&
                (dur_cur->psx_file_rec.base_rec.id == mpiio_file_rec->base_rec.id)) {
            ...
            if ((dur_cur->psx_file_rec.base_rec.id == mpiio_file_rec->base_rec.id) &&
                ...
                fprintf(stderr,
                        "WARNING: id %" PRIu64
                        " has non-shared MPI record and shared POSIX record.  Skipping POSIX "
                        "record which may have been generated by stat() calls.\n",
    ...
    for (dur_cur = dur_head; dur_cur; dur_cur = dur_cur->next) {
        ...
        if (dur_cur->psx_file_rec.counters[POSIX_BYTES_READ] &&
            ...
            if (rank == 0) {
                printf("WARNING: skipping R/W file record %lu with %ld bytes read and %ld bytes "
                       "written\n",
        ...
        if (dur_cur->mpiio_file_rec.counters[MPIIO_COLL_OPENS] ||
            ...
            if (dur_cur->mpiio_file_rec.base_rec.rank == rank ||
                ...
                file_sanity_check(&dur_cur->psx_file_rec, &dur_cur->mpiio_file_rec, &job,
                                  logfile_fd);
    ...
    if (!rank_tbl) {
        rank_tbl =
            qhash_init(darshan_rank_hash_compare, quickhash_64bit_hash, RANK_HASH_TABLE_SIZE);
        if (!rank_tbl)
            return -1;
    }
...
static void generate_mpiio_file_events(struct darshan_mpiio_file* mfile,
    ...
    if (mfile->base_rec.rank == -1) {
        /* shared file */
        total_delay = mfile->fcounters[MPIIO_F_CLOSE_TIMESTAMP] -
    ...
    int all_io_ops = mfile->counters[MPIIO_INDEP_READS] + mfile->counters[MPIIO_INDEP_WRITES] +
                     mfile->counters[MPIIO_COLL_READS] + mfile->counters[MPIIO_COLL_WRITES];
    if (mfile->base_rec.rank == -1) {
    ...
    mpiio_calc_io_delays(mfile, num_io_ops, total_delay, &first_io_delay, &close_delay,
                         &inter_io_delay);
    ...
    cur_time = generate_mpiio_io_events(mfile, inter_io_delay, cur_time, io_context);
...
static void generate_psx_file_events(struct darshan_posix_file* file,
    ...
    /* calculate synthetic delay values */
    psx_calc_io_delays(file, num_io_ops, total_delay, &first_io_delay, &close_delay,
                       &inter_io_delay);
    ...
    /* perform the calculated number of i/o operations for this file open */
    cur_time = generate_psx_io_events(file, inter_io_delay, cur_time, io_context);
...
static double generate_open_event(darshan_record_id id, enum codes_workload_op_type type,
                                  int create_flag, double meta_op_time, double cur_time,
                                  struct rank_io_context* io_context, int insert_flag) {
    struct darshan_io_op next_io_op = {.codes_op.op_type = type,
...
static double generate_mpiio_io_events(struct darshan_mpiio_file* mfile, double inter_io_delay,
    ...
    for (i = 0; i < (mfile->counters[MPIIO_COLL_WRITES] + mfile->counters[MPIIO_INDEP_WRITES]);
         i++) {
        ...
        if (mfile->base_rec.rank == io_context->my_rank ||
            (mfile->base_rec.rank == -1 && i % total_rank_cnt == io_context->my_rank)) {
...
static double generate_psx_io_events(struct darshan_posix_file* file, double inter_io_delay,
    ...
    for (i = 0; i < file->counters[POSIX_WRITES]; i++) {
        ...
        if (file->base_rec.rank == io_context->my_rank ||
            (file->base_rec.rank == -1 && i % total_rank_cnt == io_context->my_rank)) {
            /* generate a write event */
...
static void determine_mpiio_io_params(struct darshan_mpiio_file* mfile, int write_flag,
    ...
    int64_t* common_access_counts =
        &(mfile->counters[MPIIO_ACCESS1_COUNT]); /* common access counts */
    int64_t* total_io_size = NULL;
    int i, j = 0;
    const int64_t size_bin_min_vals[10] = {0,
                                           100,
                                           1024,
                                           10 * 1024,
                                           100 * 1024,
                                           ...
                                           100 * 1024 * 1024,
                                           1024 * 1024 * 1024};
    const int64_t size_bin_max_vals[10] = {100,
                                           1024,
                                           10 * 1024,
                                           100 * 1024,
                                           1024 * 1024,
...
static void determine_psx_io_params(struct darshan_posix_file* file, int write_flag, size_t* io_sz,
    ...
    int64_t* common_access_counts =
        &(file->counters[POSIX_ACCESS1_COUNT]); /* common access counts */
    int64_t* total_io_size = NULL;
    int i, j = 0;
    const int64_t size_bin_min_vals[10] = {0,
                                           100,
...
static void mpiio_calc_io_delays(struct darshan_mpiio_file* mfile, int64_t num_io_ops,
    ...
    first_io_time = MIN(mfile->fcounters[MPIIO_F_READ_START_TIMESTAMP],
    ...
    last_io_time = MAX(mfile->fcounters[MPIIO_F_READ_END_TIMESTAMP],
...
static void psx_calc_io_delays(struct darshan_posix_file* file, int64_t num_io_ops,
    ...
    if (num_io_ops == 0) {
        ...
        *close_delay = 0;
        *inter_io_delay = 0;
        // This multi-line string literal oscillates between two valid
        // layouts under clang-format-20 (a known bug with adjacent
        // string-literal continuation). Pin it so CI's single-pass
        // check stays stable.
        // clang-format off
        fprintf(stderr,
                "WARNING: id %" PRIu64 " has unaccounted total_delay, possibly stat() calls that "
                "are not supported by generator.\n",
                file->base_rec.id);
        // clang-format on
        return;
    }
    ...
    /* determine the start time of the first io operation */
    first_io_time = MIN(file->fcounters[POSIX_F_READ_START_TIMESTAMP],
    ...
    /* determine the end time of the last io operation */
    last_io_time = MAX(file->fcounters[POSIX_F_READ_END_TIMESTAMP],
...
static void file_sanity_check(struct darshan_posix_file* file, struct darshan_mpiio_file* mfile,
    ...
    if (mfile->counters[MPIIO_COLL_OPENS] > 0 && mfile->base_rec.rank >= 0) {
        fprintf(stderr,
                "WARNING: id %" PRIu64
                " has non-global collective MPIIO operations, emulating as independent.\n",
                mfile->base_rec.id);
