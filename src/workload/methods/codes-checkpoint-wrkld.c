...
static void* checkpoint_workload_read_config(ConfigHandle* handle, char const* section_name,
...
static void* checkpoint_workload_read_config(ConfigHandle* handle, char const* section_name,
    ...
    rc = configuration_get_value_double(handle, section_name, "checkpoint_sz", annotation,
                                        &p->checkpoint_sz);
    assert(!rc);

    rc = configuration_get_value_double(handle, section_name, "checkpoint_wr_bw", annotation,
                                        &p->checkpoint_wr_bw);
    assert(!rc);

    rc = configuration_get_value_int(handle, section_name, "total_checkpoints", annotation,
                                     &p->total_checkpoints);
    assert(!rc);

    rc = configuration_get_value_double(handle, section_name, "mtti", annotation, &p->mtti);
    assert(!rc);
