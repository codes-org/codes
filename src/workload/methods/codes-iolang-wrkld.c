...
static void* iolang_io_workload_read_config(ConfigHandle* handle, char const* section_name,
...
static void* iolang_io_workload_read_config(ConfigHandle* handle, char const* section_name,
    ...
    int rc =
        configuration_get_value_relpath(handle, section_name, "io_kernel_meta_path", annotation,
                                        p->io_kernel_meta_path, MAX_NAME_LENGTH_WKLD);
    assert(rc > 0);
    rc = configuration_get_value_int(handle, section_name, "num_ranks", annotation, &p->num_cns);
    if (rc != 0)
        p->num_cns = num_ranks;
    return p;
}
...
int iolang_io_workload_load(const void* params, int app_id, int rank) {
    ...
    t = codes_kernel_helper_bootstrap(i_param->io_kernel_path, i_param->io_kernel_meta_path, rank,
                                      nranks, i_param->use_relpath,
                                      &(wrkld_per_rank->codes_context),
                                      &(wrkld_per_rank->codes_pstate), &(wrkld_per_rank->task_info),
...
void iolang_io_workload_get_next(int app_id, int rank, struct codes_workload_op* op) {
    ...
    int type =
        codes_kernel_helper_parse_input(next_wrkld->codes_pstate, &(next_wrkld->codes_context),
                                        &(next_wrkld->next_event));
