...
void UNION_MPI_Finalize() {
    ...
    if (DBG_COMM) {
        ...
        printf("\nnum_sends %ld num_recvs %ld num_isends %ld num_irecvs %ld num_allreduce %ld "
               "num_barrier %ld num_waitalls %ld\n",
               num_sends, num_recvs, num_isends, num_irecvs, num_allreduce, num_barriers,
               num_waitalls);
...
void UNION_MPI_Send(const void* buf, int count, UNION_Datatype datatype, int dest, int tag,
                    UNION_Comm comm) {
    /* add an event in the shared queue and then yield */
    struct codes_workload_op wrkld_per_rank;
...
void UNION_MPI_Recv(void* buf, int count, UNION_Datatype datatype, int source, int tag,
                    UNION_Comm comm, UNION_Status* status) {
    /* Add an event in the shared queue and then yield */
    struct codes_workload_op wrkld_per_rank;
...
void UNION_MPI_Sendrecv(const void* sendbuf, int sendcount, UNION_Datatype sendtype, int dest,
                        int sendtag, void* recvbuf, int recvcount, UNION_Datatype recvtype,
                        int source, int recvtag, UNION_Comm comm, UNION_Status* status) {
    /* sendrecv events */
    ...
    if (DBG_COMM) {
        printf("\nUNION SENDRECV ssrc %d sdst %d: %lld bytes; rsrc %d rdst %d: %lld bytes ",
               sctx->my_rank, dest, send_op.u.send.num_bytes, source, sctx->my_rank,
               recv_op.u.recv.num_bytes);
    }
    ABT_thread_yield_to(global_prod_thread);
    num_sendrecv++;
}
...
void UNION_MPI_Barrier(UNION_Comm comm) {
    ...
    while (mask < size) {
        ...
        UNION_MPI_Sendrecv(NULL, 0, UNION_Int, dest, 1234, NULL, 0, UNION_Int, src, 1234, comm,
                           NULL);
...
void UNION_MPI_Isend(const void* buf, int count, UNION_Datatype datatype, int dest, int tag,
                     UNION_Comm comm, UNION_Request* request) {
    /* add an event in the shared queue and then yield */
...
void UNION_MPI_Irecv(void* buf, int count, UNION_Datatype datatype, int source, int tag,
                     UNION_Comm comm, UNION_Request* request) {
    /* Add an event in the shared queue and then yield */
    struct codes_workload_op wrkld_per_rank;
...
void UNION_MPI_Wait(UNION_Request* request, UNION_Status* status) {
    /* Add an event in the shared queue and then yield */
    struct codes_workload_op wrkld_per_rank;
...
void UNION_MPI_Waitall(int count, UNION_Request array_of_requests[],
...
void UNION_MPI_Reduce(const void* sendbuf, void* recvbuf, int count, UNION_Datatype datatype,
                      UNION_Op op, int root, UNION_Comm comm) {
    //todo
}

void UNION_MPI_Allreduce(const void* sendbuf, void* recvbuf, int count, UNION_Datatype datatype,
                         UNION_Op op, UNION_Comm comm) {
    ...
    if (newrank != -1) {
        if ((count * type_size <= 81920) || (count < pof2)) {
            ...
            while (mask < pof2) {
                ...
                UNION_MPI_Sendrecv(NULL, count, datatype, dst, -1002, NULL, count, datatype, dst,
                                   -1002, comm, NULL);
                mask <<= 1;
            }
        } else {
            ...
            while (mask < pof2) {
                ...
                UNION_MPI_Sendrecv(NULL, send_cnt, datatype, dst, -1002, NULL, recv_cnt, datatype,
                                   dst, -1002, comm, NULL);
...
void bcast_binomial(void* buffer, int rank, int count, UNION_Datatype datatype, int root,
                    UNION_Comm comm) {
...
void bcast_scatter_doubling_allgather(void* buffer, int rank, int count, UNION_Datatype datatype,
                                      int root, UNION_Comm comm) {
    ...
    while (mask < comm_size) {
        ...
        if (relative_dst < comm_size) {
            ...
            UNION_MPI_Sendrecv(buffer, curr_size, UNION_Byte, dst, -1005, buffer, recvcount,
                               UNION_Byte, dst, -1005, comm, &status);
            curr_size += recv_size;
        }
...
void bcast_scatter_ring_allgather(void* buffer, int rank, int count, UNION_Datatype datatype,
                                  int root, UNION_Comm comm) {
    ...
    curr_size =
        THISMIN(scatter_size, nbytes - ((rank - root + comm_size) % comm_size) * scatter_size);
    if (curr_size < 0)
        curr_size = 0;
    ...
    for (i = 1; i < comm_size; i++) {
        ...
        UNION_MPI_Sendrecv(buffer, right_count, UNION_Byte, right, -1005, buffer, left_count,
                           UNION_Byte, left, -1005, comm, &status);
...
void UNION_MPI_Bcast(void* buffer, int count, UNION_Datatype datatype, int root, UNION_Comm comm) {
...
void UNION_MPI_Alltoallv(const void* sendbuf, const int* sendcounts, const int* sdispls,
                         UNION_Datatype sendtype, void* recvbuf, const int* recvcounts,
                         const int* rdispls, UNION_Datatype recvtype, UNION_Comm comm) {
    int comm_size, i, j;
...
void UNION_MPI_Alltoall(const void* sendbuf, int sendcount, UNION_Datatype sendtype, void* recvbuf,
                        int recvcount, UNION_Datatype recvtype, UNION_Comm comm) {
    ...
    UNION_MPI_Alltoallv(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls,
                        recvtype, comm);
...
void SWM_Send(SWM_PEER peer, SWM_COMM_ID comm_id, SWM_TAG tag, SWM_VC reqvc, SWM_VC rspvc,
              SWM_BUF buf, SWM_BYTES bytes, SWM_BYTES pktrspbytes, SWM_ROUTING_TYPE reqrt,
 ...
 * */
void SWM_Barrier(SWM_COMM_ID comm_id, SWM_VC reqvc, SWM_VC rspvc, SWM_BUF buf, SWM_UNKNOWN auto1,
                 SWM_UNKNOWN2 auto2, SWM_ROUTING_TYPE reqrt, SWM_ROUTING_TYPE rsprt) {
    /* Add an event in the shared queue and then yield */
#if 0
    struct codes_workload_op wrkld_per_rank;
...
//     printf("\n barrier ");
    ...
    while (mask < size) {
        ...
        SWM_Sendrecv(comm_id, dest, 1234, reqvc, rspvc, 0, 0, 0, src, 1234, 0, reqrt, rsprt);
        mask <<= 1;
    }
    num_barriers++;
}

void SWM_Isend(SWM_PEER peer, SWM_COMM_ID comm_id, SWM_TAG tag, SWM_VC reqvc, SWM_VC rspvc,
               SWM_BUF buf, SWM_BYTES bytes, SWM_BYTES pktrspbytes, uint32_t* handle,
               SWM_ROUTING_TYPE reqrt, SWM_ROUTING_TYPE rsprt) {
    /* add an event in the shared queue and then yield */
    //    printf("\n Sending to rank %d ", comm_id);
    struct codes_workload_op wrkld_per_rank;
...
void SWM_Recv(SWM_PEER peer, SWM_COMM_ID comm_id, SWM_TAG tag, SWM_BUF buf) {
    /* Add an event in the shared queue and then yield */
    struct codes_workload_op wrkld_per_rank;
...
void SWM_Irecv(SWM_PEER peer, SWM_COMM_ID comm_id, SWM_TAG tag, SWM_BUF buf, uint32_t* handle) {
    /* Add an event in the shared queue and then yield */
    struct codes_workload_op wrkld_per_rank;
...
void SWM_Sendrecv(SWM_COMM_ID comm_id, SWM_PEER sendpeer, SWM_TAG sendtag, SWM_VC sendreqvc,
                  SWM_VC sendrspvc, SWM_BUF sendbuf, SWM_BYTES sendbytes, SWM_BYTES pktrspbytes,
                  SWM_PEER recvpeer, SWM_TAG recvtag, SWM_BUF recvbuf, SWM_ROUTING_TYPE reqrt,
                  SWM_ROUTING_TYPE rsprt) {
    ...
    if (DBG_COMM) {
        printf("\nSWM SENDRECV ssrc %d sdst %d: %d bytes; rsrc %d rdst %d: %lld bytes ",
               sctx->my_rank, sendpeer, sendbytes, recvpeer, sctx->my_rank,
               recv_op.u.recv.num_bytes);
    }
 ...
 * our purpose) */
void SWM_Allreduce(SWM_BYTES count, SWM_BYTES respbytes, SWM_COMM_ID comm_id, SWM_VC sendreqvc,
                   SWM_VC sendrspvc, SWM_BUF sendbuf, SWM_BUF rcvbuf) {
#if 0
    ...
    if (newrank != -1) {
        if ((count <= ALLREDUCE_SHORT_MSG_SIZE) || (count < pof2)) {
            ...
            while (mask < pof2) {
                ...
                SWM_Sendrecv(comm_id, dst, 1235, sendreqvc, sendrspvc, 0, count, 1, dst, 1235, 0, 0,
                             0);
            ...
            while (mask < pof2) {
                ...
                SWM_Sendrecv(comm_id, dst, 1235, sendreqvc, sendrspvc, 0, send_cnt, 1, dst, 1235, 0,
                             0, 0);
...
void SWM_Allreduce(SWM_BYTES bytes, SWM_BYTES respbytes, SWM_COMM_ID comm_id, SWM_VC sendreqvc,
                   SWM_VC sendrspvc, SWM_BUF sendbuf, SWM_BUF rcvbuf, SWM_UNKNOWN auto1,
                   SWM_UNKNOWN2 auto2, SWM_ROUTING_TYPE reqrt, SWM_ROUTING_TYPE rsprt) {
...
static void workload_caller(void* arg) {
    ...
    if (strncmp(sctx->workload_name, "conceptual", 10) == 0) {
        ...
        if (DBG_LINKING) {
            printf("\nLoad Union Benchmark: %s: %s", conc_params->conc_program,
                   conc_params->conc_argv[1]);
        }
        union_conc_bench_load(conc_params->conc_program, conc_params->conc_argc,
        ...
        NearestNeighborSWMUserCode* nn_swm =
            static_cast<NearestNeighborSWMUserCode*>(sctx->swm_obj);
        nn_swm->call();
    } else if (strcmp(sctx->workload_name, "incast") == 0 ||
               strcmp(sctx->workload_name, "incast1") == 0 ||
               strcmp(sctx->workload_name, "incast2") == 0) {
        AllToOneSWMUserCode* incast_swm = static_cast<AllToOneSWMUserCode*>(sctx->swm_obj);
        incast_swm->call();
    }
}

static void determine_workload_paths(online_comm_params const* o_params, string& swm_path,
                                     string& conc_path, bool& isconc) {
...
static int comm_online_workload_load(const void* params, int app_id, int rank) {
    ...
    if (isconc) {
        try {
            ...
            union_bench_param* tmp_params =
                (union_bench_param*)calloc(1, sizeof(union_bench_param));
            ...
            if (has_path) {
                strcpy(tmp_params->conc_program,
                       child.get_child("argv").begin()->second.data().c_str());
            } else {
        ...
        if (strcmp(o_params->workload_name, "lammps") == 0) {
            ...
            my_ctx->sctx.swm_obj = (void*)nn_swm;
                   ...
                   strcmp(o_params->workload_name, "incast1") == 0 ||
                   strcmp(o_params->workload_name, "incast2") == 0) {
    ...
    int rcode = ABT_thread_create_on_xstream(self_es, &workload_caller, (void*)&(my_ctx->sctx),
