...
void SWM_Sendrecv(SWM_COMM_ID comm_id, SWM_PEER sendpeer, SWM_TAG sendtag, SWM_VC sendreqvc,
    ...
    SWM_Send(sendpeer, comm_id, sendtag, sendreqvc, sendrspvc, sendbuf, sendbytes, pktrspbytes,
             reqrt, rsprt);
    SWM_Wait(handle);
#else
...
static void workload_caller(void* arg) {
    ...
    if (strcmp(sctx->workload_name, "lammps") == 0 || strcmp(sctx->workload_name, "lammps1") == 0) {
        ...
        lammps_swm->call();
               ...
               strcmp(sctx->workload_name, "nekbone1") == 0) {
        ...
        spread_swm->call();
    } else if (strcmp(sctx->workload_name, "allreduce") == 0 ||
               strcmp(sctx->workload_name, "allreduce32") == 0 ||
               strcmp(sctx->workload_name, "allreduce256") == 0) {
        AllReduceSWMUserCode* allreduce_swm = static_cast<AllReduceSWMUserCode*>(sctx->swm_obj);
        allreduce_swm->call();
    } else if (strcmp(sctx->workload_name, "many_to_many") == 0 ||
               strcmp(sctx->workload_name, "many_to_many1") == 0) {
        ManyToManySWMUserCode* many_to_many_swm =
            static_cast<ManyToManySWMUserCode*>(sctx->swm_obj);
        ...
        LayeredAllBroadcast* layered_allbcast_swm =
            static_cast<LayeredAllBroadcast*>(sctx->swm_obj);
...
static int comm_online_workload_load(const void* params, int app_id, int rank) {
    ...
    if (o_params->workload_name[0] !=
        '\0') { //then we were supplied with just the workload name, use default configs
    ...
    try {
        ...
        if (o_params->workload_name[0] ==
            '\0') { //if we instead had a configuration filename supplied, get worklaod name from jobs.cfg.app
    ...
    if (strcmp(o_params->workload_name, "lammps") == 0 ||
        ...
        my_ctx->sctx.swm_obj = (void*)spread_swm;
               ...
               strcmp(o_params->workload_name, "allreduce32") == 0 ||
               strcmp(o_params->workload_name, "allreduce256") == 0) {
        AllReduceSWMUserCode* allreduce_swm = new AllReduceSWMUserCode(root, generic_ptrs);
        my_ctx->sctx.swm_obj = (void*)allreduce_swm;
    } else if (strcmp(o_params->workload_name, "many_to_many") == 0 ||
               strcmp(o_params->workload_name, "many_to_many1") == 0) {
    ...
    ABT_thread_create_on_xstream(self_es, &workload_caller, (void*)&(my_ctx->sctx),
