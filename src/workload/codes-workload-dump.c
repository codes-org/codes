...
static struct option long_opts[] = {{"type", required_argument, NULL, 't'},
...
void usage() {
    fputs("Usage: codes-workload-dump --type TYPE --num-ranks N [OPTION...]\n"
          "--type: type of workload (\"darshan_io_workload\", \"iolang_workload\", "
          "dumpi-trace-workload\" etc.)\n"
...
int main(int argc, char* argv[]) {
    ...
    char ch;
    while ((ch = getopt_long(argc, argv, "t:n:l:b:a:m:sp:wr:S:B:R:M:Q:N:z:f:u", long_opts, NULL)) !=
           -1) {
        switch (ch) {
        case 't':
            strcpy(type, optarg);
    ...
    if (strcmp(type, "iomock_workload") == 0) {
    ...
    } else if (strcmp(type, "swm_online_comm_workload") == 0 ||
               strcmp(type, "conc_online_comm_workload") == 0) {
        if (n == -1) {
            fprintf(stderr, "Expected \"--num-ranks\" argument for online workload\n");
            usage();
            return 1;
        } else {
            oc_params.nprocs = n;
        ...
        if (n == -1) {
            fprintf(stderr, "Expected \"--num-ranks\" argument for iolang workload\n");
            usage();
            return 1;
        } else {
            i_params.num_cns = n;
        ...
        if (n == -1) {
            fprintf(stderr, "Expected \"--num-ranks\" argument for dumpi workload\n");
            usage();
            return 1;
        } else {
    ...
    if (n == -1) {
        ...
        if (n == -1) {
            fprintf(stderr, "Unable to get rank count from workload. "
    ...
    for (i = start_rank; i < start_rank + n; i++) {
        ...
        codes_workload_get_time(type, wparams, 0, i, &total_read_time, &total_write_time,
        ...
        do {
            ...
            case CODES_WK_IGNORE:
                break;
            default:
                fprintf(stderr, "WARNING: unknown workload op type (code %d)\n", op.op_type);
            }
        } while (op.op_type != CODES_WK_END);

        if (strcmp(type, "swm_online_comm_workload") == 0 ||
            strcmp(type, "conc_online_comm_workload") == 0) {
