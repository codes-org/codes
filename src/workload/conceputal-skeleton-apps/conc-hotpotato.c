 ...
 * Do not modify this file; modify /Users/xin/macworkspace/codes-dev/codes/scripts/conceptual_benchmarks/hotpotato.ncptl instead.
 *
...
ncptl_int var_task_count;   /* Copy of var_task_count to use within a piece of code */
ncptl_int var_trials;   /* Copy of var_trials to use within a piece of code */
} CONC_CODE_EVENT;
...
static ncptl_int var_trials;   /* Number of trials to perform (command-line argument) */
...
{ NCPTL_TYPE_INT, NULL, "trials", 't', "Number of trials to perform", {0}}
};
...
arguments[1].variable = (CMDLINE_VALUE *) &var_trials;
arguments[1].defaultvalue.intval = 100000LL;
...
ncptl_free (procflags);
 ...
 /* ASSERT THAT the hot-potato test requires at least two tasks. */
if (!((var_num_tasks)>=(2LL)))
ncptl_fatal ("Assertion failure: the hot-potato test requires at least two tasks");
{
 /* LET var_len BE 0LL WHILE... */
ncptl_int var_len = 0LL;

 /* FOR EACH var_task_count IN [(['2LL'], 'var_num_tasks')]... */
{
...
ncptl_int var_task_count;
if (thisrange->list_comp == NULL)
 /* The loopvar field contains the value for var_task_count. */
var_task_count = thisrange->integral ? thisrange->u.i.loopvar : CONC_DBL2INT(thisrange->u.d.loopvar);
else
 /* The queue of list-comprehension values provides the value for var_task_count. */
var_task_count = *(ncptl_int *)ncptl_queue_pop(thisrange->list_comp);
{
 /* TASK 0LL OUTPUTS ['"Performing "', 'var_trials', '" "', 'var_task_count', '"-task runs...."'] */
...
thisev->s.code.var_task_count = var_task_count;
thisev->s.code.var_trials = var_trials;
}
 /* THEN... */
 /* FOR var_trials REPETITIONS... */
{
ncptl_int loopnum;   /* 0=warmup repetitions; 1=main repetitions */

for (loopnum=0LL; loopnum<2LL; loopnum++) {
ncptl_int numreps = loopnum==0LL ? (5LL) : (var_trials);   /* Total # of repetitions to perform */
...
thisev->s.send.tag = map_tag_into_MPI_range (thisev->s.send.tag);
 ...
 /* THEN... */
 /* TASKS var_n SUCH THAT (var_n)<(var_task_count) RECEIVES FROM TASK ncptl_func_modulo((var_n)+(1LL),var_task_count) */
{
ncptl_int var_n;
 /* Loop over all tasks to see which will send to us. */
for (var_n=0; var_n<var_num_tasks; var_n++)
if ((var_n)<(var_task_count)) {
 /* var_n now represents one of the tasks that will send to us. */
ncptl_int ivar_a_task = var_n;
if (virtrank == (ncptl_func_modulo((var_n)+(1LL),var_task_count))) {
...
thisev->s.recv.tag = map_tag_into_MPI_range (thisev->s.recv.tag);
 ...
 /* THEN... */
 /* TASKS var_n SUCH THAT ((var_n)>(0LL)) && ((var_n)<(var_task_count)) SENDS TO TASK ncptl_func_modulo((var_n)+(1LL),var_task_count) */
{
ncptl_int var_n = virtrank;
if (((var_n)>(0LL)) && ((var_n)<(var_task_count))) {   /* TASKS var_n SUCH THAT ((var_n)>(0LL)) && ((var_n)<(var_task_count)) */
 /* The current coNCePTuaL statement applies to our task. */
ncptl_int virtdest = ncptl_func_modulo((var_n)+(1LL),var_task_count);
if (virtdest>=0 && virtdest<var_num_tasks) {
 /* In this scope, ncptl_func_modulo((var_n)+(1LL),var_task_count) represents a single receiver. */
 /* Prepare to send 1LL messages. */
ncptl_int numreps = 1LL;   /* Number of messages */
if (numreps > 1LL) {
CONC_EVENT * repeatev;   /* Event specifying the number of repetitions to perform */
...
CONC_EVENT *thisev = conc_allocate_event (EV_SEND);
 ...
 /* Fill in all of the fields of a send-event structure. */
thisev->s.send.dest = ncptl_virtual_to_physical (procmap, ncptl_func_modulo((var_n)+(1LL),var_task_count));
thisev->s.send.size = var_len;
thisev->s.send.alignment = 0LL;
thisev->s.send.misaligned = 0;
thisev->s.send.touching = 0;
...
thisev->s.send.tag = map_tag_into_MPI_range (thisev->s.send.tag);
 ...
 /* THEN... */
 /* TASK 0LL LOGS "# of tasks" AND "Latency (usecs)" AND "Latency (usecs)" AND "Latency (usecs)" */
if ((0LL) == virtrank) {   /* TASK 0LL */
 /* The current coNCePTuaL statement applies to our task. */
CONC_EVENT *thisev = conc_allocate_event (EV_CODE);
...
if (loopnum == 0LL) {
CONC_EVENT * thisev_sync;   /* Barrier-synchronization event */

thisev_sync =  conc_allocate_event (EV_SYNC);
thisev_sync->s.sync.communicator = MPI_COMM_WORLD;
}
...
(void) CODES_MPI_Barrier (thisev->s.sync.communicator);
break;
...
uint64_t stop_elapsed_usecs = ncptl_time();
var_elapsed_usecs = stop_elapsed_usecs - starttime;
printf ("%s%.10lg%s%.10lg%s\n", "Performing ", (double)thisev->s.code.var_trials, " ", (double)thisev->s.code.var_task_count, "-task runs....");
fflush (stdout);
starttime += ncptl_time() - stop_elapsed_usecs;
}
break;
...
uint64_t stop_elapsed_usecs = ncptl_time();
var_elapsed_usecs = stop_elapsed_usecs - starttime;
//ncptl_log_write (logstate, 0, "# of tasks", NCPTL_FUNC_ONLY, 0.0, (double)thisev->s.code.var_task_count);
//ncptl_log_write (logstate, 1, "Latency (usecs)", NCPTL_FUNC_MINIMUM, 0.0, ((double)var_elapsed_usecs)/((double)thisev->s.code.var_task_count));
//ncptl_log_write (logstate, 2, "Latency (usecs)", NCPTL_FUNC_MEAN, 0.0, ((double)var_elapsed_usecs)/((double)thisev->s.code.var_task_count));
//ncptl_log_write (logstate, 3, "Latency (usecs)", NCPTL_FUNC_VARIANCE, 0.0, ((double)var_elapsed_usecs)/((double)thisev->s.code.var_task_count));
starttime += ncptl_time() - stop_elapsed_usecs;
}
break;
...
static int hotpotato_main (int argc, char *argv[])
{
 /* Declare variables needed by all C-based backends. */
CONC_EVENT * eventlist;   /* List of events to execute */
ncptl_int numevents;   /* Number of entries in eventlist[] */
...
struct codes_conceptual_bench hotpotato_bench = 
{
.program_name = "hotpotato",
.conceptual_main = hotpotato_main,
};
