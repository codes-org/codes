                ...
                if (strcmp(lpt->name.ptr, lp_type_name) == 0) {
                    ...
                    if (ignore_anno || cmp_anno(annotation, lpt->anno.ptr)) {
                        ...
                        if (offset >= (int)lpt->count) {
                            ...
                            *gid +=
                                (rep_count * (tw_lpid)rep_id) + local_lp_count + (tw_lpid)offset;
                            return;
                        }
                    }
                }
...
int codes_mapping_get_lp_relative_id(tw_lpid gid, int group_wise, int annotation_wise) {
    ...
    codes_mapping_get_lp_info(gid, NULL, &group_index, local_lp_name, &lp_type_index,
                              local_annotation, &rep_id, &offset);
...
static void codes_mapping_init(void) {
    ...
    for (lpid = lp_start; lpid < lp_end; lpid++) {
        ...
        codes_mapping_get_lp_info(ross_gid, NULL, &grp_id, lp_type_name, &lpt_id, NULL, &rep_id,
                                  &offset);
...
tw_lpid codes_mapping_count_lps_of_type(char const lp_type_name[MAX_NAME_LENGTH]) {
    ...
    for (tw_lpid lpid = 0; lpid < g_tw_nlp; lpid++) {
        ...
        codes_mapping_get_lp_info(
            ross_gid, NULL, &grp_id, this_lp_type, &lpt_id, NULL, &rep_id,
            &offset); // This lookup could be speed up, but making this call is far simpler rn
...
void codes_mapping_setup_with_seed_offset(int offset) {
    ...
    int mem_factor_conf;
    int rc =
        configuration_get_value_int(&config, "PARAMS", "pe_mem_factor", NULL, &mem_factor_conf);
        ...
        for (tw_lpid l = 0; l < g_tw_nlp; l++) {
            for (unsigned int i = 0; i < g_tw_nRNG_per_lp; i++) {
                tw_rand_initial_seed(&g_tw_lp[l]->rng[i],
                                     (g_tw_lp[l]->gid + global_nlps * offset) * g_tw_nRNG_per_lp +
                                         i,
                                     NULL);
            }
        }
