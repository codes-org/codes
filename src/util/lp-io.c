...
int lp_io_write_rev(tw_lpid gid, char* identifier) {
    ...
    if (!id) {
        fprintf(stderr, "Error: identifier %s not found on reverse for LP %llu.", identifier,
                LLU(gid));
        return (-1);
    }
...
int lp_io_flush(lp_io_handle handle, MPI_Comm comm) {
    ...
    if (rank != comm_size - 1) {
        ...
        assert(ret == 0);
        ret = MPI_Recv(global_identifiers, 64 * sizeof(global_identifiers[0]), MPI_BYTE, rank + 1,
                       0, comm, &status);
        assert(ret == 0);
    }
    ...
    if (rank != 0) {
        ...
        assert(ret == 0);
        ret = MPI_Send(global_identifiers, 64 * sizeof(global_identifiers[0]), MPI_BYTE, rank - 1,
                       0, comm);
        assert(ret == 0);
    }
...
static int write_id(char* directory, char* identifier, MPI_Comm comm) {
    ...
    ret = MPI_File_open(comm, file, MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                        MPI_INFO_NULL, &fh);
    if (ret != 0) {
