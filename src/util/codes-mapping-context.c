...
static struct codes_mctx set_group_modulo_common(enum codes_mctx_type type, char const* annotation,
...
static struct codes_mctx set_group_ratio_common(enum codes_mctx_type type, char const* annotation,
                                                bool ignore_annotations) {
    struct codes_mctx rtn;
    rtn.type = type;
    if (ignore_annotations)
...
tw_lpid codes_mctx_to_lpid(struct codes_mctx const* ctx, char const* dest_lp_name,
    ...
    tw_lpid rtn;
    codes_mapping_get_lp_id(sender_group, dest_lp_name, anno_str, anno->cid == -1, rep_id,
                            dest_offset, &rtn);
    return rtn;
}
