...
unsigned long long Portchan_node::get_packet_count_by_port_from_term(int port_no,
...
unsigned long long Portchan_node::get_packet_count_by_port_vc_from_term(int port_no, int vc_no,
                                                                        unsigned int term_id) {
    if (type != ROOT)
        tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
map<unsigned int, unsigned long long> Portchan_node::get_term_count_map_by_port_vc(int port_no,
                                                                                   int vc_no) {
    if (type != ROOT)
        tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
map<unsigned int, unsigned long long> Portchan_node::get_app_count_map_by_port_vc(int port_no,
                                                                                  int vc_no) {
    if (type != ROOT)
        tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
Portchan_node::get_term_count_map_by_port_from_app(int port_no, int app_id) {
    if (type != ROOT)
        tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
map<unsigned int, unsigned long long>
Portchan_node::get_term_count_map_by_port_vc_from_app(int port_no, int vc_no, int app_id) {
    if (type != ROOT)
        tw_error(TW_LOC, "Portchan_node : Invalid node type for this action\n");
...
void Portchan_node::enqueue_packet(unsigned int packet_size, int port_no, int vc_no,
                                   unsigned int term_id, unsigned int app_id) {
...
void Portchan_node::dequeue_packet(unsigned int packet_size, int port_no, int vc_no,
                                   unsigned int term_id, unsigned int app_id) {
...
void cc_router_local_controller_init(rlc_state* s, tw_lp* lp, int total_terminals, int router_id,
    ...
    int rc =
        configuration_get_value_double(&config, "PARAMS", "cc_single_port_congestion_threshold",
    ...
    rc = configuration_get_value_double(&config, "PARAMS",
                                        "cc_single_port_aggressor_usage_threshold", NULL,
                                        &p->single_port_aggressor_usage_threshold);
    if (rc) {
        p->single_port_aggressor_usage_threshold = .10;
    }

    rc = configuration_get_value_double(&config, "PARAMS", "cc_minimum_abatement_active_time", NULL,
                                        &p->minimum_abatement_time);
    if (rc) {
...
void cc_router_received_packet(rlc_state* s, tw_lp* lp, unsigned int packet_size, int port_no,
    ...
    if (s->output_ports.size() > 0 && s->output_ports.count(port_no) > 0) {
        ...
        double cur_expire_time =
            s->packet_counting_tree->get_next_possible_port_normal_time(port_no);
        ...
        if (s->packet_counting_tree->is_port_congested(port_no) == true) {
            ...
            if (s->packet_counting_tree->is_abated_terminal(term_id) == false) {
                ...
                if ((this_app_occupancy / port_occupancy) >=
                    ...
                    tw_lpid term_lpgid =
                        codes_mapping_get_lpid_from_relative(term_id, NULL, terminal_lp_name, NULL,
                                                             0);
                    congestion_control_message* c_msg;
                    tw_event* e = model_net_method_congestion_event(term_lpgid,
                                                                    s->params->notification_latency,
                                                                    s->lp, (void**)&c_msg, NULL);
            ...
            s->packet_counting_tree->set_next_possible_port_normal_time(
                port_no, cur_expire_time +
                             bytes_to_ns(packet_size, s->router_bandwidths_on_each_port[port_no]));
...
void cc_router_congestion_check(rlc_state* s, tw_lp* lp, int port_no, int vc_no,
    ...
    int port_decongestion_threshold_size =
        s->params->single_port_decongestion_threshold * port_size;
    ...
    if (s->packet_counting_tree->is_port_congested(port_no) == false) {
        if (port_occupancy >= port_congestion_threshold_size) {
            ...
            for (; it != app_map.end(); it++) {
                if ((it->second / port_occupancy) >=
                    s->params->single_port_aggressor_usage_threshold) {
                    map<unsigned int, unsigned long long> tcm =
                        s->packet_counting_tree->get_term_count_map_by_port_from_app(port_no,
                                                                                     it->first);
            ...
            rc_msg->danger_rc_abated =
                (unsigned int*)malloc(sizeof(unsigned int) * rc_msg->size_abated);
        ...
        if (tw_now(lp) > s->packet_counting_tree->get_next_possible_port_normal_time(port_no)) {
            if (port_occupancy < port_decongestion_threshold_size) {
                ...
                set<unsigned int> abated_terms =
                    s->packet_counting_tree->get_abated_terminals(port_no);
                ...
                for (; it != abated_terms.end(); it++) {
                    ...
                    if (s->packet_counting_tree->is_abated_terminal(*it) == 0) {
                        ...
                        tw_lpid term_lpgid =
                            codes_mapping_get_lpid_from_relative(*it, NULL, terminal_lp_name, NULL,
                                                                 0);
                        congestion_control_message* c_msg;
                        tw_event* e =
                            model_net_method_congestion_event(term_lpgid,
                                                              s->params->notification_latency,
                                                              s->lp, (void**)&c_msg, NULL);
...
static double calculate_bandwidth_usage_percent(int bytes_transmitted,
                                                double max_configured_bandwidth, int multiplier) {
...
void save_tlc_state(tlc_state* into, tlc_state const* from) {
    ...
    into->ejected_rate_windows =
        (double*)malloc(cc_bandwidth_rolling_window_count * sizeof(double));
...
void print_tlc_state(FILE* out, char const* prefix, tlc_state* state) {
    ...
    for (int i = 0; i < cc_bandwidth_rolling_window_count; i++) {
        fprintf(out, "%g%s", state->ejected_rate_windows[i],
                i == cc_bandwidth_rolling_window_count - 1 ? "" : ", ");
    }
    fprintf(out, "]\n");
    ...
    fprintf(out, "%s  | workloads_finished_flag_ptr = %d\n", prefix,
            *state->workloads_finished_flag_ptr);
    fprintf(out, "%s  | current_injection_bandwidth_coef = %g\n", prefix,
            state->current_injection_bandwidth_coef);
}

void cc_terminal_process_bandwidth_check(tlc_state* s, congestion_control_message* msg, tw_lp* lp) {
    double usage_percent = calculate_bandwidth_usage_percent(
        s->ejected_packet_bytes, s->params->terminal_configured_bandwidth,
    ...
    if (*s->workloads_finished_flag_ptr == 0) {
        ...
        tw_event* hb_e =
            model_net_method_congestion_event(s->lp->gid, cc_bandwidth_monitoring_window, s->lp,
                                              (void**)&hb_msg, NULL);
...
void cc_terminal_local_controller_init(tlc_state* s, tw_lp* lp, int terminal_id,
    ...
    int rc = configuration_get_value_double(&config, "PARAMS", "cn_bandwidth", NULL,
                                            &p->terminal_configured_bandwidth);
    if (rc) {
        if (!g_tw_mynode)
    ...
    rc = configuration_get_value_double(&config, "PARAMS", "cc_measurement_period", NULL,
                                        &cc_bandwidth_monitoring_window);
    if (rc) {
    ...
    congestion_control_message* hb_msg;
    tw_event* hb_e = model_net_method_congestion_event(lp->gid, cc_bandwidth_monitoring_window,
                                                       s->lp, (void**)&hb_msg, NULL);
    hb_msg->type = CC_BANDWIDTH_CHECK;
    tw_event_send(hb_e);
}
...
void cc_terminal_send_ack(tlc_state* s, tw_lpid original_terminal_lpgid) {
    ...
    tw_event* ack_e = model_net_method_congestion_event(original_terminal_lpgid,
                                                        s->params->notification_latency + .00001,
                                                        s->lp, (void**)&ack_msg, NULL);
