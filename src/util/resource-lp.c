...
static void resource_lp_ind_init(resource_state* ns, tw_lp* lp);
static void resource_event_handler(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp);
static void resource_rev_handler(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp);
static void resource_finalize(resource_state* ns, tw_lp* lp);

/* ROSS function pointer table for this LP */
static tw_lptype resource_lp = {
    (init_f)resource_lp_ind_init,   (pre_run_f)NULL,        (event_f)resource_event_handler,
    (revent_f)resource_rev_handler, (commit_f)NULL,         (final_f)resource_finalize,
    (map_f)codes_mapping,           sizeof(resource_state),
};
...
void resource_lp_ind_init(resource_state* ns, tw_lp* lp) {
    // get my annotation
...
static void resource_response(struct codes_cb_params const* p, tw_lp* lp, int ret,
...
static void handle_resource_get(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    int ret = 1;
    int send_ack = 1;
    // save the previous minimum for RC
    assert(!resource_get_min_avail(m->i.tok, &m->min_avail_rc, &ns->r));
    if (!qlist_empty(&ns->pending[m->i.tok]) || (ret = resource_get(m->i.req, m->i.tok, &ns->r))) {
 ...
 * c1 - sent an ack
 * c2 - successfully got the resource */
static void handle_resource_get_rc(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    if (b->c0) {
...
static void handle_resource_free(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    (void)b;
    assert(!resource_free(m->i.req, m->i.tok, &ns->r));
...
static void handle_resource_free_rc(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    (void)b;
...
static void handle_resource_deq(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
...
static void handle_resource_deq_rc(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    if (b->c0) {
        return;
    }
...
static void handle_resource_reserve(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    (void)b;
    resource_token_t tok;
...
static void handle_resource_reserve_rc(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    (void)b;
...
void resource_event_handler(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
...
void resource_rev_handler(resource_state* ns, tw_bf* b, resource_msg* m, tw_lp* lp) {
    assert(m->i.h.magic == resource_magic);
    switch (m->i.h.event_type) {
    case RESOURCE_GET:
...
void resource_finalize(resource_state* ns, tw_lp* lp) {
    ...
    if (codes_mapping_get_lp_relative_id(lp->gid, 0, 0) == 0) {
        written = sprintf(out_buf, "# format: <LP> <max used general> <max used token...>\n");
...
void resource_lp_configure() {
    ...
    if (anno_map->has_unanno_lp > 0) {
        ret = configuration_get_value_longint(&config, RESOURCE_LP_NM, "available", NULL, &avail);
        if (ret) {
    ...
    for (int i = 0; i < anno_map->num_annos; i++) {
        ret = configuration_get_value_longint(&config, RESOURCE_LP_NM, "available",
                                              anno_map->annotations[i].ptr, &avail);
        if (ret) {
            fprintf(stderr,
...
static void resource_lp_issue_event_base(enum resource_event type, uint64_t req,
                                         resource_token_t tok, /* only used in reserve_get/free */
                                         int block_on_unavail, tw_lp* sender,
                                         struct codes_mctx const* map_ctx, int return_tag,
                                         msg_header const* return_header,
                                         struct codes_cb_info const* cb) {
    if (cb)
        SANITY_CHECK_CB(cb, resource_return);

    tw_lpid resource_lpid = codes_mctx_to_lpid(map_ctx, RESOURCE_LP_NM, sender->gid);

    tw_event* e = tw_event_new(resource_lpid, codes_local_latency(sender), sender);
...
void resource_lp_get(uint64_t req, int block_on_unavail, tw_lp* sender,
                     struct codes_mctx const* map_ctx, int return_tag,
                     msg_header const* return_header, struct codes_cb_info const* cb) {
    resource_lp_issue_event_base(RESOURCE_GET, req, 0, block_on_unavail, sender, map_ctx,
                                 return_tag, return_header, cb);
}

/* no callback for frees thus far */
void resource_lp_free(uint64_t req, tw_lp* sender, struct codes_mctx const* map_ctx) {
    resource_lp_issue_event_base(RESOURCE_FREE, req, 0, -1, sender, map_ctx, 0, NULL, NULL);
}
void resource_lp_reserve(uint64_t req, int block_on_unavail, tw_lp* sender,
                         struct codes_mctx const* map_ctx, int return_tag,
                         msg_header const* return_header, struct codes_cb_info const* cb) {
    resource_lp_issue_event_base(RESOURCE_RESERVE, req, 0, block_on_unavail, sender, map_ctx,
                                 return_tag, return_header, cb);
}
void resource_lp_get_reserved(uint64_t req, resource_token_t tok, int block_on_unavail,
                              tw_lp* sender, struct codes_mctx const* map_ctx, int return_tag,
                              msg_header const* return_header, struct codes_cb_info const* cb) {
    resource_lp_issue_event_base(RESOURCE_GET, req, tok, block_on_unavail, sender, map_ctx,
                                 return_tag, return_header, cb);
}
void resource_lp_free_reserved(uint64_t req, resource_token_t tok, tw_lp* sender,
                               struct codes_mctx const* map_ctx) {
    resource_lp_issue_event_base(RESOURCE_FREE, req, tok, -1, sender, map_ctx, 0, NULL, NULL);
}
