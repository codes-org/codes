...
void node_finalize(node_state* ns, tw_lp* lp) {
    ...
    if (ns->num_processed != num_pings * mult) {
        fprintf(stderr, "%s node %d, lp %llu: processed %d (expected %d)\n",
                ns->is_in_foo ? "foo" : "bar", ns->id_clust, LLU(lp->gid), ns->num_processed,
                num_pings * mult);
    }
}
...
void handle_node_next(node_state* ns, node_msg* m, tw_lp* lp) {
    ...
    // name must be used
    tw_lpid dest_fwd_lpid =
        codes_mapping_get_lpid_from_relative(dest_fwd_id, "FOO_FORWARDERS", "forwarder", NULL, 0);

    // as cluster nodes have only one network type (+ annotation), no need to
    // use annotation-specific messaging
    model_net_event_annotated(net_id_foo, "foo", "ping", dest_fwd_lpid, payload_sz, 0.0,
                              sizeof(m_fwd), &m_fwd, 0, NULL, lp);
}

void handle_node_recv_ping(node_state* ns, node_msg* m, tw_lp* lp) {
    ...
    // name must be used
    tw_lpid dest_fwd_lpid =
        codes_mapping_get_lpid_from_relative(dest_fwd_id, "BAR_FORWARDERS", "forwarder", NULL, 0);

    model_net_event_annotated(net_id_bar, "bar", "pong", dest_fwd_lpid, payload_sz, 0.0,
                              sizeof(m_fwd), &m_fwd, 0, NULL, lp);

    ns->num_processed++;
}

void handle_node_recv_pong(node_state* ns, node_msg* m, tw_lp* lp) {
...
static tw_lptype node_lp = {
    (init_f)node_lp_init, (pre_run_f)NULL,        (event_f)node_event_handler, (revent_f)NULL,
    (commit_f)NULL,       (final_f)node_finalize, (map_f)codes_mapping,        sizeof(node_state),
};
...
void handle_forwarder_fwd(forwarder_state* ns, forwarder_msg* m, tw_lp* lp) {
    ...
    tw_lpid dest_lpid =
        codes_mapping_get_lpid_from_relative(ns->id % mod, dest_group, "forwarder", NULL, 0);
    ...
    model_net_event_annotated(net_id_forwarding, NULL, category, dest_lpid, payload_sz, 0.0,
                              sizeof(m_fwd), &m_fwd, 0, NULL, lp);
...
void handle_forwarder_recv(forwarder_state* ns, forwarder_msg* m, tw_lp* lp) {
    ...
    tw_lpid dest_lpid =
        codes_mapping_get_lpid_from_relative(m->dest_node_clust_id, dest_group, "node", NULL, 0);
    ...
    model_net_event_annotated(net_id, annotation, category, dest_lpid, payload_sz, 0.0,
                              sizeof(m_node), &m_node, 0, NULL, lp);
...
int main(int argc, char* argv[]) {
    ...
    /* setup the globals */
    int rc = configuration_get_value_int(&config, "run_params", "num_reqs", NULL, &num_pings);
    if (rc != 0)
        tw_error(TW_LOC, "unable to read run_params:num_reqs");
    int payload_sz_d;
    rc = configuration_get_value_int(&config, "run_params", "payload_sz", NULL, &payload_sz_d);
    if (rc != 0)
    ...
    num_foo_nodes = codes_mapping_get_lp_count("FOO_CLUSTER", 0, "node", NULL, 1);
    num_bar_nodes = codes_mapping_get_lp_count("BAR_CLUSTER", 0, "node", NULL, 1);
    num_foo_forwarders = codes_mapping_get_lp_count("FOO_FORWARDERS", 0, "forwarder", NULL, 1);
    num_bar_forwarders = codes_mapping_get_lp_count("BAR_FORWARDERS", 0, "forwarder", NULL, 1);
