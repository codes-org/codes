...
/* type of events */
enum svr_event { KICKOFF = 1, PING, PONG };
...
const tw_optdef app_opt[] = {
    ...
    TWOPT_UINT(
        "num_initial_messages", num_initial_msgs,
        "Number of PING messages to be injected initially at the start (larger = more congestion)"),
...
static void svr_init(svr_state* s, tw_lp* lp) {
    ...
    for (int i = 1; i <= num_initial_msgs && i <= num_msgs; i++) {
        ...
        m->svr_event_type =
            KICKOFF; //Set the event type so we can know how to classify the event when received
        tw_event_send(e); //ROSS method to send off the event e with the encoded data in m
    }

    s->start_ts =
        kickoff_time; // the time when we're starting this LP's work is when the first ping is generated
}
...
static void handle_kickoff_event(svr_state* s, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    codes_mapping_get_lp_info(
        ...
        &offset); //gets information from CODES necessary to get the global LP ID of a server
    global_dest =
...
static void handle_pong_event(svr_state* s, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    {
        b->c1 =
            1; //flag that we didn't really do anything in this event so that if this event gets reversed, we don't over-aggressively revert state or RNGs
        return;
    }
...
static void svr_finalize(svr_state* s, tw_lp* lp) {
    ...
    printf("Sever LPID:%lu svr_id:%lu sent %d bytes in %f seconds, PINGs Sent: %d; PONGs Received: "
           "%d; PINGs Received: %d; PONGs Sent %d; Payload Sum: %d\n",
           (unsigned long)lp->gid, (unsigned long)s->svr_id, total_msg_size_sent,
           time_in_seconds_sent, s->ping_msg_sent_count, s->pong_msg_recvd_count,
           s->ping_msg_recvd_count, s->pong_msg_sent_count, s->payload_sum);
}
...
int main(int argc, char** argv) {
    ...
    num_nodes = codes_mapping_get_lp_count(
        "MODELNET_GRP", 0, "nw-lp", NULL,
        1); //get the number of nodes so we can use this value during the simulation
    assert(num_nodes);
