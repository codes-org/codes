...
static void svr_finalize(svr_state* ns, tw_lp* lp) {
    printf("server %llu recvd %d bytes in %lf seconds, %lf MiB/s sent_count %d recvd_count %d "
           "local_count %d \n",
           (unsigned long long)(lp->gid / 2), payload_sz * ns->msg_recvd_count,
           ns_to_s(ns->end_ts - ns->start_ts),
           ((double)(payload_sz * num_reqs) / (double)(1024 * 1024) /
            ns_to_s(ns->end_ts - ns->start_ts)),
...
tw_lpid get_next_server(tw_lpid sender_id) {
     ...
     * lookups */
    char grp_name[MAX_NAME_LENGTH], lp_type_name[MAX_NAME_LENGTH], annotation[MAX_NAME_LENGTH];
    int lp_type_id, grp_id, grp_rep_id, off, num_reps;
    int dest_rep_id;
    codes_mapping_get_lp_info(sender_id, grp_name, &grp_id, lp_type_name, &lp_type_id, annotation,
                              &grp_rep_id, &off);
    ...
    codes_mapping_get_lp_id(grp_name, lp_type_name, NULL, 1, dest_rep_id, 0, &rtn_id);
    return rtn_id;
}
...
static void handle_ack_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
     ...
     * calculation */
    assert(m->src == (lp->gid + offset) % (num_servers * 2) && m->src == get_next_server(lp->gid));
...
static void handle_req_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    assert(lp->gid == (m->src + offset) % (num_servers * 2) && lp->gid == get_next_server(m->src));
    ns->msg_recvd_count++;
