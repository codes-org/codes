...
struct network_model_surrogate {
    switch_surrogate_f
        switch_surrogate; // this function switches the model to and from surrogate-mode on a PE basis. It has to be called on all PEs to switch the entire simulation to its surrogate version
...
typedef bool (*model_ask_if_freeze_f)(
    tw_lp* lp,
    tw_event*
        event); // Determines whether the event should be "frozen" or should be allowed to run during surrogate-mode
typedef void (*model_check_event_f)(
    void* state, tw_lp* lp,
    tw_event*
        event); // Determines whether the event should be "frozen" or should be allowed to run during surrogate-mode
...
struct lp_types_switch {
    ...
    bool
        trigger_idle_modelnet; // Trigger idle events for model-net (prevents a model to be stuck in a schedule loop if it is to process packets during surrogate-mode). If this is true and the lpname does not start with 'modelnet_', the behaviour is undefined
    model_switch_f highdef_to_surrogate;
    model_switch_f surrogate_to_highdef;
    model_ask_if_freeze_f
        should_event_be_frozen; // true means event from LP type shouldn't be frozen
    model_ask_if_freeze_f
        should_event_be_deleted; // true means event from LP type shouldn't be deleted
...
struct network_surrogate_config {
    struct network_model_surrogate
        model; //!< functionality needed by the director to switch the model back and forth from high-fidelity to surrogate
    int total_terminals; //!< total number of terminals
    size_t n_lp_types;
    struct lp_types_switch lp_types[MAX_LP_TYPES];
};

void network_director_configure(struct network_surrogate_config*,
