...
#define qhash_lock_init(lock_ptr)                                                                  \
    do {                                                                                           \
    } while (0)
#define qhash_lock(lock_ptr)                                                                       \
    do {                                                                                           \
    } while (0)
#define qhash_unlock(lock_ptr)                                                                     \
    do {                                                                                           \
    } while (0)
 ...
 */
static inline struct qhash_table* qhash_init(int (*compare)(void* key, struct qhash_head* link),
                                             int (*hash)(void* key, int table_size),
                                             int table_size) {
    int i = 0;
    ...
    new_table->array = (struct qhash_head*)qhash_malloc(sizeof(struct qhash_head) * table_size);
    if (!new_table->array) {
 ...
 * no return value
 */
static inline void qhash_add(struct qhash_table* table, void* key, struct qhash_head* link) {
    int index = 0;
 ...
 * not found)
 */
static inline struct qhash_head* qhash_search(struct qhash_table* table, void* key) {
    int index = 0;
 ...
 * not found)
 */
static inline struct qhash_head* qhash_search_at_index(struct qhash_table* table, int index) {
    struct qhash_head* tmp_link = NULL;
 ...
 */
static inline struct qhash_head* qhash_search_and_remove(struct qhash_table* table, void* key) {
    int index = 0;
    struct qhash_head* tmp_link = NULL;
 ...
 * not found).  On success, link is removed from hashtable.
 */
static inline struct qhash_head* qhash_search_and_remove_at_index(struct qhash_table* table,
                                                                  int index) {
    struct qhash_head* tmp_link = NULL;
