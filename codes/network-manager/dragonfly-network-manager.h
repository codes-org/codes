...
enum ManagerType { MAN_ROUTER = 1, MAN_TERMINAL = 2 };
...
struct Connection {
    ...
    bool operator==(const Connection& other) const {
        return (port == other.port && src_lid == other.src_lid && src_gid == other.src_gid &&
                src_group_id == other.src_group_id && dest_lid == other.dest_lid &&
                dest_gid == other.dest_gid && dest_group_id == other.dest_group_id &&
                link_id == other.link_id && bandwidth == other.bandwidth &&
                is_failed == other.is_failed && rail_or_planar_id == other.rail_or_planar_id &&
...
template <> struct hash<Connection> {
    std::size_t operator()(const Connection& k) const {
        ...
        return ((hash<int>()(k.port) ^ (hash<int>()(k.src_gid) << 1)) >> 1) ^
               (hash<int>()(k.dest_gid) << 1) ^ (hash<int>()(k.rail_or_planar_id) << 1);
    }
};
...
class DragonflyNetworkManager {
    ...
    map<int, vector<Link_Info>>
        _router_link_failure_lists; //maps router ID to a vector of Link Infos that contain dest router GIDs that it has a FAILED connection to, one item for each failed connection
    map<int, vector<Link_Info>>
        _router_terminal_link_failure_lists; //maps router ID to a vector of Link Infos that contain dest terminal GIDs that it has a FAILED connection to, one item for each failed connection

    //storage maps - these are the 'owners' of the connections contained.
    unordered_map<int, vector<Connection*>>
        _router_connections_map; //maps router ID to a vector of all router-router connections that that router has
    unordered_map<int, vector<Connection*>>
        _router_terminal_connections_map; //maps router ID to a vector of all terminal connections that that router has
    ...
    unordered_map<pair<int, int>, vector<Connection*>>
        _router_to_router_connection_map; //maps pair(src_gid, dest_gid) to a vector of connection pointers that go from src gid to dest gid
    unordered_map<pair<int, int>, vector<Connection*>>
        _router_to_terminal_connection_map; //maps pair(src_gid, dest_term_gid) to a vector of connection pointers taht go from src gid router to dest_term id
    unordered_map<pair<int, int>, vector<Connection*>> _terminal_to_router_connection_map;
    unordered_map<pair<int, int>, vector<Connection*>>
        _router_to_group_connection_map; //maps pair(src_gid, dest_group_id) to a vector of connections that go from src_gid to any router in the dest group
    map<pair<int, int>, vector<Connection*>>
        _global_group_connection_map; //maps pair(src group id, dest group id) to a vector of connection pointers that match that pattern

    set<int> _router_ids_with_terminals;
    unordered_map<tuple<int, int, int, int>, set<Connection>> _valid_next_conn_map;

    int***
        adjacency_matrix; //total_routers x total_routers in size, 1 for if any connection exists 0 for if no connection exists
    ...
    vector<DragonflyConnectionManager>
        _connection_manager_list; //list of all connection managers in the network
    vector<DragonflyConnectionManager>
        _terminal_connection_manager_list; // list of all connection mangers for TERMINALS in the network
  ...
  public:
    ...
    DragonflyNetworkManager(int total_routers, int total_terminals, int num_routers_per_group,
                            int num_lc_per_router, int num_gc_per_router,
                            int num_cn_conns_per_router, int num_rails, int num_planes,
                            int max_local_hops, int max_global_hops);
    ...
    set<Connection> get_valid_next_hops_conns(int src_gid, int dest_gid, int max_local,
                                              int exact_global);
...
class DragonflyConnectionManager {
  public:
    ...
    map<int, Connection>
        _portMap; //Mapper for ports to connection references - includes failed connections

    // includes failed connections
    map<int, vector<Connection>>
        intraGroupConnections; //direct connections within a group - IDs are group local - maps local id to list of connections to it
    map<int, vector<Connection>>
        intraGroupConnectionsGID; //direct connections within a group - IDs are global IDs
    map<int, vector<Connection>>
        globalConnections; //direct connections between routers not in same group - IDs are global router IDs - maps global id to list of connections to it
    map<int, vector<Connection>>
        terminalConnections; //direct connections between this router and its compute node terminals - maps terminal id to connections to it
    map<int, vector<Connection>>
        injectionConnections; //this is specific for terminal origins, maps a router ID to connections to it

    map<int, vector<Connection>>
        _connections_to_groups_map; //maps group ID to connections to said group
    vector<int> _other_groups_i_connect_to;
    map<int, vector<Connection>> _all_conns_by_type_map;
    map<int, vector<Connection>>
        _routed_connections_to_group_map; //maps group ID to connections within local group that go to specified group
    map<int, vector<Connection>>
        _next_hop_routed_connections_to_group_map; //maps group ID to connections within local group that directly lead to a router that goes to the specified group
    map<int, vector<int>>
        _routed_router_gids_to_group_map; //maps group ID to a vector of Router GIDs within current group that have a connection to the group ID
    ...
    vector<int>
        _accessible_group_ids_nofail; //group IDs that this router can reach directly or with one intermediate router hop within its local group
    map<int, vector<Connection>> _all_conns_by_type_map_nofail;
    map<int, vector<Connection>>
        _routed_connections_to_group_map_nofail; //maps group ID to connections within local group that go to specified group
    map<int, vector<Connection>>
        _next_hop_routed_connections_to_group_map_nofail; //maps group ID to connections within local group that directly lead to a router that goes to the specified group
    map<int, vector<int>>
        _routed_router_gids_to_group_map_nofail; //maps group ID to a vector of Router GIDs within current group that have a connection to the group ID
    ...
    bool
        is_solidified; //flag for whether or not solidification has taken place so that this can be checked at the end of router init

  public:
    DragonflyConnectionManager();
    DragonflyConnectionManager(int src_id_local, int src_id_global, int src_group, int max_intra,
                               int max_inter, int max_term, int num_router_per_group,
                               int num_groups);
    DragonflyConnectionManager(int src_id_local, int src_id_global, int src_group, int max_intra,
                               int max_inter, int max_term, int max_injection,
                               int num_router_per_group, int num_groups, int num_planes,
                               ManagerType manType);
    /**
    ...
    void add_group_group_connection_information(
