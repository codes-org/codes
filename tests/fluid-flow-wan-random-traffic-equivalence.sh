...
case_root="${1:-fluid-flow-wan-random-traffic-seq-opt-equivalence}"
mpi_exec="${2:-mpirun}"
mpi_np_flag="${3:--np}"
...
smoke_test="$srcdir/tests/fluid-flow-wan-random-traffic-ci.sh"
...
echo "fluid-flow-wan random-traffic sequential/optimistic committed CSV logs match"
