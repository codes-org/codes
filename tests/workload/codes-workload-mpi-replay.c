...
void usage(char* exename) {
    fprintf(stderr,
            ...
            exename);
    fprintf(stderr,
            "\t<conf_file_path> : (absolute) path to a valid workload configuration file\n");
    ...
    fprintf(stderr,
            "\t\t--delay-ratio : floating point ratio applied to each delay (defaults to 1)\n");
    fprintf(stderr, "\t\t--max-delay : maximum delay (in seconds) to replay\n");
    fprintf(stderr, "\t\t--prep: instead of running replay, pre-populate files that will be needed "
...
void parse_args(int argc, char** argv, char** conf_path, char** test_dir) {
    int index;
    static struct option long_opts[] = {{"conf", 1, NULL, 'c'},        {"test-dir", 1, NULL, 'd'},
                                        {"noop", 0, NULL, 'n'},        {"prep", 0, NULL, 'P'},
                                        {"delay-ratio", 1, NULL, 'p'}, {"max-delay", 1, NULL, 'm'},
                                        {"help", 0, NULL, 0},          {0, 0, 0, 0}};
...
int load_workload(char* conf_path, int rank) {
    ...
    if (strcmp(workload_type, "darshan_io_workload") == 0) {
        ...
        configuration_get_value(&config, "PARAMS", "log_file_path", NULL, d_params.log_file_path,
                                MAX_NAME_LENGTH_WKLD);
        ...
        configuration_get_value(&config, "PARAMS", "io_kernel_meta_path", NULL,
                                i_params.io_kernel_meta_path, MAX_NAME_LENGTH_WKLD);
        ...
        configuration_get_value(&config, "PARAMS", "trace_dir_path", NULL, r_params.trace_dir_path,
                                MAX_NAME_LENGTH_WKLD);
...
int main(int argc, char* argv[]) {
    ...
    if (myrank == 0)
        printf("Note: Workload took %.2lf seconds to load.\n", load_end - load_start);
    ...
    while (1) {
        ...
        if (next_op.op_type != CODES_WK_END) {
            ...
            if (ret < 0) {
                fprintf(stderr,
                        "Error: replay_workload_op() for replay_op_number %lld failed on rank %d\n",
    ...
    printf("Rank %d:\td=%.4lf, b=%.4lf, o=%.4lf, c=%.4lf, r=%.4lf, w=%.4lf\n", myrank,
           total_delay_time, total_barrier_time, total_open_time, total_close_time, total_read_time,
           total_write_time);
#endif
...
static int do_read(struct codes_workload_op replay_op, int rank, long long int op_number) {
    ...
    if (!opt_noop) {
        ...
        if (!hash_link) {
            fprintf(
                stderr,
                "ERROR: rank %d unable to find fd_table record for file_id %llu in read path.\n",
        ...
        case CODES_WK_MPI_READ:
            ret = MPI_File_read_at(fh, replay_op.u.read.offset, buf, replay_op.u.read.size,
                                   MPI_BYTE, &status);
            break;
        case CODES_WK_MPI_COLL_READ:
            ret = MPI_File_read_at_all(fh, replay_op.u.read.offset, buf, replay_op.u.read.size,
                                       MPI_BYTE, &status);
            break;
        default:
            assert(0);
...
static int do_write(struct codes_workload_op replay_op, int rank, long long int op_number) {
    ...
    if (opt_verbose)
        fprintf(log_stream, "[Rank %d] Operation %lld : %s file %llu (sz = %llu, off = %llu)\n",
                rank, op_number, op_name, LLU(replay_op.u.write.file_id),
                LLU(replay_op.u.write.size), LLU(replay_op.u.write.offset));
    ...
    if (!opt_noop) {
        ...
        if (!hash_link) {
            fprintf(
                stderr,
                "ERROR: rank %d unable to find fd_table record for file_id %llu in write path.\n",
        ...
        case CODES_WK_MPI_WRITE:
            ret = MPI_File_write_at(fh, replay_op.u.write.offset, buf, replay_op.u.write.size,
                                    MPI_BYTE, &status);
            break;
        case CODES_WK_MPI_COLL_WRITE:
            ret = MPI_File_write_at_all(fh, replay_op.u.write.offset, buf, replay_op.u.write.size,
                                        MPI_BYTE, &status);
            break;
        default:
            assert(0);
...
static int do_close(int rank, uint64_t file_hash, enum codes_workload_op_type type,
                    long long op_number) {
    int fildes;
    MPI_File fh;
    struct qlist_head* hash_link = NULL;
    ...
    if (opt_verbose)
        fprintf(log_stream, "[Rank %d] Operation %lld : %s file %" PRIu64 "\n", rank, op_number,
                (type == CODES_WK_CLOSE) ? "CLOSE" : "MPI_CLOSE", file_hash);
    ...
    if (!opt_noop) {
        ...
        if (!hash_link) {
            fprintf(
                stderr,
                "ERROR: rank %d unable to find fd_table record for file_id %llu in close path.\n",
                rank, LLU(file_hash));
            assert(hash_link);
        }
        tmp_list = qhash_entry(hash_link, struct file_info, hash_link);
...
static int do_mpi_open(int rank, uint64_t file_hash, int create_flag, int collective_flag,
                       long long op_number) {
    ...
    if (opt_verbose)
        fprintf(log_stream, "[Rank %d] Operation %lld: %s file %" PRIu64 "\n", rank, op_number,
                collective_flag ? "MPI_FILE_OPEN (collective)" : "MPI_FILE_OPEN (independent)",
                file_hash);
...
int replay_workload_op(struct codes_workload_op replay_op, int rank, long long int op_number) {
        ...
        if (!opt_noop) {
            ...
            if (ret != MPI_SUCCESS) {
                ...
                fprintf(stderr, "Rank %d failure on operation %lld [BARRIER: %s]\n", rank,
                        op_number, "Invalid communicator");
                return -1;
            }
        ...
        if (!opt_noop) {
            ...
            if (fildes < 0) {
                fprintf(stderr, "Rank %d failure on operation %lld [%s: %s]\n", rank, op_number,
                        (replay_op.u.open.create_flag) ? "CREATE" : "OPEN", strerror(errno));
                return -1;
            }
    ...
    case CODES_WK_MPI_OPEN:
        if (opt_prep && replay_op.u.open.create_flag)
            return (0);
        return (do_mpi_open(rank, replay_op.u.open.file_id, replay_op.u.open.create_flag, 0,
                            op_number));
    case CODES_WK_MPI_COLL_OPEN:
        if (opt_prep && replay_op.u.open.create_flag)
            return (0);
        return (do_mpi_open(rank, replay_op.u.open.file_id, replay_op.u.open.create_flag, 1,
                            op_number));
    case CODES_WK_CLOSE:
    case CODES_WK_MPI_CLOSE:
        return (do_close(rank, replay_op.u.close.file_id, replay_op.op_type, op_number));
    case CODES_WK_WRITE:
    case CODES_WK_MPI_WRITE:
    case CODES_WK_MPI_COLL_WRITE:
        if (opt_prep)
