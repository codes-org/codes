...
static void init(state* ns, tw_lp* lp) {
    int dummy;
    // only need the names
    codes_mapping_get_lp_info(lp->gid, ns->group_name, &dummy, ns->lp_name, &dummy, ns->anno,
                              &dummy, &dummy);
    ...
    tw_lpid id_from_group_anno_rel =
        codes_mapping_get_lpid_from_relative(ns->id_by_group_anno, ns->group_name, ns->lp_name,
                                             anno, 1);
...
tw_lptype a_lp = {
    (init_f)init,   (pre_run_f)NULL, (event_f)NULL,        (revent_f)NULL,
    (commit_f)NULL, (final_f)NULL,   (map_f)codes_mapping, sizeof(state),
};
tw_lptype b_lp = {
    (init_f)init,   (pre_run_f)NULL, (event_f)NULL,        (revent_f)NULL,
    (commit_f)NULL, (final_f)NULL,   (map_f)codes_mapping, sizeof(state),
...
#define TEST_TO_CID(_expected_cid, _input, _name_type, _fn_type)                                   \
    do {                                                                                           \
        ...
        if (_ecid != _acid) {                                                                      \
            fprintf(stderr,                                                                        \
                    "%s:%d: " #_name_type " name->cid check error (expected %d, got %d)\n",        \
        ...
        for (int g = 0; g < 3; g++) {
            for (int a = 0; a < 3; a++) {
                ...
                printf("TEST1 %2d %6s %s\n",
                       codes_mapping_get_lp_count(groups[g], 0, lps[l], annos[a], 0), groups[g],
                       lpnm);
            }
