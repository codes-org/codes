...
const tw_optdef app_opt[] = {TWOPT_GROUP("Simple Network Test Model"),
...
static void svr_finalize(svr_state* ns, tw_lp* lp) {
    printf("server %llu : size:%d requests:%d time:%lf rate:%lf\n", (unsigned long long)lp->gid,
           PAYLOAD_SZ, NUM_REQS, ns_to_s((tw_now(lp) - ns->start_ts)),
           (double)(PAYLOAD_SZ * NUM_REQS) / (1024.0 * 1024.0) /
...
static void handle_kickoff_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    lsm_io_event("test", 0, 0, PAYLOAD_SZ, LSM_WRITE_REQUEST, 1.0, lp, CODES_MCTX_DEFAULT, 0, &h,
                 &cb_info);
    ...
    h.event_type = LOCAL;
    lsm_io_event("test", 0, 0, PAYLOAD_SZ, LSM_WRITE_REQUEST, 2.0, lp, CODES_MCTX_DEFAULT, 1, &h,
                 &cb_info);
}
...
static void handle_ack_event(svr_state* ns, tw_bf* b, svr_msg* m, tw_lp* lp) {
    ...
    if (ns->msg_sent_count < NUM_REQS) {
        ...
        msg_header h;
        msg_set_header(magic, ACK, lp->gid, &h);
        lsm_io_event("test", 0, 0, PAYLOAD_SZ, LSM_WRITE_REQUEST, 0.0, lp, CODES_MCTX_DEFAULT, 0,
                     &h, &cb_info);
        ...
        // make a parallel dummy request to test out sched
        h.event_type = LOCAL;
        lsm_io_event("test", 0, 0, PAYLOAD_SZ, LSM_WRITE_REQUEST, 2.0, lp, CODES_MCTX_DEFAULT, 1,
                     &h, &cb_info);
    } else {
