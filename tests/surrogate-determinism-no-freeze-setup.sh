...
#
# Generates the surrogate ping-pong config with NO network freezing
# (NETWORK_TREATMENT=nothing) for example-ping-pong-surrogate-determinism-no-freeze.
# A single PACKET_LATENCY_TRACE_PATH is fine because each run executes in its own
# subdir, so the two runs' trace output cannot collide. $bindir is exported by
# run-test.sh.
