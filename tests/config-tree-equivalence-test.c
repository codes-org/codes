 ...
 *
 * With two configs: loads both through the production loader (configuration_load,
 * so a .conf goes through the legacy text parser and a .yaml/.json through the
 * YAML front-end, including collective include resolution) and asserts the two
 ...
 *
 * With one config: just loads it (a config linter / no-run load). Combined with
 * CODES_RESOLVED_CONFIG_DUMP this prints the resolved config tree without running
 * a model, which is how the resolved-config-dump test exercises that knob.
 *
 * Exit status: 0 on success (one config loaded, or two equal trees), 1 if two
 * trees differ (cf_equal_report has printed which section/key diverged), 2 on a
 * usage or load error.
 */
...
int main(int argc, char** argv) {
    ...
    /* One config: load-only (a linter; with CODES_RESOLVED_CONFIG_DUMP set, the
     * loader dumps the resolved tree). */
    if (argc == 2) {
        if (rank == 0)
