...
static int test_jobmap_identity(int num_ranks) {
    ...
    if (!c)
        ERR("jobmap-identity: configure failure");
    ...
    for (int i = 0; i < num_ranks; i++) {
        ...
        if (lid.job != 0 || lid.rank != i)
            ERR("jobmap-identity: expected lid (%d,%d), got (%d,%d) for gid %d", 0, i, lid.job,
                lid.rank, i);
        gid = codes_jobmap_to_global_id(lid, c);
        if (gid != i)
            ERR("jobmap-identity: expected gid %d, got %d for lid (%d,%d)", i, gid, lid.job,
                lid.rank);
    }
    return 0;
}
...
static int test_jobmap_list(char* fname) {
    ...
    if (!c)
        ERR("jobmap-list: configure failure");
    ...
    for (int i = 0; i < num_jobs; i++) {
        ...
        for (int j = 0; j < rank_count_per_job[i]; j++) {
            ...
            if (gid != gid_expected)
                ERR("jobmap-list: expected gid %d for lid (%d,%d), got %d", gid_expected,
                    lid_expected.job, lid_expected.rank, gid);
            lid = codes_jobmap_to_local_id(gid_expected, c);
            if (lid.job != lid_expected.job || lid.rank != lid_expected.rank)
                ERR("jobmap-list: expected lid (%d,%d) for gid %d, got (%d,%d)", lid_expected.job,
                    lid_expected.rank, gid_expected, lid.job, lid.rank);
            gid_expected++;
        }
    }
...
static int test_jobmap_dummy(int num_jobs) {
    ...
    for (int i = 0; i < num_jobs; i++) {
        ...
        if (lid.job != i || lid.rank != 0)
            ERR("lookup failure for %d: expected (%d,%d), got (%d,%d)", i, i, 0, lid.job, lid.rank);
        else {
            lid.job = -1;
            lid.rank = -1;
        }
    ...
    if (lid.job != -1 || lid.rank != -1)
        ERR("lookup expected failure for %d: expected (%d,%d), got (%d,%d)", num_jobs, -1, -1,
            lid.job, lid.rank);
    ...
    for (lid.job = 0; lid.job < num_jobs; lid.job++) {
        gid = codes_jobmap_to_global_id(lid, c);
        if (gid != lid.job)
            ERR("lookup failure for (%d,%d): expected %d, got %d", lid.job, lid.rank, lid.job, gid);
    }
    /* bad global lookup */
    lid.job = num_jobs;
    gid = codes_jobmap_to_global_id(lid, c);
    if (gid != -1)
        ERR("lookup expected failure for (%d,0): expected -1, got %d", num_jobs, gid);
