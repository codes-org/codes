...
static void handle_kickoff_event(svr_state* ns, svr_msg* m, tw_lp* lp) {
    ...
    num_routers_per_rep =
        codes_mapping_get_lp_count("MODELNET_GRP", 1, "dragonfly_router", NULL, 1);
    ...
    if (TRAFFIC == UNIFORM_RANDOM)
        ...
        dest_svr = ((rep_id * num_servers_per_rep) + (num_servers_per_rep * 2)) %
                   (num_reps * num_servers_per_rep);
    }
    dest_svr =
        dest_svr + ((dest_svr / num_servers_per_rep) * (num_routers_per_rep + num_servers_per_rep));

    if (do_pull) {
        m->ret = model_net_pull_event(net_id, "test", dest_svr, PAYLOAD_SZ, 0.0, sizeof(svr_msg),
                                      (const void*)m_remote, lp);
    } else {
        m->ret = model_net_event(net_id, "test", dest_svr, PAYLOAD_SZ, 0.0, sizeof(svr_msg),
...
static void handle_ack_event(svr_state* ns, svr_msg* m, tw_lp* lp) {
    ...
    if (ns->msg_sent_count < NUM_REQS) {
        ...
        if (do_pull) {
            ...
            m->ret =
                model_net_event(net_id, "test", dest_svr, PAYLOAD_SZ, 0.0, sizeof(svr_msg),
                                (const void*)m_remote, sizeof(svr_msg), (const void*)m_local, lp);
        }
        ns->msg_sent_count++;
