...
int main(int argc, char** argv) {
    ...
    if (net_id == SLIMFLY || net_id == DRAGONFLY || net_id == EXPRESS_MESH) {
        num_routers = codes_mapping_get_lp_count("MODELNET_GRP", 0, router_name, NULL, 1);
        offset = 1;
    }
...
static void handle_kickoff_event(svr_state* ns, svr_msg* m, tw_lp* lp) {
    ...
    num_routers_per_rep = codes_mapping_get_lp_count("MODELNET_GRP", 1, router_name, NULL, 1);
    ...
    if ((net_id == DRAGONFLY || net_id == SLIMFLY || net_id == EXPRESS_MESH) &&
        (lp->gid % lps_per_rep == num_servers_per_rep - 1))
        opt_offset = num_servers_per_rep + num_routers_per_rep;

    /* each server sends a request to the next highest server */
    int dest_id = (lp->gid + offset + opt_offset) % total_lps;
    if (do_pull) {
        m->ret = model_net_pull_event(net_id, "test", dest_id, PAYLOAD_SZ, 0.0, sizeof(svr_msg),
                                      (const void*)m_remote, lp);
    } else {
...
static void handle_ack_event(svr_state* ns, svr_msg* m, tw_lp* lp) {
    ...
    if ((net_id == DRAGONFLY || net_id == SLIMFLY || net_id == EXPRESS_MESH) &&
        (lp->gid % lps_per_rep == num_servers_per_rep - 1))
        opt_offset = num_servers_per_rep +
                     num_routers_per_rep; /* optional offset due to dragonfly mapping */
...
static void handle_req_event(svr_state* ns, svr_msg* m, tw_lp* lp) {
    ...
    if ((net_id == DRAGONFLY || net_id == SLIMFLY || net_id == EXPRESS_MESH) &&
        (m->src % lps_per_rep == num_servers_per_rep - 1))
        opt_offset = num_servers_per_rep +
                     num_routers_per_rep; /* optional offset due to dragonfly mapping */
